...
/datum/saymode/changeling/handle_message(mob/living/user, message, datum/language/language)
	switch(user.lingcheck())
		if(LINGHIVE_LINK)
			var/msg = "<span_class='changeling'><b>[user.mind]:</b> [message]</span>"
				...
				if(M in GLOB.dead_mob_list)
					...
					switch(M.lingcheck())
						...
						if(LINGHIVE_OUTSIDER)
							if(prob(40))
								to_chat(M, "<span_class='changeling'>We can faintly sense an outsider trying to communicate through the hivemind...</span>")
		if(LINGHIVE_LING)
			if (user.has_trait(CHANGELING_HIVEMIND_MUTE))
				to_chat(user, "<span_class='warning'>The poison in the air hinders our ability to interact with the hivemind.</span>")
				return FALSE
			var/datum/antagonist/changeling/changeling = user.mind.has_antag_datum(/datum/antagonist/changeling)
			var/msg = "<span_class='changeling'><b>[changeling.changelingID]:</b> [message]</span>"
			...
			for(var/_M in GLOB.player_list)
				...
				if(M in GLOB.dead_mob_list)
					...
					switch(M.lingcheck())
						...
						if(LINGHIVE_OUTSIDER)
							if(prob(40))
								to_chat(M, "<span_class='changeling'>We can faintly sense another of our kind trying to communicate through the hivemind...</span>")
		if(LINGHIVE_OUTSIDER)
			to_chat(user, "<span_class='changeling'>Our senses have not evolved enough to be able to communicate this way...</span>")
	return FALSE
...
/datum/saymode/monkey/handle_message(mob/living/user, message, datum/language/language)
	...
	if(is_monkey_leader(mind) || (ismonkey(user) && is_monkey(mind)))
		...
		if(prob(75) && ismonkey(user))
			user.visible_message("<span_class='notice'>\The [user] chimpers.</span>")
		var/msg = "<span_class='[is_monkey_leader(mind) ? "monkeylead" : "monkeyhive"]'><b><font size=2>\[[is_monkey_leader(mind) ? "Monkey Leader" : "Monkey"]\]</font> [user]</b>: [message]</span>"
