...
/datum/martial_art/mushpunch/harm_act(mob/living/carbon/human/A, mob/living/carbon/human/D)
	var/atk_verb
	to_chat(A, "<span_class='spider'>You begin to wind up an attack...</span>")
	if(!do_after(A, 25, target = D))
		to_chat(A, "<span_class='spider'><b>Your attack was interrupted!</b></span>")
		return TRUE //martial art code was a mistake
	A.do_attack_animation(D, ATTACK_EFFECT_PUNCH)
	atk_verb = pick("punches", "smashes", "ruptures", "cracks")
	D.visible_message("<span_class='danger'>[A] [atk_verb] [D] with inhuman strength, sending [D.p_them()] flying backwards!</span>", \
					  "<span_class='userdanger'>[A] [atk_verb] you with inhuman strength, sending you flying backwards!</span>")
...
/obj/item/mushpunch/attack_self(mob/living/carbon/human/user)
	if(!istype(user) || !user)
		return
	var/message = "<span_class='spider'>You devour [src], and a confluence of skill and power from the mushroom enhances your punches! You do need a short moment to charge these powerful punches.</span>"
	to_chat(user, message)
	var/datum/martial_art/mushpunch/mush = new(null)
	mush.teach(user)
	qdel(src)
	visible_message("<span_class='warning'>[user] devours [src].</span>")
