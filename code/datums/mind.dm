...
/datum/mind/proc/enslave_mind_to_creator(mob/living/creator)
	...
	if(creator.mind.special_role)
		...
		to_chat(current, "<span_class='userdanger'>Despite your creators current allegiances, your true master remains [creator.real_name]. If their loyalties change, so do yours. This will never change unless your creator's body is destroyed.</span>")
...
/datum/mind/Topic(href, href_list)
	...
	if(href_list["remove_antag"])
		...
		if(!istype(A))
			to_chat(usr,"<span_class='warning'>Invalid antagonist ref to be removed.</span>")
			return
		A.admin_remove(usr)
	...
	if (href_list["role_edit"])
		...
		switch(href_list["common"])
			...
			if("uplink")
				if(!equip_traitor())
					to_chat(usr, "<span_class='danger'>Equipping a syndicate failed!</span>")
...
/datum/mind/proc/announce_objectives()
	var/obj_count = 1
	to_chat(current, "<span_class='notice'>Your current objectives:</span>")
