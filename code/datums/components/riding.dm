...
/datum/component/riding/proc/ride_check(mob/living/M)
	...
	if((ride_check_rider_restrained && M.restrained(TRUE)) || (ride_check_rider_incapacitated && M.incapacitated(FALSE, TRUE)) || (ride_check_ridden_incapacitated && istype(AMM) && AMM.incapacitated(FALSE, TRUE)))
		AM.visible_message("<span_class='warning'>[M] falls off of [AM]!</span>")
		AM.unbuckle_mob(M)
	return TRUE
...
/datum/component/riding/proc/handle_ride(mob/user, direction)
	...
	if(keycheck(user))
		...
		to_chat(user, "<span_class='notice'>You'll need the keys in one of your hands to [drive_verb] [AM].</span>")
...
/datum/component/riding/human/force_dismount(mob/living/user)
	...
	user.Paralyze(60)
	user.visible_message("<span_class='warning'>[AM] pushes [user] off of [AM.p_them()]!</span>")
	...
	if(user.incapacitated())
		...
		if(iscyborg(AM))
			...
			if(R.module && R.module.ride_allow_incapacitated)
				kick = FALSE
		if(kick)
			to_chat(user, "<span_class='userdanger'>You fall off of [AM]!</span>")
			Unbuckle(user)
			return
	if(iscarbon(user))
		...
		if(!carbonuser.get_num_arms())
			Unbuckle(user)
			to_chat(user, "<span_class='userdanger'>You can't grab onto [AM] with no hands!</span>")
			return
...
/datum/component/riding/cyborg/force_dismount(mob/living/M)
	...
	M.Move(targetm)
	M.visible_message("<span_class='warning'>[M] is thrown clear of [AM]!</span>")
