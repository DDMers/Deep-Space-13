	...
	if(ismob(AM))
		...
		if(ishuman(AM))
			...
			if(istype(H.belt, /obj/item/wormhole_jaunter))
				...
				H.visible_message("<span_class='boldwarning'>[H] falls into [parent]!</span>")
				J.chasm_react(H)
				return FALSE
	return TRUE
...
/datum/component/chasm/proc/drop(atom/movable/AM)
	...
	if(T)
		...
		AM.visible_message("<span_class='boldwarning'>[AM] falls into [parent]!</span>", "<span_class='userdanger'>[fall_message]</span>")
		T.visible_message("<span_class='boldwarning'>[AM] falls from above!</span>")
		AM.forceMove(T)
		if(isliving(AM))
			var/mob/living/L = AM
			L.Paralyze(100)
		...
		// send to oblivion
		AM.visible_message("<span_class='boldwarning'>[AM] falls into [parent]!</span>", "<span_class='userdanger'>[oblivion_message]</span>")
		if (isliving(AM))
			var/mob/living/L = AM
			L.notransform = TRUE
			L.Stun(200)
		...
		if(AM && !QDELETED(AM))	//It's indestructible
			var/atom/parent = src.parent
			parent.visible_message("<span_class='boldwarning'>[parent] spits out [AM]!</span>")
			AM.alpha = oldalpha
			AM.color = oldcolor
