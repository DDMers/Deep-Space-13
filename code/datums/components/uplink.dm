...
/datum/component/uplink/proc/LoadTC(mob/user, obj/item/stack/telecrystal/TC, silent = FALSE)
	if(!silent)
		to_chat(user, "<span_class='notice'>You slot [TC] into [parent] and charge its internal uplink.</span>")
	var/amt = TC.amount
	telecrystals += amt
	TC.use(amt)
	...
	for(var/category in uplink_items)
		for(var/item in uplink_items[category])
			...
			if(I.type == path && UI.refundable && I.check_uplink_validity())
				...
				to_chat(user, "<span_class='notice'>[I] refunded.</span>")
				qdel(I)
				return
...
/datum/component/uplink/proc/pen_rotation(datum/source, degrees, mob/living/carbon/user)
	...
	if(compare_list(previous_attempts, unlock_code))
		...
		master.degrees = 0
		interact(null, user)
		to_chat(user, "<span_class='warning'>Your pen makes a clicking noise, before quickly rotating back to 0 degrees!</span>")
