...
/datum/component/butchering/proc/Butcher(mob/living/butcher, mob/living/meat)
	...
	for(var/V in meat.butcher_results)
		...
		for(var/_i in 1 to amount)
			if(!prob(final_effectiveness))
				if(butcher)
					to_chat(butcher, "<span_class='warning'>You fail to harvest some of the [initial(bones.name)] from [meat].</span>")
			else if(prob(bonus_chance))
				if(butcher)
					to_chat(butcher, "<span_class='info'>You harvest some extra [initial(bones.name)] from [meat]!</span>")
				for(var/i in 1 to 2)
					new bones (T)
			else
				new bones (T)
	...
	if(butcher)
		meat.visible_message("<span_class='notice'>[butcher] butchers [meat].</span>")
	ButcherEffects(meat)
