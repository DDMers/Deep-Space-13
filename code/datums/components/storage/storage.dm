...
/datum/component/storage/proc/attack_self(datum/source, mob/M)
	if(locked)
		to_chat(M, "<span_class='warning'>[parent] seems to be locked!</span>")
		return FALSE
...
/datum/component/storage/proc/preattack_intercept(datum/source, obj/O, mob/M, params)
	...
	if(!len)
		to_chat(M, "<span_class='notice'>You failed to pick up anything with [parent].</span>")
		return
	...
	while(do_after(M, 10, TRUE, parent, FALSE, CALLBACK(src, .proc/handle_mass_pickup, things, I.loc, rejections, progress)))
		stoplag(1)
	qdel(progress)
	to_chat(M, "<span_class='notice'>You put everything you could [insert_preposition] [parent].</span>")
...
/datum/component/storage/proc/quick_empty(mob/M)
	...
	if(locked)
		...
		return FALSE
	A.add_fingerprint(M)
	to_chat(M, "<span_class='notice'>You start dumping out [parent].</span>")
...
/datum/component/storage/proc/can_be_inserted(obj/item/I, stop_messages = FALSE, mob/M)
	...
	if(locked)
		if(M && !stop_messages)
			...
			to_chat(M, "<span_class='warning'>[host] seems to be locked!</span>")
		return FALSE
	if(real_location.contents.len >= max_items)
		if(!stop_messages)
			to_chat(M, "<span_class='warning'>[host] is full, make some space!</span>")
		return FALSE //Storage item is full
	if(length(can_hold))
		if(!is_type_in_typecache(I, can_hold))
			if(!stop_messages)
				to_chat(M, "<span_class='warning'>[host] cannot hold [I]!</span>")
			return FALSE
	...
	if(I.w_class > max_w_class)
		if(!stop_messages)
			to_chat(M, "<span_class='warning'>[I] is too big for [host]!</span>")
		return FALSE
	...
	if(sum_w_class > max_combined_w_class)
		if(!stop_messages)
			to_chat(M, "<span_class='warning'>[I] won't fit in [host], make some space!</span>")
		return FALSE
	if(isitem(host))
		...
		if((I.w_class >= IP.w_class) && STR_I && !allow_big_nesting)
			if(!stop_messages)
				to_chat(M, "<span_class='warning'>[IP] cannot hold [I] as it's a storage item of the same size!</span>")
			return FALSE //To prevent the stacking of same sized storage items.
	if(I.item_flags & NODROP) //SHOULD be handled in unEquip, but better safe than sorry.
		if(!stop_messages)
			to_chat(M, "<span_class='warning'>\the [I] is stuck to your hand, you can't put it in \the [host]!</span>")
		return FALSE
	var/datum/component/storage/concrete/master = master()
	if(!istype(master))
		return FALSE
...
/datum/component/storage/proc/mob_item_insertion_feedback(mob/user, mob/M, obj/item/I, override = FALSE)
	...
	for(var/mob/viewing in viewers(user, null))
		if(M == viewing)
			to_chat(usr, "<span_class='notice'>You put [I] [insert_preposition]to [parent].</span>")
		else if(in_range(M, viewing)) //If someone is standing close enough, they can tell what it is...
			viewing.show_message("<span_class='notice'>[M] puts [I] [insert_preposition]to [parent].</span>", 1)
...
/datum/component/storage/proc/on_attack_hand(datum/source, mob/user)
	...
	if(A.loc == user)
		. = COMPONENT_NO_ATTACK_HAND
		if(locked)
			to_chat(user, "<span_class='warning'>[parent] seems to be locked!</span>")
		else
			show_to(user)
...
/datum/component/storage/proc/on_alt_click(datum/source, mob/user)
	...
	if(!user.incapacitated())
		...
		if(!user.put_in_hands(I))
			to_chat(user, "<span_class='notice'>You fumble for [I] and it falls on the floor.</span>")
			return
		user.visible_message("<span_class='warning'>[user] draws [I] from [parent]!</span>", "<span_class='notice'>You draw [I] from [parent].</span>")
		return
