/datum/component/storage/concrete/bluespace/bag_of_holding/handle_item_insertion(obj/item/W, prevent_warning = FALSE, mob/living/user)
	...
	if(istype(W, /obj/item/storage/backpack/holding) || matching.len)
		...
		if(is_reebe(loccheck.z))
			user.visible_message("<span_class='warning'>An unseen force knocks [user] to the ground!</span>", "<span_class='big_brass'>\"I think not!\"</span>")
			user.Paralyze(60)
			return
		if(istype(loccheck.loc, /area/fabric_of_reality))
			to_chat(user, "<span_class='danger'>You can't do that here!</span>")
		to_chat(user, "<span_class='danger'>The Bluespace interfaces of the two devices catastrophically malfunction!</span>")
		qdel(W)
		...
		for(var/turf/T in range(6,loccheck))
			...
			for(var/mob/living/M in T)
				if(M.movement_type & FLYING)
					M.visible_message("<span_class='danger'>The bluespace collapse crushes the air towards it, pulling [M] towards the ground...</span>")
