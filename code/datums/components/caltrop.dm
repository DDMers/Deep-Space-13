	...
	if(ishuman(AM))
		...
		if(cooldown < world.time - 10) //cooldown to avoid message spam.
			if(!H.incapacitated(ignore_restraints = TRUE))
				H.visible_message("<span_class='danger'>[H] steps on [A].</span>", \
						"<span_class='userdanger'>You step on [A]!</span>")
			else
				H.visible_message("<span_class='danger'>[H] slides on [A]!</span>", \
						"<span_class='userdanger'>You slide on [A]!</span>")
