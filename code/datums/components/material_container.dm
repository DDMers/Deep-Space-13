...
/datum/component/material_container/proc/OnExamine(datum/source, mob/user)
	if(show_on_examine)
		for(var/I in materials)
			...
			var/amt = amount(M.id)
			if(amt)
				to_chat(user, "<span_class='notice'>It has [amt] units of [lowertext(M.name)] stored.</span>")
...
/datum/component/material_container/proc/OnAttackBy(datum/source, obj/item/I, mob/living/user)
	...
	if((I.flags_1 & HOLOGRAM_1) || (I.item_flags & NO_MAT_REDEMPTION) || (tc && !is_type_in_typecache(I, tc)))
		to_chat(user, "<span_class='warning'>[parent] won't accept [I]!</span>")
		return
	...
	if(!material_amount)
		to_chat(user, "<span_class='warning'>[I] does not contain sufficient amounts of metal or glass to be accepted by [parent].</span>")
		return
	if(!has_space(material_amount))
		to_chat(user, "<span_class='warning'>[parent] is full. Please remove metal or glass from [parent] in order to insert more.</span>")
		return
	user_insert(I, user)
...
/datum/component/material_container/proc/user_insert(obj/item/I, mob/living/user)
	...
	if(!user.temporarilyRemoveItemFromInventory(I))
		to_chat(user, "<span_class='warning'>[I] is stuck to you and cannot be placed into [parent].</span>")
		return
	var/inserted = insert_item(I, stack_amt = requested_amount)
	if(inserted)
		if(istype(I, /obj/item/stack))
			var/obj/item/stack/S = I
			to_chat(user, "<span_class='notice'>You insert [inserted] [S.singular_name][inserted>1 ? "s" : ""] into [parent].</span>")
			...
			to_chat(user, "<span_class='notice'>You insert a material total of [inserted] into [parent].</span>")
			qdel(I)
		if(after_insert)
