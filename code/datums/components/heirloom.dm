...
/datum/component/heirloom/proc/examine(datum/source, mob/user)
	if(user.mind == owner)
		to_chat(user, "<span_class='notice'>It is your precious [family_name] family heirloom. Keep it safe!</span>")
	...
	if(creeper && creeper.trauma.obsession == owner)
		to_chat(user, "<span_class='nicegreen'>This must be [owner]'s family heirloom! It smells just like them...</span>")