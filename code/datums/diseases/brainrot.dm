...
/datum/disease/brainrot/stage_act() //Removed toxloss because damaging diseases are pretty horrible. Last round it killed the entire station because the cure didn't work -- Urist -ACTUALLY Removed rather than commented out, I don't see it returning - RR
	...
	switch(stage)
		if(2)
			...
			if(prob(2))
				to_chat(affected_mob, "<span_class='danger'>You don't feel like yourself.</span>")
			if(prob(5))
		...
		if(3)
			...
			if(prob(10))
				...
				affected_mob.updatehealth()
				if(prob(2))
					to_chat(affected_mob, "<span_class='danger'>Your try to remember something important...but can't.</span>")
		...
		if(4)
			...
			if(prob(15))
				...
				affected_mob.updatehealth()
				if(prob(2))
					to_chat(affected_mob, "<span_class='danger'>Strange buzzing fills your head, removing all thoughts.</span>")
			if(prob(3))
				to_chat(affected_mob, "<span_class='danger'>You lose consciousness...</span>")
				affected_mob.visible_message("<span_class='warning'>[affected_mob] suddenly collapses</span>")
