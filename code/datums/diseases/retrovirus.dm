		...
		if(1)
			if(restcure)
				if(!(affected_mob.mobility_flags & MOBILITY_STAND) && prob(30))
					...
					return
			if (prob(8))
				to_chat(affected_mob, "<span_class='danger'>Your head hurts.</span>")
			if (prob(9))
				to_chat(affected_mob, "You feel a tingling sensation in your chest.")
			if (prob(9))
				to_chat(affected_mob, "<span_class='danger'>You feel angry.</span>")
		if(2)
			if(restcure)
				if(!(affected_mob.mobility_flags & MOBILITY_STAND) && prob(20))
					...
					return
			if (prob(8))
				to_chat(affected_mob, "<span_class='danger'>Your skin feels loose.</span>")
			if (prob(10))
			...
			if (prob(4))
				to_chat(affected_mob, "<span_class='danger'>Your stomach churns.</span>")
		if(3)
			if(restcure)
				if(!(affected_mob.mobility_flags & MOBILITY_STAND) && prob(20))
					to_chat(affected_mob, "<span_class='notice'>You feel better.</span>")
					...
					return
			if (prob(10))
				to_chat(affected_mob, "<span_class='danger'>Your entire body vibrates.</span>")
