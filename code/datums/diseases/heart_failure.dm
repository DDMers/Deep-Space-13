...
/datum/disease/heart_failure/stage_act()
	...
	if(O && H.can_heartattack())
		switch(stage)
			if(1 to 2)
				if(prob(2))
					to_chat(H, "<span_class='warning'>You feel [pick("discomfort", "pressure", "a burning sensation", "pain")] in your chest.</span>")
				if(prob(2))
					to_chat(H, "<span_class='warning'>You feel dizzy.</span>")
					H.confused += 6
				if(prob(3))
					to_chat(H, "<span_class='warning'>You feel [pick("full", "nauseated", "sweaty", "weak", "tired", "short on breath", "uneasy")].</span>")
			if(3 to 4)
				if(!sound)
					H.playsound_local(H, 'sound/health/slowbeat.ogg',40,0, channel = CHANNEL_HEARTBEAT)
					sound = TRUE
				if(prob(3))
					to_chat(H, "<span_class='danger'>You feel a sharp pain in your chest!</span>")
					if(prob(25))
					...
					H.losebreath += 4
				if(prob(3))
					to_chat(H, "<span_class='danger'>You feel very weak and dizzy...</span>")
					H.confused += 8
			...
			if(5)
				...
				if(H.stat == CONSCIOUS)
					H.visible_message("<span_class='userdanger'>[H] clutches at [H.p_their()] chest as if [H.p_their()] heart is stopping!</span>")
