...
/datum/symptom/narcolepsy/Activate(var/datum/disease/advance/A)
	...
	switch(A.stage)
		if(1)
			if(prob(10))
				to_chat(M, "<span_class='warning'>You feel tired.</span>")
		if(2)
			if(prob(10))
				to_chat(M, "<span_class='warning'>You feel very tired.</span>")
				sleepy_ticks += rand(10,14)
				if(stamina)
					M.adjustStaminaLoss(10)
		if(3)
			if(prob(15))
				to_chat(M, "<span_class='warning'>You try to focus on staying awake.</span>")
				sleepy_ticks += rand(10,14)
				if(stamina)
					M.adjustStaminaLoss(15)
		if(4)
			if(prob(20))
				to_chat(M, "<span_class='warning'>You nod off for a moment.</span>")
				sleepy_ticks += rand(10,14)
				if(stamina)
					M.adjustStaminaLoss(20)
		if(5)
			if(prob(25))
				to_chat(M, "<span_class='warning'>[pick("So tired...","You feel very sleepy.","You have a hard time keeping your eyes open.","You try to stay awake.")]</span>")
