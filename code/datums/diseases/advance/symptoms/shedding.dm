...
/datum/symptom/shedding/Activate(datum/disease/advance/A)
	...
	if(prob(base_message_chance))
		to_chat(M, "<span_class='warning'>[pick("Your scalp itches.", "Your skin feels flaky.")]</span>")
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
		switch(A.stage)
			if(3, 4)
				if(!(H.hair_style == "Bald") && !(H.hair_style == "Balding Hair"))
					to_chat(H, "<span_class='warning'>Your hair starts to fall out in clumps...</span>")
