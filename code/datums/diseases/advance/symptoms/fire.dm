	...
	switch(A.stage)
		if(3)
			if(prob(base_message_chance) && !suppress_warning)
				to_chat(M, "<span_class='warning'>[pick("You feel hot.", "You hear a crackling noise.", "You smell smoke.")]</span>")
		if(4)
			Firestacks_stage_4(M, A)
			M.IgniteMob()
			to_chat(M, "<span_class='userdanger'>Your skin bursts into flames!</span>")
			M.emote("scream")
		if(5)
			Firestacks_stage_5(M, A)
			M.IgniteMob()
			to_chat(M, "<span_class='userdanger'>Your skin erupts into an inferno!</span>")
			M.emote("scream")
	...
	switch(A.stage)
		if(3)
			if(prob(base_message_chance))
				to_chat(M, "<span_class='warning'>[pick("Your veins boil.", "You feel hot.", "You smell meat cooking.")]</span>")
		if(4)
			Alkali_fire_stage_4(M, A)
			M.IgniteMob()
			to_chat(M, "<span_class='userdanger'>Your sweat bursts into flames!</span>")
			M.emote("scream")
		if(5)
			...
			M.emote("scream")
			if(M.fire_stacks < 0)
				M.visible_message("<span_class='warning'>[M]'s sweat sizzles and pops on contact with water!</span>")
