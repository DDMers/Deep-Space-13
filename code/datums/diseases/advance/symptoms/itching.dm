...
/datum/symptom/itching/Activate(datum/disease/advance/A)
	...
	if(bodypart && bodypart.status == BODYPART_ORGANIC && !bodypart.is_pseudopart)	 //robotic limbs will mean less scratching overall
		...
		M.visible_message("[can_scratch ? "<span_class='warning'>[M] scratches [M.p_their()] [bodypart.name].</span>" : ""]", "<span_class='warning'>Your [bodypart.name] itches. [can_scratch ? " You scratch it." : ""]</span>")
		if(can_scratch)
