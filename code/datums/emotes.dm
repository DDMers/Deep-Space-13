...
/datum/emote/proc/can_run_emote(mob/user, status_check = TRUE, intentional = FALSE)
	...
	if(status_check && !is_type_in_typecache(user, mob_type_ignore_stat_typecache))
		if(user.stat > stat_allowed)
			...
			switch(user.stat)
				if(SOFT_CRIT)
					to_chat(user, "<span_class='notice'>You cannot [key] while in a critical condition.</span>")
				if(UNCONSCIOUS)
					to_chat(user, "<span_class='notice'>You cannot [key] while unconscious.</span>")
				if(DEAD)
					to_chat(user, "<span_class='notice'>You cannot [key] while dead.</span>")
			return FALSE
		if(restraint_check)
			if(isliving(user))
				var/mob/living/L = user
				if(L.IsParalyzed() || L.IsStun())
					if(!intentional)
						return FALSE
					to_chat(user, "<span_class='notice'>You cannot [key] while stunned.</span>")
					return FALSE
		if(restraint_check && user.restrained())
			if(!intentional)
				return FALSE
			to_chat(user, "<span_class='notice'>You cannot [key] while restrained.</span>")
			return FALSE
