...
/obj/screen/area_creator/Click()
	...
	if(!A.outdoors)
		to_chat(usr, "<span_class='warning'>There is already a defined structure here.</span>")
		return TRUE
	create_area(usr)
...
/obj/screen/internals/Click()
	...
	if(C.internal)
		C.internal = null
		to_chat(C, "<span_class='notice'>You are no longer running on internals.</span>")
		icon_state = "internal0"
	else
		if(!C.getorganslot(ORGAN_SLOT_BREATHING_TUBE))
			if(!istype(C.wear_mask, /obj/item/clothing/mask))
				to_chat(C, "<span_class='warning'>You are not wearing an internals mask!</span>")
				return 1
			else
		...
		if(I)
			to_chat(C, "<span_class='notice'>You are now running on internals from [I] in your [C.get_held_index_name(C.get_held_index_of_item(I))].</span>")
			C.internal = I
		else if(ishuman(C))
			var/mob/living/carbon/human/H = C
			if(istype(H.s_store, /obj/item/tank))
				to_chat(H, "<span_class='notice'>You are now running on internals from [H.s_store] on your [H.wear_suit.name].</span>")
				H.internal = H.s_store
			else if(istype(H.belt, /obj/item/tank))
				to_chat(H, "<span_class='notice'>You are now running on internals from [H.belt] on your belt.</span>")
				H.internal = H.belt
			else if(istype(H.l_store, /obj/item/tank))
				to_chat(H, "<span_class='notice'>You are now running on internals from [H.l_store] in your left pocket.</span>")
				H.internal = H.l_store
			else if(istype(H.r_store, /obj/item/tank))
				to_chat(H, "<span_class='notice'>You are now running on internals from [H.r_store] in your right pocket.</span>")
				H.internal = H.r_store

		//Separate so CO2 jetpacks are a little less cumbersome.
		if(!C.internal && istype(C.back, /obj/item/tank))
			to_chat(C, "<span_class='notice'>You are now running on internals from [C.back] on your back.</span>")
			C.internal = C.back

		if(C.internal)
			icon_state = "internal1"
		else
			to_chat(C, "<span_class='warning'>You don't have an oxygen tank!</span>")
			return
