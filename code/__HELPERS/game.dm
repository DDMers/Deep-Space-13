...
/proc/showCandidatePollWindow(mob/M, poll_time, Question, list/candidates, ignore_category, time_passed, flashwindow = TRUE)
	...
	switch(ignore_category ? askuser(M,Question,"Please answer in [DisplayTimeText(poll_time)]!","Yes","No","Never for this round", StealFocus=0, Timeout=poll_time) : askuser(M,Question,"Please answer in [DisplayTimeText(poll_time)]!","Yes","No", StealFocus=0, Timeout=poll_time))
		if(1)
			to_chat(M, "<span_class='notice'>Choice registered: Yes.</span>")
			if(time_passed + poll_time <= world.time)
				to_chat(M, "<span_class='danger'>Sorry, you answered too late to be considered!</span>")
				SEND_SOUND(M, 'sound/machines/buzz-sigh.ogg')
				candidates -= M
			else
				candidates += M
		if(2)
			to_chat(M, "<span_class='danger'>Choice registered: No.</span>")
			candidates -= M
		if(3)
			...
			to_chat(M, "<span_class='danger'>Choice registered: Never for this round.</span>")
			candidates -= M
		else
			candidates -= M
...
/proc/AnnounceArrival(var/mob/living/carbon/human/character, var/rank)
	...
	var/message = "<span_class='game deadsay'><span_class='name'>\
		[character.real_name]</span> ([rank]) has arrived at the station at \
		<span_class='name'>[A.name]</span>.</span>"
