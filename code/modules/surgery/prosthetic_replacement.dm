...
/datum/surgery_step/add_prosthetic/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	if(istype(tool, /obj/item/organ_storage))
		if(!tool.contents.len)
			to_chat(user, "<span_class='notice'>There is nothing inside [tool]!</span>")
			return -1
		var/obj/item/I = tool.contents[1]
		if(!isbodypart(I))
			to_chat(user, "<span_class='notice'>[I] cannot be attached!</span>")
			return -1
		tool = I
	if(istype(tool, /obj/item/bodypart))
		...
		if(ismonkey(target))// monkey patient only accept organic monkey limbs
			if(BP.status == BODYPART_ROBOTIC || BP.animal_origin != MONKEY_BODYPART)
				to_chat(user, "<span_class='warning'>[BP] doesn't match the patient's morphology.</span>")
				return -1
		...
		if(target_zone == BP.body_zone) //so we can't replace a leg with an arm, or a human arm with a monkey arm.
			user.visible_message("[user] begins to replace [target]'s [parse_zone(target_zone)].", "<span_class ='notice'>You begin to replace [target]'s [parse_zone(target_zone)]...</span>")
		else
			to_chat(user, "<span_class='warning'>[tool] isn't the right type for [parse_zone(target_zone)].</span>")
			return -1
	else if(target_zone == BODY_ZONE_L_ARM || target_zone == BODY_ZONE_R_ARM)
		user.visible_message("[user] begins to attach [tool] onto [target].", "<span_class='notice'>You begin to attach [tool] onto [target]...</span>")
	else
		to_chat(user, "<span_class='warning'>[tool] must be installed onto an arm.</span>")
		return -1
...
/datum/surgery_step/add_prosthetic/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(istype(tool, /obj/item/bodypart) && user.temporarilyRemoveItemFromInventory(tool))
		...
		user.visible_message("[user] successfully replaces [target]'s [parse_zone(target_zone)]!", "<span_class='notice'>You succeed in replacing [target]'s [parse_zone(target_zone)].</span>")
		return 1
	else
		var/obj/item/bodypart/L = target.newBodyPart(target_zone, FALSE, FALSE)
		L.is_pseudopart = TRUE
		L.attach_limb(target)
		user.visible_message("[user] finishes attaching [tool]!", "<span_class='notice'>You attach [tool].</span>")
		qdel(tool)
