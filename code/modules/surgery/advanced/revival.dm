...
/datum/surgery_step/revive/tool_check(mob/user, obj/item/tool)
	...
	if(istype(tool, /obj/item/twohanded/shockpaddles))
		...
		if((S.req_defib && !S.defib.powered) || !S.wielded || S.cooldown || S.busy)
			to_chat(user, "<span_class='warning'>You need to wield both paddles, and [S.defib] must be powered!</span>")
			return FALSE
	if(istype(tool, /obj/item/melee/baton))
		var/obj/item/melee/baton/B = tool
		if(!B.status)
			to_chat(user, "<span_class='warning'>[B] needs to be active!</span>")
			return FALSE
	if(istype(tool, /obj/item/gun/energy))
		...
		if(E.chambered && istype(E.chambered, /obj/item/ammo_casing/energy/electrode))
			...
			to_chat(user, "<span_class='warning'>You need an electrode for this!</span>")
			return FALSE

/datum/surgery_step/revive/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	user.visible_message("[user] prepares to shock [target]'s brain with [tool].", "<span_class='notice'>You prepare to give [target]'s brain the spark of life with [tool].</span>")
...
/datum/surgery_step/revive/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	user.visible_message("[user] send a powerful shock to [target]'s brain with [tool]...", "<span_class='notice'>You successfully shock [target]'s brain with [tool]...</span>")
	playsound(get_turf(target), 'sound/magic/lightningbolt.ogg', 50, 1)
	target.adjustOxyLoss(-50, 0)
	target.updatehealth()
	if(target.revive())
		user.visible_message("...[target] wakes up, alive and aware!", "<span_class='notice'><b>IT'S ALIVE!</b></span>")
		target.emote("gasp")
...
/datum/surgery_step/revive/failure(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	user.visible_message("[user] send a powerful shock to [target]'s brain with [tool], but [target.p_they()] doesn't react.", "<span_class='notice'>You shock [target]'s brain with [tool], but [target.p_they()] doesn't react.</span>")
	playsound(get_turf(target), 'sound/magic/lightningbolt.ogg', 50, 1)
