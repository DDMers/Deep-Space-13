...
/obj/item/organ/cyberimp/arm/examine(mob/user)
	..()
	to_chat(user, "<span_class='info'>[src] is assembled in the [zone == BODY_ZONE_R_ARM ? "right" : "left"] arm configuration. You can use a screwdriver to reassemble it.</span>")
...
/obj/item/organ/cyberimp/arm/screwdriver_act(mob/living/user, obj/item/I)
	...
	if(zone == BODY_ZONE_R_ARM)
		...
		zone = BODY_ZONE_R_ARM
	SetSlotFromZone()
	to_chat(user, "<span_class='notice'>You modify [src] to be installed on the [zone == BODY_ZONE_R_ARM ? "right" : "left"] arm.</span>")
	update_icon()
...
/obj/item/organ/cyberimp/arm/emp_act(severity)
	...
	if(prob(15/severity) && owner)
		to_chat(owner, "<span_class='warning'>[src] is hit by EMP!</span>")
...
/obj/item/organ/cyberimp/arm/proc/Retract()
	...
	owner.visible_message("<span_class='notice'>[owner] retracts [holder] back into [owner.p_their()] [zone == BODY_ZONE_R_ARM ? "right" : "left"] arm.</span>",
		"<span_class='notice'>[holder] snaps back into your [zone == BODY_ZONE_R_ARM ? "right" : "left"] arm.</span>",
		"<span_class='italics'>You hear a short mechanical noise.</span>")
...
/obj/item/organ/cyberimp/arm/proc/Extend(var/obj/item/item)
	...
	if(arm_item)
		if(!owner.dropItemToGround(arm_item))
			to_chat(owner, "<span_class='warning'>Your [arm_item] interferes with [src]!</span>")
			return
		else
			to_chat(owner, "<span_class='notice'>You drop [arm_item] to activate [src]!</span>")

	var/result = (zone == BODY_ZONE_R_ARM ? owner.put_in_r_hand(holder) : owner.put_in_l_hand(holder))
	if(!result)
		to_chat(owner, "<span_class='warning'>Your [name] fails to activate!</span>")
		return

	// Activate the hand that now holds our item.
	owner.swap_hand(result)//... or the 1st hand if the index gets lost somehow

	owner.visible_message("<span_class='notice'>[owner] extends [holder] from [owner.p_their()] [zone == BODY_ZONE_R_ARM ? "right" : "left"] arm.</span>",
		"<span_class='notice'>You extend [holder] from your [zone == BODY_ZONE_R_ARM ? "right" : "left"] arm.</span>",
		"<span_class='italics'>You hear a short mechanical noise.</span>")
	playsound(get_turf(owner), 'sound/mecha/mechmove03.ogg', 50, 1)

/obj/item/organ/cyberimp/arm/ui_action_click()
	if(crit_fail || (!holder && !contents.len))
		to_chat(owner, "<span_class='warning'>The implant doesn't respond. It seems to be broken...</span>")
		return
...
/obj/item/organ/cyberimp/arm/gun/emp_act(severity)
	...
	if(prob(30/severity) && owner && !crit_fail)
		Retract()
		owner.visible_message("<span_class='danger'>A loud bang comes from [owner]\'s [zone == BODY_ZONE_R_ARM ? "right" : "left"] arm!</span>")
		playsound(get_turf(owner), 'sound/weapons/flashbang.ogg', 100, 1)
		to_chat(owner, "<span_class='userdanger'>You feel an explosion erupt inside your [zone == BODY_ZONE_R_ARM ? "right" : "left"] arm as your implant breaks!</span>")
...
/obj/item/organ/cyberimp/arm/toolset/emag_act()
	if(!(locate(/obj/item/kitchen/knife/combat/cyborg) in items_list))
		to_chat(usr, "<span_class='notice'>You unlock [src]'s integrated knife!</span>")
