...
/obj/item/organ/vocal_cords/adamantine/handle_speech(message)
	var/msg = "<span_class='resonate'><span_class='name'>[owner.real_name]</span> <span_class='message'>resonates, \"[message]\"</span></span>"
...
/datum/action/item_action/organ_action/colossus/Trigger()
	...
	if(!IsAvailable())
		if(world.time < cords.next_command)
			to_chat(owner, "<span_class='notice'>You must wait [DisplayTimeText(cords.next_command - world.time)] before Speaking again.</span>")
		return
...
/obj/item/organ/vocal_cords/colossus/can_speak_with()
	if(world.time < next_command)
		to_chat(owner, "<span_class='notice'>You must wait [DisplayTimeText(next_command - world.time)] before Speaking again.</span>")
		return FALSE
	if(!owner)
		return FALSE
	if(!owner.can_speak())
		to_chat(owner, "<span_class='warning'>You are unable to speak!</span>")
		return FALSE
	return TRUE
