...
/obj/item/autosurgeon/attack_self(mob/user)//when the object it used...
	if(!uses)
		to_chat(user, "<span_class='warning'>[src] has already been used. The tools are dull and won't reactivate.</span>")
		return
	else if(!storedorgan)
		to_chat(user, "<span_class='notice'>[src] currently has no implant stored.</span>")
		return
	storedorgan.Insert(user)//insert stored organ into the user
	user.visible_message("<span_class='notice'>[user] presses a button on [src], and you hear a short mechanical noise.</span>", "<span_class='notice'>You feel a sharp sting as [src] plunges into your body.</span>")
...
/obj/item/autosurgeon/attackby(obj/item/I, mob/user, params)
	if(istype(I, organ_type))
		if(storedorgan)
			to_chat(user, "<span_class='notice'>[src] already has an implant stored.</span>")
			return
		else if(!uses)
			to_chat(user, "<span_class='notice'>[src] has already been used up.</span>")
			return
		if(!user.transferItemToLoc(I, src))
			return
		storedorgan = I
		to_chat(user, "<span_class='notice'>You insert the [I] into [src].</span>")
	else
		return ..()
...
/obj/item/autosurgeon/screwdriver_act(mob/living/user, obj/item/I)
	if(..())
		return TRUE
	if(!storedorgan)
		to_chat(user, "<span_class='notice'>There's no implant in [src] for you to remove.</span>")
	else
		...
		to_chat(user, "<span_class='notice'>You remove the [storedorgan] from [src].</span>")
