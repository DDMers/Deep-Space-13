...
/obj/machinery/gateway/centerstation/toggleon(mob/user)
	...
	if(!powered())
		return
	if(!awaygate)
		to_chat(user, "<span_class='notice'>Error: No destination found.</span>")
		return
	if(world.time < wait)
		to_chat(user, "<span_class='notice'>Error: Warpspace triangulation in progress. Estimated time to completion: [DisplayTimeText(wait - world.time)].</span>")
		return
...
/obj/machinery/gateway/centeraway/multitool_act(mob/living/user, obj/item/I)
	if(calibrated)
		...
		to_chat(user, "<span_class='boldnotice'>Recalibration successful!</span>: \black This gate's systems have been fine tuned.  Travel to this gate will now be on target.")
		calibrated = TRUE
	return TRUE
