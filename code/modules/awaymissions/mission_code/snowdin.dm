...
/turf/open/lava/plasma/attackby(obj/item/I, mob/user, params)
	...
	if(C.reagents.total_volume >= C.volume)
		to_chat(user, "<span_class='danger'>[C] is full.</span>")
		return
	C.reagents.add_reagent("plasma", rand(5, 10))
	user.visible_message("[user] scoops some plasma from the [src] with \the [C].", "<span_class='notice'>You scoop out some plasma from the [src] using \the [C].</span>")
				...
				if(ishuman(L))//are they a carbon?
					...
					if(prob(35)) //checking if the delay is over & if the victim actually has any parts to nom
						...
						if(plasma_parts.len)
							...
							PP.visible_message("<span_class='warning'>[L] screams in pain as [L.p_their()] [NB] melts down to the bone!</span>", \
											  "<span_class='userdanger'>You scream out in pain as your [NB] melts down to the bone, leaving an eerie plasma-like glow where flesh used to be!</span>")
						if(!plasma_parts.len && !robo_parts.len) //a person with no potential organic limbs left AND no robotic limbs, time to turn them into a plasmaman
							PP.IgniteMob()
							PP.set_species(/datum/species/plasmaman)
							PP.visible_message("<span_class='warning'>[L] bursts into a brilliant purple flame as [L.p_their()] entire body is that of a skeleton!</span>", \
											  "<span_class='userdanger'>Your senses numb as all of your remaining flesh is turned into a purple slurry, sloshing off your body and leaving only your bones to show in a vibrant purple!</span>")
...
/obj/effect/mob_spawn/human/syndicatesoldier/coldres/alive
	...
	flavour_text = "<span_class='big bold'>You are a syndicate operative recently awoken from cryostasis in an underground outpost. Monitor Nanotrasen communications and record information. All intruders should be \
