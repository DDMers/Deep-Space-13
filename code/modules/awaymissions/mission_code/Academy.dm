...
/obj/structure/academy_wizard_spawner/deconstruct(disassembled = TRUE)
	if(!broken)
		broken = 1
		visible_message("<span_class='warning'>[src] breaks down!</span>")
...
/obj/item/dice/d20/fate/diceroll(mob/user)
	...
	if(!used)
		if(!ishuman(user) || !user.mind || (user.mind in SSticker.mode.wizards))
			to_chat(user, "<span_class='warning'>You feel the magic of the dice is restricted to ordinary humans!</span>")
			return
		...
		T.visible_message("<span_class='userdanger'>[src] flares briefly.</span>")

		addtimer(CALLBACK(src, .proc/effect, user, .), 1 SECONDS)

/obj/item/dice/d20/fate/equipped(mob/user, slot)
	if(!ishuman(user) || !user.mind || (user.mind in SSticker.mode.wizards))
		to_chat(user, "<span_class='warning'>You feel the magic of the dice is restricted to ordinary humans! You should leave it alone.</span>")
...
/obj/item/dice/d20/fate/proc/effect(var/mob/living/carbon/human/user,roll)
	...
	switch(roll)
		if(1)
			//Dust
			T.visible_message("<span_class='userdanger'>[user] turns to dust!</span>")
			user.dust()
		if(2)
			//Death
			T.visible_message("<span_class='userdanger'>[user] suddenly dies!</span>")
			user.death()
		if(3)
			//Swarm of creatures
			T.visible_message("<span_class='userdanger'>A swarm of creatures surround [user]!</span>")
			for(var/direction in GLOB.alldirs)
				new /mob/living/simple_animal/hostile/netherworld(get_step(get_turf(user),direction))
		if(4)
			//Destroy Equipment
			T.visible_message("<span_class='userdanger'>Everything [user] is holding and wearing disappears!</span>")
		...
		if(5)
			//Monkeying
			T.visible_message("<span_class='userdanger'>[user] transforms into a monkey!</span>")
			user.monkeyize()
		if(6)
			//Cut speed
			T.visible_message("<span_class='userdanger'>[user] starts moving slower!</span>")
			user.add_movespeed_modifier(MOVESPEED_ID_DIE_OF_FATE, update=TRUE, priority=100, multiplicative_slowdown=1)
		if(7)
			//Throw
			T.visible_message("<span_class='userdanger'>Unseen forces throw [user]!</span>")
			user.Stun(60)
		...
		if(8)
			//Fueltank Explosion
			T.visible_message("<span_class='userdanger'>An explosion bursts into existence around [user]!</span>")
			explosion(get_turf(user),-1,0,2, flame_range = 2)
		if(9)
			//Cold
			var/datum/disease/D = new /datum/disease/cold()
			T.visible_message("<span_class='userdanger'>[user] looks a little under the weather!</span>")
			user.ForceContractDisease(D, FALSE, TRUE)
		if(10)
			//Nothing
			T.visible_message("<span_class='userdanger'>Nothing seems to happen.</span>")
		if(11)
			//Cookie
			T.visible_message("<span_class='userdanger'>A cookie appears out of thin air!</span>")
			var/obj/item/reagent_containers/food/snacks/cookie/C = new(drop_location())
			do_smoke(0, drop_location())
			C.name = "Cookie of Fate"
		if(12)
			//Healing
			T.visible_message("<span_class='userdanger'>[user] looks very healthy!</span>")
			user.revive(full_heal = 1, admin_revive = 1)
		if(13)
			//Mad Dosh
			T.visible_message("<span_class='userdanger'>Mad dosh shoots out of [src]!</span>")
		...
		if(14)
			//Free Gun
			T.visible_message("<span_class='userdanger'>An impressive gun appears!</span>")
			do_smoke(0, drop_location())
			new /obj/item/gun/ballistic/revolver/mateba(drop_location())
		if(15)
			//Random One-use spellbook
			T.visible_message("<span_class='userdanger'>A magical looking book drops to the floor!</span>")
			do_smoke(0, drop_location())
			new /obj/item/book/granter/spell/random(drop_location())
		if(16)
			//Servant & Servant Summon
			T.visible_message("<span_class='userdanger'>A Dice Servant appears in a cloud of smoke!</span>")
		...
		if(17)
			//Tator Kit
			T.visible_message("<span_class='userdanger'>A suspicious box appears!</span>")
			new /obj/item/storage/box/syndicate(drop_location())
			do_smoke(0, drop_location())
		if(18)
			//Captain ID
			T.visible_message("<span_class='userdanger'>A golden identification card appears!</span>")
			new /obj/item/card/id/captains_spare(drop_location())
			do_smoke(0, drop_location())
		if(19)
			//Instrinct Resistance
			T.visible_message("<span_class='userdanger'>[user] looks very robust!</span>")
			user.physiology.brute_mod *= 0.5
			user.physiology.burn_mod *= 0.5

		if(20)
			//Free wizard!
			T.visible_message("<span_class='userdanger'>Magic flows out of [src] and into [user]!</span>")
...
/obj/structure/ladder/unbreakable/rune/show_fluff_message(up,mob/user)
	user.visible_message("[user] activates \the [src].","<span_class='notice'>You activate \the [src].</span>")
