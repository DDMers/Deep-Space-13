...
/obj/structure/bookcase/examine(mob/user)
	..()
	if(!anchored)
		to_chat(user, "<span_class='notice'>The <i>bolts</i> on the bottom are unsecured.</span>")
	if(anchored)
		to_chat(user, "<span_class='notice'>It's secured in place with <b>bolts</b>.</span>")
	switch(state)
		if(0)
			to_chat(user, "<span_class='notice'>There's a <b>small crack</b> visible on the back panel.</span>")
		if(1)
			to_chat(user, "<span_class='notice'>There's space inside for a <i>wooden</i> shelf.</span>")
		if(2)
			to_chat(user, "<span_class='notice'>There's a <b>small crack</b> visible on the shelf.</span>")
	...
	switch(state)
		if(0)
			if(I.tool_behaviour == TOOL_WRENCH)
				if(I.use_tool(src, user, 20, volume=50))
					to_chat(user, "<span_class='notice'>You wrench the frame into place.</span>")
					anchored = TRUE
					state = 1
			if(I.tool_behaviour == TOOL_CROWBAR)
				if(I.use_tool(src, user, 20, volume=50))
					to_chat(user, "<span_class='notice'>You pry the frame apart.</span>")
					deconstruct(TRUE)
		...
		if(1)
			if(istype(I, /obj/item/stack/sheet/mineral/wood))
				...
				if(W.get_amount() >= 2)
					W.use(2)
					to_chat(user, "<span_class='notice'>You add a shelf.</span>")
					state = 2
					icon_state = "book-0"
			if(I.tool_behaviour == TOOL_WRENCH)
				I.play_tool_sound(src, 100)
				to_chat(user, "<span_class='notice'>You unwrench the frame.</span>")
				anchored = FALSE
				state = 0
		...
		if(2)
			...
			if(is_type_in_list(I, allowed_books))
				...
				to_chat(user, "<span_class='notice'>You empty \the [I] into \the [src].</span>")
				update_icon()
			else if(istype(I, /obj/item/pen))
				if(!user.is_literate())
					to_chat(user, "<span_class='notice'>You scribble illegibly on the side of [src]!</span>")
					return
				...
				if(contents.len)
					to_chat(user, "<span_class='warning'>You need to remove the books first!</span>")
				else
					I.play_tool_sound(src, 100)
					to_chat(user, "<span_class='notice'>You pry the shelf out.</span>")
...
/obj/item/book/attack_self(mob/user)
	...
	if(dat)
		...
		to_chat(user, "<span_class='notice'>This book is completely blank!</span>")


/obj/item/book/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/pen))
		if(is_blind(user))
			to_chat(user, "<span_class='warning'> As you are trying to write on the book, you suddenly feel very stupid!</span>")
			return
		if(unique)
			to_chat(user, "<span_class='warning'>These pages don't seem to take the ink well! Looks like you can't modify it.</span>")
			return
		var/literate = user.is_literate()
		if(!literate)
			to_chat(user, "<span_class='notice'>You scribble illegibly on the cover of [src]!</span>")
			return
		...
		to_chat(user, "<span_class='notice'>You begin to carve out [title]...</span>")
		if(do_after(user, 30, target = src))
			to_chat(user, "<span_class='notice'>You carve out the pages from [title]! You didn't want to read it anyway.</span>")
