...
/obj/item/soapstone/afterattack(atom/target, mob/user, proximity)
	...
	if(!remaining_uses && !existing_message)
		to_chat(user, "<span_class='warning'>[src] is too worn out to use.</span>")
		return

	if(!good_chisel_message_location(T))
		to_chat(user, "<span_class='warning'>It's not appropriate to engrave on [T].</span>")
		return

	if(existing_message)
		user.visible_message("<span_class='notice'>[user] starts erasing [existing_message].</span>", "<span_class='notice'>You start erasing [existing_message].</span>", "<span_class='italics'>You hear a chipping sound.</span>")
		playsound(loc, 'sound/items/gavel.ogg', 50, 1, -1)
		if(do_after(user, tool_speed, target = existing_message))
			user.visible_message("<span_class='notice'>[user] erases [existing_message].</span>", "<span_class='notice'>You erase [existing_message][existing_message.creator_key == user.ckey ? ", refunding a use" : ""].</span>")
	...
	if(!message)
		to_chat(user, "<span_class='notice'>You decide not to engrave anything.</span>")
		return

	if(!target.Adjacent(user) && locate(/obj/structure/chisel_message) in T)
		to_chat(user, "<span_class='warning'>Someone wrote here before you chose! Find another spot.</span>")
		return
	playsound(loc, 'sound/items/gavel.ogg', 50, 1, -1)
	user.visible_message("<span_class='notice'>[user] starts engraving a message into [T]...</span>", "<span_class='notice'>You start engraving a message into [T]...</span>", "<span_class='italics'>You hear a chipping sound.</span>")
	if(can_use() && do_after(user, tool_speed, target = T) && can_use()) //This looks messy but it's actually really clever!
		if(!locate(/obj/structure/chisel_message) in T)
			user.visible_message("<span_class='notice'>[user] leaves a message for future spacemen!</span>", "<span_class='notice'>You engrave a message into [T]!</span>", "<span_class='italics'>You hear a chipping sound.</span>")
			playsound(loc, 'sound/items/gavel.ogg', 50, 1, -1)
