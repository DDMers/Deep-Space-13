...
/obj/item/book/codex_gigas/attack_self(mob/user)
	if(!user.can_read(src))
		return FALSE
	if(inUse)
		to_chat(user, "<span_class='notice'>Someone else is reading it.</span>")
	if(ishuman(user))
		var/mob/living/carbon/human/U = user
		if(U.check_acedia())
			to_chat(user, "<span_class='notice'>None of this matters, why are you reading this? You put [title] down.</span>")
			return
