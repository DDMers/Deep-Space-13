...
/obj/machinery/power/terminal/proc/dismantle(mob/living/user, obj/item/I)
	if(isturf(loc))
		var/turf/T = loc
		if(T.intact)
			to_chat(user, "<span_class='warning'>You must first expose the power terminal!</span>")
			return
	...
	user.visible_message("[user.name] dismantles the power terminal from [master].",
		"<span_class='notice'>You begin to cut the cables...</span>")
	...
	if(I.use_tool(src, user, 50))
		...
		to_chat(user, "<span_class='notice'>You cut the cables and dismantle the power terminal.</span>")
		qdel(src)
