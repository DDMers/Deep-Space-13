...
/obj/machinery/power/rad_collector/interact(mob/user)
	if(anchored)
		if(!src.locked)
			...
			"<span_class='notice'>You turn the [src.name] [active? "on":"off"].</span>")
			var/fuel
			if(loaded_tank)
...
/obj/machinery/power/rad_collector/can_be_unfasten_wrench(mob/user, silent)
	if(loaded_tank)
		if(!silent)
			to_chat(user, "<span_class='warning'>Remove the plasma tank first!</span>")
...
/obj/machinery/power/rad_collector/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/tank/internals/plasma))
		...
		if(loaded_tank)
			...
			return TRUE
		if(panel_open)
			to_chat(user, "<span_class='warning'>Close the maintenance panel first!</span>")
			return TRUE
		...
		if(allowed(user))
			if(active)
				...
				to_chat(user, "<span_class='warning'>The controls can only be locked when \the [src] is active!</span>")
		else
...
/obj/machinery/power/rad_collector/crowbar_act(mob/living/user, obj/item/I)
	...
	if(default_deconstruction_crowbar(I))
		return TRUE
	to_chat(user, "<span_class='warning'>There isn't a tank loaded!</span>")
	return TRUE

/obj/machinery/power/rad_collector/multitool_act(mob/living/user, obj/item/I)
	if(!is_station_level(z) && !SSresearch.science_tech)
		to_chat(user, "<span_class='warning'>[src] isn't linked to a research system!</span>")
		return TRUE
	if(locked)
		to_chat(user, "<span_class='warning'>[src] is locked!</span>")
		return TRUE
	if(active)
		to_chat(user, "<span_class='warning'>[src] is currently active, producing [bitcoinmining ? "research points":"power"].</span>")
		return TRUE
	bitcoinmining = !bitcoinmining
	to_chat(user, "<span_class='warning'>You [bitcoinmining ? "enable":"disable"] the research point production feature of [src].</span>")
	return TRUE
...
/obj/machinery/power/rad_collector/examine(mob/user)
	...
	if(active)
		if(!bitcoinmining)
			to_chat(user, "<span_class='notice'>[src]'s display states that it has stored <b>[DisplayPower(stored_power)]</b>, and processing <b>[DisplayPower(RAD_COLLECTOR_OUTPUT)]</b>.</span>")
		else
			to_chat(user, "<span_class='notice'>[src]'s display states that it has stored a total of <b>[stored_power*RAD_COLLECTOR_MINING_CONVERSION_RATE]</b>, and producing [RAD_COLLECTOR_OUTPUT*RAD_COLLECTOR_MINING_CONVERSION_RATE] research points per minute.</span>")
	else
		if(!bitcoinmining)
			to_chat(user,"<span_class='notice'><b>[src]'s display displays the words:</b> \"Power production mode. Please insert <b>Plasma</b>. Use a multitool to change production modes.\"</span>")
		else
			to_chat(user,"<span_class='notice'><b>[src]'s display displays the words:</b> \"Research point production mode. Please insert <b>Tritium</b> and <b>Oxygen</b>. Use a multitool to change production modes.\"</span>")
