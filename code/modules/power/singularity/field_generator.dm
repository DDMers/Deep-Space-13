	...
	if(state == FG_WELDED)
		if(get_dist(src, user) <= 1)//Need to actually touch the thing to turn it on
			if(active >= FG_CHARGING)
				to_chat(user, "<span_class='warning'>You are unable to turn off [src] once it is online!</span>")
				return 1
			else
				user.visible_message("[user] turns on [src].", \
					"<span_class='notice'>You turn on [src].</span>", \
...
/obj/machinery/field/generator/welder_act(mob/living/user, obj/item/I)
	if(active)
		to_chat(user, "<span_class='warning'>[src] needs to be off!</span>")
		return TRUE

	switch(state)
		if(FG_UNSECURED)
			to_chat(user, "<span_class='warning'>[src] needs to be wrenched to the floor!</span>")
		...
		if(FG_SECURED)
			...
			if(I.use_tool(src, user, 20, volume=50) && state == FG_SECURED)
				state = FG_WELDED
				to_chat(user, "<span_class='notice'>You weld the field generator to the floor.</span>")
...
/obj/machinery/field/generator/proc/calc_power(set_power_draw)
	...
	if(draw_power(round(power_draw/2,1)))
		...
		visible_message("<span_class='danger'>The [name] shuts down!</span>", "<span_class='italics'>You hear something shutting down.</span>")
		turn_off()
