...
/obj/machinery/power/emitter/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		to_chat(user, "<span_class='notice'>The status display reads: Emitting one beam each <b>[fire_delay*0.1]</b> seconds.<br>Power consumption at <b>[active_power_usage]W</b>.<span>")
...
/obj/machinery/power/emitter/proc/can_be_rotated(mob/user,rotation_type)
	if (anchored)
		to_chat(user, "<span_class='warning'>It is fastened to the floor!</span>")
		return FALSE
	return TRUE
...
/obj/machinery/power/emitter/interact(mob/user)
	...
	if(state == EMITTER_WELDED)
		if(!powernet)
			to_chat(user, "<span_class='warning'>\The [src] isn't connected to a wire!</span>")
			return TRUE
		if(!locked && allow_switch_interact)
			if(active == TRUE)
				active = FALSE
				to_chat(user, "<span_class='notice'>You turn off [src].</span>")
			else
				active = TRUE
				to_chat(user, "<span_class='notice'>You turn on [src].</span>")
				shot_number = 0
		...
		to_chat(user, "<span_class='warning'>[src] needs to be firmly secured to the floor first!</span>")
		return TRUE
...
/obj/machinery/power/emitter/attack_animal(mob/living/simple_animal/M)
	if(ismegafauna(M) && anchored)
		...
		anchored = FALSE
		M.visible_message("<span_class='warning'>[M] rips [src] free from its moorings!</span>")
	else
		..()
	if(!anchored)
...
/obj/machinery/power/emitter/can_be_unfasten_wrench(mob/user, silent)
	if(active)
		if(!silent)
			to_chat(user, "<span_class='warning'>Turn \the [src] off first!</span>")
		return FAILED_UNFASTEN

	else if(state == EMITTER_WELDED)
		if(!silent)
			to_chat(user, "<span_class='warning'>[src] is welded to the floor!</span>")
		return FAILED_UNFASTEN
...
/obj/machinery/power/emitter/welder_act(mob/living/user, obj/item/I)
	...
	switch(state)
		if(EMITTER_UNWRENCHED)
			to_chat(user, "<span_class='warning'>The [src.name] needs to be wrenched to the floor!</span>")
		if(EMITTER_WRENCHED)
			if(!I.tool_start_check(user, amount=0))
				return TRUE
			user.visible_message("[user.name] starts to weld the [name] to the floor.", \
				"<span_class='notice'>You start to weld \the [src] to the floor...</span>", \
				"<span_class='italics'>You hear welding.</span>")
			if(I.use_tool(src, user, 20, volume=50))
				state = EMITTER_WELDED
				to_chat(user, "<span_class='notice'>You weld \the [src] to the floor.</span>")
				connect_to_network()
		if(EMITTER_WELDED)
			if(!I.tool_start_check(user, amount=0))
				return TRUE
			user.visible_message("[user.name] starts to cut the [name] free from the floor.", \
				"<span_class='notice'>You start to cut \the [src] free from the floor...</span>", \
				"<span_class='italics'>You hear welding.</span>")
			if(I.use_tool(src, user, 20, volume=50))
				state = EMITTER_WRENCHED
				to_chat(user, "<span_class='notice'>You cut \the [src] free from the floor.</span>")
...
/obj/machinery/power/emitter/attackby(obj/item/I, mob/user, params)
	if(I.GetID())
		if(obj_flags & EMAGGED)
			to_chat(user, "<span_class='warning'>The lock seems to be broken!</span>")
			return
		if(allowed(user))
			if(active)
				locked = !locked
				...
				to_chat(user, "<span_class='warning'>The controls can only be locked when \the [src] is online!</span>")
		else
...
/obj/machinery/power/emitter/emag_act(mob/user)
	...
	obj_flags |= EMAGGED
	if(user)
		user.visible_message("[user.name] emags [src].","<span_class='notice'>You short out the lock.</span>")
