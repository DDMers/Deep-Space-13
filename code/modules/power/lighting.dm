...
/obj/item/wallframe/light_fixture/try_build(turf/on_wall, user)
	...
	if(!IS_DYNAMIC_LIGHTING(A))
		to_chat(user, "<span_class='warning'>You cannot place [src] in this area!</span>")
		return
	return TRUE
...
/obj/structure/light_construct/examine(mob/user)
	...
	if(cell_connectors)
		...
		to_chat(user, "<span_class='danger'>This casing doesn't support power cells for backup power.</span>")
		return
...
/obj/structure/light_construct/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/stock_parts/cell))
		if(!cell_connectors)
			to_chat(user, "<span_class='warning'>This [name] can't support a power cell!</span>")
			return
		if(W.item_flags & NODROP)
			to_chat(user, "<span_class='warning'>[W] is stuck to your hand!</span>")
			return
		user.dropItemToGround(W)
		if(cell)
			user.visible_message("<span_class='notice'>[user] swaps [W] out for [src]'s cell.</span>", \
			"<span_class='notice'>You swap [src]'s power cells.</span>")
			cell.forceMove(drop_location())
			user.put_in_hands(cell)
		else
			user.visible_message("<span_class='notice'>[user] hooks up [W] to [src].</span>", \
			"<span_class='notice'>You add [W] to [src].</span>")
	...
	switch(stage)
		if(1)
			if(W.tool_behaviour == TOOL_WRENCH)
				to_chat(usr, "<span_class='notice'>You begin deconstructing [src]...</span>")
				if (W.use_tool(src, user, 30, volume=50))
					new /obj/item/stack/sheet/metal(drop_location(), sheets_refunded)
					user.visible_message("[user.name] deconstructs [src].", \
						"<span_class='notice'>You deconstruct [src].</span>", "<span_class='italics'>You hear a ratchet.</span>")
			...
			if(istype(W, /obj/item/stack/cable_coil))
				...
				if(coil.use(1))
					...
					user.visible_message("[user.name] adds wires to [src].", \
						"<span_class='notice'>You add wires to [src].</span>")
				else
					to_chat(user, "<span_class='warning'>You need one length of cable to wire [src]!</span>")
				return
		if(2)
			if(W.tool_behaviour == TOOL_WRENCH)
				to_chat(usr, "<span_class='warning'>You have to remove the wires first!</span>")
				return
			...
			if(W.tool_behaviour == TOOL_WIRECUTTER)
				...
				user.visible_message("[user.name] removes the wiring from [src].", \
					"<span_class='notice'>You remove the wiring from [src].</span>", "<span_class='italics'>You hear clicking.</span>")
				W.play_tool_sound(src, 100)
				return

			if(W.tool_behaviour == TOOL_SCREWDRIVER)
				user.visible_message("[user.name] closes [src]'s casing.", \
					"<span_class='notice'>You close [src]'s casing.</span>", "<span_class='italics'>You hear screwing.</span>")
...
/obj/machinery/light/attackby(obj/item/W, mob/living/user, params)
	...
	// attempt to insert light
		...
		if(status == LIGHT_OK)
			to_chat(user, "<span_class='warning'>There is a [fitting] already inserted!</span>")
		else
			...
			if(istype(L, light_type))
				...
				if(status != LIGHT_EMPTY)
					...
					to_chat(user, "<span_class='notice'>You replace [L].</span>")
				else
					to_chat(user, "<span_class='notice'>You insert [L].</span>")
				status = L.status
				...
				to_chat(user, "<span_class='warning'>This type of light requires a [fitting]!</span>")
	...
	// attempt to stick weapon into light socket
		...
		if(W.tool_behaviour == TOOL_SCREWDRIVER) //If it's a screwdriver open it.
			...
			user.visible_message("[user.name] opens [src]'s casing.", \
				"<span_class='notice'>You open [src]'s casing.</span>", "<span_class='italics'>You hear a noise.</span>")
			deconstruct()
		else
			to_chat(user, "<span_class='userdanger'>You stick \the [W] into the light socket!</span>")
...
/obj/machinery/light/proc/use_emergency_power(pwr = LIGHT_EMERGENCY_POWER_USE)
	...
	if(cell.charge > 300) //it's meant to handle 120 W, ya doofus
		visible_message("<span_class='warning'>[src] short-circuits from too powerful of a power cell!</span>")
		burn_out()
		return FALSE
	cell.use(pwr)
...
/obj/machinery/light/attack_ai(mob/user)
	...
	to_chat(user, "<span_class='notice'>Emergency lights for this fixture have been [no_emergency ? "disabled" : "enabled"].</span>")
	update(FALSE)
	return
	...
	if(on)
		...
		if(istype(H))
			...
			if(istype(eth_species))
				to_chat(H, "<span_class='notice'>You start channeling some power through the [fitting] into your body.</span>")
				if(do_after(user, 50, target = src))
					to_chat(H, "<span_class='notice'>You receive some charge from the [fitting].</span>")
		...
		if(prot > 0 || user.has_trait(TRAIT_RESISTHEAT) || user.has_trait(TRAIT_RESISTHEATHANDS))
			to_chat(user, "<span_class='notice'>You remove the light [fitting].</span>")
		else if(istype(user) && user.dna.check_mutation(TK))
			to_chat(user, "<span_class='notice'>You telekinetically remove the light [fitting].</span>")
		else
			to_chat(user, "<span_class='warning'>You try to remove the light [fitting], but you burn your hand on it!</span>")
...
/obj/item/light/suicide_act(mob/living/carbon/user)
	if (status == LIGHT_BROKEN)
		user.visible_message("<span_class='suicide'>[user] begins to stab [user.p_them()]self with \the [src]! It looks like [user.p_theyre()] trying to commit suicide!</span>")
		return BRUTELOSS
	else
		user.visible_message("<span_class='suicide'>[user] begins to eat \the [src]! It looks like [user.p_theyre()] not very bright!</span>")
		shatter()
		return BRUTELOSS
...
/obj/item/light/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/reagent_containers/syringe))
		...
		to_chat(user, "<span_class='notice'>You inject the solution into \the [src].</span>")
...
/obj/item/light/proc/shatter()
	if(status == LIGHT_OK || status == LIGHT_BURNED)
		visible_message("<span_class='danger'>[src] shatters.</span>","<span_class='italics'>You hear a small glass object shatter.</span>")
		status = LIGHT_BROKEN
		force = 5
		playsound(src.loc, 'sound/effects/glasshit.ogg', 75, 1)
		if(rigged)
