...
/datum/buildmode_mode/copy/show_help(client/c)
	...
	to_chat(c, "<span_class='notice'>Left Mouse Button on obj/turf/mob   = Spawn a Copy of selected target</span>")
	to_chat(c, "<span_class='notice'>Right Mouse Button on obj/mob = Select target to copy</span>")
	to_chat(c, "<span_class='notice'>***********************************************************</span>")
...
/datum/buildmode_mode/copy/handle_click(client/c, params, obj/object)
	...
	if(left_click)
		...
		if(ismovableatom(object)) // No copying turfs for now.
			to_chat(c, "<span_class='notice'>[object] set as template.</span>")
			stored = object
