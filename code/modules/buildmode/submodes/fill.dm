...
/datum/buildmode_mode/fill/show_help(client/c)
	...
	to_chat(c, "<span_class='notice'>Left Mouse Button + Alt on turf/obj/mob = Delete region</span>")
	to_chat(c, "<span_class='notice'>Right Mouse Button on buildmode button = Select object type</span>")
	to_chat(c, "<span_class='notice'>***********************************************************</span>")
...
/datum/buildmode_mode/fill/handle_click(client/c, params, obj/object)
	if(isnull(objholder))
		to_chat(c, "<span_class='warning'>Select an object type first.</span>")
		deselect_region()
		return
	..()
