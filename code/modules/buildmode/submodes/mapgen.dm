...
/datum/buildmode_mode/mapgen/show_help(client/c)
	...
	to_chat(c, "<span_class='notice'>Left Mouse Button on turf/obj/mob      = Select corner</span>")
	to_chat(c, "<span_class='notice'>Right Mouse Button on buildmode button = Select generator</span>")
	to_chat(c, "<span_class='notice'>***********************************************************</span>")
...
/datum/buildmode_mode/mapgen/handle_click(client/c, params, obj/object)
	if(isnull(generator_path))
		to_chat(c, "<span_class='warning'>Select generator type first.</span>")
		deselect_region()
		return
	..()
	...
	if(left_click)
		...
		if(istype(G, /datum/mapGenerator/repair/reload_station_map))
			if(GLOB.reloading_map)
				to_chat(c, "<span_class='boldwarning'>You are already reloading an area! Please wait for it to fully finish loading before trying to load another!</span>")
				deselect_region()
				return
