...
/datum/buildmode_mode/area_edit/show_help(client/c)
	...
	to_chat(c, "<span_class='notice'>Left Mouse Button on obj/turf/mob  = Paint area</span>")
	to_chat(c, "<span_class='notice'>Right Mouse Button on obj/turf/mob = Select area to paint</span>")
	to_chat(c, "<span_class='notice'>Right Mouse Button on buildmode button = Create new area</span>")
	to_chat(c, "<span_class='notice'>***********************************************************</span>")
...
/datum/buildmode_mode/area_edit/handle_click(client/c, params, object)
	...
	if(left_click)
		if(!storedarea)
			to_chat(c, "<span_class='warning'>Configure or select the area you want to paint first!</span>")
			return
