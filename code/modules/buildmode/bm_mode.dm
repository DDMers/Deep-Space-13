...
/datum/buildmode_mode/proc/change_settings(client/c)
	to_chat(c, "<span_class='warning'>There is no configuration available for this mode</span>")
	return
	...
	if(use_corner_selection)
		if(left_click)
			...
			if(cornerA && !cornerB)
				...
				to_chat(c, "<span_class='boldwarning'>Region selected, if you're happy with your selection left click again, otherwise right click.</span>")
				return
			handle_selected_area(c, params)
			deselect_region()
		else
			to_chat(c, "<span_class='notice'>Region selection canceled!</span>")
			deselect_region()
	return
