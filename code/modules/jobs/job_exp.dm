...
/client/proc/update_exp_list(minutes, announce_changes = FALSE)
	...
	if(isliving(mob))
		if(mob.stat != DEAD)
			...
			if(announce_changes)
				to_chat(src,"<span_class='notice'>You got: [minutes] Living EXP!</span>")
			if(mob.mind.assigned_role)
				for(var/job in SSjob.name_occupations)
					if(mob.mind.assigned_role == job)
						...
						if(announce_changes)
							to_chat(src,"<span_class='notice'>You got: [minutes] [job] EXP!</span>")
				if(!rolefound)
					for(var/role in GLOB.exp_specialmap[EXP_TYPE_SPECIAL])
						if(mob.mind.assigned_role == role)
							...
							if(announce_changes)
								to_chat(mob,"<span_class='notice'>You got: [minutes] [role] EXP!</span>")
				if(mob.mind.special_role && !(mob.mind.datum_flags & DF_VAR_EDITED))
					...
					if(announce_changes)
						to_chat(src,"<span_class='notice'>You got: [minutes] [trackedrole] EXP!</span>")
			if(!rolefound)
			...
			if(holder && !holder.deadmined)
				...
				if(announce_changes)
					to_chat(src,"<span_class='notice'>You got: [minutes] Admin EXP!</span>")
			else
				play_records[EXP_TYPE_GHOST] += minutes
				if(announce_changes)
					to_chat(src,"<span_class='notice'>You got: [minutes] Ghost EXP!</span>")
