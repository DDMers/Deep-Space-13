		...
		if (merge_stacks)
			if (istype(active_stack) && istype(I_stack, active_stack.merge_type))
				if (I_stack.merge(active_stack))
					to_chat(usr, "<span_class='notice'>Your [active_stack.name] stack now contains [active_stack.get_amount()] [active_stack.singular_name]\s.</span>")
					return TRUE
			else
				var/obj/item/stack/inactive_stack = get_inactive_held_item()
				if (istype(inactive_stack) && istype(I_stack, inactive_stack.merge_type))
					if (I_stack.merge(inactive_stack))
						to_chat(usr, "<span_class='notice'>Your [inactive_stack.name] stack now contains [inactive_stack.get_amount()] [inactive_stack.singular_name]\s.</span>")
						return TRUE
...
/obj/item/proc/equip_to_best_slot(mob/M)
	if(src != M.get_active_held_item())
		to_chat(M, "<span_class='warning'>You are not holding anything to equip!</span>")
		return FALSE
	...
	to_chat(M, "<span_class='warning'>You are unable to equip that!</span>")
	return FALSE
