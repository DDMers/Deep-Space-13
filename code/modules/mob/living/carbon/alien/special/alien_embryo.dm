	...
	switch(stage)
		if(2, 3)
			...
			if(prob(2))
				owner.emote("cough")
			if(prob(2))
				to_chat(owner, "<span_class='danger'>Your throat feels sore.</span>")
			if(prob(2))
				to_chat(owner, "<span_class='danger'>Mucous runs down the back of your throat.</span>")
		if(4)
			if(prob(2))
				owner.emote("sneeze")
			if(prob(2))
				owner.emote("cough")
			if(prob(4))
				to_chat(owner, "<span_class='danger'>Your muscles ache.</span>")
				if(prob(20))
					owner.take_bodypart_damage(1)
			if(prob(4))
				to_chat(owner, "<span_class='danger'>Your stomach hurts.</span>")
				if(prob(20))
					owner.adjustToxLoss(1)
		if(5)
			to_chat(owner, "<span_class='danger'>You feel something tearing its way out of your stomach...</span>")
...
/obj/item/organ/body_egg/alien_embryo/proc/AttemptGrow(gib_on_success=TRUE)
	...
	if(gib_on_success)
		new_xeno.visible_message("<span_class='danger'>[new_xeno] bursts out of [owner] in a shower of gore!</span>", "<span_class='userdanger'>You exit [owner], your previous host.</span>", "<span_class='italics'>You hear organic matter ripping and tearing!</span>")
		owner.gib(TRUE)
	else
		new_xeno.visible_message("<span_class='danger'>[new_xeno] wriggles out of [owner]!</span>", "<span_class='userdanger'>You exit [owner], your previous host.</span>")
