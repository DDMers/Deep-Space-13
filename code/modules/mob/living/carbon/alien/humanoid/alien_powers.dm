...
/obj/effect/proc_holder/alien/proc/cost_check(check_turf = FALSE, mob/living/carbon/user, silent = FALSE)
	if(user.stat)
		if(!silent)
			to_chat(user, "<span_class='noticealien'>You must be conscious to do this.</span>")
		return FALSE
	if(user.getPlasma() < plasma_cost)
		if(!silent)
			to_chat(user, "<span_class='noticealien'>Not enough plasma stored.</span>")
		return FALSE
	if(check_turf && (!isturf(user.loc) || isspaceturf(user.loc)))
		if(!silent)
			to_chat(user, "<span_class='noticealien'>Bad place for a garden!</span>")
		return FALSE
	return TRUE
...
/obj/effect/proc_holder/alien/plant/fire(mob/living/carbon/user)
	if(locate(/obj/structure/alien/weeds/node) in get_turf(user))
		...
		return 0
	user.visible_message("<span_class='alertalien'>[user] has planted some alien weeds!</span>")
...
/obj/effect/proc_holder/alien/whisper/fire(mob/living/carbon/user)
	...
	if(msg)
		...
		to_chat(M, "<span_class='noticealien'>You hear a strange, alien voice in your head...</span>[msg]")
		to_chat(user, "<span_class='noticealien'>You said: \"[msg]\" to [M]</span>")
		for(var/ded in GLOB.dead_mob_list)
			if(!isobserver(ded))
				continue
			var/follow_link_user = FOLLOW_LINK(ded, user)
			var/follow_link_whispee = FOLLOW_LINK(ded, M)
			to_chat(ded, "[follow_link_user] <span_class='name'>[user]</span> <span_class='alertalien'>Alien Whisper --> </span> [follow_link_whispee] <span_class='name'>[M]</span> <span_class='noticealien'>[msg]</span>")
	else
		return 0
	return 1
...
/obj/effect/proc_holder/alien/transfer/fire(mob/living/carbon/user)
	...
	if (amount)
		...
		if (get_dist(user,M) <= 1)
			...
			to_chat(M, "<span_class='noticealien'>[user] has transferred [amount] plasma to you.</span>")
			to_chat(user, "<span_class='noticealien'>You transfer [amount] plasma to [M]</span>")
		else
			to_chat(user, "<span_class='noticealien'>You need to be closer!</span>")
	return
...
/obj/effect/proc_holder/alien/acid/proc/corrode(atom/target,mob/living/carbon/user = usr)
	if(target in oview(1,user))
		if(target.acid_act(200, 100))
			user.visible_message("<span_class='alertalien'>[user] vomits globs of vile stuff all over [target]. It begins to sizzle and melt under the bubbling mess of acid!</span>")
			return 1
		else
			to_chat(user, "<span_class='noticealien'>You cannot dissolve this object.</span>")


			return 0
	else
		to_chat(src, "<span_class='noticealien'>[target] is too far away.</span>")
		return 0
...
/obj/effect/proc_holder/alien/neurotoxin/fire(mob/living/carbon/user)
	var/message
	if(active)
		message = "<span_class='notice'>You empty your neurotoxin gland.</span>"
		remove_ranged_ability(message)
	else
		message = "<span_class='notice'>You prepare your neurotoxin gland. <B>Left-click to fire at a target!</B></span>"
...
/obj/effect/proc_holder/alien/neurotoxin/InterceptClickOn(mob/living/caller, params, atom/target)
	...
	if(user.getPlasma() < p_cost)
		to_chat(user, "<span_class='warning'>You need at least [p_cost] plasma to spit.</span>")
		remove_ranged_ability()
		return
	...
	user.visible_message("<span_class='danger'>[user] spits neurotoxin!", "<span_class='alertalien'>You spit neurotoxin.</span>")
...
/obj/effect/proc_holder/alien/resin/fire(mob/living/carbon/user)
	if(locate(/obj/structure/alien/resin) in user.loc)
		to_chat(user, "<span_class='danger'>There is already a resin structure there.</span>")
		return FALSE
	...
	if (!cost_check(check_turf,user))
		return FALSE
	to_chat(user, "<span_class='notice'>You shape a [choice].</span>")
	user.visible_message("<span_class='notice'>[user] vomits up a thick purple substance and begins to shape it.</span>")
	...
	if(user.stomach_contents.len)
		for(var/atom/movable/A in user.stomach_contents)
			...
			if(isliving(A))
				var/mob/M = A
				M.reset_perspective()
		user.visible_message("<span_class='alertealien'>[user] hurls out the contents of their stomach!</span>")
	return
...
/obj/effect/proc_holder/alien/sneak/fire(mob/living/carbon/alien/humanoid/user)
	if(!active)
		...
		user.sneaking = 1
		active = 1
		to_chat(user, "<span_class='noticealien'>You blend into the shadows...</span>")
	else
		user.alpha = initial(user.alpha)
		user.sneaking = 0
		active = 0
		to_chat(user, "<span_class='noticealien'>You reveal yourself!</span>")
