...
/obj/effect/proc_holder/alien/royal/praetorian/evolve/fire(mob/living/carbon/alien/humanoid/user)
	...
	if(!node) //Just in case this particular Praetorian gets violated and kept by the RD as a replacement for Lamarr.
		to_chat(user, "<span_class='danger'>Without the hivemind, you would be unfit to rule as queen!</span>")
		return 0
	if(node.recent_queen_death)
		to_chat(user, "<span_class='danger'>You are still too burdened with guilt to evolve into a queen.</span>")
		return 0
	if(!get_alien_type(/mob/living/carbon/alien/humanoid/royal/queen))
		...
		to_chat(user, "<span_class='notice'>We already have an alive queen.</span>")
		return 0
