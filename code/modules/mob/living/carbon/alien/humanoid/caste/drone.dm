...
/obj/effect/proc_holder/alien/evolve/fire(mob/living/carbon/alien/humanoid/user)
	...
	if(!node) //Players are Murphy's Law. We may not expect there to ever be a living xeno with no hivenode, but they _WILL_ make it happen.
		to_chat(user, "<span_class='danger'>Without the hivemind, you can't possibly hold the responsibility of leadership!</span>")
		return 0
	if(node.recent_queen_death)
		to_chat(user, "<span_class='danger'>Your thoughts are still too scattered to take up the position of leadership.</span>")
		return 0

	if(!isturf(user.loc))
		to_chat(user, "<span_class='notice'>You can't evolve here!</span>")
		return 0
	if(!get_alien_type(/mob/living/carbon/alien/humanoid/royal))
		...
		to_chat(user, "<span_class='notice'>We already have a living royal!</span>")
		return 0