...
/obj/effect/proc_holder/alien/lay_egg/fire(mob/living/carbon/user)
	if(locate(/obj/structure/alien/egg) in get_turf(user))
		to_chat(user, "<span_class='alertalien'>There's already an egg here.</span>")
		return FALSE

	if(!check_vent_block(user))
		return FALSE

	user.visible_message("<span_class='alertalien'>[user] has laid an egg!</span>")
...
/obj/effect/proc_holder/alien/royal/queen/promote/fire(mob/living/carbon/alien/user)
	...
	if(get_alien_type(/mob/living/carbon/alien/humanoid/royal/praetorian/))
		to_chat(user, "<span_class='noticealien'>You already have a Praetorian!</span>")
		return 0
	else
		for(prom in user)
			to_chat(user, "<span_class='noticealien'>You discard [prom].</span>")
			qdel(prom)
			return 0

		prom = new (user.loc)
		if(!user.put_in_active_hand(prom, 1))
			to_chat(user, "<span_class='warning'>You must empty your hands before preparing the parasite.</span>")
			return 0
		else //Just in case telling the player only once is not enough!
			to_chat(user, "<span_class='noticealien'>Use the royal parasite on one of your children to promote her to Praetorian!</span>")
	return 0
...
/obj/item/queenpromote/attack(mob/living/M, mob/living/carbon/alien/humanoid/user)
	if(!isalienadult(M) || isalienroyal(M))
		to_chat(user, "<span_class='noticealien'>You may only use this with your adult, non-royal children!</span>")
		return
	if(get_alien_type(/mob/living/carbon/alien/humanoid/royal/praetorian/))
		to_chat(user, "<span_class='noticealien'>You already have a Praetorian!</span>")
		return
	...
	if(A.stat == CONSCIOUS && A.mind && A.key)
		if(!user.usePlasma(500))
			to_chat(user, "<span_class='noticealien'>You must have 500 plasma stored to use this!</span>")
			return

		to_chat(A, "<span_class='noticealien'>The queen has granted you a promotion to Praetorian!</span>")
		user.visible_message("<span_class='alertalien'>[A] begins to expand, twist and contort!</span>")
		...
		to_chat(user, "<span_class='warning'>This child must be alert and responsive to become a Praetorian!</span>")

/obj/item/queenpromote/attack_self(mob/user)
	to_chat(user, "<span_class='noticealien'>You discard [src].</span>")
	qdel(src)
