...
/obj/item/organ/alien/hivenode/proc/queen_death()
	...
	if(isalien(owner)) //Different effects for aliens than humans
		to_chat(owner, "<span_class='userdanger'>Your Queen has been struck down!</span>")
		to_chat(owner, "<span_class='danger'>You are struck with overwhelming agony! You feel confused, and your connection to the hivemind is severed.</span>")
		owner.emote("roar")
		owner.Stun(200) //Actually just slows them down a bit.

	else if(ishuman(owner)) //Humans, being more fragile, are more overwhelmed by the mental backlash.
		to_chat(owner, "<span_class='danger'>You feel a splitting pain in your head, and are struck with a wave of nausea. You cannot hear the hivemind anymore!</span>")
		owner.emote("scream")
		owner.Paralyze(100)
...
/obj/item/organ/alien/hivenode/proc/clear_queen_death()
	...
	if(!owner) //In case the xeno is butchered or subjected to surgery after death.
		return
	to_chat(owner, "<span_class='noticealien'>The pain of the queen's death is easing. You begin to hear the hivemind again.</span>")
