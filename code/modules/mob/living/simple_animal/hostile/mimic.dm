...
/mob/living/simple_animal/hostile/mimic/crate/AttackingTarget()
	...
	if(.)
		...
		if(prob(15) && iscarbon(target))
			...
			C.Paralyze(40)
			C.visible_message("<span_class='danger'>\The [src] knocks down \the [C]!</span>", \
					"<span_class='userdanger'>\The [src] knocks you down!</span>")
	...
	if(Zapgun)
		...
		if(Pewgun.chambered)
			if(Pewgun.chambered.BB)
				...
				visible_message("<span_class='danger'>The <b>[src]</b> clears a jam!</span>")
			...
			Pewgun.chambered = Pewgun.magazine.get_round(0)
			Pewgun.chambered.forceMove(Pewgun)
			visible_message("<span_class='danger'>The <b>[src]</b> cocks itself!</span>")
	else
		ranged = 0 //BANZAIIII
		retreat_distance = 0
		minimum_distance = 1
