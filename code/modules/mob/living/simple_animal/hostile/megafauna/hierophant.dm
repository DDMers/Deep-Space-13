...
/mob/living/simple_animal/hostile/megafauna/hierophant/Life()
	...
	if(. && spawned_beacon && !QDELETED(spawned_beacon) && !client)
		...
		if(timeout_time <= 0 && !did_reset)
			did_reset = TRUE
			visible_message("<span_class='hierophant_warning'>\"Vixyvrmrk xs fewi...\"</span>")
			...
			if(health > maxHealth * 0.9)
				visible_message("<span_class='hierophant'>\"Vitemvw gsqtpixi. Stivexmrk ex qebmqyq ijjmgmirgc.\"</span>")
			else
				visible_message("<span_class='hierophant'>\"Vitemvw gsqtpixi. Stivexmsrep ijjmgmirgc gsqtvsqmwih.\"</span>")
...
/mob/living/simple_animal/hostile/megafauna/hierophant/death()
	if(health > 0 || stat == DEAD)
		...
		burst_range = 10
		visible_message("<span_class='hierophant'>\"Mrmxmexmrk wipj-hiwxvygx wiuyirgi...\"</span>")
		visible_message("<span_class='hierophant_warning'>[src] shrinks, releasing a massive burst of energy!</span>")
		burst(get_turf(src))
		..()
...
/mob/living/simple_animal/hostile/megafauna/hierophant/devour(mob/living/L)
	for(var/obj/item/W in L)
		if(!L.dropItemToGround(W))
			qdel(W)
	visible_message("<span_class='hierophant_warning'>\"[pick(kill_phrases)]\"</span>")
	visible_message("<span_class='hierophant_warning'>[src] annihilates [L]!</span>","<span_class='userdanger'>You annihilate [L], restoring your health!</span>")
...
/mob/living/simple_animal/hostile/megafauna/hierophant/GiveTarget(new_target)
	...
	if(. && target && !targets_the_same)
		visible_message("<span_class='hierophant_warning'>\"[pick(target_phrases)]\"</span>")
			...
			switch(pick(possibilities))
				if("blink_spam") //blink either once or multiple times.
					if(health < maxHealth * 0.5 && blink_counter > 1)
						visible_message("<span_class='hierophant'>\"Mx ampp rsx iwgeti.\"</span>")
						var/oldcolor = color
				...
				if("cross_blast_spam") //fire a lot of cross blasts at a target.
					visible_message("<span_class='hierophant'>\"Piezi mx rsalivi xs vyr.\"</span>")
					blinking = TRUE
					var/oldcolor = color
					animate(src, color = "#660099", time = 6)
					sleep(6)
				...
				if("chaser_swarm") //fire four fucking chasers at a target and their friends.
					visible_message("<span_class='hierophant'>\"Mx gerrsx lmhi.\"</span>")
					blinking = TRUE
					var/oldcolor = color
					animate(src, color = "#660099", time = 6)
					sleep(6)
...
/mob/living/simple_animal/hostile/megafauna/hierophant/proc/blink(mob/victim) //blink to a target
	...
	sleep(1)
	visible_message("<span_class='hierophant_warning'>[src] fades out!</span>")
	density = FALSE
	sleep(2)
	forceMove(T)
	sleep(1)
	...
	sleep(1)
	density = TRUE
	visible_message("<span_class='hierophant_warning'>[src] fades in!</span>")
...
/obj/effect/temp_visual/hierophant/blast/proc/do_damage(turf/T)
	...
	for(var/mob/living/L in T.contents - hit_things) //find and damage mobs...
		...
		to_chat(L, "<span_class='userdanger'>You're struck by a [name]!</span>")
	...
	for(var/obj/mecha/M in T.contents - hit_things) //also damage mechs.
		...
		if(M.occupant)
			if(friendly_fire_check && caster && caster.faction_check_mob(M.occupant))
				continue
			to_chat(M.occupant, "<span_class='userdanger'>Your [M.name] is struck by a [name]!</span>")
...
/obj/effect/hierophant/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/hierophant_club))
		...
		if(H.timer > world.time)
			return
		if(H.beacon == src)
			to_chat(user, "<span_class='notice'>You start removing your hierophant beacon...</span>")
			H.timer = world.time + 51
			INVOKE_ASYNC(H, /obj/item/hierophant_club.proc/prepare_icon_update)
			if(do_after(user, 50, target = src))
				playsound(src,'sound/magic/blind.ogg', 200, 1, -4)
				new /obj/effect/temp_visual/hierophant/telegraph/teleport(get_turf(src), user)
				to_chat(user, "<span_class='hierophant_warning'>You collect [src], reattaching it to the club!</span>")
				H.beacon = null
			...
			to_chat(user, "<span_class='hierophant_warning'>You touch the beacon with the club, but nothing happens.</span>")
	else
		return ..()
