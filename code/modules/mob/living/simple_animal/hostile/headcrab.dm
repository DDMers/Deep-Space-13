...
/mob/living/simple_animal/hostile/headcrab/proc/Infect(mob/living/carbon/victim)
	...
	for(var/obj/item/organ/I in src)
		I.forceMove(egg)
	visible_message("<span_class='warning'>[src] plants something in [victim]'s flesh!</span>", \
					"<span_class='danger'>We inject our egg into [victim]'s body!</span>")
	egg_lain = 1
	...
	if(. && !egg_lain && iscarbon(target) && !ismonkey(target))
		...
		if(C.stat == DEAD)
			if(C.has_trait(TRAIT_XENO_HOST))
				to_chat(src, "<span_class='userdanger'>A foreign presence repels us from this body. Perhaps we should try to infest another?</span>")
				return
			Infect(target)
			to_chat(src, "<span_class='userdanger'>With our egg laid, our death approaches rapidly...</span>")
