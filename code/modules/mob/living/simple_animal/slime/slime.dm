...
/mob/living/simple_animal/slime/attack_slime(mob/living/simple_animal/slime/M)
	if(..()) //successful slime attack
		...
		if(buckled)
			...
			visible_message("<span_class='danger'>[M] pulls [src] off!</span>")
			return
		attacked += 5
...
/mob/living/simple_animal/slime/attack_hand(mob/living/carbon/human/M)
	if(buckled)
		...
		if(buckled == M)
			if(prob(60))
				visible_message("<span_class='warning'>[M] attempts to wrestle \the [name] off!</span>")
				playsound(loc, 'sound/weapons/punchmiss.ogg', 25, 1, -1)

			else
				visible_message("<span_class='warning'>[M] manages to wrestle \the [name] off!</span>")
			...
			if(prob(30))
				visible_message("<span_class='warning'>[M] attempts to wrestle \the [name] off of [buckled]!</span>")
				playsound(loc, 'sound/weapons/punchmiss.ogg', 25, 1, -1)

			else
				visible_message("<span_class='warning'>[M] manages to wrestle \the [name] off of [buckled]!</span>")
				playsound(loc, 'sound/weapons/thudswoosh.ogg', 50, 1, -1)
...
/mob/living/simple_animal/slime/attackby(obj/item/W, mob/living/user, params)
	...
	if(istype(W, /obj/item/stack/sheet/mineral/plasma) && !stat) //Let's you feed slimes plasma.
		if (user in Friends)
			...
			Friends[user] = 1
		to_chat(user, "<span_class='notice'>You feed the slime the plasma. It chirps happily.</span>")
	...
	if(W.force > 0)
		...
		if(prob(25))
			...
			to_chat(user, "<span_class='danger'>[W] passes right through [src]!</span>")
			return
	...
	if(istype(W, /obj/item/storage/bag/bio))
		...
		if(!effectmod)
			to_chat(user, "<span_class='warning'>The slime is not currently being mutated.</span>")
			return
		...
		for(var/obj/item/slime_extract/S in P.contents)
			...
			if(applied >= SLIME_EXTRACT_CROSSING_REQUIRED)
				to_chat(user, "<span_class='notice'>You feed the slime as many of the extracts from the bag as you can, and it mutates!</span>")
		...
		if(!hasOutput)
			if(!hasFound)
				to_chat(user, "<span_class='warning'>There are no extracts in the bag that this slime will accept!</span>")
			else
				to_chat(user, "<span_class='notice'>You feed the slime some extracts from the bag.</span>")
				playsound(src, 'sound/effects/attackblob.ogg', 50, 1)
		return
	..()
...
/mob/living/simple_animal/slime/proc/spawn_corecross()
	...
	visible_message("<span_class='danger'>[src] shudders, its mutated core consuming the rest of its body!</span>")
	...
	if(crosspath)
		...
		visible_message("<span_class='warning'>The mutated core shudders, and collapses into a puddle, unable to maintain its form.</span>")
	qdel(src)
...
/mob/living/simple_animal/slime/examine(mob/user)
	...
	if (src.stat == DEAD)
		msg += "<span_class='deadsay'>It is limp and unresponsive.</span>\n"
	else
		if (stat == UNCONSCIOUS) // Slime stasis
			msg += "<span_class='deadsay'>It appears to be alive but unresponsive.</span>\n"
		if (src.getBruteLoss())
		...
		switch(powerlevel)
			...
			if(6 to 9)
				msg += "<span_class='warning'>It is glowing brightly with high levels of electrical activity.</span>\n"

			if(10)
				msg += "<span_class='warning'><B>It is radiating with massive levels of electrical activity!</B></span>\n"
