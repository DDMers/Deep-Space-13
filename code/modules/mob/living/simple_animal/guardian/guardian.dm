...
/mob/living/simple_animal/hostile/guardian
	...
	var/playstyle_string = "<span_class='holoparasite bold'>You are a standard Guardian. You shouldn't exist!</span>"
	var/magic_fluff_string = "<span_class='holoparasite'>You draw the Coder, symbolizing bugs and errors. This shouldn't happen! Submit a bug report!</span>"
	var/tech_fluff_string = "<span_class='holoparasite'>BOOT SEQUENCE COMPLETE. ERROR MODULE LOADED. THIS SHOULDN'T HAPPEN. Submit a bug report!</span>"
	var/carp_fluff_string = "<span_class='holoparasite'>CARP CARP CARP SOME SORT OF HORRIFIC BUG BLAME THE CODERS CARP CARP CARP</span>"
...
/mob/living/simple_animal/hostile/guardian/Login() //if we have a mind, set its name to ours when it logs in
	...
	if(mind)
		mind.name = "[real_name]"
	if(!summoner)
		to_chat(src, "<span_class='holoparasite bold'>For some reason, somehow, you have no summoner. Please report this bug immediately.</span>")
		return
	to_chat(src, "<span_class='holoparasite'>You are <font color=\"[namedatum.colour]\"><b>[real_name]</b></font>, bound to serve [summoner.real_name].</span>")
	to_chat(src, "<span_class='holoparasite'>You are capable of manifesting or recalling to your master with the buttons on your HUD. You will also find a button to communicate with [summoner.p_them()] privately there.</span>")
	to_chat(src, "<span_class='holoparasite'>While personally invincible, you will die if [summoner.real_name] does, and any damage dealt to you will have a portion passed on to [summoner.p_them()] as you feed upon [summoner.p_them()] to sustain yourself.</span>")
...
/mob/living/simple_animal/hostile/guardian/Life() //Dies if the summoner dies
	...
	if(!QDELETED(summoner))
		if(summoner.stat == DEAD)
			...
			to_chat(src, "<span_class='danger'>Your summoner has died!</span>")
			visible_message("<span_class='danger'><B>\The [src] dies along with its user!</B></span>")
			summoner.visible_message("<span_class='danger'><B>[summoner]'s body is completely consumed by the strain of sustaining [src]!</B></span>")
		...
		to_chat(src, "<span_class='danger'>Your summoner has died!</span>")
		visible_message("<span_class='danger'><B>[src] dies along with its user!</B></span>")
		death(TRUE)
		qdel(src)
	snapback()
...
/mob/living/simple_animal/hostile/guardian/proc/snapback()
	if(summoner)
		if(get_dist(get_turf(summoner),get_turf(src)) <= range)
			...
			to_chat(src, "<span_class='holoparasite'>You moved out of range, and were pulled back! You can only move [range] meters from [summoner.real_name]!</span>")
			visible_message("<span_class='danger'>\The [src] jumps back to its user.</span>")
...
/mob/living/simple_animal/hostile/guardian/AttackingTarget()
	if(!is_deployed())
		to_chat(src, "<span_class='danger'><B>You must be manifested to attack!</span></B>")
		return FALSE
	else
		return ..()
...
/mob/living/simple_animal/hostile/guardian/death()
	...
	..()
	if(summoner)
		to_chat(summoner, "<span_class='danger'><B>Your [name] died somehow!</span></B>")
		summoner.death()
...
/mob/living/simple_animal/hostile/guardian/adjustHealth(amount, updating_health = TRUE, forced = FALSE) //The spirit is invincible, but passes on damage to the summoner
	...
	if(summoner)
		...
		if(amount > 0)
			to_chat(summoner, "<span_class='danger'><B>Your [name] is under attack! You take damage!</span></B>")
			summoner.visible_message("<span_class='danger'><B>Blood sprays from [summoner] as [src] takes damage!</B></span>")
			if(summoner.stat == UNCONSCIOUS)
				to_chat(summoner, "<span_class='danger'><B>Your body can't take the strain of sustaining [src] in this condition, it begins to fall apart!</span></B>")
...
/mob/living/simple_animal/hostile/guardian/gib()
	if(summoner)
		to_chat(summoner, "<span_class='danger'><B>Your [src] was blown up!</span></B>")
		summoner.gib()
	ghostize()
	qdel(src)
...
/mob/living/simple_animal/hostile/guardian/proc/ToggleMode()
	to_chat(src, "<span_class='danger'><B>You don't have another mode!</span></B>")

/mob/living/simple_animal/hostile/guardian/proc/ToggleLight()
	if(light_range<3)
		to_chat(src, "<span_class='notice'>You activate your light.</span>")
		set_light(3)
	else
		to_chat(src, "<span_class='notice'>You deactivate your light.</span>")
		set_light(0)
...
/mob/living/simple_animal/hostile/guardian/proc/Communicate()
	if(summoner)
		...
		var/preliminary_message = "<span_class='holoparasite bold'>[input]</span>" //apply basic color/bolding
...
/mob/living/proc/guardian_comm()
	...
	var/preliminary_message = "<span_class='holoparasite bold'>[input]</span>" //apply basic color/bolding
	var/my_message = "<span_class='holoparasite bold'><i>[src]:</i> [preliminary_message]</span>" //add source, color source with default grey...
...
/mob/living/proc/guardian_reset()
	...
	if(guardians.len)
		...
		if(G)
			to_chat(src, "<span_class='holoparasite'>You attempt to reset <font color=\"[G.namedatum.colour]\"><b>[G.real_name]</b></font>'s personality...</span>")
			...
			if(LAZYLEN(candidates))
				...
				to_chat(G, "<span_class='holoparasite'>Your user reset you, and your body was taken over by a ghost. Looks like they weren't happy with your performance.</span>")
				to_chat(src, "<span_class='holoparasite bold'>Your <font color=\"[G.namedatum.colour]\">[G.real_name]</font> has been successfully reset.</span>")
				...
				switch(G.namedatum.theme)
					if("tech")
						to_chat(src, "<span_class='holoparasite'><font color=\"[G.namedatum.colour]\"><b>[G.real_name]</b></font> is now online!</span>")
					if("magic")
						to_chat(src, "<span_class='holoparasite'><font color=\"[G.namedatum.colour]\"><b>[G.real_name]</b></font> has been summoned!</span>")
				guardians -= G
				if(!guardians.len)
					verbs -= /mob/living/proc/guardian_reset
			else
				to_chat(src, "<span_class='holoparasite'>There were no ghosts willing to take control of <font color=\"[G.namedatum.colour]\"><b>[G.real_name]</b></font>. Looks like you're stuck with it for now.</span>")
		else
			to_chat(src, "<span_class='holoparasite'>You decide not to reset [guardians.len > 1 ? "any of your guardians":"your guardian"].</span>")
	else
		verbs -= /mob/living/proc/guardian_reset
...
/obj/item/guardiancreator
	...
	var/use_message = "<span_class='holoparasite'>You shuffle the deck...</span>"
	var/used_message = "<span_class='holoparasite'>All the cards seem to be blank now.</span>"
	var/failure_message = "<span_class='holoparasite bold'>..And draw a card! It's...blank? Maybe you should try again later.</span>"
	var/ling_failure = "<span_class='holoparasite bold'>The deck refuses to respond to a souless creature such as you.</span>"
...
/obj/item/guardiancreator/attack_self(mob/living/user)
	if(isguardian(user) && !allowguardian)
		to_chat(user, "<span_class='holoparasite'>[mob_name] chains are not allowed.</span>")
		return
	var/list/guardians = user.hasparasites()
	if(guardians.len && !allowmultiple)
		to_chat(user, "<span_class='holoparasite'>You already have a [mob_name]!</span>")
		return
...
/obj/item/guardiancreator/proc/spawn_guardian(var/mob/living/user, var/key)
	...
	switch(theme)
		if("tech")
			...
			to_chat(user, "<span_class='holoparasite'><font color=\"[G.namedatum.colour]\"><b>[G.real_name]</b></font> is now online!</span>")
		if("magic")
			to_chat(user, "[G.magic_fluff_string]")
			to_chat(user, "<span_class='holoparasite'><font color=\"[G.namedatum.colour]\"><b>[G.real_name]</b></font> has been summoned!</span>")
		if("carp")
			to_chat(user, "[G.carp_fluff_string]")
			to_chat(user, "<span_class='holoparasite'><font color=\"[G.namedatum.colour]\"><b>[G.real_name]</b></font> has been caught!</span>")
...
/obj/item/guardiancreator/tech
	...
	use_message = "<span_class='holoparasite'>You start to power on the injector...</span>"
	used_message = "<span_class='holoparasite'>The injector has already been used.</span>"
	failure_message = "<span_class='holoparasite bold'>...ERROR. BOOT SEQUENCE ABORTED. AI FAILED TO INTIALIZE. PLEASE CONTACT SUPPORT OR TRY AGAIN LATER.</span>"
	ling_failure = "<span_class='holoparasite bold'>The holoparasites recoil in horror. They want nothing to do with a creature like you.</span>"
...
/obj/item/guardiancreator/carp
	...
	theme = "carp"
	mob_name = "Holocarp"
	use_message = "<span_class='holoparasite'>You put the fishsticks in your mouth...</span>"
	used_message = "<span_class='holoparasite'>Someone's already taken a bite out of these fishsticks! Ew.</span>"
	failure_message = "<span_class='holoparasite bold'>You couldn't catch any carp spirits from the seas of Lake Carp. Maybe there are none, maybe you fucked up.</span>"
	ling_failure = "<span_class='holoparasite bold'>Carp'sie is fine with changelings, so you shouldn't be seeing this message.</span>"
	allowmultiple = TRUE
	allowling = TRUE
	random = TRUE
