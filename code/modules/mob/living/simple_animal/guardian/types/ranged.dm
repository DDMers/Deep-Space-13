...
/mob/living/simple_animal/hostile/guardian/ranged
	...
	ranged = 1
	range = 13
	playstyle_string = "<span_class='holoparasite'>As a <b>ranged</b> type, you have only light damage resistance, but are capable of spraying shards of crystal at incredibly high speed. You can also deploy surveillance snares to monitor enemy movement. Finally, you can switch to scout mode, in which you can't attack, but can move without limit.</span>"
	magic_fluff_string = "<span_class='holoparasite'>..And draw the Sentinel, an alien master of ranged combat.</span>"
	tech_fluff_string = "<span_class='holoparasite'>Boot sequence complete. Ranged combat modules active. Holoparasite swarm online.</span>"
	carp_fluff_string = "<span_class='holoparasite'>CARP CARP CARP! Caught one, it's a ranged carp. This fishy can watch people pee in the ocean.</span>"
...
/mob/living/simple_animal/hostile/guardian/ranged/ToggleMode()
	if(src.loc == summoner)
		if(toggle)
			...
			alpha = 45
			range = 255
			to_chat(src, "<span_class='danger'><B>You switch to scout mode.</span></B>")
			toggle = TRUE
	else
...
/mob/living/simple_animal/hostile/guardian/ranged/ToggleLight()
	...
	to_chat(src, "<span_class='notice'>[msg]</span>")
...
/mob/living/simple_animal/hostile/guardian/ranged/verb/Snare()
	...
	if(src.snares.len <6)
		...
		src.snares |= S
		to_chat(src, "<span_class='danger'><B>Surveillance snare deployed!</span></B>")
	else
		to_chat(src, "<span_class='danger'><B>You have too many snares deployed. Remove some first.</span></B>")
...
/mob/living/simple_animal/hostile/guardian/ranged/verb/DisarmSnare()
	...
	if(picked_snare)
		...
		qdel(picked_snare)
		to_chat(src, "<span_class='danger'><B>Snare disarmed.</span></B>")
...
/obj/effect/snare/Crossed(AM as mob|obj)
	if(isliving(AM) && spawner && spawner.summoner && AM != spawner && !spawner.hasmatchingsummoner(AM))
		to_chat(spawner.summoner, "<span_class='danger'><B>[AM] has crossed surveillance snare, [name].</span></B>")
		var/list/guardians = spawner.summoner.hasparasites()
		for(var/para in guardians)
			to_chat(para, "<span_class='danger'><B>[AM] has crossed surveillance snare, [name].</span></B>")
