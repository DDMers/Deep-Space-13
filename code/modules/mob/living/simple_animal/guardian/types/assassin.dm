...
/mob/living/simple_animal/hostile/guardian/assassin
	...
	playstyle_string = "<span_class='holoparasite'>As an <b>assassin</b> type you do medium damage and have no damage resistance, but can enter stealth, massively increasing the damage of your next attack and causing it to ignore armor. Stealth is broken when you attack or take damage.</span>"
	magic_fluff_string = "<span_class='holoparasite'>..And draw the Space Ninja, a lethal, invisible assassin.</span>"
	tech_fluff_string = "<span_class='holoparasite'>Boot sequence complete. Assassin modules loaded. Holoparasite swarm online.</span>"
	carp_fluff_string = "<span_class='holoparasite'>CARP CARP CARP! Caught one! It's an assassin carp! Just when you thought it was safe to go back to the water... which is unhelpful, because we're in space.</span>"
...
/mob/living/simple_animal/hostile/guardian/assassin/ToggleMode(forced = 0)
	if(toggle)
		...
		alpha = initial(alpha)
		if(!forced)
			to_chat(src, "<span_class='danger'><B>You exit stealth.</span></B>")
		else
			visible_message("<span_class='danger'>\The [src] suddenly appears!</span>")
		...
		if(src.loc == summoner)
			to_chat(src, "<span_class='danger'><B>You have to be manifested to enter stealth!</span></B>")
			return
		...
		alpha = 15
		if(!forced)
			to_chat(src, "<span_class='danger'><B>You enter stealth, empowering your next attack.</span></B>")
		updatestealthalert()
		toggle = TRUE
	else if(!forced)
		to_chat(src, "<span_class='danger'><B>You cannot yet enter stealth, wait another [DisplayTimeText(stealthcooldown - world.time)]!</span></B>")
