...
/mob/living/simple_animal/hostile/guardian/beam
	...
	range = 7
	playstyle_string = "<span_class='holoparasite'>As a <b>lightning</b> type, you will apply lightning chains to targets on attack and have a lightning chain to your summoner. Lightning chains will shock anyone near them.</span>"
	magic_fluff_string = "<span_class='holoparasite'>..And draw the Tesla, a shocking, lethal source of power.</span>"
	tech_fluff_string = "<span_class='holoparasite'>Boot sequence complete. Lightning modules active. Holoparasite swarm online.</span>"
	carp_fluff_string = "<span_class='holoparasite'>CARP CARP CARP! Caught one! It's a lightning carp! Everyone else goes zap zap.</span>"
		...
		for(var/mob/living/L in T)
			if(L.stat != DEAD && L != src && L != summoner)
				...
				if(successfulshocks > 4)
					...
					L.visible_message(
						"<span_class='danger'>[L] was shocked by the lightning chain!</span>", \
						"<span_class='userdanger'>You are shocked by the lightning chain!</span>", \
