...
/mob/living/simple_animal/parrot/Topic(href, href_list)
	...
	if(href_list["remove_inv"])
		...
		switch(remove_from)
			if("ears")
				if(!ears)
					to_chat(usr, "<span_class='warning'>There is nothing to remove from its [remove_from]!</span>")
					return
				if(!stat)
	...
	//Adding things to inventory
		...
		if(!usr.get_active_held_item())
			to_chat(usr, "<span_class='warning'>You have nothing in your hand to put on its [add_to]!</span>")
			return
		switch(add_to)
			if("ears")
				if(ears)
					...
					if( !istype(item_to_add,  /obj/item/radio/headset) )
						to_chat(usr, "<span_class='warning'>This object won't fit!</span>")
						return
					...
					if(!usr.transferItemToLoc(headset_to_add, src))
						return
					ears = headset_to_add
					to_chat(usr, "<span_class='notice'>You fit the headset onto [src].</span>")
...
/mob/living/simple_animal/parrot/attackby(obj/item/O, mob/living/user, params)
	if(!stat && !client && !istype(O, /obj/item/stack/medical) && !istype(O, /obj/item/reagent_containers/food/snacks/cracker))
		...
		to_chat(user, "<span_class='notice'>[src] eagerly devours the cracker.</span>")
	..()
	return
		...
		if(Adjacent(parrot_interest))
			...
			if(isliving(parrot_interest))
				...
				if(!parrot_perch || parrot_interest.loc != parrot_perch.loc)
					...
					visible_message("[src] grabs [held_item]!", "<span_class='notice'>You grab [held_item]!</span>", "<span_class='italics'>You hear the sounds of wings flapping furiously.</span>")
...
/mob/living/simple_animal/parrot/proc/steal_from_ground()
	...
	if(held_item)
		to_chat(src, "<span_class='warning'>You are already holding [held_item]!</span>")
		return 1
	...
	to_chat(src, "<span_class='warning'>There is nothing of interest to take!</span>")
	return 0
...
/mob/living/simple_animal/parrot/proc/steal_from_mob()
	...
	for(var/mob/living/carbon/C in view(1,src))
		...
		if(stolen_item)
			...
			visible_message("[src] grabs [held_item] out of [C]'s hand!", "<span_class='notice'>You snag [held_item] out of [C]'s hand!</span>", "<span_class='italics'>You hear the sounds of wings flapping furiously.</span>")
			return held_item
...
/mob/living/simple_animal/parrot/proc/drop_held_item(drop_gently = 1)
	...
	if(!held_item)
		if(src == usr) //So that other mobs wont make this message appear when they're bludgeoning you.
			to_chat(src, "<span_class='danger'>You have nothing to drop!</span>")
		return 0
		...
		for(var/atom/movable/AM in view(src,1))
			for(var/perch_path in desired_perches)
				if(istype(AM, perch_path))
					...
					parrot_state = PARROT_PERCH
					return
	to_chat(src, "<span_class='warning'>There is no perch nearby to sit on!</span>")
	return
...
/mob/living/simple_animal/parrot/proc/perch_mob_player()
	...
	if(!buckled)
		for(var/mob/living/carbon/human/H in view(src,1))
			...
			perch_on_human(H)
			return
		to_chat(src, "<span_class='warning'>There is nobody nearby that you can sit on!</span>")
	else
		icon_state = icon_living
		parrot_state = PARROT_WANDER
		if(buckled)
			to_chat(src, "<span_class='notice'>You are no longer sitting on [buckled]'s shoulder.</span>")
			buckled.unbuckle_mob(src, TRUE)
		buckled = null
		pixel_x = initial(pixel_x)
		pixel_y = initial(pixel_y)
...
/mob/living/simple_animal/parrot/proc/perch_on_human(mob/living/carbon/human/H)
	...
	if(H.buckle_mob(src, TRUE))
		...
		icon_state = icon_sit
		parrot_state = PARROT_PERCH
		to_chat(src, "<span_class='notice'>You sit on [H]'s shoulder.</span>")
...
/mob/living/simple_animal/parrot/Poly/ghost/proc/Possess(mob/living/carbon/human/H)
	...
	parrot_interest = null
	H.visible_message("<span_class='danger'>[src] dive bombs into [H]'s chest and vanishes!</span>", "<span_class='userdanger'>[src] dive bombs into your chest, vanishing! This can't be good!</span>")
