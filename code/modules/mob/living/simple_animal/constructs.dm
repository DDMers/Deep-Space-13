/mob/living/simple_animal/hostile/construct
	...
	var/playstyle_string = "<span_class='big bold'>You are a generic construct!</span><b> Your job is to not exist, and you should probably adminhelp this.</b>"
	var/master = null
	var/seeking = FALSE
	...
	if(isconstruct(M)) //is it a construct?
		...
		if(health < maxHealth)
			...
			if(src != M)
				...
				M.visible_message("<span_class='danger'>[M] repairs some of \the <b>[src]'s</b> dents.</span>", \
						   "<span_class='cult'>You repair some of <b>[src]'s</b> dents, leaving <b>[src]</b> at <b>[health]/[maxHealth]</b> health.</span>")
			else
				M.visible_message("<span_class='danger'>[M] repairs some of [p_their()] own dents.</span>", \
						   "<span_class='cult'>You repair some of your own dents, leaving you at <b>[M.health]/[M.maxHealth]</b> health.</span>")
		else
			if(src != M)
				to_chat(M, "<span_class='cult'>You cannot repair <b>[src]'s</b> dents, as [p_they()] [p_have()] none!</span>")
			else
				to_chat(M, "<span_class='cult'>You cannot repair your own dents, as you have none!</span>")
	else if(src != M)
		return ..()
...
/mob/living/simple_animal/hostile/construct/armored/bullet_act(obj/item/projectile/P)
	if(istype(P, /obj/item/projectile/energy) || istype(P, /obj/item/projectile/beam))
		...
		if(prob(reflectchance))
			...
			visible_message("<span_class='danger'>The [P.name] is reflected by [src]'s armored shell!</span>", \
							"<span_class='userdanger'>The [P.name] is reflected by your armored shell!</span>")
	...
	if(iscarbon(target))
		...
		if(!LAZYLEN(parts))
			if(undismembermerable_limbs) //they have limbs we can't remove, and no parts we can, attack!
				return ..()
			C.Paralyze(60)
			visible_message("<span_class='danger'>[src] knocks [C] down!</span>")
			to_chat(src, "<span_class='cultlarge'>\"Bring [C.p_them()] to me.\"</span>")
			return FALSE
...
/datum/action/innate/seek_master/Activate()
	...
	if(!the_construct.master)
		to_chat(the_construct, "<span_class='cult italic'>You have no master to seek!</span>")
		the_construct.seeking = FALSE
		return
	if(tracking)
		tracking = FALSE
		the_construct.seeking = FALSE
		to_chat(the_construct, "<span_class='cult italic'>You are no longer tracking your master.</span>")
		return
	else
		tracking = TRUE
		the_construct.seeking = TRUE
		to_chat(the_construct, "<span_class='cult italic'>You are now tracking your master.</span>")
...
/datum/action/innate/seek_prey/Activate()
	...
	if(the_construct.seeking)
		...
		button_icon_state = "cult_mark"
		the_construct.seeking = FALSE
		to_chat(the_construct, "<span_class='cult italic'>You are now tracking Nar'Sie, return to reap the harvest!</span>")
		return
	else
		if(LAZYLEN(GLOB.cult_narsie.souls_needed))
			...
			to_chat(the_construct, "<span_class='cult italic'>You are now tracking your prey, [real_target.real_name] - harvest [real_target.p_them()]!</span>")
		else
			to_chat(the_construct, "<span_class='cult italic'>Nar'Sie has completed her harvest!</span>")
			return
