			...
			switch(d_input)
				...
				if("Cannibalize")
					if(D.health < D.maxHealth)
						D.visible_message("<span_class='notice'>[D] begins to cannibalize parts from [src].</span>", "<span_class='notice'>You begin to cannibalize parts from [src]...</span>")
						if(do_after(D, 60, 0, target = src))
							D.visible_message("<span_class='notice'>[D] repairs itself using [src]'s remains!</span>", "<span_class='notice'>You repair yourself using [src]'s remains.</span>")
							D.adjustBruteLoss(-src.maxHealth)
							new /obj/effect/decal/cleanable/oil/streak(get_turf(src))
							qdel(src)
						else
							to_chat(D, "<span_class='warning'>You need to remain still to cannibalize [src]!</span>")
					else
						to_chat(D, "<span_class='warning'>You're already in perfect condition!</span>")
				if("Nothing")
					return
...
/mob/living/simple_animal/drone/attack_hand(mob/user)
	if(ishuman(user))
		...
		if(user.get_active_held_item())
			...
			return
		visible_message("<span_class='warning'>[user] starts picking up [src].</span>", \
						"<span_class='userdanger'>[user] starts picking you up!</span>")
		if(!do_after(user, 20, target = src))
			return
		visible_message("<span_class='warning'>[user] picks up [src]!</span>", \
						"<span_class='userdanger'>[user] picks you up!</span>")
		if(buckled)
			to_chat(user, "<span_class='warning'>[src] is buckled to [buckled] and cannot be picked up!</span>")
			return
		to_chat(user, "<span_class='notice'>You pick [src] up.</span>")
		drop_all_held_items()
...
/mob/living/simple_animal/drone/proc/try_reactivate(mob/living/user)
	...
	if(!client && (!G || !G.client))
		...
		to_chat(user, "<span_class='warning'>You can't seem to find the [pick(faux_gadgets)]! Without it, [src] [pick(faux_problems)].</span>")
		return
	user.visible_message("<span_class='notice'>[user] begins to reactivate [src].</span>", "<span_class='notice'>You begin to reactivate [src]...</span>")
	if(do_after(user, 30, 1, target = src))
		revive(full_heal = 1)
		user.visible_message("<span_class='notice'>[user] reactivates [src]!</span>", "<span_class='notice'>You reactivate [src].</span>")
		alert_drones(DRONE_NET_CONNECT)
		if(G)
			to_chat(G, "<span_class='ghostalert'>You([name]) were reactivated by [user]!</span>")
	else
		to_chat(user, "<span_class='warning'>You need to remain still to reactivate [src]!</span>")


/mob/living/simple_animal/drone/attackby(obj/item/I, mob/user)
	if(I.tool_behaviour == TOOL_SCREWDRIVER && stat != DEAD)
		if(health < maxHealth)
			to_chat(user, "<span_class='notice'>You start to tighten loose screws on [src]...</span>")
			if(I.use_tool(src, user, 80))
				adjustBruteLoss(-getBruteLoss())
				visible_message("<span_class='notice'>[user] tightens [src == user ? "[user.p_their()]" : "[src]'s"] loose screws!</span>", "<span_class='notice'>You tighten [src == user ? "your" : "[src]'s"] loose screws.</span>")
			else
				to_chat(user, "<span_class='warning'>You need to remain still to tighten [src]'s screws!</span>")
		else
			to_chat(user, "<span_class='warning'>[src]'s screws can't get any tighter!</span>")
		...
		user.visible_message("<span_class='notice'>[user] starts resetting [src]...</span>", \
							 "<span_class='notice'>You press down on [src]'s factory reset control...</span>")
		if(I.use_tool(src, user, 50, volume=50))
			user.visible_message("<span_class='notice'>[user] resets [src]!</span>", \
								 "<span_class='notice'>You reset [src]'s directives to factory defaults!</span>")
...
/mob/living/simple_animal/drone/proc/update_drone_hack(hack, clockwork)
	...
	if(hack)
		if(hacked)
			...
			Stun(40)
		if(clockwork)
			to_chat(src, "<span_class='large_brass'><b>ERROR: LAW OVERRIDE DETECTED</b></span>")
			to_chat(src, "<span_class='heavy_brass'>From now on, these are your laws:</span>")
			laws = "1. Purge all untruths and honor Ratvar."
		else
			visible_message("<span_class='warning'>[src]'s display glows a vicious red!</span>", \
							"<span_class='userdanger'>ERROR: LAW OVERRIDE DETECTED</span>")
			to_chat(src, "<span_class='boldannounce'>From now on, these are your laws:</span>")
			laws = \
		...
		if(!hacked)
			return
		Stun(40)
		visible_message("<span_class='info'>[src]'s display glows a content blue!</span>", \
						"<font size=3 color='#0000CC'><b>ERROR: LAW OVERRIDE DETECTED</b></font>")
		to_chat(src, "<span_class='info'><b>From now on, these are your laws:</b></span>")
		laws = initial(laws)
		to_chat(src, laws)
