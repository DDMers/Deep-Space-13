...
/obj/item/bot_assembly/proc/can_finish_build(obj/item/I, mob/user)
	if(istype(loc, /obj/item/storage/backpack))
		to_chat(user, "<span_class='warning'>You must take [src] out of [loc] first!</span>")
		return FALSE
...
/obj/item/bot_assembly/cleanbot/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/bodypart/l_arm/robot) || istype(W, /obj/item/bodypart/r_arm/robot))
		...
		A.name = created_name
		A.robot_arm = W.type
		to_chat(user, "<span_class='notice'>You add [W] to [src]. Beep boop!</span>")
		qdel(W)
		qdel(src)
	...
	switch(build_step)
		if(ASSEMBLY_FIRST_STEP, ASSEMBLY_SECOND_STEP)
			if(istype(W, /obj/item/bodypart/l_leg/robot) || istype(W, /obj/item/bodypart/r_leg/robot))
				if(!user.temporarilyRemoveItemFromInventory(W))
					return
				to_chat(user, "<span_class='notice'>You add [W] to [src].</span>")
				qdel(W)
		...
		if(ASSEMBLY_FOURTH_STEP)
			if(W.tool_behaviour == TOOL_WELDER)
				if(W.use_tool(src, user, 0, volume=40))
					...
					to_chat(user, "<span_class='notice'>You weld the vest to [src].</span>")
					build_step++
		...
		if(6)
			if(istype(W, /obj/item/stack/cable_coil))
				...
				if(coil.get_amount() < 1)
					to_chat(user, "<span_class='warning'>You need one length of cable to wire the ED-209!</span>")
					return
				to_chat(user, "<span_class='notice'>You start to wire [src]...</span>")
				if(do_after(user, 40, target = src))
					if(coil.get_amount() >= 1 && build_step == 6)
						coil.use(1)
						to_chat(user, "<span_class='notice'>You wire [src].</span>")
		...
		if(8)
			if(W.tool_behaviour == TOOL_SCREWDRIVER)
				to_chat(user, "<span_class='notice'>You start attaching the gun to the frame...</span>")
				if(W.use_tool(src, user, 40, volume=100))
					name = "armed [name]"
					to_chat(user, "<span_class='notice'>Taser gun attached.</span>")
					build_step++
		...
		if(9)
			if(istype(W, /obj/item/stock_parts/cell))
				...
				to_chat(user, "<span_class='notice'>You complete the ED-209.</span>")
				B.cell_type = W.type
				qdel(W)
...
/obj/item/storage/toolbox/mechanical/attackby(obj/item/stack/tile/plasteel/T, mob/user, params)
	if(!istype(T, /obj/item/stack/tile/plasteel))
		...
		return
	if(contents.len >= 1)
		to_chat(user, "<span_class='warning'>They won't fit in, as there is already stuff inside!</span>")
		return
	if(T.use(10))
		var/obj/item/bot_assembly/floorbot/B = new
		B.toolbox = type
		user.put_in_hands(B)
		to_chat(user, "<span_class='notice'>You add the tiles into the empty [src.name]. They protrude from the top.</span>")
		qdel(src)
	else
		to_chat(user, "<span_class='warning'>You need 10 floor tiles to start building a floorbot!</span>")
		return
	...
	switch(build_step)
		...
		if(ASSEMBLY_SECOND_STEP)
			if(istype(W, /obj/item/bodypart/l_arm/robot) || istype(W, /obj/item/bodypart/r_arm/robot))
				...
				A.robot_arm = W.type
				A.toolbox = toolbox
				to_chat(user, "<span_class='notice'>You add [W] to [src]. Boop beep!</span>")
				qdel(W)
				qdel(src)
...
/obj/item/storage/firstaid/attackby(obj/item/bodypart/S, mob/user, params)
	...
	//Making a medibot!
	if(contents.len >= 1)
		to_chat(user, "<span_class='warning'>You need to empty [src] out first!</span>")
		return
	...
	if(istype(src, /obj/item/storage/firstaid/fire))
		...
		A.skin = "brute"
	user.put_in_hands(A)
	to_chat(user, "<span_class='notice'>You add [S] to [src].</span>")
	A.robot_arm = S.type
	A.firstaid = type
	qdel(S)
	qdel(src)
	...
	switch(build_step)
		...
		if(ASSEMBLY_SECOND_STEP)
			if(isprox(W))
				...
				to_chat(user, "<span_class='notice'>You complete the Medbot. Beep boop!</span>")
				S.name = created_name
				S.firstaid = firstaid
		...
		if(ASSEMBLY_FIRST_STEP)
			if(isprox(I))
				if(!user.temporarilyRemoveItemFromInventory(I))
					return
				to_chat(user, "<span_class='notice'>You add the [I] to [src]!</span>")
		...
		if(ASSEMBLY_SECOND_STEP)
			if(istype(I, /obj/item/bikehorn))
				if(!can_finish_build(I, user))
					return
				to_chat(user, "<span_class='notice'>You add the [I] to [src]! Honk!</span>")
	...
	switch(build_step)
		if(ASSEMBLY_FIRST_STEP)
			if(I.tool_behaviour == TOOL_WELDER)
				if(I.use_tool(src, user, 0, volume=40))
					add_overlay("hs_hole")
					to_chat(user, "<span_class='notice'>You weld a hole in [src]!</span>")
					build_step++
				...
				to_chat(user, "<span_class='notice'>You disconnect the signaler from the helmet.</span>")
				qdel(src)
		...
		if(ASSEMBLY_SECOND_STEP)
			if(isprox(I))
				...
				if(I.use_tool(src, user, 0, volume=40))
					cut_overlay("hs_hole")
					to_chat(user, "<span_class='notice'>You weld the hole in [src] shut!</span>")
					build_step--
		...
		if(ASSEMBLY_THIRD_STEP)
			if((istype(I, /obj/item/bodypart/l_arm/robot)) || (istype(I, /obj/item/bodypart/r_arm/robot)))
				...
				to_chat(user, "<span_class='notice'>You detach the proximity sensor from [src].</span>")
				build_step--
		...
		if(ASSEMBLY_FOURTH_STEP)
			if(istype(I, /obj/item/melee/baton))
				if(!can_finish_build(I, user))
					return
				to_chat(user, "<span_class='notice'>You complete the Securitron! Beep boop.</span>")
			...
			if(istype(I, /obj/item/toy/sword))
				if(toyswordamt < 3 && swordamt <= 0)
					...
					to_chat(user, "<span_class='notice'>You superglue [I] onto one of [src]'s arm slots.</span>")
					qdel(I)
					toyswordamt ++
				else
					if(!can_finish_build(I, user))
						return
					to_chat(user, "<span_class='notice'>You complete the Securitron!...Something seems a bit wrong with it..?</span>")
				...
				robot_arm = null
				to_chat(user, "<span_class='notice'>You remove [dropped_arm] from [src].</span>")
				build_step--
				if(toyswordamt > 0 || toyswordamt)
					icon_state = initial(icon_state)
					to_chat(user, "<span_class='notice'>The superglue binding [src]'s toy swords to its chassis snaps!</span>")
		...
		if(ASSEMBLY_FIFTH_STEP)
			if(istype(I, /obj/item/melee/transforming/energy/sword/saber))
				if(swordamt < 3)
					...
					icon_state = "grievous_assembly"
					to_chat(user, "<span_class='notice'>You bolt [I] onto one of [src]'s arm slots.</span>")
					qdel(I)
					swordamt ++
				else
					if(!can_finish_build(I, user))
				...
				build_step--
				icon_state = initial(icon_state)
				to_chat(user, "<span_class='notice'>You unbolt [src]'s energy swords</span>")
