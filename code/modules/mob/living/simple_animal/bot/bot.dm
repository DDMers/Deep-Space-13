...
/mob/living/simple_animal/bot/proc/get_mode()
	if(client) //Player bots do not have modes, thus the override. Also an easy way for PDA users/AI to know when a bot is a player.
		...
		return "<span_class='bad'>Inactive</span>"
	else if(!mode)
		return "<span_class='good'>Idle</span>"
	else
		return "<span_class='average'>[mode_name[mode]]</span>"
...
/mob/living/simple_animal/bot/emag_act(mob/user)
	if(locked) //First emag application unlocks the bot's interface. Apply a screwdriver to use the emag again.
		locked = FALSE
		emagged = 1
		to_chat(user, "<span_class='notice'>You bypass [src]'s controls.</span>")
		return
	if(!locked && open) //Bot panel is unlocked by ID or emag, and the panel is screwed open. Ready for emagging.
		...
		to_chat(src, "<span_class='userdanger'>(#$*#$^^( OVERRIDE DETECTED</span>")
		log_combat(user, src, "emagged")
		return
	else //Bot is unlocked, but the maint panel has not been opened with a screwdriver yet.
		to_chat(user, "<span_class='warning'>You need to open maintenance panel first!</span>")
...
/mob/living/simple_animal/bot/attack_ai(mob/user)
	if(!topic_denied(user))
		...
		to_chat(user, "<span_class='warning'>[src]'s interface is not responding!</span>")
...
/mob/living/simple_animal/bot/attackby(obj/item/W, mob/user, params)
	if(W.tool_behaviour == TOOL_SCREWDRIVER)
		if(!locked)
			open = !open
			to_chat(user, "<span_class='notice'>The maintenance panel is now [open ? "opened" : "closed"].</span>")
		else
			to_chat(user, "<span_class='warning'>The maintenance panel is locked.</span>")
		...
		if(bot_core.allowed(user) && !open && !emagged)
			...
			if(emagged)
				to_chat(user, "<span_class='danger'>ERROR</span>")
			if(open)
				to_chat(user, "<span_class='warning'>Please close the access panel before locking it.</span>")
			else
		...
		if(open)
			to_chat(user, "<span_class='warning'>Close the access panel before manipulating the personality slot!</span>")
		else
			to_chat(user, "<span_class='notice'>You attempt to pull [paicard] free...</span>")
			if(do_after(user, 30, target = src))
				if (paicard)
					user.visible_message("<span_class='notice'>[user] uses [W] to pull [paicard] out of [bot_name]!</span>","<span_class='notice'>You pull [paicard] out of [bot_name] with [W].</span>")
					ejectpai(user)
	else
		user.changeNext_move(CLICK_CD_MELEE)
		if(W.tool_behaviour == TOOL_WELDER && user.a_intent != INTENT_HARM)
			if(health >= maxHealth)
				to_chat(user, "<span_class='warning'>[src] does not need a repair!</span>")
				return
			if(!open)
				to_chat(user, "<span_class='warning'>Unable to repair with the maintenance panel closed!</span>")
				return

			if(W.use_tool(src, user, 0, volume=40))
				adjustHealth(-10)
				user.visible_message("[user] repairs [src]!","<span_class='notice'>You repair [src].</span>")
		else
...
/mob/living/simple_animal/bot/emp_act(severity)
	...
	if(paicard)
		...
		src.visible_message("[paicard] is flies out of [bot_name]!","<span_class='warning'>You are forcefully ejected from [bot_name]!</span>")
		ejectpai(0)
	if(on)
		turn_off()
	spawn(severity*300)
...
/mob/living/simple_animal/bot/proc/call_bot(caller, turf/waypoint, message=TRUE)
	...
	if(path && path.len) //Ensures that a valid path is calculated!
		...
		if(client)
			...
			to_chat(src, "<span_class='notice'><span_class='big'>Priority waypoint set by [icon2html(calling_ai, src)] <b>[caller]</b>. Proceed to <b>[end_area]</b>.</span><br>[path.len-1] meters to destination. You have been granted additional door access for 60 seconds.</span>")
		if(message)
			to_chat(calling_ai, "<span_class='notice'>[icon2html(src, calling_ai)] [name] called to [end_area]. [path.len-1] meters to destination.</span>")
		pathset = 1
		mode = BOT_RESPONDING
		tries = 0
	else
		if(message)
			to_chat(calling_ai, "<span_class='danger'>Failed to calculate a valid route. Ensure destination is clear of obstructions and within range.</span>")
		calling_ai = null
		set_path(null)
...
//Handles the bot's movement during a call.
	...
	if(!success)
		if(calling_ai)
			to_chat(calling_ai, "[icon2html(src, calling_ai)] [get_turf(src) == ai_waypoint ? "<span_class='notice'>[src] successfully arrived to waypoint.</span>" : "<span_class='danger'>[src] failed to reach waypoint.</span>"]")
			calling_ai = null
		bot_reset()
...
/mob/living/simple_animal/bot/proc/bot_reset()
	if(calling_ai) //Simple notification to the AI if it called a bot. It will not know the cause or identity of the bot.
		to_chat(calling_ai, "<span_class='danger'>Call command to a bot has been reset.</span>")
		calling_ai = null
...
/mob/living/simple_animal/bot/proc/bot_control_message(command,user,user_turf,user_access)
	switch(command)
		if("patroloff")
			to_chat(src, "<span_class='warning big'>STOP PATROL</span>")
		if("patrolon")
			to_chat(src, "<span_class='warning big'>START PATROL</span>")
		if("summon")
			var/area/a = get_area(user_turf)
			to_chat(src, "<span_class='warning big'>PRIORITY ALERT:[user] in [a.name]!</span>")
		if("stop")
			to_chat(src, "<span_class='warning big'>STOP!</span>")

		if("go")
			to_chat(src, "<span_class='warning big'>GO!</span>")

		if("home")
			to_chat(src, "<span_class='warning big'>RETURN HOME!</span>")
		if("ejectpai")
			return
		else
			to_chat(src, "<span_class='warning'>Unidentified control sequence received:[command]</span>")
...
/mob/living/simple_animal/bot/Topic(href, href_list)
	...
	if(topic_denied(usr))
		to_chat(usr, "<span_class='warning'>[src]'s interface is not responding!</span>")
		return TRUE
	add_fingerprint(usr)
	...
	switch(href_list["operation"])
		...
		if("hack")
			if(emagged != 2)
				...
				hacked = TRUE
				locked = TRUE
				to_chat(usr, "<span_class='warning'>[text_hack]</span>")
				bot_reset()
			else if(!hacked)
				to_chat(usr, "<span_class='boldannounce'>[text_dehack_fail]</span>")
			else
				emagged = FALSE
				hacked = FALSE
				to_chat(usr, "<span_class='notice'>[text_dehack]</span>")
				bot_reset()
		if("ejectpai")
			if(paicard && (!locked || issilicon(usr) || IsAdminGhost(usr)))
				to_chat(usr, "<span_class='notice'>You eject [paicard] from [bot_name]</span>")
				ejectpai(usr)
	update_controls()
...
/mob/living/simple_animal/bot/proc/hack(mob/user)
	...
	if(issilicon(user) || IsAdminGhost(user)) //Allows silicons or admins to toggle the emag status of a bot.
		...
		hack += "Harm Prevention Safety System: <A href='?src=[REF(src)];operation=hack'>[emagged ? "<span_class='bad'>DANGER</span>" : "Engaged"]</A><BR>"
...
/mob/living/simple_animal/bot/proc/insertpai(mob/user, obj/item/paicard/card)
	if(paicard)
		to_chat(user, "<span_class='warning'>A [paicard] is already inserted!</span>")
	else if(allow_pai && !key)
		if(!locked && !open)
			if(card.pai && card.pai.mind)
				if(!user.transferItemToLoc(card, src))
					return
				paicard = card
				user.visible_message("[user] inserts [card] into [src]!","<span_class='notice'>You insert [card] into [src].</span>")
				paicard.pai.mind.transfer_to(src)
				to_chat(src, "<span_class='notice'>You sense your form change as you are uploaded into [src].</span>")
				bot_name = name
				...
				to_chat(user, "<span_class='warning'>[card] is inactive.</span>")
		else
			to_chat(user, "<span_class='warning'>The personality slot is locked.</span>")
	else
		to_chat(user, "<span_class='warning'>[src] is not compatible with [card]</span>")
...
/mob/living/simple_animal/bot/proc/ejectpai(mob/user = null, announce = 1)
	if(paicard)
		...
		if(announce)
			to_chat(paicard.pai, "<span_class='notice'>You feel your control fade as [paicard] ejects from [bot_name].</span>")
		paicard = null
		name = bot_name
