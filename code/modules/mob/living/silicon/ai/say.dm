...
/mob/living/silicon/ai/radio(message, message_mode, list/spans, language)
	...
	if(!radio_enabled) //AI cannot speak if radio is disabled (via intellicard) or depowered.
		to_chat(src, "<span_class='danger'>Your radio transmitter is offline!</span>")
		return FALSE
	..()
...
/mob/living/silicon/ai/proc/holopad_talk(message, language)
	...
	if(istype(T) && T.masters[src])//If there is a hologram and its master is the user.
		...
		to_chat(src, "<i><span_class='game say'>Holopad transmitted, <span_class='name'>[real_name]</span> <span_class='message robot'>\"[message]\"</span></span></i>")
	else
...
/mob/living/silicon/ai/proc/announcement()
	...
	if(announcing_vox > world.time)
		to_chat(src, "<span_class='notice'>Please wait [DisplayTimeText(announcing_vox - world.time)].</span>")
		return
	...
	if(control_disabled)
		to_chat(src, "<span_class='warning'>Wireless interface disabled, unable to interact with announcement PA.</span>")
		return
	...
	if(incorrect_words.len)
		to_chat(src, "<span_class='notice'>These words are not available on the announcement system: [english_list(incorrect_words)].</span>")
		return
