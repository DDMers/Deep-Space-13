...
/mob/living/silicon/ai/proc/drop_new_multicam(silent = FALSE)
	if(!CONFIG_GET(flag/allow_ai_multicam))
		if(!silent)
			to_chat(src, "<span_class='warning'>This action is currently disabled. Contact an administrator to enable this feature.</span>")
		return
	if(!eyeobj)
		return
	if(multicam_screens.len >= max_multicams)
		if(!silent)
			to_chat(src, "<span_class='warning'>Cannot place more than [max_multicams] multicamera windows.</span>")
		return
	...
	C.set_ai(src)
	if(!silent)
		to_chat(src, "<span_class='notice'>Added new multicamera window.</span>")
	return C
...
/mob/living/silicon/ai/proc/start_multicam()
	...
	if(!GLOB.ai_camera_room_landmark)
		to_chat(src, "<span_class='warning'>This function is not available at this time.</span>")
		return
	multicam_on = TRUE
	refresh_multicam()
	to_chat(src, "<span_class='notice'>Multiple-camera viewing mode activated.</span>")
...
/mob/living/silicon/ai/proc/end_multicam()
	...
	if(client)
		for(var/V in multicam_screens)
			...
			P.unshow_to(client)
	reset_perspective()
	to_chat(src, "<span_class='notice'>Multiple-camera viewing mode deactivated.</span>")
