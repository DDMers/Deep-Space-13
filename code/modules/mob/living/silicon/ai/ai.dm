...
/mob/living/silicon/ai/Initialize(mapload, datum/ai_laws/L, mob/target_ai)
	...
	if(target_ai.mind)
		...
		if(mind.special_role)
			...
			to_chat(src, "<span_class='userdanger'>You have been installed as an AI! </span>")
			to_chat(src, "<span_class='danger'>You must obey your silicon laws above all else. Your objectives will consider you to be dead.</span>")
...
/mob/living/silicon/ai/proc/ai_call_shuttle()
	if(control_disabled)
		to_chat(usr, "<span_class='warning'>Wireless control is disabled!</span>")
		return
...
/mob/living/silicon/ai/proc/ai_cancel_call()
	...
	if(control_disabled)
		to_chat(src, "<span_class='warning'>Wireless control is disabled!</span>")
		return
...
/mob/living/silicon/ai/Topic(href, href_list)
	...
	if(href_list["jumptoholopad"])
		...
		if(H)
			...
			to_chat(src, "<span_class='notice'>Unable to locate the holopad.</span>")
	if(href_list["track"])
	...
	if(href_list["callbot"]) //Command a bot to move to a selected location.
		if(call_bot_cooldown > world.time)
			to_chat(src, "<span_class='danger'>Error: Your last call bot command is still processing, please wait for the bot to finish calculating a route.</span>")
			return
		Bot = locate(href_list["callbot"]) in GLOB.alive_mob_list
		if(!Bot || Bot.remote_disabled || control_disabled)
			return //True if there is no bot found, the bot is manually emagged, or the AI is carded with wireless off.
		waypoint_mode = 1
		to_chat(src, "<span_class='notice'>Set your waypoint by clicking on a valid location free of obstructions.</span>")
		return
	...
	if (href_list["ai_take_control"]) //Mech domination
		...
		if(controlled_mech)
			to_chat(src, "<span_class='warning'>You are already loaded into an onboard computer!</span>")
			return
		if(!GLOB.cameranet.checkCameraVis(M))
			to_chat(src, "<span_class='warning'>Exosuit is no longer near active cameras.</span>")
			return
		if(!isturf(loc))
			to_chat(src, "<span_class='warning'>You aren't in your core!</span>")
			return
		if(M)
...
/mob/living/silicon/ai/proc/botcall()
	...
	for (Bot in GLOB.alive_mob_list)
		if(Bot.z == ai_Zlevel && !Bot.remote_disabled) //Only non-emagged bots on the same Z-level are detected!
			...
			d += "<tr><td width='30%'>[Bot.hacked ? "<span_class='bad'>(!)</span>" : ""] [Bot.name]</A> ([Bot.model])</td>"
...
/mob/living/silicon/ai/proc/set_waypoint(atom/A)
	...
	if(turf_check in range(get_turf(src)))
		...
		to_chat(src, "<span_class='danger'>Selected location is not visible.</span>")
...
/mob/living/silicon/ai/proc/call_bot(turf/waypoint)
	...
	if(Bot.calling_ai && Bot.calling_ai != src) //Prevents an override if another AI is controlling this bot.
		to_chat(src, "<span_class='danger'>Interface error. Unit is already in use.</span>")
		return
	to_chat(src, "<span_class='notice'>Sending command to bot...</span>")
	...
	if(isnull(network))
		...
		for(var/obj/machinery/camera/C in GLOB.cameranet.cameras)
			...
			if(network in C.network)
				...
				break
	to_chat(src, "<span_class='notice'>Switched to the \"[uppertext(network)]\" camera network.</span>")
...
/mob/living/silicon/ai/proc/corereturn()
	...
	if(!istype(apc))
		to_chat(src, "<span_class='notice'>You are already in your Main Core.</span>")
		return
	apc.malfvacate()
...
/mob/living/silicon/ai/transfer_ai(interaction, mob/user, mob/living/silicon/ai/AI, obj/item/aicard/card)
	...
	if(interaction == AI_TRANS_TO_CARD)//The only possible interaction. Upload AI mob to a card.
		if(!can_be_carded)
			to_chat(user, "<span_class='boldwarning'>Transfer failed.</span>")
			return
		disconnect_shell() //If the AI is controlling a borg, force the player back to core!
		if(!mind)
			to_chat(user, "<span_class='warning'>No intelligence patterns detected.</span>"    )
			return
		...
		to_chat(user, "<span_class='boldnotice'>Transfer successful</span>: [name] ([rand(1000,9999)].exe) removed from host terminal and stored within local memory.")
...
/mob/living/silicon/ai/proc/relay_speech(message, atom/movable/speaker, datum/language/message_language, raw_message, radio_freq, list/spans, message_mode)
	...
	var/rendered = "<i><span_class='game say'>[start]<span_class='name'>[hrefpart][namepart] ([jobpart])</a> </span><span_class='message'>[raw_message]</span></span></i>"
...
/mob/living/silicon/ai/proc/malfhacked(obj/machinery/power/apc/apc)
	...
	if(!istype(apc) || QDELETED(apc) || apc.stat & BROKEN)
		to_chat(src, "<span_class='danger'>Hack aborted. The designated APC no longer exists on the power network.</span>")
		playsound(get_turf(src), 'sound/machines/buzz-two.ogg', 50, 1)
	else if(apc.aidisabled)
		to_chat(src, "<span_class='danger'>Hack aborted. \The [apc] is no longer responding to our systems.</span>")
...
/mob/living/silicon/ai/verb/deploy_to_shell(var/mob/living/silicon/robot/target)
	...
	if(incapacitated())
		return
	if(control_disabled)
		to_chat(src, "<span_class='warning'>Wireless networking module is offline.</span>")
		return
...
/mob/living/silicon/ai/proc/disconnect_shell()
	if(deployed_shell) //Forcibly call back AI in event of things such as damage, EMP or power loss.
		to_chat(src, "<span_class='danger'>Your remote connection has been reset!</span>")
