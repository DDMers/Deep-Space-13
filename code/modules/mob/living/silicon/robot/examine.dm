/mob/living/silicon/robot/examine(mob/user)
	...
	if(opened)
		msg += "<span_class='warning'>Its cover is open and the power cell is [cell ? "installed" : "missing"].</span>\n"
	else
		msg += "Its cover is closed[locked ? "" : ", and looks unlocked"].\n"

	if(cell && cell.charge <= 0)
		msg += "<span_class='warning'>Its battery indicator is blinking red!</span>\n"

	if(is_servant_of_ratvar(src) && get_dist(user, src) <= 1 && !stat) //To counter pseudo-stealth by using headlamps
		msg += "<span_class='warning'>Its eyes are glowing a blazing yellow!</span>\n"
	...
	switch(stat)
		...
		if(UNCONSCIOUS)
			msg += "<span_class='warning'>It doesn't seem to be responding.</span>\n"
		if(DEAD)
			msg += "<span_class='deadsay'>It looks like its system is corrupted and requires a reset.</span>\n"
	msg += "*---------*</span>"
