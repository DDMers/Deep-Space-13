...
/mob/living/proc/phaseout(obj/effect/decal/cleanable/B)
	if(iscarbon(src))
		...
		for(var/obj/item/I in C.held_items)
			...
			to_chat(C, "<span_class='warning'>You may not hold items while blood crawling!</span>")
			return 0
...
/mob/living/proc/bloodpool_sink(obj/effect/decal/cleanable/B)
	...
	src.visible_message("<span_class='warning'>[src] sinks into the pool of blood!</span>")
	...
	if(victim.stat == CONSCIOUS)
		src.visible_message("<span_class='warning'>[victim] kicks free of the blood pool just before entering it!</span>", null, "<span_class='notice'>You hear splashing and struggling.</span>")
	else if(victim.reagents && victim.reagents.has_reagent("demonsblood"))
		visible_message("<span_class='warning'>Something prevents [victim] from entering the pool!</span>", "<span_class='warning'>A strange force is blocking [victim] from entering!</span>", "<span_class='notice'>You hear a splash and a thud.</span>")
	else
		victim.forceMove(src)
		victim.emote("scream")
		src.visible_message("<span_class='warning'><b>[src] drags [victim] into the pool of blood!</b></span>", null, "<span_class='notice'>You hear a splash.</span>")
		kidnapped = TRUE

	if(kidnapped)
		var/success = bloodcrawl_consume(victim)
		if(!success)
			to_chat(src, "<span_class='danger'>You happily devour... nothing? Your meal vanished at some point!</span>")
	return 1

/mob/living/proc/bloodcrawl_consume(mob/living/victim)
	to_chat(src, "<span_class='danger'>You begin to feast on [victim]. You can not move while you are doing this.</span>")
	...
	if(victim.reagents && victim.reagents.has_reagent("devilskiss"))
		to_chat(src, "<span_class='warning'><b>AAH! THEIR FLESH! IT BURNS!</b></span>")
		...
		for(var/obj/effect/decal/cleanable/target in range(1,get_turf(victim)))
			if(target.can_bloodcrawl_in())
				...
				victim.visible_message("<span_class='warning'>[target] violently expels [victim]!</span>")
		...
		if(!found_bloodpool)
			...
			victim.visible_message("<span_class='warning'>[victim] appears from nowhere, covered in blood!</span>")
			victim.exit_blood_effect()
		return TRUE

	to_chat(src, "<span_class='danger'>You devour [victim]. Your health is fully restored.</span>")
...
/mob/living/proc/phasein(obj/effect/decal/cleanable/B)
	if(src.notransform)
		to_chat(src, "<span_class='warning'>Finish eating first!</span>")
		return 0
	B.visible_message("<span_class='warning'>[B] starts to bubble...</span>")
	...
	forceMove(B.loc)
	src.client.eye = src
	src.visible_message("<span_class='warning'><B>[src] rises out of the pool of blood!</B></span>")
	exit_blood_effect(B)
	if(iscarbon(src))
		var/mob/living/carbon/C = src
