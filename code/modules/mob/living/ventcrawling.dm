...
/mob/living/proc/handle_ventcrawl(atom/A)
	...
	if(vent_found)
		...
		if(vent_found_parent && (vent_found_parent.members.len || vent_found_parent.other_atmosmch))
			visible_message("<span_class='notice'>[src] begins climbing into the ventilation system...</span>" ,"<span_class='notice'>You begin climbing into the ventilation system...</span>")
			...
			if(iscarbon(src) && ventcrawler < 2)//It must have atleast been 1 to get this far
				...
				for(var/obj/item/I in held_items)
					...
					break
				if(failed)
					to_chat(src, "<span_class='warning'>You can't crawl around in the ventilation ducts with items!</span>")
					return

			visible_message("<span_class='notice'>[src] scrambles into the ventilation ducts!</span>","<span_class='notice'>You climb into the ventilation ducts.</span>")
			forceMove(vent_found)
	else
		to_chat(src, "<span_class='warning'>This ventilation duct is not connected to anything!</span>")
