...
/mob/proc/change_mob_type(new_type = null, turf/location = null, new_name = null as text, delete_old_mob = FALSE)
	...
	if(isnewplayer(src))
		to_chat(usr, "<span_class='danger'>Cannot convert players who have not entered yet.</span>")
		return
	...
	if(ispath(new_type, /mob/dead/new_player))
		to_chat(usr, "<span_class='danger'>Cannot convert into a new_player mob type.</span>")
		return
