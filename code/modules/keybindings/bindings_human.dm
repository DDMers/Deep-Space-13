		...
		switch(_key)
			if("E") // Put held thing in belt or take out most recent thing from belt
				...
				if(!equipped_belt) // We also let you equip a belt like this
					if(!thing)
						to_chat(user, "<span_class='notice'>You have no belt to take something out of.</span>")
						return
				...
				if(!SEND_SIGNAL(equipped_belt, COMSIG_CONTAINS_STORAGE)) // not a storage item
					if(!thing)
						...
						to_chat(user, "<span_class='notice'>You can't fit anything in.</span>")
					return
				...
				if(!equipped_belt.contents.len) // nothing to take out
					to_chat(user, "<span_class='notice'>There's nothing in your belt to take out.</span>")
					return
			...
			if("B") // Put held thing in backpack or take out most recent thing from backpack
				...
				if(!equipped_back) // We also let you equip a backpack like this
					if(!thing)
						to_chat(user, "<span_class='notice'>You have no backpack to take something out of.</span>")
						return
				...
				if(!equipped_back.contents.len) // nothing to take out
					to_chat(user, "<span_class='notice'>There's nothing in your backpack to take out.</span>")
					return
