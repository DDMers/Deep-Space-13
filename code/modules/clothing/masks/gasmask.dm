...
/obj/item/clothing/mask/gas/clown_hat/ui_action_click(mob/user)
	...
	if(src && choice && !user.incapacitated() && in_range(user,src))
		...
		for(var/X in actions)
			...
			A.UpdateButtonIcon()
		to_chat(user, "<span_class='notice'>Your Clown Mask has now morphed into [choice], all praise the Honkmother!</span>")
		return 1
...
/obj/item/clothing/mask/gas/mime/ui_action_click(mob/user)
	...
	if(src && choice && !user.incapacitated() && in_range(user,src))
		...
		for(var/X in actions)
			var/datum/action/A = X
			A.UpdateButtonIcon()
		to_chat(user, "<span_class='notice'>Your Mime Mask has now morphed into [choice]!</span>")
		return 1
