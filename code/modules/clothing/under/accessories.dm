...
/obj/item/clothing/accessory/proc/can_attach_accessory(obj/item/clothing/U, mob/user)
	if(!attachment_slot || (U && U.body_parts_covered & attachment_slot))
		return TRUE
	if(user)
		to_chat(user, "<span_class='warning'>There doesn't seem to be anywhere to put [src]...</span>")
...
/obj/item/clothing/accessory/examine(mob/user)
	..()
	to_chat(user, "<span_class='notice'>\The [src] can be attached to a uniform. Alt-click to remove it once attached.</span>")
	if(initial(above_suit))
		to_chat(user, "<span_class='notice'>\The [src] can be worn above or below your suit. Alt-click to toggle.</span>")
	...
	if(ishuman(M) && (user.a_intent == INTENT_HELP))
		...
		if(M.wear_suit)
			if((M.wear_suit.flags_inv & HIDEJUMPSUIT)) //Check if the jumpsuit is covered
				to_chat(user, "<span_class='warning'>Medals can only be pinned on jumpsuits.</span>")
				return
		...
		if(M.w_uniform)
			...
			if(user == M)
				...
				user.visible_message("[user] is trying to pin [src] on [M]'s chest.", \
									 "<span_class='notice'>You try to pin [src] on [M]'s chest.</span>")
			var/input
			if(!commended && user != M)
			...
			if(do_after(user, delay, target = M))
				if(U.attach_accessory(src, user, 0)) //Attach it, do not notify the user of the attachment
					if(user == M)
						to_chat(user, "<span_class='notice'>You attach [src] to [U].</span>")
					else
						user.visible_message("[user] pins \the [src] on [M]'s chest.", \
											 "<span_class='notice'>You pin \the [src] on [M]'s chest.</span>")
						if(input)
							SSblackbox.record_feedback("associative", "commendation", 1, list("commender" = "[user.real_name]", "commendee" = "[M.real_name]", "medal" = "[src]", "reason" = input))
							GLOB.commendations += "[user.real_name] awarded <b>[M.real_name]</b> the <span_class='medaltext'>[name]</span>! \n- [input]"
							commended = TRUE
...
/obj/item/clothing/accessory/medal/plasma/temperature_expose(datum/gas_mixture/air, exposed_temperature, exposed_volume)
	if(exposed_temperature > 300)
		...
		visible_message("<span_class='danger'> \The [src] bursts into flame!</span>","<span_class='userdanger'>Your [src] bursts into flame!</span>")
		qdel(src)
...
/obj/item/clothing/accessory/lawyers_badge/attack_self(mob/user)
	...
	user.visible_message("[user] shows [user.p_their()] attorney's badge.", "<span_class='notice'>You show your attorney's badge.</span>")
