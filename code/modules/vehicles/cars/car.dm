...
/obj/vehicle/sealed/car/driver_move(mob/user, direction)
	if(key_type && !is_key(inserted_key))
		to_chat(user, "<span_class='warning'>[src] has no key inserted!</span>")
		return FALSE
...
/obj/vehicle/sealed/car/MouseDrop_T(atom/dropping, mob/M)
	...
	if((car_traits & CAN_KIDNAP) && isliving(dropping) && M != dropping)
		...
		L.visible_message("<span_class='warning'>[M] starts forcing [L] into [src]!</span>")
...
/obj/vehicle/sealed/car/mob_try_exit(mob/M, mob/user, silent = FALSE)
	if(M == user && (occupants[M] & VEHICLE_CONTROL_KIDNAPPED))
		to_chat(user, "<span_class='notice'>You push against the back of [src] trunk to try and get out.</span>")
		if(!do_after(user, escape_time, target = src))
			return FALSE
		to_chat(user,"<span_class='danger'>[user] gets out of [src]</span>")
		mob_exit(M, silent)
		return TRUE
	mob_exit(M, silent)
	return TRUE
...
/obj/vehicle/sealed/car/attacked_by(obj/item/I, mob/living/user)
	if(!I.force)
		return
	if(occupants[user])
		to_chat(user, "<span_class='notice'>Your attack bounces off of the car's padded interior.</span>")
		return
	return ..()
...
/obj/vehicle/sealed/car/attack_hand(mob/living/user)
	...
	if(occupants[user])
		return
	to_chat(user, "<span_class='notice'>You start opening [src]'s trunk.</span>")
	if(do_after(user, 30))
		if(return_amount_of_controllers_with_flag(VEHICLE_CONTROL_KIDNAPPED))
			to_chat(user, "<span_class='notice'>The people stuck in [src]'s trunk all come tumbling out.</span>")
			DumpSpecificMobs(VEHICLE_CONTROL_KIDNAPPED)
		else
			to_chat(user, "<span_class='notice'>It seems [src]'s trunk was empty.</span>")
...
/obj/vehicle/sealed/car/proc/mob_forced_enter(mob/M, silent = FALSE)
	if(!silent)
		M.visible_message("<span_class='warning'>[M] is forced into \the [src]!</span>")
	M.forceMove(src)
