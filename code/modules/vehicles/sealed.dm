...
/obj/vehicle/sealed/proc/mob_enter(mob/M, silent = FALSE)
	if(!istype(M))
		return FALSE
	if(!silent)
		M.visible_message("<span_class='notice'>[M] climbs into \the [src]!</span>")
	M.forceMove(src)
	add_occupant(M)
	return TRUE
...
/obj/vehicle/sealed/proc/mob_exit(mob/M, silent = FALSE, randomstep = FALSE)
	...
	if(!silent)
		M.visible_message("<span_class='notice'>[M] drops out of \the [src]!</span>")
	return TRUE
...
/obj/vehicle/sealed/proc/remove_key(mob/user)
	if(!inserted_key)
		to_chat(user, "<span_class='notice'>There is no key in [src]!</span>")
		return
	if(!is_occupant(user) || !(occupants[user] & VEHICLE_CONTROL_DRIVE))
		to_chat(user, "<span_class='notice'>You must be driving [src] to remove [src]'s key!</span>")
		return
	to_chat(user, "<span_class='notice'>You remove [inserted_key] from [src].</span>")
	inserted_key.forceMove(drop_location())
