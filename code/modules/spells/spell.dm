...
/obj/effect/proc_holder/proc/InterceptClickOn(mob/living/caller, params, atom/A)
	if(caller.ranged_ability != src || ranged_ability_user != caller) //I'm not actually sure how these would trigger, but, uh, safety, I guess?
		to_chat(caller, "<span_class='warning'><b>[caller.ranged_ability.name]</b> has been disabled.</span>")
...
/obj/effect/proc_holder/proc/add_ranged_ability(mob/living/user, msg, forced)
	...
	if(user.ranged_ability && user.ranged_ability != src)
		if(forced)
			to_chat(user, "<span_class='warning'><b>[user.ranged_ability.name]</b> has been replaced by <b>[name]</b>.</span>")
...
/obj/effect/proc_holder/spell
	...
	var/still_recharging_msg = "<span_class='notice'>The spell is still recharging.</span>"
	var/recharging = TRUE
...
/obj/effect/proc_holder/spell/proc/cast_check(skipcharge = 0,mob/user = usr) //checks if the spell can be cast based on its settings; skipcharge is used when an additional cast_check is called inside the spell
	if(player_lock)
		if(!user.mind || !(src in user.mind.spell_list) && !(src in user.mob_spell_list))
			to_chat(user, "<span_class='warning'>You shouldn't have this spell! Something's wrong.</span>")
			return FALSE
	else
	...
	if(is_centcom_level(T.z) && !centcom_cancast) //Certain spells are not allowed on the centcom zlevel
		to_chat(user, "<span_class='notice'>You can't cast this spell here.</span>")
		return FALSE
	...
	if(user.stat && !stat_allowed)
		to_chat(user, "<span_class='notice'>Not when you're incapacitated.</span>")
		return FALSE
	...
	if(!antimagic_allowed)
		...
		if(antimagic)
			if(isitem(antimagic))
				to_chat(user, "<span_class='notice'>[antimagic] is interfering with your magic.</span>")
			else
				to_chat(user, "<span_class='notice'>Magic seems to flee from you, you can't gather enough power to cast this spell.</span>")
			return FALSE

	if(!phase_allowed && istype(user.loc, /obj/effect/dummy))
		to_chat(user, "<span_class='notice'>[name] cannot be cast unless you are completely manifested in the material plane.</span>")
		return FALSE
	...
	if(ishuman(user))
		...
		if(clothes_req) //clothes check
			if(!is_type_in_typecache(H.wear_suit, casting_clothes))
				to_chat(H, "<span_class='notice'>I don't feel strong enough without my robe.</span>")
				return FALSE
			if(!is_type_in_typecache(H.head, casting_clothes))
				to_chat(H, "<span_class='notice'>I don't feel strong enough without my hat.</span>")
				return FALSE
		if(cult_req) //CULT_REQ CLOTHES CHECK
			if(!istype(H.wear_suit, /obj/item/clothing/suit/magusred) && !istype(H.wear_suit, /obj/item/clothing/suit/space/hardsuit/cult))
				to_chat(H, "<span_class='notice'>I don't feel strong enough without my armor.</span>")
				return FALSE
			if(!istype(H.head, /obj/item/clothing/head/magus) && !istype(H.head, /obj/item/clothing/head/helmet/space/hardsuit/cult))
				to_chat(H, "<span_class='notice'>I don't feel strong enough without my helmet.</span>")
				return FALSE
	else
		if(clothes_req || human_req)
			to_chat(user, "<span_class='notice'>This spell can only be cast by humans!</span>")
			return FALSE
		if(nonabstract_req && (isbrain(user) || ispAI(user)))
			to_chat(user, "<span_class='notice'>This spell can only be cast by physical beings!</span>")
			return FALSE
...
/obj/effect/proc_holder/spell/proc/charge_check(mob/user, silent = FALSE)
	switch(charge_type)
		...
		if("charges")
			if(!charge_counter)
				if(!silent)
					to_chat(user, "<span_class='notice'>[name] has no charges left.</span>")
				return FALSE
	return TRUE
...
/obj/effect/proc_holder/spell/Initialize()
	...
	still_recharging_msg = "<span_class='notice'>[name] is still recharging.</span>"
...
/obj/effect/proc_holder/spell/proc/perform(list/targets, recharge = TRUE, mob/user = usr) //if recharge is started is important for the trigger spells
	...
	invocation(user)
	if(user && user.ckey)
		user.log_message("<span_class='danger'>cast the spell [name].</span>", LOG_ATTACK)
	if(recharge)
		recharging = TRUE
	if(sound)
		playMagSound()
...
/obj/effect/proc_holder/spell/self/basic_heal/cast(mob/living/carbon/human/user) //Note the lack of "list/targets" here. Instead, use a "user" var depending on mob requirements.
	...
	user.visible_message("<span_class='warning'>A wreath of gentle light passes over [user]!</span>", "<span_class='notice'>You wreath yourself in healing light!</span>")
