...
/obj/effect/proc_holder/spell/targeted/abyssal_gaze/cast(list/targets, mob/user = usr)
	...
	if(!(target in oview(range)))
		to_chat(user, "<span_class='notice'>[target] is too far away!</span>")
		revert_cast()
		return

	if(target.anti_magic_check(TRUE, TRUE))
		to_chat(target, "<span_class='warning'>You feel a freezing darkness closing in on you, but it rapidly dissipates.</span>")
		return

	to_chat(target, "<span_class='userdanger'>A freezing darkness surrounds you...</span>")
...
/obj/effect/proc_holder/spell/targeted/dominate/cast(list/targets, mob/user = usr)
	...
	if(S.ckey)
		to_chat(user, "<span_class='warning'>[S] is too intelligent to dominate!</span>")
		revert_cast()
		return

	if(S.stat)
		to_chat(user, "<span_class='warning'>[S] is dead!</span>")
		revert_cast()
		return

	if(S.sentience_type != SENTIENCE_ORGANIC)
		to_chat(user, "<span_class='warning'>[S] cannot be dominated!</span>")
		revert_cast()
		return

	if(!(S in oview(range)))
		to_chat(user, "<span_class='notice'>[S] is too far away!</span>")
		revert_cast()
		return
