...
/obj/effect/proc_holder/spell/self/tap/cast(mob/living/user = usr)
	if(!user.mind.hasSoul)
		to_chat(user, "<span_class='warning'>You do not possess a soul to tap into!</span>")
		return
	to_chat(user, "<span_class='danger'>Your body feels drained and there is a burning pain in your chest.</span>")
	user.maxHealth -= HEALTH_LOST_PER_SOUL_TAP
	user.health = min(user.health, user.maxHealth)
	if(user.maxHealth <= 0)
		to_chat(user, "<span_class='userdanger'>Your weakened soul is completely consumed by the tap!</span>")
