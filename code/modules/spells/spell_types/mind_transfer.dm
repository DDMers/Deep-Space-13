...
/obj/effect/proc_holder/spell/targeted/mind_transfer/cast(list/targets, mob/living/user = usr, distanceoverride, silent = FALSE)
	if(!targets.len)
		if(!silent)
			to_chat(user, "<span_class='warning'>No mind found!</span>")
		return

	if(targets.len > 1)
		if(!silent)
			to_chat(user, "<span_class='warning'>Too many minds! You're not a hive damnit!</span>")
		return
	...
	if(!(target in oview(range)) && !distanceoverride)//If they are not in overview after selection. Do note that !() is necessary for in to work because ! takes precedence over it.
		if(!silent)
			to_chat(user, "<span_class='warning'>[t_He] [t_is] too far away!</span>")
		return

	if(ismegafauna(target))
		if(!silent)
			to_chat(user, "<span_class='warning'>This creature is too powerful to control!</span>")
		return

	if(target.stat == DEAD)
		if(!silent)
			to_chat(user, "<span_class='warning'>You don't particularly want to be dead!</span>")
		return

	if(!target.key || !target.mind)
		if(!silent)
			to_chat(user, "<span_class='warning'>[t_He] appear[target.p_s()] to be catatonic! Not even magic can affect [target.p_their()] vacant mind.</span>")
		return

	if(user.suiciding)
		if(!silent)
			to_chat(user, "<span_class='warning'>You're killing yourself! You can't concentrate enough to do this!</span>")
		return
	...
	if((target.anti_magic_check() || TM.has_antag_datum(/datum/antagonist/wizard) || TM.has_antag_datum(/datum/antagonist/cult) || TM.has_antag_datum(/datum/antagonist/clockcult) || TM.has_antag_datum(/datum/antagonist/changeling) || TM.has_antag_datum(/datum/antagonist/rev)) || cmptext(copytext(target.key,1,2),"@"))
		if(!silent)
			to_chat(user, "<span_class='warning'>[target.p_their(TRUE)] mind is resisting your spell!</span>")
		return
