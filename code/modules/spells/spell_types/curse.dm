...
/proc/curse_of_madness(mob/user, message)
	if(user) //in this case either someone holding a spellbook or a badmin
		to_chat(user, "<span_class='warning'>You sent a curse of madness with the message \"[message]\"!</span>")
	...
	deadchat_broadcast("<span_class='deadsay'>A <span_class='name'>Curse of Madness</span> has stricken the station, shattering their minds with the awful secret: \"<span_class='big hypnophrase'>[message]</span>\"</span>")
	...
	for(var/mob/living/carbon/human/H in GLOB.player_list)
		...
		if(H.anti_magic_check(TRUE, FALSE))
			to_chat(H, "<span_class='notice'>You have a strange feeling for a moment, but then it passes.</span>")
			continue
		if(istype(H.get_item_by_slot(SLOT_HEAD), /obj/item/clothing/head/foilhat))
			to_chat(H, "<span_class='warning'>Your protective headgear successfully deflects mind controlling brainwaves!</span>")
			continue
		give_madness(H, message)

/proc/give_madness(mob/living/carbon/human/H, message)
	H.playsound_local(H,'sound/magic/curse.ogg',40,1)
	to_chat(H, "<span_class='reallybig hypnophrase'>[message]</span>")
	to_chat(H, "<span_class='warning'>Your mind shatters!</span>")
	switch(rand(1,10))
		if(1 to 3)
