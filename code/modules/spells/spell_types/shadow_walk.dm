...
/obj/effect/proc_holder/spell/targeted/shadowwalk/cast(list/targets,mob/living/user = usr)
	...
	if(istype(user.loc, /obj/effect/dummy/phased_mob/shadow))
		...
		if(light_amount < SHADOW_SPECIES_LIGHT_THRESHOLD)
			...
			visible_message("<span_class='boldwarning'>[user] melts into the shadows!</span>")
			...
			to_chat(user, "<span_class='warning'>It isn't dark enough here!</span>")
...
/obj/effect/dummy/phased_mob/shadow/relaymove(mob/user, direction)
	...
	if(isspaceturf(newLoc))
		to_chat(user, "<span_class='warning'>It really would not be wise to go into space.</span>")
		return
	forceMove(newLoc)
	check_light_level()
...
/obj/effect/dummy/phased_mob/shadow/proc/end_jaunt(forced = FALSE)
	if(jaunter)
		if(forced)
			visible_message("<span_class='boldwarning'>[jaunter] is revealed by the light!</span>")
		else
			visible_message("<span_class='boldwarning'>[jaunter] emerges from the darkness!</span>")
