...
/obj/effect/proc_holder/spell/targeted/summon_contract/cast(list/targets, mob/user = usr)
	for(var/mob/living/carbon/C in targets)
		if(C.mind && user.mind)
			...
			to_chat(user, "<span_class='notice'>[C] seems to not be sentient.  You cannot summon a contract for [C.p_them()].</span>")
	...
	if(istype(user))
		if(istype(user.loc, /obj/effect/dummy/phased_mob/slaughter/))
			if(valid_location(user))
				to_chat(user, "<span_class='warning'>You are now phasing in.</span>")
				if(do_mob(user,user,150))
					if(valid_location(user))
						user.infernalphasein()
					else
						to_chat(user, "<span_class='warning'>You are no longer near a potential signer.</span>")

			else
				to_chat(user, "<span_class='warning'>You can only re-appear near a potential signer.</span>")
				revert_cast()
				return ..()
		else
			user.notransform = TRUE
			user.fakefire()
			to_chat(src, "<span_class='warning'>You begin to phase back into sinful flames.</span>")
			if(do_mob(user,user,150))
				user.infernalphaseout()
			else
				to_chat(user, "<span_class='warning'>You must remain still while exiting.</span>")
...
/mob/living/proc/infernalphaseout()
	dust_animation()
	spawn_dust()
	visible_message("<span_class='warning'>[src] disappears in a flashfire!</span>")
...
/mob/living/proc/infernalphasein()
	if(notransform)
		to_chat(src, "<span_class='warning'>You're too busy to jaunt in.</span>")
		return FALSE
	fakefire()
	forceMove(drop_location())
	client.eye = src
	visible_message("<span_class='warning'><B>[src] appears in a fiery blaze!</B></span>")
...
/obj/effect/proc_holder/spell/targeted/summon_dancefloor/cast(list/targets, mob/user = usr)
	...
	if(dancefloor_exists)
		...
		for(var/turf/closed/solid in funky_turfs)
			to_chat(user, "<span_class='warning'>You're too close to a wall.</span>")
			return
