...
/obj/effect/proc_holder/spell/targeted/lichdom/cast(list/targets,mob/user = usr)
	for(var/mob/M in targets)
		...
		if(!hand_items.len)
			to_chat(M, "<span_class='caution'>You must hold an item you wish to make your phylactery...</span>")
			return
		if(!M.mind.hasSoul)
			to_chat(user, "<span_class='caution'>You do not possess a soul.</span>")
			return
		...
		for(var/obj/item/item in hand_items)
			...
			if((item.item_flags & ABSTRACT) || (item.item_flags & NODROP) || SEND_SIGNAL(item, COMSIG_ITEM_IMBUE_SOUL, user))
				continue
			marked_item = item
			to_chat(M, "<span_class='warning'>You begin to focus your very being into [item]...</span>")
			break

		if(!marked_item)
			to_chat(M, "<span_class='warning'>None of the items you hold are suitable for emplacement of your fragile soul.</span>")
			return

		playsound(user, 'sound/effects/pope_entry.ogg', 100)

		if(!do_after(M, 50, needhand=FALSE, target=marked_item))
			to_chat(M, "<span_class='warning'>Your soul snaps back to your body as you stop ensouling [marked_item]!</span>")
			return
		...
		to_chat(M, "<span_class='userdanger'>With a hideous feeling of emptiness you watch in horrified fascination as skin sloughs off bone! Blood boils, nerves disintegrate, eyes boil in their sockets! As your organs crumble to dust in your fleshless chest you come to terms with your choice. You're a lich!</span>")
...
/obj/item/phylactery/proc/rise()
	...
	to_chat(lich, "<span_class='warning'>Your bones clatter and shudder as you are pulled back into this world!</span>")
	...
	if(old_body && old_body.loc)
		...
		if(wheres_wizdo)
			old_body.visible_message("<span_class='warning'>Suddenly [old_body.name]'s corpse falls to pieces! You see a strange energy rise from the remains, and speed off towards the [wheres_wizdo]!</span>")
