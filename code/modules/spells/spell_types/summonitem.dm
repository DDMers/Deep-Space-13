...
/obj/effect/proc_holder/spell/targeted/summonitem/cast(list/targets,mob/user = usr)
	for(var/mob/living/L in targets)
		...
		if(!marked_item) //linking item to the spell
			...
			if(!marked_item)
				if(hand_items)
					message = "<span_class='caution'>You aren't holding anything that can be marked for recall.</span>"
				else
					message = "<span_class='notice'>You must hold the desired item in your hands to mark it for recall.</span>"

		else if(marked_item && marked_item in hand_items) //unlinking item to the spell
			message = "<span_class='notice'>You remove the mark on [marked_item] to use elsewhere.</span>"
			name = "Instant Summons"
			marked_item = 		null

		else if(marked_item && QDELETED(marked_item)) //the item was destroyed at some point
			message = "<span_class='warning'>You sense your marked item has been destroyed!</span>"
			name = "Instant Summons"
			marked_item = 		null
				...
				while(!isturf(item_to_retrieve.loc) && infinite_recursion < 10) //if it's in something you get the whole thing.
					...
					if(ismob(item_to_retrieve.loc)) //If its on someone, properly drop it
						...
						if(issilicon(M)) //Items in silicons warp the whole silicon
							M.loc.visible_message("<span_class='warning'>[M] suddenly disappears!</span>")
							M.forceMove(L.loc)
							M.loc.visible_message("<span_class='caution'>[M] suddenly appears!</span>")
						...
						if(iscarbon(M)) //Edge case housekeeping
							...
							for(var/X in C.bodyparts)
								...
								if(item_to_retrieve in part.embedded_objects)
									...
									to_chat(C, "<span_class='warning'>The [item_to_retrieve] that was embedded in your [L] has mysteriously vanished. How fortunate!</span>")
			...
			if(item_to_retrieve.loc)
				item_to_retrieve.loc.visible_message("<span_class='warning'>The [item_to_retrieve.name] suddenly disappears!</span>")
			if(!L.put_in_hands(item_to_retrieve))
				item_to_retrieve.forceMove(L.drop_location())
				item_to_retrieve.loc.visible_message("<span_class='caution'>The [item_to_retrieve.name] suddenly appears!</span>")
				playsound(get_turf(L), 'sound/magic/summonitems_generic.ogg', 50, 1)
			else
				item_to_retrieve.loc.visible_message("<span_class='caution'>The [item_to_retrieve.name] suddenly appears in [L]'s hand!</span>")
				playsound(get_turf(L), 'sound/magic/summonitems_generic.ogg', 50, 1)
