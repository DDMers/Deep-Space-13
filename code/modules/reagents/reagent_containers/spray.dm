...
/obj/item/reagent_containers/spray/afterattack(atom/A, mob/user)
	...
	if((A.is_drainable() && !A.is_refillable()) && get_dist(src,A) <= 1 && can_fill_from_container)
		if(!A.reagents.total_volume)
			to_chat(user, "<span_class='warning'>[A] is empty.</span>")
			return
		...
		to_chat(user, "<span_class='notice'>You fill \the [src] with [trans] units of the contents of \the [A].</span>")
		return

	if(reagents.total_volume < amount_per_transfer_from_this)
		to_chat(user, "<span_class='warning'>Not enough left!</span>")
		return
...
/obj/item/reagent_containers/spray/attack_self(mob/user)
	...
	to_chat(user, "<span_class='notice'>You switch the nozzle setting to [stream_mode ? "\"stream\"":"\"spray\""]. You'll now use [amount_per_transfer_from_this] units per use.</span>")
...
/obj/item/reagent_containers/spray/verb/empty()
	...
	if(isturf(usr.loc) && src.loc == usr)
		to_chat(usr, "<span_class='notice'>You empty \the [src] onto the floor.</span>")
...
/obj/item/reagent_containers/spray/cleaner/suicide_act(mob/user)
	user.visible_message("<span_class='suicide'>[user] is putting the nozzle of \the [src] in [user.p_their()] mouth.  It looks like [user.p_theyre()] trying to commit suicide!</span>")
	if(do_mob(user,user,30))
		if(reagents.total_volume >= amount_per_transfer_from_this)//if not empty
			user.visible_message("<span_class='suicide'>[user] pulls the trigger!</span>")
			src.spray(user)
			return BRUTELOSS
		else
			user.visible_message("<span_class='suicide'>[user] pulls the trigger...but \the [src] is empty!</span>")
			return SHAME
	else
		user.visible_message("<span_class='suicide'>[user] decided life was worth living.</span>")
		return
...
/obj/item/reagent_containers/spray/pepper/suicide_act(mob/living/carbon/user)
	user.visible_message("<span_class='suicide'>[user] begins huffing \the [src]! It looks like [user.p_theyre()] getting a dirty high!</span>")
	return OXYLOSS
...
/obj/item/reagent_containers/spray/waterflower/cyborg/empty()
	to_chat(usr, "<span_class='warning'>You can not empty this!</span>")
	return
