...
/obj/item/reagent_containers/medspray/attack_self(mob/user)
	...
	to_chat(user, "<span_class='notice'>You will now apply the medspray's contents in [squirt_mode ? "short bursts":"extended sprays"]. You'll now use [amount_per_transfer_from_this] units per use.</span>")
...
/obj/item/reagent_containers/medspray/attack(mob/M, mob/user, def_zone)
	...
	if(M == user)
		M.visible_message("<span_class='notice'>[user] attempts to [apply_method] [src] on [user.p_them()]self.</span>")
		if(self_delay)
			if(!do_mob(user, M, self_delay))
				return
			if(!reagents || !reagents.total_volume)
				return
		to_chat(M, "<span_class='notice'>You [apply_method] yourself with [src].</span>")

	else
		log_combat(user, M, "attempted to apply", src, reagents.log_list())
		M.visible_message("<span_class='danger'>[user] attempts to [apply_method] [src] on [M].</span>", \
							"<span_class='userdanger'>[user] attempts to [apply_method] [src] on [M].</span>")
		if(!do_mob(user, M))
			return
		if(!reagents || !reagents.total_volume)
			return
		M.visible_message("<span_class='danger'>[user] [apply_method]s [M] down with [src].</span>", \
							"<span_class='userdanger'>[user] [apply_method]s [M] down with [src].</span>")
