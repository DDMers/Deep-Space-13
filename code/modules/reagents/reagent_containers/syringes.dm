	...
	switch(mode)
		if(SYRINGE_DRAW)
			...
			if(reagents.total_volume >= reagents.maximum_volume)
				to_chat(user, "<span_class='notice'>The syringe is full.</span>")
				return

			if(L) //living mob
				var/drawn_amount = reagents.maximum_volume - reagents.total_volume
				if(target != user)
					target.visible_message("<span_class='danger'>[user] is trying to take a blood sample from [target]!</span>", \
									"<span_class='userdanger'>[user] is trying to take a blood sample from [target]!</span>")
					busy = TRUE
				...
				if(L.transfer_blood_to(src, drawn_amount))
					...
					to_chat(user, "<span_class='warning'>You are unable to draw any blood from [L]!</span>")
				...
				if(!target.is_drawable(user))
					to_chat(user, "<span_class='warning'>You cannot directly remove reagents from [target]!</span>")
					return

				var/trans = target.reagents.trans_to(src, amount_per_transfer_from_this, transfered_by = user) // transfer from, transfer to - who cares?

				to_chat(user, "<span_class='notice'>You fill [src] with [trans] units of the solution. It now contains [reagents.total_volume] units.</span>")
			if (reagents.total_volume >= reagents.maximum_volume)
				mode=!mode
				update_icon()
		...
		if(SYRINGE_INJECT)
			...
			if(!reagents.total_volume)
				to_chat(user, "<span_class='notice'>[src] is empty.</span>")
				return

			if(!L && !target.is_injectable(user)) //only checks on non-living mobs, due to how can_inject() handles
				to_chat(user, "<span_class='warning'>You cannot directly fill [target]!</span>")
				return
			...
			if(L) //living mob
				if(!L.can_inject(user, TRUE))
					return
				if(L != user)
					L.visible_message("<span_class='danger'>[user] is trying to inject [L]!</span>", \
											"<span_class='userdanger'>[user] is trying to inject [L]!</span>")
					...
					if(L.reagents.total_volume >= L.reagents.maximum_volume)
						return
					L.visible_message("<span_class='danger'>[user] injects [L] with the syringe!", \
									"<span_class='userdanger'>[user] injects [L] with the syringe!</span>")
			...
			to_chat(user, "<span_class='notice'>You inject [amount_per_transfer_from_this] units of the solution. The syringe now contains [reagents.total_volume] units.</span>")
