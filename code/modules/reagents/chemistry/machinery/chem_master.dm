...
/obj/machinery/chem_master/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/reagent_containers) && !(I.item_flags & ABSTRACT) && I.is_open_container())
		. = TRUE // no afterattack
		if(panel_open)
			to_chat(user, "<span_class='warning'>You can't use the [src.name] while its panel is opened!</span>")
			return
		...
		if(bottle)
			to_chat(user, "<span_class='warning'>A pill bottle is already loaded into [src]!</span>")
			return
		if(!user.transferItemToLoc(I, src))
			return
		bottle = I
		to_chat(user, "<span_class='notice'>You add [I] into the dispenser slot.</span>")
		updateUsrDialog()
	else
		return ..()
