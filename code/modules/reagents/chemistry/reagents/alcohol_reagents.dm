...
/datum/reagent/consumable/ethanol/reaction_obj(obj/O, reac_volume)
	if(istype(O, /obj/item/paper))
		...
		to_chat(usr, "<span_class='notice'>[paperaffected]'s ink washes away.</span>")
	if(istype(O, /obj/item/book))
		if(reac_volume >= 5)
			var/obj/item/book/affectedbook = O
			affectedbook.dat = null
			O.visible_message("<span_class='notice'>[O]'s writing is washed away by [name]!</span>")
		else
			O.visible_message("<span_class='warning'>[O]'s ink is smeared by [name], but doesn't wash away!</span>")
	return
...
/datum/reagent/consumable/ethanol/thirteenloko/overdose_start(mob/living/M)
	to_chat(M, "<span_class='userdanger'>Your entire body violently jitters as you start to feel queasy. You really shouldn't have drank all of that [name]!</span>")
	M.Jitter(20)
	M.Stun(15)
...
/datum/reagent/consumable/ethanol/thirteenloko/overdose_process(mob/living/M)
	if(prob(7) && iscarbon(M))
		...
		if(I)
			...
			to_chat(M, "<span_class ='notice'>Your hands jitter and you drop what you were holding!</span>")
			M.Jitter(10)

	if(prob(7))
		to_chat(M, "<span_class='notice'>[pick("You have a really bad headache.", "Your eyes hurt.", "You find it hard to stay still.", "You feel your heart practically beating out of your chest.")]</span>")
	...
	if(prob(5) && iscarbon(M))
		if(M.has_trait(TRAIT_BLIND))
			...
			if(istype(eye))
				...
				to_chat(M, "<span_class='userdanger'>You double over in pain as you feel your eyeballs liquify in your head!</span>")
				M.emote("scream")
				M.adjustBruteLoss(15)
		else
			to_chat(M, "<span_class='userdanger'>You scream in terror as you go blind!</span>")
	...
	if(prob(1) && iscarbon(M))
		...
		to_chat(M, "<span_class='userdanger'>You're pretty sure you just felt your heart stop for a second there..</span>")
...
/datum/reagent/consumable/ethanol/brave_bull/on_mob_add(mob/living/M)
	...
	to_chat(M, "<span_class='notice'>You feel [tough_text]!</span>")
...
/datum/reagent/consumable/ethanol/brave_bull/on_mob_delete(mob/living/M)
	to_chat(M, "<span_class='notice'>You no longer feel [tough_text].</span>")
	M.maxHealth -= 10
...
/datum/reagent/consumable/ethanol/tequila_sunrise/on_mob_add(mob/living/M)
	to_chat(M, "<span_class='notice'>You feel gentle warmth spread through your body!</span>")
	light_holder = new(M)
...
/datum/reagent/consumable/ethanol/tequila_sunrise/on_mob_delete(mob/living/M)
	to_chat(M, "<span_class='notice'>The warmth in your body fades.</span>")
...
/datum/reagent/consumable/ethanol/manly_dorf/on_mob_add(mob/living/M)
	if(ishuman(M))
		...
		if(H.dna.check_mutation(DWARFISM) || H.has_trait(TRAIT_ALCOHOL_TOLERANCE))
			to_chat(H, "<span_class='notice'>Now THAT is MANLY!</span>")
...
/datum/reagent/consumable/ethanol/bastion_bourbon/on_mob_add(mob/living/L)
	...
	L.visible_message("<span_class='warning'>[L] shivers with renewed vigor!</span>", "<span_class='notice'>One taste of [lowertext(name)] fills you with energy!</span>")
	if(!L.stat && heal_points == 20) //brought us out of softcrit
		L.visible_message("<span_class='danger'>[L] lurches to [L.p_their()] feet!</span>", "<span_class='boldnotice'>Up and at 'em, kid.</span>")
...
/datum/reagent/consumable/ethanol/alexander/on_mob_add(mob/living/L)
	if(ishuman(L))
		...
		for(var/obj/item/shield/theshield in thehuman.contents)
			...
			to_chat(thehuman, "<span_class='notice'>[theshield] appears polished, although you don't recall polishing it.</span>")
			return TRUE
...
/datum/reagent/consumable/ethanol/alexander/on_mob_delete(mob/living/L)
	if(mighty_shield)
		mighty_shield.block_chance -= 10
		to_chat(L,"<span_class='notice'>You notice [mighty_shield] looks worn again. Weird.</span>")
	..()
