			...
			for(var/addiction in cached_addictions)
				...
				if(C && R)
					...
					switch(R.addiction_stage)
						...
						if(40 to INFINITY)
							to_chat(C, "<span_class='notice'>You feel like you've gotten over your need for [R.name].</span>")
			...
			if(cached_my_atom)
				if(!ismob(cached_my_atom)) // No bubbling mobs
					...
					for(var/mob/M in seen)
						to_chat(M, "<span_class='notice'>[iconhtml] [selected_reaction.mix_message]</span>")
				...
				if(istype(cached_my_atom, /obj/item/slime_extract))
					...
					if(ME2.Uses <= 0) // give the notification that the slime core is dead
						for(var/mob/M in seen)
							to_chat(M, "<span_class='notice'>[iconhtml] \The [my_atom]'s power is consumed in the reaction.</span>")
