	...
	if(istype(W, /obj/item/reagent_containers/food/snacks))
		...
		if(!S.junkiness)
			if(!iscompartmentfull(user))
				if(!user.transferItemToLoc(W, src))
					return
				food_load(W)
				to_chat(user, "<span_class='notice'>You insert [W] into [src]'s chef compartment.</span>")
		else
			to_chat(user, "<span_class='notice'>[src]'s chef compartment does not accept junk food.</span>")
		...
		for(var/obj/item/reagent_containers/food/snacks/S in T.contents)
			...
			if(!S.junkiness)
				...
				denied_items++
		if(denied_items)
			to_chat(user, "<span_class='notice'>[src] refuses some items.</span>")
		if(loaded)
			to_chat(user, "<span_class='notice'>You insert [loaded] dishes into [src]'s chef compartment.</span>")
		updateUsrDialog()
		return
...
/obj/machinery/vending/snack/proc/compartment_access_check(user)
	...
	if(!allowed(user) && !(obj_flags & EMAGGED) && scan_id)
		to_chat(user, "<span_class='warning'>[src]'s chef compartment blinks red: Access denied.</span>")
		req_access_txt = "0"
		return 0
	req_access_txt = "0"
	return 1
...
/obj/machinery/vending/snack/proc/iscompartmentfull(mob/user)
	if(contents.len >= 30) // no more than 30 dishes can fit inside
		to_chat(user, "<span_class='warning'>[src]'s chef compartment is full.</span>")
		return 1
	return 0
