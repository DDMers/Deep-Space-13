...
/datum/action/innate/ai/proc/adjust_uses(amt, silent)
	uses += amt
	if(!silent && uses)
		to_chat(owner, "<span_class='notice'>[name] now has <b>[uses]</b> use[uses > 1 ? "s" : ""] remaining.</span>")
	if(!uses)
		if(initial(uses) > 1) //no need to tell 'em if it was one-use anyway!
			to_chat(owner, "<span_class='warning'>[name] has run out of uses!</span>")
		qdel(src)
...
/obj/effect/proc_holder/ranged_ai
	var/enable_text = "<span_class='notice'>Hello World!</span>" //Appears when the user activates the ability
	var/disable_text = "<span_class='danger'>Goodbye Cruel World!</span>" //Context clues!
...
/datum/module_picker/Topic(href, href_list)
	...
	if(A.stat == DEAD)
		to_chat(A, "<span_class='warning'>You are already dead!</span>")
		return
...
/datum/AI_Module
	...
	var/unlock_text = "<span_class='notice'>Hello World!</span>" //Text shown when an ability is unlocked
...
/datum/AI_Module/large/nuke_station
	...
	unlock_text = "<span_class='notice'>You slowly, carefully, establish a connection with the on-station self-destruct. You can now activate it at any time.</span>"
...
/datum/action/innate/ai/nuke_station/Activate()
	...
	if(!istype(T) || !is_station_level(T.z))
		to_chat(owner, "<span_class='warning'>You cannot activate the doomsday device while off-station!</span>")
		return
...
/datum/action/innate/ai/nuke_station/proc/set_us_up_the_bomb(mob/living/owner)
	set waitfor = FALSE
	to_chat(owner, "<span_class='small boldannounce'>run -o -a 'selfdestruct'</span>")
	sleep(5)
	if(!owner || QDELETED(owner))
		return
	to_chat(owner, "<span_class='small boldannounce'>Running executable 'selfdestruct'...</span>")
	sleep(rand(10, 30))
	if(!owner || QDELETED(owner))
		return
	owner.playsound_local(owner, 'sound/misc/bloblarm.ogg', 50, 0)
	to_chat(owner, "<span_class='userdanger'>!!! UNAUTHORIZED SELF-DESTRUCT ACCESS !!!</span>")
	to_chat(owner, "<span_class='boldannounce'>This is a class-3 security violation. This incident will be reported to Central Command.</span>")
	for(var/i in 1 to 3)
		sleep(20)
		if(!owner || QDELETED(owner))
			return
		to_chat(owner, "<span_class='boldannounce'>Sending security report to Central Command.....[rand(0, 9) + (rand(20, 30) * i)]%</span>")
	sleep(3)
	if(!owner || QDELETED(owner))
		return
	to_chat(owner, "<span_class='small boldannounce'>auth 'akjv9c88asdf12nb' ******************</span>")
	owner.playsound_local(owner, 'sound/items/timer.ogg', 50, 0)
	sleep(30)
	if(!owner || QDELETED(owner))
		return
	to_chat(owner, "<span_class='boldnotice'>Credentials accepted. Welcome, akjv9c88asdf12nb.</span>")
	owner.playsound_local(owner, 'sound/misc/server-ready.ogg', 50, 0)
	sleep(5)
	if(!owner || QDELETED(owner))
		return
	to_chat(owner, "<span_class='boldnotice'>Arm self-destruct device? (Y/N)</span>")
	owner.playsound_local(owner, 'sound/misc/compiler-stage1.ogg', 50, 0)
	sleep(20)
	if(!owner || QDELETED(owner))
		return
	to_chat(owner, "<span_class='small boldannounce'>Y</span>")
	sleep(15)
	if(!owner || QDELETED(owner))
		return
	to_chat(owner, "<span_class='boldnotice'>Confirm arming of self-destruct device? (Y/N)</span>")
	...
	if(!owner || QDELETED(owner))
		return
	to_chat(owner, "<span_class='boldnotice'>Please repeat password to confirm.</span>")
	owner.playsound_local(owner, 'sound/misc/compiler-stage2.ogg', 50, 0)
	sleep(14)
	if(!owner || QDELETED(owner))
		return
	to_chat(owner, "<span_class='small boldannounce'>******************</span>")
	sleep(40)
	if(!owner || QDELETED(owner))
		return
	to_chat(owner, "<span_class='boldnotice'>Credentials accepted. Transmitting arming signal...</span>")
	owner.playsound_local(owner, 'sound/misc/server-ready.ogg', 50, 0)
	sleep(30)
	if(!owner || QDELETED(owner))
		return
...
/obj/machinery/doomsday_device/proc/detonate()
	...
	for(var/i in GLOB.mob_living_list)
		...
		if(issilicon(L))
			continue
		to_chat(L, "<span_class='userdanger'>The blast wave from [src] tears you atom from atom!</span>")
		L.dust()
...
/datum/AI_Module/large/upgrade_turrets
	...
	cost = 30
	upgrade = TRUE
	unlock_text = "<span_class='notice'>You establish a power diversion to your turrets, upgrading their health and damage.</span>"
...
/datum/AI_Module/large/lockdown
	...
	unlock_text = "<span_class='notice'>You upload a sleeper trojan into the door control systems. You can send a signal to set it off at any time.</span>"
...
/datum/action/innate/ai/lockdown/Activate()
	...
	to_chat(owner, "<span_class='danger'>Lockdown initiated. Network reset in 90 seconds.</span>")
...
/datum/AI_Module/large/destroy_rcd
	...
	unlock_text = "<span_class='notice'>After some improvisation, you rig your onboard radio to be able to send a signal to detonate all RCDs.</span>"
...
/datum/action/innate/ai/destroy_rcds/Activate()
	for(var/I in GLOB.rcd_list)
		if(!istype(I, /obj/item/construction/rcd/borg)) //Ensures that cyborg RCDs are spared.
			...
			RCD.detonate_pulse()
	to_chat(owner, "<span_class='danger'>RCD detonation pulse emitted.</span>")
...
/datum/AI_Module/large/mecha_domination
	...
	cost = 30
	upgrade = TRUE
	unlock_text = "<span_class='notice'>Virus package compiled. Select a target mech at any time. <b>You must remain on the station at all times. Loss of signal will result in total system lockout.</b></span>"
...
/datum/AI_Module/large/break_fire_alarms
	...
	unlock_text = "<span_class='notice'>You replace the thermal sensing capabilities of all fire alarms with a manual override, allowing you to turn them off at will.</span>"
...
/datum/action/innate/ai/break_fire_alarms/Activate()
	for(var/obj/machinery/firealarm/F in GLOB.machines)
		if(!is_station_level(F.z))
			continue
		F.obj_flags |= EMAGGED
	to_chat(owner, "<span_class='notice'>All thermal sensors on the station have been disabled. Fire alerts will no longer be recognized.</span>")
...
/datum/AI_Module/large/break_air_alarms
	...
	unlock_text = "<span_class='notice'>You remove the safety overrides on all air alarms, but you leave the confirm prompts open. You can hit 'Yes' at any time... you bastard.</span>"
...
/datum/action/innate/ai/break_air_alarms/Activate()
	...
	to_chat(owner, "<span_class='notice'>All air alarm safeties on the station have been overridden. Air alarms may now use the Flood environmental mode.</span>")
	owner.playsound_local(owner, 'sound/machines/terminal_off.ogg', 50, 0)
...
/datum/AI_Module/small/overload_machine
	...
	unlock_text = "<span_class='notice'>You enable the ability for the station's APCs to direct intense energy into machinery.</span>"
...
/obj/effect/proc_holder/ranged_ai/overload_machine
	...
	enable_text = "<span_class='notice'>You tap into the station's powernet. Click on a machine to detonate it, or use the ability again to cancel.</span>"
	disable_text = "<span_class='notice'>You release your hold on the powernet.</span>"
...
/obj/effect/proc_holder/ranged_ai/overload_machine/InterceptClickOn(mob/living/caller, params, obj/machinery/target)
	...
	if(ranged_ability_user.incapacitated())
		...
		return
	if(!istype(target))
		to_chat(ranged_ability_user, "<span_class='warning'>You can only overload machines!</span>")
		return
	if(is_type_in_typecache(target, GLOB.blacklisted_malf_machines))
		to_chat(ranged_ability_user, "<span_class='warning'>You cannot overload that device!</span>")
		return
	ranged_ability_user.playsound_local(ranged_ability_user, "sparks", 50, 0)
	attached_action.adjust_uses(-1)
	target.audible_message("<span_class='userdanger'>You hear a loud electrical buzzing sound coming from [target]!</span>")
	addtimer(CALLBACK(attached_action, /datum/action/innate/ai/ranged/overload_machine.proc/detonate_machine, target), 50) //kaboom!
	remove_ranged_ability("<span_class='danger'>Overcharging machine...</span>")
	return TRUE
...
/datum/AI_Module/small/override_machine
	...
	unlock_text = "<span_class='notice'>You procure a virus from the Space Dark Web and distribute it to the station's machines.</span>"
...
/obj/effect/proc_holder/ranged_ai/override_machine
	...
	enable_text = "<span_class='notice'>You tap into the station's powernet. Click on a machine to animate it, or use the ability again to cancel.</span>"
	disable_text = "<span_class='notice'>You release your hold on the powernet.</span>"
...
/obj/effect/proc_holder/ranged_ai/override_machine/InterceptClickOn(mob/living/caller, params, obj/machinery/target)
	...
	if(ranged_ability_user.incapacitated())
		...
		return
	if(!istype(target))
		to_chat(ranged_ability_user, "<span_class='warning'>You can only animate machines!</span>")
		return
	if(!target.can_be_overridden() || is_type_in_typecache(target, GLOB.blacklisted_malf_machines))
		to_chat(ranged_ability_user, "<span_class='warning'>That machine can't be overridden!</span>")
		return
	...
	remove_ranged_ability("<span_class='danger'>Sending override signal...</span>")
	return TRUE
...
/datum/AI_Module/large/place_cyborg_transformer
	...
	unlock_text = "<span_class='notice'>You make contact with Space Amazon and request a robotics factory for delivery.</span>"
...
/datum/action/innate/ai/place_transformer/Activate()
	...
	to_chat(owner, "<span_class='warning'>You are no longer able to shunt your core to APCs.</span>")
	adjust_uses(-1)
...
/mob/living/silicon/ai/proc/can_place_transformer(datum/action/innate/ai/place_transformer/action)
	...
	if(!success)
		to_chat(src, "<span_class='warning'>[alert_msg]</span>")
	return success
...
/datum/AI_Module/small/blackout
	...
	unlock_text = "<span_class='notice'>You hook into the powernet and route bonus power towards the station's lighting.</span>"
	unlock_sound = "sparks"
...
/datum/action/innate/ai/blackout/Activate()
	for(var/obj/machinery/power/apc/apc in GLOB.apcs_list)
		if(prob(30 * apc.overload))
			...
			apc.overload++
	to_chat(owner, "<span_class='notice'>Overcurrent applied to the powernet.</span>")
...
/datum/AI_Module/small/emergency_lights
	...
	unlock_text = "<span_class='notice'>You hook into the powernet and locate the connections between light fixtures and their fallbacks.</span>"
	unlock_sound = "sparks"
...
/datum/action/innate/ai/emergency_lights/Activate()
	for(var/obj/machinery/light/L in GLOB.machines)
		...
		CHECK_TICK
	to_chat(owner, "<span_class='notice'>Emergency light connections severed.</span>")
...
/datum/AI_Module/small/reactivate_cameras
	...
	unlock_text = "<span_class='notice'>You deploy nanomachines to the cameranet.</span>"
...
/datum/action/innate/ai/reactivate_cameras/Activate()
	...
	to_chat(owner, "<span_class='notice'>Diagnostic complete! Cameras reactivated: <b>[fixed_cameras]</b>. Reactivations remaining: <b>[uses]</b>.</span>")
...
/datum/AI_Module/large/upgrade_cameras
	...
	upgrade = TRUE
	unlock_text = "<span_class='notice'>OTA firmware distribution complete! Cameras upgraded: CAMSUPGRADED. Light amplification system online.</span>"
	unlock_sound = 'sound/items/rped.ogg'
...
/datum/AI_Module/large/eavesdrop
	...
	one_purchase = TRUE
	upgrade = TRUE
	unlock_text = "<span_class='notice'>OTA firmware distribution complete! Cameras upgraded: Enhanced surveillance package online.</span>"
	unlock_sound = 'sound/items/rped.ogg'
