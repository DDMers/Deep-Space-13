...
/datum/antagonist/traitor/internal_affairs/proc/steal_targets(datum/mind/victim)
	if(!owner.current||owner.current.stat==DEAD)
		return
	to_chat(owner.current, "<span_class='userdanger'> Target eliminated: [victim.name]</span>")
	for(var/objective_ in victim.get_all_objectives())
		if(istype(objective_, /datum/objective/assassinate/internal))
			...
			if(objective.target==owner)
				...
				to_chat(owner.current, "<span_class='userdanger'> New target added to database: [objective.target.name] ([status_text]) </span>")
	...
	if(last_man_standing)
		if(syndicate)
			to_chat(owner.current,"<span_class='userdanger'> All the loyalist agents are dead, and no more is required of you. Die a glorious death, agent. </span>")
		else
			to_chat(owner.current,"<span_class='userdanger'> All the other agents are dead, and you're the last loose end. Stage a Syndicate terrorist attack to cover up for today's events. You no longer have any limits on collateral damage.</span>")
	...
	if(owner&&owner.current&&owner.current.stat!=DEAD)
		for(var/objective_ in objectives)
			...
			if(objective.check_completion())
				...
				if(objective.stolen)
					var/fail_msg = "<span_class='userdanger'>Your sensors tell you that [objective.target.current.real_name], one of the targets you were meant to have killed, pulled one over on you, and is still alive - do the job properly this time! </span>"
					if(last_man_standing)
						if(syndicate)
							fail_msg += "<span_class='userdanger'> You no longer have permission to die. </span>"
						else
							fail_msg += "<span_class='userdanger'> The truth could still slip out!</font><B><font size=5 color=red> Cease any terrorist actions as soon as possible, unneeded property damage or loss of employee life will lead to your contract being terminated.</span>"
...
/datum/antagonist/traitor/internal_affairs/proc/greet_iaa()
	...
	to_chat(owner.current, "<span_class='userdanger'>You are the [special_role].</span>")
	if(syndicate)
		to_chat(owner.current, "<span_class='userdanger'>Your target has been framed for [crime], and you have been tasked with eliminating them to prevent them defending themselves in court.</span>")
		...
		to_chat(owner.current, "<span_class='userdanger'> You have been provided with a standard uplink to accomplish your task. </span>")
	else
		to_chat(owner.current, "<span_class='userdanger'>Your target is suspected of [crime], and you have been tasked with eliminating them by any means necessary to avoid a costly and embarrassing public trial.</span>")
		...
		to_chat(owner.current, "<span_class='userdanger'>For the sake of plausible deniability, you have been equipped with an array of captured Syndicate weaponry available via uplink.</span>")

	to_chat(owner.current, "<span_class='userdanger'>Finally, watch your back. Your target has friends in high places, and intel suggests someone may have taken out a contract of their own to protect them.</span>")
