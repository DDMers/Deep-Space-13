...
/obj/effect/clockwork/spatial_gateway/examine(mob/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		to_chat(user, "<span_class='brass'>It has [uses] use\s remaining.</span>")
...
/obj/effect/clockwork/spatial_gateway/attack_hand(mob/living/user)
	...
	if(user.pulling && user.a_intent == INTENT_GRAB && isliving(user.pulling))
		...
		if(L.buckled || L.anchored || L.has_buckled_mobs())
			return FALSE
		user.visible_message("<span_class='warning'>[user] shoves [L] into [src]!</span>", "<span_class='danger'>You shove [L] into [src]!</span>")
		user.stop_pulling()
		pass_through_gateway(L)
		return TRUE
	if(!user.canUseTopic(src))
		return FALSE
	user.visible_message("<span_class='warning'>[user] climbs through [src]!</span>", "<span_class='danger'>You brace yourself and step through [src]...</span>")
	pass_through_gateway(user)
	return TRUE

/obj/effect/clockwork/spatial_gateway/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/nullrod))
		user.visible_message("<span_class='warning'>[user] dispels [src] with [I]!</span>", "<span_class='danger'>You close [src] with [I]!</span>")
		qdel(linked_gateway)
		qdel(src)
		return TRUE
	if(istype(I, /obj/item/clockwork/slab))
		to_chat(user, "<span_class='heavy_brass'>\"I don't think you want to drop your slab into that.\"\n\"If you really want to, try throwing it.\"</span>")
		return TRUE
	if(uses && user.dropItemToGround(I))
		user.visible_message("<span_class='warning'>[user] drops [I] into [src]!</span>", "<span_class='danger'>You drop [I] into [src]!</span>")
...
/obj/effect/clockwork/spatial_gateway/ex_act(severity)
	if(severity == 1 && uses)
		uses = 0
		visible_message("<span_class='warning'>[src] is disrupted!</span>")
		animate(src, alpha = 0, transform = matrix()*2, time = 10, flags = ANIMATION_END_NOW)
		deltimer(timerid)
		timerid = QDEL_IN(src, 10)
		linked_gateway.uses = 0
		linked_gateway.visible_message("<span_class='warning'>[linked_gateway] is disrupted!</span>")
...
/obj/effect/clockwork/spatial_gateway/proc/pass_through_gateway(atom/movable/A, no_cost)
	if(!linked_gateway)
		...
		return FALSE
	if(!sender)
		visible_message("<span_class='warning'>[A] bounces off [src]!</span>")
		return FALSE
	if(!uses)
		return FALSE
	...
	if(isliving(A))
		...
		to_chat(user, "<span_class='warning'><b>You pass through [src] and appear elsewhere!</b></span>")
	linked_gateway.visible_message("<span_class='warning'>A shape appears in [linked_gateway] before emerging!</span>")
...
/atom/movable/proc/procure_gateway(mob/living/invoker, time_duration, gateway_uses, two_way)
	...
	if(!possible_targets.len)
		to_chat(invoker, "<span_class='warning'>There are no other eligible targets for a Spatial Gateway!</span>")
		return FALSE
	...
	if(!target) //if we have no target, but did have a key, let them retry
		to_chat(invoker, "<span_class='warning'>That target no longer exists!</span>")
		return procure_gateway(invoker, time_duration, gateway_uses, two_way)
	if(isliving(target))
		var/mob/living/L = target
		if(!is_servant_of_ratvar(L))
			to_chat(invoker, "<span_class='warning'>That target is no longer a Servant!</span>")
			return procure_gateway(invoker, time_duration, gateway_uses, two_way)
		if(L.stat != CONSCIOUS)
			to_chat(invoker, "<span_class='warning'>That Servant is no longer conscious!</span>")
			return procure_gateway(invoker, time_duration, gateway_uses, two_way)
	...
	if(issrcobelisk)
		if(!anchored)
			to_chat(invoker, "<span_class='warning'>[src] is no longer secured!</span>")
			return FALSE
		...
		if(CO.active)
			to_chat(invoker, "<span_class='warning'>[src] is now sustaining a gateway!</span>")
			return FALSE
	if(istargetobelisk)
		if(!target.anchored)
			to_chat(invoker, "<span_class='warning'>That [target.name] is no longer secured!</span>")
			return procure_gateway(invoker, time_duration, gateway_uses, two_way)
		var/obj/structure/destructible/clockwork/powered/clockwork_obelisk/CO = target
		if(CO.active)
			to_chat(invoker, "<span_class='warning'>That [target.name] is sustaining a gateway, and cannot receive another!</span>")
			return procure_gateway(invoker, time_duration, gateway_uses, two_way)
	...
	invoker.visible_message("<span_class='warning'>The air in front of [invoker] ripples before suddenly tearing open!</span>", \
	"<span_class='brass'>With a word, you rip open a [two_way ? "two-way":"one-way"] rift to [input_target_key]. It will last for [DisplayTimeText(time_duration)] and has [gateway_uses] use[gateway_uses > 1 ? "s" : ""].</span>")
	...
	S2.visible_message("<span_class='warning'>The air in front of [target] ripples before suddenly tearing open!</span>")
	return TRUE
