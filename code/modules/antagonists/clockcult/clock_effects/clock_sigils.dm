...
/obj/effect/clockwork/sigil/attackby(obj/item/I, mob/living/user, params)
	if(I.force)
		if(is_servant_of_ratvar(user) && user.a_intent != INTENT_HARM)
			return ..()
		user.visible_message("<span_class='warning'>[user] scatters [src] with [I]!</span>", "<span_class='danger'>You scatter [src] with [I]!</span>")
		qdel(src)
		return 1
	return ..()
...
/obj/effect/clockwork/sigil/attack_hand(mob/user)
	if(iscarbon(user) && !user.stat)
		if(is_servant_of_ratvar(user) && user.a_intent != INTENT_HARM)
			return ..()
		user.visible_message("<span_class='warning'>[user] stamps out [src]!</span>", "<span_class='danger'>You stomp on [src], scattering it into thousands of particles.</span>")
		qdel(src)
		return TRUE
	. = ..()

/obj/effect/clockwork/sigil/ex_act(severity)
	visible_message("<span_class='warning'>[src] scatters into thousands of particles.</span>")
	qdel(src)
	...
	if(isliving(AM))
		...
		if(L.stat <= stat_affected)
			if((!is_servant_of_ratvar(L) || (affects_servants && is_servant_of_ratvar(L))) && (L.mind || L.has_status_effect(STATUS_EFFECT_SIGILMARK)) && !isdrone(L))
				...
				if(I)
					if(isitem(I))
						L.visible_message("<span_class='warning'>[L]'s [I.name] [resist_string], protecting [L.p_them()] from [src]'s effects!</span>", \
						"<span_class='userdanger'>Your [I.name] [resist_string], protecting you!</span>")
					return
				sigil_effects(L)
...
/obj/effect/clockwork/sigil/transgression/sigil_effects(mob/living/L)
	...
	for(var/mob/living/M in viewers(5, src))
		if(!is_servant_of_ratvar(M) && M != L)
			M.flash_act()
	if(iscultist(L))
		to_chat(L, "<span_class='heavy_brass'>\"Watch your step, wretch.\"</span>")
		L.adjustBruteLoss(10)
		L.Paralyze(80, FALSE)
	L.visible_message("<span_class='warning'>[src] appears around [L] in a burst of light!</span>", \
	"<span_class='userdanger'>[target_flashed ? "An unseen force":"The glowing sigil around you"] holds you in place!</span>")
	L.Stun(40)
...
/obj/effect/clockwork/sigil/submission/sigil_effects(mob/living/L)
	...
	if(!has_sigil && !has_servant)
		visible_message("<span_class='danger'>[src] strains into a gentle violet color, but quietly fades...</span>")
		return
	L.visible_message("<span_class='warning'>[src] begins to glow a piercing magenta!</span>", "<span_class='sevtug'>You feel something start to invade your mind...</span>")
	var/oldcolor = color
	...
	if(get_turf(L) != get_turf(src))
		...
		visible_message("<span_class='warning'>[src] slowly stops glowing!</span>")
		return
	if(is_eligible_servant(L))
		to_chat(L, "<span_class='heavy_brass'>\"You belong to me now.\"</span>")
		if(!GLOB.application_scripture_unlocked)
			GLOB.application_scripture_unlocked = TRUE
			hierophant_message("<span_class='large_brass bold'>With the conversion of a new servant the Ark's power grows. Application scriptures are now available.</span>")
	...
	if(iscarbon(L))
		...
		C.silent += 5
	var/message = "[sigil_name] in [get_area(src)] <span_class='sevtug'>[is_servant_of_ratvar(L) ? "successfully converted" : "failed to convert"]</span>"
	for(var/M in GLOB.mob_list)
		if(isobserver(M))
			var/link = FOLLOW_LINK(M, L)
			to_chat(M,  "[link] <span_class='heavy_brass'>[message] [L.real_name]!</span>")
		else if(is_servant_of_ratvar(M))
			if(M == L)
				to_chat(M, "<span_class='heavy_brass'>[message] you!</span>")
			else
				to_chat(M, "<span_class='heavy_brass'>[message] [L.real_name]!</span>")
	animate(src, color = oldcolor, time = 20, flags = ANIMATION_END_NOW)
	addtimer(CALLBACK(src, /atom/proc/update_atom_colour), 20)
	visible_message("<span_class='warning'>[src] slowly stops glowing!</span>")
...
/obj/effect/clockwork/sigil/transmission/ex_act(severity)
	if(severity == 3)
		...
		visible_message("<span_class='warning'>[src] flares a brilliant orange!</span>")
	else
		..()
...
/obj/effect/clockwork/sigil/transmission/examine(mob/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		...
		for(var/obj/structure/destructible/clockwork/powered/P in range(SIGIL_ACCESS_RANGE, src))
			structure_number++
		to_chat(user, "<span_class='[get_clockwork_power() ? "brass":"alloy"]'>It is storing <b>[DisplayPower(get_clockwork_power())]</b> of shared power, \
		and <b>[structure_number]</b> clockwork structure[structure_number == 1 ? " is":"s are"] in range.</span>")
		if(iscyborg(user))
			to_chat(user, "<span_class='brass'>You can recharge from the [sigil_name] by crossing it.</span>")
...
/obj/effect/clockwork/sigil/transmission/sigil_effects(mob/living/L)
	if(is_servant_of_ratvar(L))
		...
		to_chat(L, "<span_class='brass'>You feel a slight, static shock.</span>")

/obj/effect/clockwork/sigil/transmission/proc/charge_cyborg(mob/living/silicon/robot/cyborg)
	if(!cyborg_checks(cyborg))
		return
	to_chat(cyborg, "<span_class='brass'>You start to charge from the [sigil_name]...</span>")
	...
	if(adjust_clockwork_power(-giving_power))
		cyborg.visible_message("<span_class='warning'>[cyborg] glows a brilliant orange!</span>")
...
/obj/effect/clockwork/sigil/transmission/proc/cyborg_checks(mob/living/silicon/robot/cyborg, silent)
	if(!cyborg.cell)
		if(!silent)
			to_chat(cyborg, "<span_class='warning'>You have no cell!</span>")
		return FALSE
	if(!get_clockwork_power())
		if(!silent)
			to_chat(cyborg, "<span_class='warning'>There is no power available across sigils!</span>")
		return FALSE
	if(cyborg.cell.charge > cyborg.cell.maxcharge - MIN_CLOCKCULT_POWER)
		if(!silent)
			to_chat(cyborg, "<span_class='warning'>You are already at maximum charge!</span>")
		return FALSE
	if(cyborg.has_status_effect(STATUS_EFFECT_POWERREGEN))
		if(!silent)
			to_chat(cyborg, "<span_class='warning'>You are already regenerating power!</span>")
		return FALSE
	return TRUE
...
/obj/effect/clockwork/sigil/vitality/examine(mob/user)
	..()
	if(is_servant_of_ratvar(user) || isobserver(user))
		to_chat(user, "<span_class='[GLOB.clockwork_vitality ? "inathneq_small":"alloy"]'>It has access to <b>[GLOB.ratvar_awakens ? "INFINITE":GLOB.clockwork_vitality]</b> units of vitality.</span>")
		if(GLOB.ratvar_awakens)
			to_chat(user, "<span_class='inathneq_small'>It can revive Servants at no cost!</span>")
		else
			to_chat(user, "<span_class='inathneq_small'>It can revive Servants at a cost of <b>[revive_cost]</b> vitality.</span>")
	...
	while(L && (!is_servant_of_ratvar(L) || (is_servant_of_ratvar(L) && (GLOB.ratvar_awakens || GLOB.clockwork_vitality))) && get_turf(L) == get_turf(src) && !L.buckled)
		...
		if(!is_servant_of_ratvar(L))
			...
			if(L.stat == DEAD && !consumed_vitality)
				...
				L.visible_message("<span_class='warning'>[L] collapses in on [L.p_them()]self as [src] flares bright blue!</span>")
				to_chat(L, "<span_class='inathneq_large'>\"[text2ratvar("Your life will not be wasted.")]\"</span>")
			...
			if(L.stat == DEAD)
				...
				if(GLOB.clockwork_vitality >= revival_cost && (ghost || (L.mind && L.mind.active)))
					if(L.has_status_effect(STATUS_EFFECT_ICHORIAL_STAIN))
						visible_message("<span_class='boldwarning'>[src] strains, but nothing happens...</span>")
						if(L.pulledby)
							to_chat(L.pulledby, "<span_class='userdanger'>[L] was already revived recently by a vitality matrix! Wait a bit longer!</span>")
						break
					else
						if(ghost)
						...
						to_chat(L, "<span_class='inathneq'>\"[text2ratvar("You will be okay, child.")]\"</span>")
			...
			if(!L.client || L.client.is_afk())
				...
				if(LAZYLEN(candidates))
					...
					to_chat(L, "<span_class='userdanger'>Your physical form has been taken over by another soul due to your inactivity! Ahelp if you wish to regain your form!</span>")
					...
					animate(V, alpha = 0, transform = matrix()*2, time = 8)
					playsound(L, 'sound/magic/staff_healing.ogg', 50, 1)
					L.visible_message("<span_class='warning'>[L]'s eyes suddenly open wide, gleaming with renewed vigor for the cause!</span>", "<span_class='inathneq'>\"[text2ratvar("Awaken!")]\"</span>")
					break
