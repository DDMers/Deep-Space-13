...
/obj/effect/clockwork/city_of_cogs_rift/Initialize()
	. = ..()
	visible_message("<span_class='warning'>The air above [loc] shimmers and pops as a [name] forms there!</span>")
...
/obj/effect/clockwork/city_of_cogs_rift/Destroy()
	visible_message("<span_class='warning'>[src] cracks as it destabilizes and breaks apart!</span>")
	return ..()

/obj/effect/clockwork/city_of_cogs_rift/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/nullrod))
		to_chat(user, "<span_class='warning'>Your [I.name] seems to have no effect on [src]!</span>")
		return
	. = ..()
	...
	if(!QDELETED(AM))
		if(isliving(AM))
			...
			if(L.client && !L.incapacitated())
				L.visible_message("<span_class='notice'>[L] starts climbing through [src]...</span>", \
				"<span_class='notice'>You begin climbing through [src]...</span>")
...
/obj/effect/clockwork/city_of_cogs_rift/proc/beckon(atom/movable/AM)
	...
	if(ismob(AM) && is_servant_of_ratvar(AM))
		...
		for(var/mob/living/L in target_contents)
			if(is_servant_of_ratvar(L) && L.stat != DEAD) // Having a living cultist in your inventory sends you to the cultist spawn
				T = GLOB.ark_of_the_clockwork_justiciar ? get_step(GLOB.ark_of_the_clockwork_justiciar, SOUTH) : get_turf(pick(GLOB.servant_spawns))
				break
	AM.visible_message("<span_class='danger'>[AM] passes through [src]!</span>", null, null, null, AM)
	AM.forceMove(T)
	AM.visible_message("<span_class='danger'>[AM] materializes from the air!</span>", \
	"<span_class='boldannounce'>You pass through [src] and appear [is_servant_of_ratvar(AM) ? "back at the City of Cogs" : "somewhere unfamiliar. Looks like it was a one-way trip.."].</span>")
	...
	if(isliving(AM))
		...
		if(TTV)
			to_chat(L, "<span_class='userdanger'>The air resonates with the Ark's presence; your explosives will be significantly dampened here!</span>")
