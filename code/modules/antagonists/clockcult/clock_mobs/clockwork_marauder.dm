...
/mob/living/simple_animal/hostile/clockwork/marauder
	...
	light_range = 2
	light_power = 1.1
	playstyle_string = "<span_class='big bold'><span_class='neovgre'>You are a clockwork marauder,</span></span><b> a well-rounded frontline construct of Ratvar. Although you have no \
	...
	empower_string = "<span_class='neovgre'>The Anima Bulwark's power flows through you! Your weapon will strike harder, your armor is sturdier, and your shield is more durable.</span>"
...
/mob/living/simple_animal/hostile/clockwork/marauder/examine_info()
	if(!shield_health)
		return "<span_class='warning'>Its shield has been destroyed!</span>"
...
/mob/living/simple_animal/hostile/clockwork/marauder/Life()
	...
	if(shield_health < max_shield_health && world.time >= shield_health_regen)
		...
		to_chat(src, "<span_class='neovgre'>Your shield has recovered, <b>[shield_health]</b> blocks remaining!</span>")
...
/mob/living/simple_animal/hostile/clockwork/marauder/death(gibbed)
	visible_message("<span_class='danger'>[src]'s equipment clatters lifelessly to the ground as the red flames within dissipate.</span>", \
	"<span_class='userdanger'>Dented and scratched, your armor falls away, and your fragile form breaks apart without its protection.</span>")
	. = ..()
	...
	if(amount > 0)
		for(var/mob/living/L in view(2, src))
			if(L.is_holding_item_of_type(/obj/item/nullrod))
				to_chat(src, "<span_class='userdanger'>The presence of a brandished holy artifact weakens your armor!</span>")
...
/mob/living/simple_animal/hostile/clockwork/marauder/proc/deflect_projectile(obj/item/projectile/P)
	...
	visible_message("<span_class='danger'>[src] deflects [P] with [p_their()] shield!</span>", \
	"<span_class='danger'>You block [P] with your shield! <i>Blocks left:</i> <b>[shield_health - 1]</b></span>")
	...
	shield_health--
	if(!shield_health)
		visible_message("<span_class='warning'>[src]'s shield breaks from deflecting the attack!</span>", "<span_class='boldwarning'>Your shield breaks! Give it some time to recover...</span>")
