...
/obj/item/clockwork/construct_chassis
	...
	var/construct_desc = "<span_class='alloy'>There is no construct for this chassis. Report this to a coder.</span>"
	icon_state = "anime_fragment"
	resistance_flags = FIRE_PROOF | ACID_PROOF
	w_class = WEIGHT_CLASS_HUGE
	var/creation_message = "<span_class='brass'>The chassis shudders and hums to life!</span>"
...
/obj/item/clockwork/construct_chassis/attack_hand(mob/living/user)
	if(w_class >= WEIGHT_CLASS_HUGE)
		to_chat(user, "<span_class='warning'>[src] is too cumbersome to carry! Drag it around instead!</span>")
		return
	. = ..()
...
/obj/item/clockwork/construct_chassis/attack_ghost(mob/user)
	if(!SSticker.mode)
		to_chat(user, "<span_class='danger'>You cannot use that before the game has started.</span>")
		return
	if(QDELETED(src))
		to_chat(user, "<span_class='danger'>You were too late! Better luck next time.</span>")
		return
...
/obj/item/clockwork/construct_chassis/clockwork_marauder
	...
	construct_desc = "<span_class='neovgre_small'>It will become a <b>clockwork marauder,</b> a well-rounded frontline combatant.</span>"
	creation_message = "<span_class='neovgre_small bold'>Crimson fire begins to rage in the armor as it rises into the air with its armaments!</span>"
...
/obj/item/clockwork/construct_chassis/cogscarab
	...
	construct_desc = "<span_class='alloy'>It will become a <b>cogscarab,</b> a small and fragile drone that builds, repairs, and maintains.</span>"
	creation_message = "<span_class='alloy bold'>The cogscarab clicks and whirrs as it hops up and springs to life!</span>"
