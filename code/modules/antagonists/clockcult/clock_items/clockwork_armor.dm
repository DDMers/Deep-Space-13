...
/obj/item/clothing/head/helmet/clockwork/equipped(mob/living/user, slot)
	...
	if(slot == SLOT_HEAD && !is_servant_of_ratvar(user))
		if(!iscultist(user))
			to_chat(user, "<span_class='heavy_brass'>\"Now now, this is for my servants, not you.\"</span>")
			user.visible_message("<span_class='warning'>As [user] puts [src] on, it flickers off [user.p_their()] head!</span>", "<span_class='warning'>The helmet flickers off your head, leaving only nausea!</span>")
			if(iscarbon(user))
				var/mob/living/carbon/C = user
				C.vomit(20)
		else
			to_chat(user, "<span_class='heavy_brass'>\"Do you have a hole in your head? You're about to.\"</span>")
			to_chat(user, "<span_class='userdanger'>The helmet tries to drive a spike through your head as you scramble to remove it!</span>")
			user.emote("scream")
...
/obj/item/clothing/suit/armor/clockwork/equipped(mob/living/user, slot)
	...
	if(slot == SLOT_WEAR_SUIT && !is_servant_of_ratvar(user))
		if(!iscultist(user))
			to_chat(user, "<span_class='heavy_brass'>\"Now now, this is for my servants, not you.\"</span>")
			user.visible_message("<span_class='warning'>As [user] puts [src] on, it flickers off [user.p_their()] body!</span>", "<span_class='warning'>The cuirass flickers off your body, leaving only nausea!</span>")
			if(iscarbon(user))
				var/mob/living/carbon/C = user
				C.vomit(20)
		else
			to_chat(user, "<span_class='heavy_brass'>\"I think this armor is too hot for you to handle.\"</span>")
			to_chat(user, "<span_class='userdanger'>The cuirass emits a burst of flame as you scramble to get it off!</span>")
			user.emote("scream")
...
/obj/item/clothing/gloves/clockwork/equipped(mob/living/user, slot)
	...
	if(slot == SLOT_GLOVES && !is_servant_of_ratvar(user))
		if(!iscultist(user))
			to_chat(user, "<span_class='heavy_brass'>\"Now now, this is for my servants, not you.\"</span>")
			user.visible_message("<span_class='warning'>As [user] puts [src] on, it flickers off [user.p_their()] arms!</span>", "<span_class='warning'>The gauntlets flicker off your arms, leaving only nausea!</span>")
			if(iscarbon(user))
				var/mob/living/carbon/C = user
				C.vomit()
		else
			to_chat(user, "<span_class='heavy_brass'>\"Did you like having arms?\"</span>")
			to_chat(user, "<span_class='userdanger'>The gauntlets suddenly squeeze tight, crushing your arms before you manage to get them off!</span>")
			user.emote("scream")
...
/obj/item/clothing/shoes/clockwork/equipped(mob/living/user, slot)
	...
	if(slot == SLOT_SHOES && !is_servant_of_ratvar(user))
		if(!iscultist(user))
			to_chat(user, "<span_class='heavy_brass'>\"Now now, this is for my servants, not you.\"</span>")
			user.visible_message("<span_class='warning'>As [user] puts [src] on, it flickers off [user.p_their()] feet!</span>", "<span_class='warning'>The treads flicker off your feet, leaving only nausea!</span>")
			if(iscarbon(user))
				var/mob/living/carbon/C = user
				C.vomit()
		else
			to_chat(user, "<span_class='heavy_brass'>\"Let's see if you can dance with these.\"</span>")
			to_chat(user, "<span_class='userdanger'>The treads turn searing hot as you scramble to get them off!</span>")
			user.emote("scream")
