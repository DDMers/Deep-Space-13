...
/datum/action/innate/call_weapon/Activate()
	if(!owner.get_empty_held_indexes())
		to_chat(usr, "<span_class='warning'>You need an empty hand to call forth your [initial(weapon_type.name)]!</span>")
		return
	if(weapon)
		if(weapon.loc == owner)
			owner.visible_message("<span_class='danger'>[owner]'s [weapon.name] flickers and disappears!</span>")
			to_chat(owner, "<span_class='brass'>You dismiss [weapon].</span>")
			QDEL_NULL(weapon)
			weapon_reset(RATVARIAN_SPEAR_COOLDOWN * 0.5)
			return
		else
			weapon.visible_message("<span_class='warning'>[weapon] suddenly flickers and disappears!</span>")
			owner.visible_message("<span_class='danger'>A [weapon.name] suddenly flickers into [owner]'s hands!</span>", "<span_class='brass'>You recall [weapon] to you.</span>")
	else
		weapon = new weapon_type (get_turf(usr), src)
		owner.visible_message("<span_class='warning'>A [weapon.name] materializes in [owner]'s hands!</span>", "<span_class='brass'>You call forth your [weapon.name]!</span>")
