...
/obj/item/clockwork/weapon/ratvarian_spear/examine(mob/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		to_chat(user, "<span_class='inathneq_small'>Attacks on living non-Servants will generate <b>[bonus_burn]</b> units of vitality.</span>")
		if(!iscyborg(user))
			to_chat(user, "<span_class='brass'>Throwing the spear will do massive damage, break the spear, and knock down the target.</span>")
...
/obj/item/clockwork/weapon/ratvarian_spear/attack(mob/living/target, mob/living/carbon/human/user)
	...
	if(!QDELETED(target) && target.stat != DEAD && !target.anti_magic_check(major = FALSE) && !is_servant_of_ratvar(target)) //we do bonus damage on attacks unless they're a servant, have a null rod, or are dead
		...
		if(issilicon(target))
			target.visible_message("<span_class='warning'>[target] shudders violently at [src]'s touch!</span>", "<span_class='userdanger'>ERROR: Temperature rising!</span>")
			bonus_damage *= 5 //total 40 damage on borgs, 70 with ratvar
		else if(iscultist(target) || isconstruct(target))
			to_chat(target, "<span_class='userdanger'>Your body flares with agony at [src]'s presence!</span>")
...
/obj/item/clockwork/weapon/ratvarian_spear/proc/break_spear(turf/T)
	if(src)
		...
		if(T) //make sure we're not in null or something
			T.visible_message("<span_class='warning'>[src] [pick("cracks in two and fades away", "snaps in two and dematerializes")]!</span>")
