...
/obj/item/clothing/glasses/judicial_visor/equipped(mob/living/user, slot)
	...
	if(iscultist(user)) //Cultists spontaneously combust
		to_chat(user, "<span_class='heavy_brass'>\"Consider yourself judged, whelp.\"</span>")
		to_chat(user, "<span_class='userdanger'>You suddenly catch fire!</span>")
...
/obj/item/clothing/glasses/judicial_visor/proc/update_status(change_to)
	...
	switch(active)
		if(TRUE)
			to_chat(L, "<span_class='notice'>As you put on [src], its lens begins to glow, information flashing before your eyes.</span>\n\
			<span_class='heavy_brass'>Judicial visor active. Use the action button to gain the ability to smite the unworthy.</span>")
		if(FALSE)
			to_chat(L, "<span_class='notice'>As you take off [src], its lens darkens once more.</span>")
	return 1
...
/obj/item/clothing/glasses/judicial_visor/proc/recharge_visor(mob/living/user)
	...
	if(user && src == user.get_item_by_slot(SLOT_GLASSES))
		to_chat(user, "<span_class='brass'>Your [name] hums. It is ready.</span>")
	else
		active = FALSE
	icon_state = "judicial_visor_[active]"
	if(user)
...
/obj/effect/proc_holder/judicial_visor/proc/toggle(mob/user)
	var/message
	if(active)
		message = "<span_class='brass'>You dispel the power of [visor].</span>"
		remove_ranged_ability(message)
	else
		message = "<span_class='brass'><i>You harness [visor]'s power.</i> <b>Left-click to place a judicial marker!</b></span>"
...
/obj/effect/proc_holder/judicial_visor/InterceptClickOn(mob/living/caller, params, atom/target)
	...
	if(target in view(7, get_turf(ranged_ability_user)))
		...
		ranged_ability_user.visible_message("<span_class='warning'>[ranged_ability_user]'s judicial visor fires a stream of energy at [target], creating a strange mark!</span>", "<span_class='heavy_brass'>You direct [visor]'s power to [target]. You must wait for some time before doing this again.</span>")
...
/obj/effect/clockwork/judicial_marker/proc/judicialblast()
	...
	for(var/mob/living/L in range(1, src))
		...
		if(I)
			if(isitem(I))
				L.visible_message("<span_class='warning'>Strange energy flows into [L]'s [I.name]!</span>", \
				"<span_class='userdanger'>Your [I.name] shields you from [src]!</span>")
			continue
		L.Paralyze(15) //knocks down briefly when exploding
		if(!iscultist(L))
			L.visible_message("<span_class='warning'>[L] is struck by a judicial explosion!</span>", \
			"<span_class='userdanger'>[!issilicon(L) ? "An unseen force slams you into the ground!" : "ERROR: Motor servos disabled by external source!"]</span>")
		else
			L.visible_message("<span_class='warning'>[L] is struck by a judicial explosion!</span>", \
			"<span_class='heavy_brass'>\"Keep an eye out, filth.\"</span>\n<span_class='userdanger'>A burst of heat crushes you against the ground!</span>")
	...
	to_chat(user, "<span_class='brass'><b>[targetsjudged ? "Successfully judged <span_class='neovgre'>[targetsjudged]</span>":"Judged no"] heretic[targetsjudged == 1 ? "":"s"].</b></span>")
