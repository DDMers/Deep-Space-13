	...
	if(ishuman(loc))
		...
		if(src == H.glasses && !up)
			if(H.has_trait(TRAIT_BLIND))
				to_chat(H, "<span_class='heavy_brass'>\"You're blind, idiot. Stop embarrassing yourself.\"</span>")
				return
			if(blind_cultist(H))
				return
			if(is_servant_of_ratvar(H))
				to_chat(H, "<span_class='heavy_brass'>You push the spectacles down, and all is revealed to you.[GLOB.ratvar_awakens ? "" : " Your eyes begin to itch - you cannot do this for long."]</span>")
				...
				to_chat(H, "<span_class='heavy_brass'>You push the spectacles down, but you can't see through the glass.</span>")

/obj/item/clothing/glasses/wraith_spectacles/proc/blind_cultist(mob/living/victim)
	if(iscultist(victim))
		to_chat(victim, "<span_class='heavy_brass'>\"It looks like Nar'Sie's dogs really don't value their eyes.\"</span>")
		to_chat(victim, "<span_class='userdanger'>Your eyes explode with horrific pain!</span>")
		victim.emote("scream")
...
/obj/item/clothing/glasses/wraith_spectacles/equipped(mob/living/user, slot)
	...
	if(user.has_trait(TRAIT_BLIND))
		to_chat(user, "<span_class='heavy_brass'>\"You're blind, idiot. Stop embarrassing yourself.\"</span>" )
		return
	if(blind_cultist(user)) //Cultists instantly go blind
		return
	set_vision_vars(TRUE)
	if(is_servant_of_ratvar(user))
		to_chat(user, "<span_class='heavy_brass'>As you put on the spectacles, all is revealed to you.[GLOB.ratvar_awakens || GLOB.ratvar_approaches ? "" : " Your eyes begin to itch - you cannot do this for long."]</span>")
		...
		to_chat(user, "<span_class='heavy_brass'>You put on the spectacles, but you can't see through the glass.</span>")
...
/datum/status_effect/wraith_spectacles/proc/apply_eye_damage(mob/living/carbon/human/H)
	...
	if(eye_damage_done >= nearsight_breakpoint)
		if(!H.has_trait(TRAIT_NEARSIGHT))
			to_chat(H, "<span_class='nzcrentr'>Your vision doubles, then trembles. Darkness begins to close in. You can't keep this up!</span>")
		H.become_nearsighted(EYE_DAMAGE)
	if(eye_damage_done >= blind_breakpoint)
		if(!H.has_trait(TRAIT_BLIND))
			to_chat(H, "<span_class='nzcrentr_large'>A piercing white light floods your vision. Suddenly, all goes dark!</span>")
		H.become_blind(EYE_DAMAGE)

	if(prob(min(20, 5 + eye_damage_done)))
		to_chat(H, "<span_class='nzcrentr_small'><i>Your eyes continue to burn.</i></span>")
