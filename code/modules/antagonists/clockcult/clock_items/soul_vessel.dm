...
/obj/item/mmi/posibrain/soul_vessel
	...
	<span_class='brass'>If active, can serve as a positronic brain, placable in cyborg shells or clockwork construct shells.</span>"
	icon = 'icons/obj/clockwork_objects.dmi'
	icon_state = "soul_vessel"
	req_access = list()
	braintype = "Servant"
	begin_activation_message = "<span_class='brass'>You activate the cogwheel. It hitches and stalls as it begins spinning.</span>"
	success_message = "<span_class='brass'>The cogwheel's rotation smooths out as the soul vessel activates.</span>"
	fail_message = "<span_class='warning'>The cogwheel creaks and grinds to a halt. Maybe you could try again?</span>"
	new_role = "Soul Vessel"
	welcome_message = "<span_class='warning'>ALL PAST LIVES ARE FORGOTTEN.</span>\n\
	...
	new_mob_message = "<span_class='brass'>The soul vessel emits a jet of steam before its cogwheel smooths out.</span>"
	dead_message = "<span_class='deadsay'>Its cogwheel, scratched and dented, lies motionless.</span>"
	recharge_message = "<span_class='warning'>The soul vessel's internal geis capacitor is still recharging!</span>"
...
/obj/item/mmi/posibrain/soul_vessel/attack(mob/living/target, mob/living/carbon/human/user)
	...
	if(QDELETED(brainmob))
		return
	if(brainmob.key)
		to_chat(user, "<span_class='nezbere'>\"This vessel is filled, friend. Provide it with a body.\"</span>")
		return
	if(is_servant_of_ratvar(target))
		to_chat(user, "<span_class='nezbere'>\"It would be more wise to revive your allies, friend.\"</span>")
		return
	if(target.suiciding)
		to_chat(user, "<span_class='nezbere'>\"This ally isn't able to be revived.\"</span>")
		return
	var/mob/living/carbon/human/H = target
	if(H.stat == CONSCIOUS)
		to_chat(user, "<span_class='warning'>[H] must be dead or unconscious for you to claim [H.p_their()] mind!</span>")
		return
	if(H.head)
		var/obj/item/I = H.head
		if(I.flags_inv & HIDEHAIR) //they're wearing a hat that covers their skull
			to_chat(user, "<span_class='warning'>[H]'s head is covered, remove [H.p_their()] [H.head] first!</span>")
			return
	if(H.wear_mask)
		var/obj/item/I = H.wear_mask
		if(I.flags_inv & HIDEHAIR) //they're wearing a mask that covers their skull
			to_chat(user, "<span_class='warning'>[H]'s head is covered, remove [H.p_their()] [H.wear_mask] first!</span>")
			return
	var/obj/item/bodypart/head/HE = H.get_bodypart(BODY_ZONE_HEAD)
	if(!HE) //literally headless
		to_chat(user, "<span_class='warning'>[H] has no head, and thus no mind to claim!</span>")
		return
	var/obj/item/organ/brain/B = H.getorgan(/obj/item/organ/brain)
	if(!B) //either somebody already got to them or robotics did
		to_chat(user, "<span_class='warning'>[H] has no brain, and thus no mind to claim!</span>")
		return
	if(B.suicided || B.brainmob?.suiciding)
		to_chat(user, "<span_class='nezbere'>\"This ally isn't able to be revived.\"</span>")
		return
	if(!H.key) //nobody's home
		to_chat(user, "<span_class='warning'>[H] has no mind to claim!</span>")
		return
	...
	user.visible_message("<span_class='warning'>[user] presses [src] to [H]'s head, ripping through the skull and carefully extracting the brain!</span>", \
	"<span_class='brass'>You extract [H]'s consciousness from [H.p_their()] body, trapping it in the soul vessel.</span>")
