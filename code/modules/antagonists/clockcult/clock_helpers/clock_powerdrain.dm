...
/obj/machinery/power/apc/power_drain(clockcult_user)
	if(cell && cell.charge)
		...
		if(!cell.charge && !shorted)
			shorted = 1
			visible_message("<span_class='warning'>The [name]'s screen blurs with static.</span>")
		update()
		update_icon()
...
/obj/machinery/power/smes/power_drain(clockcult_user)
	if(charge)
		...
		if(!charge && !panel_open)
			...
			visible_message("<span_class='warning'>[src]'s panel flies open with a flurry of sparks!</span>")
		update_icon()
