...
/turf/open/floor/clockwork/fabrication_vals(mob/living/user, obj/item/clockwork/replica_fabricator/fabricator, silent)
	...
	if(locate(/obj/structure/falsewall) in contents)
		to_chat(user, "<span_class='warning'>There is a false wall in the way, preventing you from fabricating a clockwork wall on [src].</span>")
		return
	if(is_blocked_turf(src, TRUE))
		to_chat(user, "<span_class='warning'>Something is in the way, preventing you from fabricating a clockwork wall on [src].</span>")
		return TRUE
...
/obj/item/stack/tile/plasteel/fabrication_vals(mob/living/user, obj/item/clockwork/replica_fabricator/fabricator, silent)
	...
	var/no_delete = FALSE
	if(amount_temp < 2)
		to_chat(user, "<span_class='warning'>You need at least <b>2</b> floor tiles to convert into power.</span>")
		return TRUE
...
/obj/structure/destructible/clockwork/fabrication_vals(mob/living/user, obj/item/clockwork/replica_fabricator/fabricator, silent)
	...
	if(!fabricator.fabricator_repair_checks(repair_values, src, user))
		return
	user.visible_message("<span_class='notice'>[user]'s [fabricator.name] starts covering [src] in glowing orange energy...</span>", \
	"<span_class='alloy'>You start repairing [src]...</span>")
	...
	if(fabricator)
		...
		if(user)
			user.visible_message("<span_class='notice'>[user]'s [fabricator.name] stops covering [src] with glowing orange energy.</span>", \
			"<span_class='alloy'>You finish repairing [src]. It is now at <b>[obj_integrity]/[max_integrity]</b> integrity.</span>")
...
/mob/living/proc/fabricator_heal(mob/living/user, obj/item/clockwork/replica_fabricator/fabricator)
	...
	if(!fabricator.fabricator_repair_checks(repair_values, src, user))
		return
	user.visible_message("<span_class='notice'>[user]'s [fabricator.name] starts covering [src == user ? "[user.p_them()]" : "[src]"] in glowing orange energy...</span>", \
	"<span_class='alloy'>You start repairing [src == user ? "yourself" : "[src]"]...</span>")
	fabricator.repairing = src
	while(fabricator && user && src)
		if(!do_after(user, repair_values["healing_for_cycle"] * fabricator.speed_multiplier, target = src, \
			extra_checks = CALLBACK(fabricator, /obj/item/clockwork/replica_fabricator.proc/fabricator_repair_checks, repair_values, src, user, TRUE)))
...
/mob/living/silicon/fabrication_vals(mob/living/user, obj/item/clockwork/replica_fabricator/fabricator, silent)
	...
	if(health == maxHealth) //if we're at maximum health, replace the turf under us
		...
		user.visible_message("<span_class='notice'>[user]'s [fabricator.name] stops covering [src == user ? "[user.p_them()]" : "[src]"] with glowing orange energy.</span>", \
		"<span_class='alloy'>You finish repairin[src == user ? "g yourself. You are":"g [src]. [p_theyre(TRUE)]"] now at <b>[abs(HEALTH_THRESHOLD_DEAD - health)]/[abs(HEALTH_THRESHOLD_DEAD - maxHealth)]</b> health.</span>")
...
/mob/living/simple_animal/hostile/clockwork/fabrication_vals(mob/living/user, obj/item/clockwork/replica_fabricator/fabricator, silent)
	...
	if(health == maxHealth) //if we're at maximum health, replace the turf under us
		...
		user.visible_message("<span_class='notice'>[user]'s [fabricator.name] stops covering [src == user ? "[user.p_them()]" : "[src]"] with glowing orange energy.</span>", \
		"<span_class='alloy'>You finish repairin[src == user ? "g yourself. You are":"g [src]. [p_theyre(TRUE)]"] now at <b>[health]/[maxHealth]</b> health.</span>")
...
/mob/living/simple_animal/drone/cogscarab/fabrication_vals(mob/living/user, obj/item/clockwork/replica_fabricator/fabricator, silent)
	...
	if(health == maxHealth)
		...
		if(do_after(user, (maxHealth - health)*2, target=src))
			...
			user.visible_message("<span_class='notice'>[user]'s [fabricator.name] stops covering [src == user ? "[user.p_them()]" : "[src]"] with glowing orange energy.</span>", \
			"<span_class='alloy'>You finish repairing [src == user ? "yourself" : "[src]"].</span>")
		if(fabricator)
			fabricator.repairing = null
