...
/obj/structure/destructible/clockwork/wall_gear
	...
	break_message = "<span_class='warning'>The gear breaks apart into shards of alloy!</span>"
...
/obj/structure/destructible/clockwork/wall_gear/attackby(obj/item/I, mob/user, params)
	if(I.tool_behaviour == TOOL_WRENCH)
		...
		if(anchored)
			to_chat(user, "<span_class='warning'>[src] needs to be unsecured to disassemble it!</span>")
		else
			user.visible_message("<span_class='warning'>[user] starts to disassemble [src].</span>", "<span_class='notice'>You start to disassemble [src]...</span>")
			if(I.use_tool(src, user, 30, volume=100) && !anchored)
				to_chat(user, "<span_class='notice'>You disassemble [src].</span>")
				deconstruct(TRUE)
		return 1
		...
		if(iswallturf(T))
			...
			return
		if(!isfloorturf(T))
			to_chat(user, "<span_class='warning'>A floor must be present to build a [anchored ? "false ":""]wall!</span>")
			return
		...
		if(do_after(user, 20, target = src))
			...
			if(W.use(2 - brass_floor))
				...
				to_chat(user, "<span_class='warning'>You need more brass to make a [anchored ? "false ":""]wall!</span>")
		return 1
	return ..()
