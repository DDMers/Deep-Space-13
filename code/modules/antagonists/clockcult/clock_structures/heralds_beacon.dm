...
/obj/structure/destructible/clockwork/heralds_beacon
	...
	break_message = "<span_class='warning'>The beacon crackles with power before collapsing into pieces!</span>"
	max_integrity = 250
	light_color = "#EF078E"
...
/obj/structure/destructible/clockwork/heralds_beacon/process()
	...
	time_remaining--
	if(!time_remaining)
		hierophant_message("<span_class='bold sevtug_small'>[src] has lost its power, and can no longer be activated.</span>")
	...
	if(isobserver(user) || is_servant_of_ratvar(user))
		if(!available)
			if(!GLOB.ratvar_approaches)
				to_chat(user, "<span_class='bold alloy'>It can no longer be activated.</span>")
			else
				to_chat(user, "<span_class='bold neovgre_small'>It has been activated!</span>")
		else
			to_chat(user, "<span_class='brass'>There are <b>[time_remaining]</b> second[time_remaining != 1 ? "s" : ""] remaining to vote.</span>")
			to_chat(user, "<span_class='big brass'>There are <b>[voters.len]/[votes_needed]</b> votes to activate the beacon!</span>")
...
/obj/structure/destructible/clockwork/heralds_beacon/attack_hand(mob/living/user)
	...
	if(!is_servant_of_ratvar(user))
		...
		return
	if(!available)
		to_chat(user, "<span_class='danger'>You can no longer vote with [src].</span>")
		return
	...
	hierophant_message("<span_class='brass'><b>[user.real_name]</b> has [voting ? "voted" : "undone their vote"] to activate [src]! The beacon needs [votes_left] more votes to activate.")
	for(var/mob/M in GLOB.player_list)
		if(isobserver(M) || is_servant_of_ratvar(M))
...
/obj/structure/destructible/clockwork/heralds_beacon/proc/herald_the_justiciar()
	...
	hierophant_message("<span_class='big bold brass'>The beacon's activation has given your team great power! Many of your objects are permanently empowered!</span>")
	...
	for(var/mob/living/carbon/human/H in GLOB.alive_mob_list)
		if(is_servant_of_ratvar(H))
			to_chat(H, "<span_class='bold alloy'>The beacon's power warps your body into a clockwork form! You are now immune to many hazards, and your body is more robust against damage!</span>")
