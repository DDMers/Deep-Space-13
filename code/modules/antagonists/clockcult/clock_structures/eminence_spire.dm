...
/obj/structure/destructible/clockwork/eminence_spire
	...
	break_message = "<span_class='warning'>The spire screeches with crackling power and collapses into scrap!</span>"
	max_integrity = 400
...
/obj/structure/destructible/clockwork/eminence_spire/attack_hand(mob/living/user)
	...
	if(!is_servant_of_ratvar(user))
		to_chat(user, "<span_class='notice'>You can tell how powerful [src] is; you know better than to touch it.</span>")
		return
	if(kingmaking)
		return
	...
	if(C.clock_team.eminence)
		to_chat(user, "<span_class='warning'>There's already an Eminence!</span>")
		return
	if(!GLOB.servants_active)
		to_chat(user, "<span_class='warning'>The Ark isn't active!</span>")
		return
...
/obj/structure/destructible/clockwork/eminence_spire/attack_drone(mob/living/simple_animal/drone/user)
	if(!is_servant_of_ratvar(user))
		...
		to_chat(user, "<span_class='warning'>You feel the omniscient gaze turn into a puzzled frown. Perhaps you should just stick to building.</span>")
		return
...
/obj/structure/destructible/clockwork/eminence_spire/attack_ghost(mob/user)
	...
	if(rando)
		...
		if(random_cultist && random_cultist.clock_team && random_cultist.clock_team.eminence)
			to_chat(user, "<span_class='warning'>There's already an Eminence - too late!</span>")
			return
	if(!GLOB.servants_active)
		to_chat(user, "<span_class='warning'>The Ark must be active first!</span>")
		return
	if(alert(user, "Become the Eminence using admin?", "Become Eminence", "Yes", "No") != "Yes")
		return
	message_admins("<span_class='danger'>Admin [key_name_admin(user)] directly became the Eminence of the cult!</span>")
	...
	hierophant_message("<span_class='bold large_brass'>Ratvar has directly assigned the Eminence!</span>")
...
/obj/structure/destructible/clockwork/eminence_spire/proc/nomination(mob/living/nominee) //A user is nominating themselves or ghosts to become Eminence
	...
	switch(nomination_choice)
		...
		if("Nominate Yourself")
			...
			hierophant_message("<span_class='brass'><b>[nominee] nominates [nominee.p_them()]self as the Eminence!</b> You may object by interacting with the eminence spire. The vote will otherwise pass in 30 seconds.</span>")
		if("Nominate Ghosts")
			eminence_nominee = "ghosts"
			hierophant_message("<span_class='brass'><b>[nominee] proposes selecting an Eminence from ghosts!</b> You may object by interacting with the eminence spire. The vote will otherwise pass in 30 seconds.</span>")
	for(var/mob/M in servants_and_ghosts())
...
/obj/structure/destructible/clockwork/eminence_spire/proc/objection(mob/living/wright)
	if(alert(wright, "Object to the selection of [eminence_nominee] as Eminence?", "Objection!", "Object", "Cancel") == "Cancel" || !is_servant_of_ratvar(wright) || !wright.canUseTopic(src) || !eminence_nominee)
		return
	hierophant_message("<span_class='brass'><b>[wright] objects to the nomination of [eminence_nominee]!</b> The eminence spire has been reset.</span>")
	for(var/mob/M in servants_and_ghosts())
...
/obj/structure/destructible/clockwork/eminence_spire/proc/cancelation(mob/living/cold_feet)
	if(alert(cold_feet, "Cancel your nomination?", "Cancel Nomination", "Withdraw Nomination", "Cancel") == "Cancel" || !is_servant_of_ratvar(cold_feet) || !cold_feet.canUseTopic(src) || !eminence_nominee)
		return
	hierophant_message("<span_class='brass'><b>[eminence_nominee] has withdrawn their nomination!</b> The eminence spire has been reset.</span>")
	for(var/mob/M in servants_and_ghosts())
		M.playsound_local(M, 'sound/machines/clockcult/integration_cog_install.ogg', 50, FALSE)
	eminence_nominee = null
	deltimer(selection_timer)
...
/obj/structure/destructible/clockwork/eminence_spire/proc/kingmaker()
	...
	if(ismob(eminence_nominee))
		if(!eminence_nominee.client || !eminence_nominee.mind)
			hierophant_message("<span_class='brass'><b>[eminence_nominee] somehow lost their sentience!</b> The eminence spire has been reset.</span>")
			for(var/mob/M in servants_and_ghosts())
				M.playsound_local(M, 'sound/machines/clockcult/integration_cog_install.ogg', 50, FALSE)
			eminence_nominee = null
			return
		playsound(eminence_nominee, 'sound/machines/clockcult/ark_damage.ogg', 50, FALSE)
		eminence_nominee.visible_message("<span_class='warning'>A blast of white-hot light flows into [eminence_nominee], vaporizing [eminence_nominee.p_them()] in an instant!</span>", \
		"<span_class='userdanger'>allthelightintheuniverseflowing.into.YOU</span>")
		...
		hierophant_message("<span_class='bold large_brass'>[eminence_nominee] has ascended into the Eminence!</span>")
	else if(eminence_nominee == "ghosts")
		kingmaking = TRUE
		hierophant_message("<span_class='brass'><b>The eminence spire is now selecting a ghost to be the Eminence...</b></span>")
		...
		if(!LAZYLEN(candidates))
			for(var/mob/M in servants_and_ghosts())
				M.playsound_local(M, 'sound/machines/clockcult/integration_cog_install.ogg', 50, FALSE)
			hierophant_message("<span_class='brass'><b>No ghosts accepted the offer!</b> The eminence spire has been reset.</span>")
			eminence_nominee = null
			return
		visible_message("<span_class='warning'>A blast of white-hot light spirals from [src] in waves!</span>")
		...
		hierophant_message("<span_class='bold large_brass'>A ghost has ascended into the Eminence!</span>")
	for(var/mob/M in servants_and_ghosts())
		M.playsound_local(M, 'sound/machines/clockcult/eminence_selected.ogg', 50, FALSE)
	eminence_nominee = null
