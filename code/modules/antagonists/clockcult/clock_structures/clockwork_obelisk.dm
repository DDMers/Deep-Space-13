...
/obj/structure/destructible/clockwork/powered/clockwork_obelisk
	...
	max_integrity = 150
	break_message = "<span_class='warning'>The obelisk falls to the ground, undamaged!</span>"
...
/obj/structure/destructible/clockwork/powered/clockwork_obelisk/examine(mob/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		to_chat(user, "<span_class='nzcrentr_small'>It requires <b>[DisplayPower(hierophant_cost)]</b> to broadcast over the Hierophant Network, and <b>[DisplayPower(gateway_cost)]</b> to open a Spatial Gateway.</span>")

/obj/structure/destructible/clockwork/powered/clockwork_obelisk/can_be_unfasten_wrench(mob/user, silent)
	if(active)
		if(!silent)
			to_chat(user, "<span_class='warning'>[src] is currently sustaining a gateway!</span>")
...
/obj/structure/destructible/clockwork/powered/clockwork_obelisk/attack_hand(mob/living/user)
	...
	if(!is_servant_of_ratvar(user) || !can_access_clockwork_power(src, hierophant_cost) || !anchored)
		to_chat(user, "<span_class='warning'>You place your hand on [src], but it doesn't react.</span>")
		return
	var/choice = alert(user,"You place your hand on [src]...",,"Hierophant Broadcast","Spatial Gateway","Cancel")
	switch(choice)
		if("Hierophant Broadcast")
			if(active)
				to_chat(user, "<span_class='warning'>[src] is sustaining a gateway and cannot broadcast!</span>")
				return
			if(!user.can_speak_vocal())
				to_chat(user, "<span_class='warning'>You cannot speak through [src]!</span>")
				return
			...
			if(!is_servant_of_ratvar(user) || !input || !user.canUseTopic(src, !issilicon(user)))
				return
			if(!anchored)
				to_chat(user, "<span_class='warning'>[src] is no longer secured!</span>")
				return FALSE
			if(active)
				to_chat(user, "<span_class='warning'>[src] is sustaining a gateway and cannot broadcast!</span>")
				return
			...
			if(!try_use_power(hierophant_cost))
				to_chat(user, "<span_class='warning'>[src] lacks the power to broadcast!</span>")
				return
		...
		if("Spatial Gateway")
			if(active)
				to_chat(user, "<span_class='warning'>[src] is already sustaining a gateway!</span>")
				return
			if(!user.can_speak_vocal())
				to_chat(user, "<span_class='warning'>You need to be able to speak to open a gateway!</span>")
				return
			if(!try_use_power(gateway_cost))
				to_chat(user, "<span_class='warning'>[src] lacks the power to open a gateway!</span>")
				return
