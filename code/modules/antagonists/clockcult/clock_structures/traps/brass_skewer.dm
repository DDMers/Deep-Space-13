...
/obj/structure/destructible/clockwork/trap/brass_skewer
	...
	break_message = "<span_class='warning'>The skewer snaps in two!</span>"
	max_integrity = 40
	density = FALSE
	can_buckle = TRUE
...
/obj/structure/destructible/clockwork/trap/brass_skewer/Destroy()
	...
	if(buckled_mobs && LAZYLEN(buckled_mobs))
		...
		if(iscarbon(L))
			L.Paralyze(100)
			L.visible_message("<span_class='warning'>[L] is maimed as the skewer shatters while still in [L.p_their()] body!</span>")
...
/obj/structure/destructible/clockwork/trap/attackby(obj/item/I, mob/living/user, params)
	if(buckled_mobs && (user in buckled_mobs))
		to_chat(user, "<span_class='warning'>You can't reach!</span>")
		return
	..()
...
/obj/structure/destructible/clockwork/trap/brass_skewer/activate()
	...
	if(squirrel)
		if(iscyborg(squirrel))
			if(!squirrel.stat)
				squirrel.visible_message("<span_class='boldwarning'>A massive brass spike erupts from the ground, rending [squirrel]'s chassis but shattering into pieces!</span>", \
				"<span_class='userdanger'>A massive brass spike rips through your chassis and bursts into shrapnel in your casing!</span>")
				squirrel.adjustBruteLoss(50)
				squirrel.Stun(20)
				addtimer(CALLBACK(src, .proc/take_damage, max_integrity), 1)
		else
			squirrel.visible_message("<span_class='boldwarning'>A massive brass spike erupts from the ground, impaling [squirrel]!</span>", \
			"<span_class='userdanger'>A massive brass spike rams through your chest, hoisting you into the air!</span>")
		...
		visible_message("<span_class='danger'>A massive brass spike erupts from the ground!</span>")
...
/obj/structure/destructible/clockwork/trap/brass_skewer/user_unbuckle_mob(mob/living/skewee, mob/living/user)
	if(user == skewee)
		if(wiggle_wiggle)
			return
		user.visible_message("<span_class='warning'>[user] starts wriggling off of [src]!</span>", \
		"<span_class='danger'>You start agonizingly working your way off of [src]...</span>")
		wiggle_wiggle = TRUE
		if(!do_after(user, 300, target = user))
			user.visible_message("<span_class='warning'>[user] slides back down [src]!</span>")
			user.emote("scream")
		...
		user.visible_message("<span_class='danger'>[user] starts tenderly lifting [skewee] off of [src]...</span>", \
		"<span_class='danger'>You start tenderly lifting [skewee] off of [src]...</span>")
		if(!do_after(user, 60, target = skewee))
			skewee.visible_message("<span_class='warning'>[skewee] painfully slides back down [src].</span>")
			skewee.emote("moan")
			return
	skewee.visible_message("<span_class='danger'>[skewee] comes free of [src] with a squelching pop!</span>", \
	"<span_class='boldannounce'>You come free of [src]!</span>")
	skewee.Paralyze(30)
