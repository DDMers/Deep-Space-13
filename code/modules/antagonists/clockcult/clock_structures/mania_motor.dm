...
/obj/structure/destructible/clockwork/powered/mania_motor
	...
	break_message = "<span_class='warning'>The antenna break off, leaving a pile of shards!</span>"
	max_integrity = 100
	light_color = "#AF0AAF"
...
/obj/structure/destructible/clockwork/powered/mania_motor/examine(mob/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		to_chat(user, "<span_class='sevtug_small'>It requires <b>[DisplayPower(mania_cost)]</b> to run.</span>")

/obj/structure/destructible/clockwork/powered/mania_motor/forced_disable(bad_effects)
	if(active)
		if(bad_effects)
			try_use_power(MIN_CLOCKCULT_POWER*4)
		visible_message("<span_class='warning'>[src] hums loudly, then the sockets at its base fall dark!</span>")
...
/obj/structure/destructible/clockwork/powered/mania_motor/attack_hand(mob/living/user)
	...
	if(user.canUseTopic(src, !issilicon(user), NO_DEXTERY) && is_servant_of_ratvar(user))
		if(!can_access_clockwork_power(src, mania_cost))
			to_chat(user, "<span_class='warning'>[src] needs more power to function!</span>")
			return 0
		toggle(0, user)
