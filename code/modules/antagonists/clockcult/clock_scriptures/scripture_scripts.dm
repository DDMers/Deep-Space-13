...
/datum/clockwork_scripture/create_object/replica_fabricator
	...
	creator_message = "<span_class='brass'>You form a replica fabricator.</span>"
...
/datum/clockwork_scripture/create_object/ocular_warden
	...
	creator_message = "<span_class='brass'>You form an ocular warden, which will automatically attack nearby unrestrained non-Servants that can see it.</span>"
	observer_message = "<span_class='warning'>A brass eye takes shape and slowly rises into the air, its red iris glaring!</span>"
...
/datum/clockwork_scripture/create_object/ocular_warden/check_special_requirements()
	for(var/obj/structure/destructible/clockwork/ocular_warden/W in range(OCULAR_WARDEN_EXCLUSION_RANGE, invoker))
		to_chat(invoker, "<span_class='neovgre'>You sense another ocular warden too near this location. Placing another this close would cause them to fight.</span>" )
		return FALSE
	return ..()
...
/datum/clockwork_scripture/create_object/vitality_matrix
	...
	creator_message = "<span_class='brass'>A vitality matrix appears below you. It will drain life from non-Servants and heal Servants that cross it.</span>"
...
/datum/clockwork_scripture/create_object/vitality_matrix/check_special_requirements()
	if(locate(object_path) in range(1, invoker))
		to_chat(invoker, "<span_class='danger'>Vitality matrices placed next to each other could interfere and cause a feedback loop! Move away from the other ones!</span>")
		return FALSE
	return ..()
...
/datum/clockwork_scripture/create_object/judicial_visor
	...
	creator_message = "<span_class='brass'>You form a judicial visor, which is capable of smiting a small area.</span>"
...
/datum/clockwork_scripture/clockwork_armaments/check_special_requirements()
	for(var/datum/action/innate/clockwork_armaments/F in invoker.actions)
		to_chat(invoker, "<span_class='warning'>You have already bound a Ratvarian spear to yourself!</span>")
		return FALSE
	return invoker.can_hold_items()

/datum/clockwork_scripture/clockwork_armaments/scripture_effects()
	invoker.visible_message("<span_class='warning'>A shimmer of yellow light infuses [invoker]!</span>", \
	"<span_class='brass'>You bind clockwork equipment to yourself. Use Clockwork Armaments and Call Spear to summon them.</span>")
...
/datum/action/innate/clockwork_armaments/Activate()
	...
	if(do_message)
		owner.visible_message("<span_class='warning'>Strange armor appears on [owner]!</span>", "<span_class='heavy_brass'>A bright shimmer runs down your body, equipping you with Ratvarian armor.</span>")
...
/datum/clockwork_scripture/spatial_gateway/check_special_requirements()
	if(!isturf(invoker.loc))
		to_chat(invoker, "<span_class='warning'>You must not be inside an object to use this scripture!</span>")
		return FALSE
	...
	for(var/obj/structure/destructible/clockwork/powered/clockwork_obelisk/O in GLOB.all_clockwork_objects)
		if(O.anchored)
			other_servants++
	if(!other_servants)
		to_chat(invoker, "<span_class='warning'>There are no other conscious servants or anchored clockwork obelisks!</span>")
		return FALSE
	return TRUE
