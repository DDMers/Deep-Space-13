...
/datum/action/innate/cult/comm/proc/cultist_commune(mob/living/user, message)
	...
	if(user.mind && user.mind.has_antag_datum(/datum/antagonist/cult/master))
		...
		title = "Construct"
	my_message = "<span_class='[span]'><b>[title] [findtextEx(user.name, user.real_name) ? user.name : "[user.real_name] (as [user.name])"]:</b> [message]</span>"
...
/datum/action/innate/cult/comm/spirit/cultist_commune(mob/living/user, message)
	...
	if(!message)
		return
	my_message = "<span_class='cultboldtalic'>The [user.name]: [message]</span>"
	for(var/i in GLOB.player_list)
		var/mob/M = i
		if(iscultist(M))
			to_chat(M, my_message)
...
/proc/pollCultists(var/mob/living/Nominee,datum/team/cult/team) //Cult Master Poll
	...
	for(var/datum/mind/B in team.members)
		if(B.current)
			...
			if(!B.current.incapacitated())
				...
				to_chat(B.current, "<span_class='cultlarge'>Acolyte [Nominee] has asserted that [Nominee.p_theyre()] worthy of leading the cult. A vote will be called shortly.</span>")
	sleep(100)
	...
	if(QDELETED(Nominee) || Nominee.incapacitated())
		...
		for(var/datum/mind/B in team.members)
			if(B.current)
				B.current.update_action_buttons_icon()
				if(!B.current.incapacitated())
					to_chat(B.current,"<span_class='cultlarge'>[Nominee] has died in the process of attempting to win the cult's support!</span>")
		return FALSE
	if(!Nominee.mind)
		team.cult_vote_called = FALSE
		for(var/datum/mind/B in team.members)
			if(B.current)
				B.current.update_action_buttons_icon()
				if(!B.current.incapacitated())
					to_chat(B.current,"<span_class='cultlarge'>[Nominee] has gone catatonic in the process of attempting to win the cult's support!</span>")
		return FALSE
	if(LAZYLEN(yes_voters) <= LAZYLEN(asked_cultists) * 0.5)
		...
		for(var/datum/mind/B in team.members)
			if(B.current)
				B.current.update_action_buttons_icon()
				if(!B.current.incapacitated())
					to_chat(B.current, "<span_class='cultlarge'>[Nominee] could not win the cult's support and shall continue to serve as an acolyte.</span>")
		return FALSE
	...
	for(var/datum/mind/B in team.members)
		if(B.current)
			...
			if(!B.current.incapacitated())
				to_chat(B.current,"<span_class='cultlarge'>[Nominee] has won the cult's support and is now their master. Follow [Nominee.p_their()] orders to the best of your ability!</span>")
	return TRUE
...
/datum/action/innate/cult/master/finalreck/Activate()
	...
	for(var/i in 1 to 4)
		...
		if(!LAZYLEN(destinations))
			to_chat(owner, "<span_class='warning'>You need more space to summon your cult!</span>")
			return
...
/datum/action/innate/cult/master/cultmark/IsAvailable()
	if(cooldown > world.time)
		if(!CM.active)
			to_chat(owner, "<span_class='cultlarge'><b>You need to wait [DisplayTimeText(cooldown - world.time)] before you can mark another target!</b></span>")
		return FALSE
	return ..()
...
/obj/effect/proc_holder/cultmark/proc/toggle(mob/user)
	if(active)
		remove_ranged_ability("<span_class='cult'>You cease the marking ritual.</span>")
	else
		add_ranged_ability(user, "<span_class='cult'>You prepare to mark a target for your cult...</span>")
...
/obj/effect/proc_holder/cultmark/InterceptClickOn(mob/living/caller, params, atom/target)
	...
	if(target in view(7, get_turf(ranged_ability_user)))
		if(C.cult_team.blood_target)
			to_chat(ranged_ability_user, "<span_class='cult'>The cult has already designated a target!</span>")
			return FALSE
		...
		for(var/datum/mind/B in SSticker.mode.cult)
			if(B.current && B.current.stat != DEAD && B.current.client)
				to_chat(B.current, "<span_class='cultlarge'><b>[ranged_ability_user] has marked [C.cult_team.blood_target] in the [A.name] as the cult's top priority, get there immediately!</b></span>")
		...
		remove_ranged_ability("<span_class='cult'>The marking rite is complete! It will last for 90 seconds.</span>")
	...
	for(var/datum/mind/B in team.members)
		if(B.current && B.current.stat != DEAD && B.current.client)
			if(team.blood_target)
				to_chat(B.current,"<span_class='cultlarge'><b>The blood mark has expired!</b></span>")
...
/datum/action/innate/cult/ghostmark/proc/reset_button()
	if(owner)
		...
		to_chat(owner,"<span_class='cultbold'>Your previous mark is gone - you are now ready to create a new blood mark.</span>")
...
/datum/action/innate/cult/ghostmark/Activate()
	...
	if(C.cult_team.blood_target)
		if(cooldown>world.time)
			...
			to_chat(owner, "<span_class='cultbold'>You have cleared the cult's blood target!</span>")
			deltimer(C.cult_team.blood_target_reset_timer)
			return
		else
			to_chat(owner, "<span_class='cultbold'>The cult has already designated a target!</span>")
			return
	if(cooldown>world.time)
		to_chat(owner, "<span_class='cultbold'>You aren't ready to place another blood mark yet!</span>")
		return
	...
	for(var/datum/mind/B in SSticker.mode.cult)
		if(B.current && B.current.stat != DEAD && B.current.client)
			to_chat(B.current, "<span_class='cultlarge'><b>[owner] has marked [C.cult_team.blood_target] in the [A.name] as the cult's top priority, get there immediately!</b></span>")
			SEND_SOUND(B.current, sound(pick('sound/hallucinations/over_here2.ogg','sound/hallucinations/over_here3.ogg'),0,1,75))
			B.current.client.images += C.cult_team.blood_target_image
	to_chat(owner,"<span_class='cultbold'>You have marked the [target] for the cult! It will last for [DisplayTimeText(base_cooldown)].</span>")
...
/datum/action/innate/cult/master/pulse/IsAvailable()
	...
	if(cooldown > world.time)
		if(!PM.active)
			to_chat(owner, "<span_class='cultlarge'><b>You need to wait [DisplayTimeText(cooldown - world.time)] before you can pulse again!</b></span>")
		return FALSE
	return ..()
...
/obj/effect/proc_holder/pulse/proc/toggle(mob/user)
	if(active)
		remove_ranged_ability("<span_class='cult'>You cease your preparations...</span>")
		attached_action.throwing = FALSE
	else
		add_ranged_ability(user, "<span_class='cult'>You prepare to tear through the fabric of reality...</span>")
...
/obj/effect/proc_holder/pulse/InterceptClickOn(mob/living/caller, params, atom/target)
	...
	if(target in view(7, get_turf(ranged_ability_user)))
		...
		if(!attached_action.throwing)
			...
			to_chat(ranged_ability_user,"<span_class='cult'><b>You reach through the veil with your mind's eye and seize [target]!</b></span>")
			return
		else
			...
			attached_action.throwing = FALSE
			attached_action.cooldown = world.time + attached_action.base_cooldown
			remove_ranged_ability("<span_class='cult'>A pulse of blood magic surges through you as you shift [attached_action.throwee] through time and space.</span>")
