...
/obj/effect/rune/attackby(obj/I, mob/user, params)
	if(istype(I, /obj/item/melee/cultblade/dagger) && iscultist(user))
		...
		if(do_after(user, 15, target = src))
			to_chat(user, "<span_class='notice'>You carefully erase the [lowertext(cultist_name)] rune.</span>")
			qdel(src)
	else if(istype(I, /obj/item/nullrod))
		user.say("BEGONE FOUL MAGIKS!!", forced = "nullrod")
		to_chat(user, "<span_class='danger'>You disrupt the magic of [src] with [I].</span>")
		qdel(src)
...
/obj/effect/rune/attack_hand(mob/living/user)
	...
	if(.)
		return
	if(!iscultist(user))
		to_chat(user, "<span_class='warning'>You aren't able to understand the words of [src].</span>")
		return
	var/list/invokers = can_invoke(user)
	if(invokers.len >= req_cultists)
		invoke(invokers)
	else
		to_chat(user, "<span_class='danger'>You need [req_cultists - invokers.len] more adjacent cultists to use this rune in such a manner.</span>")
		fail_invoke()
...
/obj/effect/rune/attack_animal(mob/living/simple_animal/M)
	if(istype(M, /mob/living/simple_animal/shade) || istype(M, /mob/living/simple_animal/hostile/construct))
		if(construct_invoke || !iscultist(M)) //if you're not a cult construct we want the normal fail message
			...
			to_chat(M, "<span_class='warning'>You are unable to invoke the rune!</span>")

/obj/effect/rune/proc/conceal() //for talisman of revealing/hiding
	visible_message("<span_class='danger'>[src] fades away.</span>")
	invisibility = INVISIBILITY_OBSERVER
	alpha = 100 //To help ghosts distinguish hidden runes

/obj/effect/rune/proc/reveal() //for talisman of revealing/hiding
	invisibility = 0
	visible_message("<span_class='danger'>[src] suddenly appears!</span>")
	alpha = initial(alpha)
	...
	for(var/M in invokers)
		if(isliving(M))
			...
			if(invoke_damage)
				...
				to_chat(L, "<span_class='cult italic'>[src] saps your strength!</span>")
		else if(istype(M, /obj/item/toy/plush/narplush))
			var/obj/item/toy/plush/narplush/P = M
			P.visible_message("<span_class='cult italic'>[P] squeaks loudly!</span>")
	do_invoke_glow()
...
/obj/effect/rune/proc/fail_invoke()
	...
	visible_message("<span_class='warning'>The markings pulse with a small flash of red light, then fall dark.</span>")
	var/oldcolor = color
	color = rgb(255, 0, 0)
...
/obj/effect/rune/convert/invoke(var/list/invokers)
	...
	if(!myriad_targets.len)
		...
		return
	rune_in_use = TRUE
	visible_message("<span_class='warning'>[src] pulses blood red!</span>")
	var/oldcolor = color
	...
	if(L.stat != DEAD && (is_clock || is_convertable))
		...
		..()
		if(is_clock)
			L.visible_message("<span_class='warning'>[L]'s eyes glow a defiant yellow!</span>", \
			"<span_class='cultlarge'>\"Stop resisting. You <i>will</i> be mi-\"</span>\n\
			<span_class='large_brass'>\"Give up and you will feel pain unlike anything you've ever felt!\"</span>")
			L.Paralyze(80)
...
/obj/effect/rune/convert/proc/do_convert(mob/living/convertee, list/invokers)
	if(invokers.len < 2)
		for(var/M in invokers)
			to_chat(M, "<span_class='danger'>You need at least two invokers to convert [convertee]!</span>")
		log_game("Offer rune failed - tried conversion with one invoker")
		return 0
	if(convertee.anti_magic_check(TRUE, TRUE, major = FALSE)) //Not major because it can be spammed
		for(var/M in invokers)
			to_chat(M, "<span_class='warning'>Something is shielding [convertee]'s mind!</span>")
	...
	convertee.visible_message("<span_class='warning'>[convertee] writhes in pain \
	[brutedamage || burndamage ? "even as [convertee.p_their()] wounds heal and close" : "as the markings below [convertee.p_them()] glow a bloody red"]!</span>", \
 	"<span_class='cultlarge'><i>AAAAAAAAAAAAAA-</i></span>")
	SSticker.mode.add_cultist(convertee.mind, 1)
	new /obj/item/melee/cultblade/dagger(get_turf(src))
	convertee.mind.special_role = ROLE_CULTIST
	to_chat(convertee, "<span_class='cult italic'><b>Your blood pulses. Your head throbs. The world goes red. All at once you are aware of a horrible, horrible, truth. The veil of reality has been ripped away \
	and something evil takes root.</b></span>")
	to_chat(convertee, "<span_class='cult italic'><b>Assist your new compatriots in their dark dealings. Your goal is theirs, and theirs is yours. You serve the Geometer above all else. Bring it back.\
	</b></span>")
...
/obj/effect/rune/convert/proc/do_sacrifice(mob/living/sacrificial, list/invokers)
	...
	if((((ishuman(sacrificial) || iscyborg(sacrificial)) && sacrificial.stat != DEAD) || C.cult_team.is_sacrifice_target(sacrificial.mind)) && invokers.len < 3)
		for(var/M in invokers)
			to_chat(M, "<span_class='cult italic'>[sacrificial] is too greatly linked to the world! You need three acolytes!</span>")
	...
	for(var/M in invokers)
		if(big_sac)
			to_chat(M, "<span_class='cultlarge'>\"Yes! This is the one I desire! You have done well.\"</span>")
		else
			if(ishuman(sacrificial) || iscyborg(sacrificial))
				to_chat(M, "<span_class='cultlarge'>\"I accept this sacrifice.\"</span>")
			else
				to_chat(M, "<span_class='cultlarge'>\"I accept this meager sacrifice.\"</span>")
...
/obj/effect/rune/teleport/invoke(var/list/invokers)
	...
	if(!potential_runes.len)
		to_chat(user, "<span_class='warning'>There are no valid runes to teleport to!</span>")
	...
	if(is_away_level(T.z))
		to_chat(user, "<span_class='cult italic'>You are not in the right dimension!</span>")
	...
	if(is_blocked_turf(target, TRUE))
		to_chat(user, "<span_class='warning'>The target rune is blocked. Attempting to teleport to it would be massively unwise.</span>")
		fail_invoke()
		return
	...
	if(movedsomething)
		...
		if(moveuserlater)
			if(do_teleport(user, target, channel = TELEPORT_CHANNEL_CULT))
				movesuccess = TRUE
		if(movesuccess)
			visible_message("<span_class='warning'>There is a sharp crack of inrushing air, and everything above the rune disappears!</span>", null, "<i>You hear a sharp crack.</i>")
			to_chat(user, "<span_class='cult'>You[moveuserlater ? "r vision blurs, and you suddenly appear somewhere else":" send everything above the rune away"].</span>")
		else
			to_chat(user, "<span_class='cult'>You[moveuserlater ? "r vision blurs briefly, but nothing happens":"  try send everything above the rune away, but the teleportation fails"].</span>")
		...
		if(movesuccess)
			target.visible_message("<span_class='warning'>There is a boom of outrushing air as something appears above the rune!</span>", null, "<i>You hear a boom.</i>")
	else
		fail_invoke()
...
/obj/effect/rune/narsie/invoke(var/list/invokers)
	...
	if(!(place in summon_objective.summon_spots))
		to_chat(user, "<span_class='cultlarge'>The Geometer can only be summoned where the veil is weak - in [english_list(summon_objective.summon_spots)]!</span>")
		return
	if(locate(/obj/singularity/narsie) in GLOB.poi_list)
		for(var/M in invokers)
			to_chat(M, "<span_class='warning'>Nar'Sie is already on this plane!</span>")
...
/obj/effect/rune/narsie/attackby(obj/I, mob/user, params)	//Since the narsie rune takes a long time to make, add logging to removal.
	if((istype(I, /obj/item/melee/cultblade/dagger) && iscultist(user)))
		user.visible_message("<span_class='warning'>[user.name] begins erasing [src]...</span>", "<span_class='notice'>You begin erasing [src]...</span>")
...
/obj/effect/rune/raise_dead/invoke(var/list/invokers)
	...
	if(!potential_revive_mobs.len)
		to_chat(user, "<span_class='cult italic'>There are no dead cultists on the rune!</span>")
	...
	if(mob_to_revive.stat == DEAD)
		...
		if(diff < 0)
			to_chat(user, "<span_class='warning'>Your cult must carry out [abs(diff)] more sacrifice\s before it can revive another cultist!</span>")
			fail_invoke()
			return
	...
	to_chat(mob_to_revive, "<span_class='cultlarge'>\"PASNAR SAVRAE YAM'TOTH. Arise.\"</span>")
	mob_to_revive.visible_message("<span_class='warning'>[mob_to_revive] draws in a huge breath, red light shining from [mob_to_revive.p_their()] eyes.</span>", \
								  "<span_class='cultlarge'>You awaken suddenly from the void. You're alive!</span>")
	rune_in_use = FALSE
...
/obj/effect/rune/raise_dead/proc/validness_checks(mob/living/target_mob, mob/living/user)
	...
	if(!(target_mob in T.contents))
		to_chat(user, "<span_class='cult italic'>The cultist to revive has been moved!</span>")
...
/obj/effect/rune/raise_dead/fail_invoke()
	...
	for(var/mob/living/M in range(1,src))
		if(iscultist(M) && M.stat == DEAD)
			M.visible_message("<span_class='warning'>[M] twitches.</span>")
...
/obj/effect/rune/wall/examine(mob/user)
	...
	if(density && iscultist(user))
		...
		if(TMR)
			to_chat(user, "<span_class='cultitalic'>The air above this rune has hardened into a barrier that will last [DisplayTimeText(TMR.timeToRun - world.time)].</span>")
...
/obj/effect/rune/wall/invoke(var/list/invokers)
	...
	user.visible_message("<span_class='warning'>[user] [carbon_user ? "places [user.p_their()] hands on":"stares intently at"] [src], and [density ? "the air above it begins to shimmer" : "the shimmer above it fades"].</span>", \
						 "<span_class='cult italic'>You channel [carbon_user ? "your life ":""]energy into [src], [density ? "temporarily preventing" : "allowing"] passage above it.</span>")
	if(carbon_user)
...
/obj/effect/rune/summon/invoke(var/list/invokers)
	...
	if(!cultist_to_summon)
		to_chat(user, "<span_class='cult italic'>You require a summoning target!</span>")
		fail_invoke()
		log_game("Summon Cultist rune failed - no target")
		return
	if(cultist_to_summon.stat == DEAD)
		to_chat(user, "<span_class='cult italic'>[cultist_to_summon] has died!</span>")
		fail_invoke()
		log_game("Summon Cultist rune failed - target died")
		return
	if(cultist_to_summon.pulledby || cultist_to_summon.buckled)
		to_chat(user, "<span_class='cult italic'>[cultist_to_summon] is being held in place!</span>")
		fail_invoke()
		log_game("Summon Cultist rune failed - target restrained")
		return
	if(!iscultist(cultist_to_summon))
		to_chat(user, "<span_class='cult italic'>[cultist_to_summon] is not a follower of the Geometer!</span>")
		fail_invoke()
		log_game("Summon Cultist rune failed - target was deconverted")
		return
	if(is_away_level(cultist_to_summon.z))
		to_chat(user, "<span_class='cult italic'>[cultist_to_summon] is not in our dimension!</span>")
		fail_invoke()
		log_game("Summon Cultist rune failed - target in away mission")
		return
	cultist_to_summon.visible_message("<span_class='warning'>[cultist_to_summon] suddenly disappears in a flash of red light!</span>", \
									  "<span_class='cult italic'><b>Overwhelming vertigo consumes you as you are hurled through the air!</b></span>")
	..()
	visible_message("<span_class='warning'>A foggy shape materializes atop [src] and solidifes into [cultist_to_summon]!</span>")
...
/obj/effect/rune/blood_boil/invoke(var/list/invokers)
	...
	rune_in_use = TRUE
	var/turf/T = get_turf(src)
	visible_message("<span_class='warning'>[src] turns a bright, glowing orange!</span>")
	color = "#FC9B54"
	set_light(6, 1, color)
	for(var/mob/living/L in viewers(T))
		if(!iscultist(L) && L.blood_volume)
			...
			if(I)
				if(isitem(I))
					to_chat(L, "<span_class='userdanger'>[I] suddenly burns hotly before returning to normal!</span>")
				continue
			to_chat(L, "<span_class='cultlarge'>Your blood boils in your veins!</span>")
			if(is_servant_of_ratvar(L))
				to_chat(L, "<span_class='userdanger'>You feel an unholy darkness dimming the Justiciar's light!</span>")
...
/obj/effect/rune/manifest/can_invoke(mob/living/user)
	if(!(user in get_turf(src)))
		to_chat(user, "<span_class='cult italic'>You must be standing on [src]!</span>")
		fail_invoke()
		log_game("Manifest rune failed - user not standing on rune")
		return list()
	if(user.has_status_effect(STATUS_EFFECT_SUMMONEDGHOST))
		to_chat(user, "<span_class='cult italic'>Ghosts can't summon more ghosts!</span>")
		fail_invoke()
...
/obj/effect/rune/manifest/invoke(var/list/invokers)
	...
	if(choice == "Summon a Cult Ghost")
		...
		if(A.map_name == "Space" || is_mining_level(T.z))
			to_chat(user, "<span_class='cultitalic'><b>The veil is not weak enough here to manifest spirits, you must be on station!</b></span>")
			return
		if(ghosts >= ghost_limit)
			to_chat(user, "<span_class='cultitalic'>You are sustaining too many ghosts to summon more!</span>")
			fail_invoke()
		...
		if(!ghosts_on_rune.len)
			to_chat(user, "<span_class='cultitalic'>There are no spirits near [src]!</span>")
			fail_invoke()
		...
		visible_message("<span_class='warning'>A cloud of red mist forms above [src], and from within steps... a [new_human.gender == FEMALE ? "wo":""]man.</span>")
		to_chat(user, "<span_class='cultitalic'>Your blood begins flowing into [src]. You must remain in place and conscious to maintain the forms of those summoned. This will hurt you slowly but surely...</span>")
		var/obj/structure/emergency_shield/invoker/N = new(T)
		new_human.key = ghost_to_spawn.key
		SSticker.mode.add_cultist(new_human.mind, 0)
		to_chat(new_human, "<span_class='cultitalic'><b>You are a servant of the Geometer. You have been made semi-corporeal by the cult of Nar'Sie, and you are to serve them at all costs.</b></span>")
		...
		ghosts--
		if(new_human)
			new_human.visible_message("<span_class='warning'>[new_human] suddenly dissolves into bones and ashes.</span>", \
									  "<span_class='cultlarge'>Your link to the world fades. Your form breaks apart.</span>")
		...
		affecting.visible_message("<span_class='warning'>[affecting] freezes statue-still, glowing an unearthly red.</span>", \
						 "<span_class='cult'>You see what lies beyond. All is revealed. In this form you find that your voice booms louder and you can mark targets for the entire cult</span>")
		...
		while(!QDELETED(affecting))
			if(!(affecting in T))
				user.visible_message("<span_class='warning'>A spectral tendril wraps around [affecting] and pulls [affecting.p_them()] back to the rune!</span>")
				Beam(affecting, icon_state="drainbeam", time=2)
				affecting.forceMove(get_turf(src)) //NO ESCAPE :^)
			if(affecting.key)
				affecting.visible_message("<span_class='warning'>[affecting] slowly relaxes, the glow around [affecting.p_them()] dimming.</span>", \
									 "<span_class='danger'>You are re-united with your physical form. [src] releases its hold over you.</span>")
				affecting.Paralyze(40)
				break
			if(affecting.health <= 10)
				to_chat(G, "<span_class='cultitalic'>Your body can no longer sustain the connection!</span>")
				break
			sleep(5)
		CM.Remove(G)
		GM.Remove(G)
...
/obj/effect/rune/apocalypse/invoke(var/list/invokers)
	...
	if(summon_objective.summon_spots.len <= 1)
		to_chat(user, "<span_class='cultlarge'>Only one ritual site remains - it must be reserved for the final summoning!</span>")
		return
	if(!(place in summon_objective.summon_spots))
		to_chat(user, "<span_class='cultlarge'>The Apocalypse rune will remove a ritual site, where Nar'Sie can be summoned, it can only be scribed in [english_list(summon_objective.summon_spots)]!</span>")
		return
	...
	visible_message("<span_class='warning'>A colossal shockwave of energy bursts from the rune, disintegrating it in the process!</span>")
	...
	for(var/mob/living/M in GLOB.alive_mob_list)
		...
		if(!iscultist(M))
			...
			to_chat(M, "<span_class='cultlarge'>An Apocalypse Rune was invoked in the [place.name], it is no longer available as a summoning site!</span>")
