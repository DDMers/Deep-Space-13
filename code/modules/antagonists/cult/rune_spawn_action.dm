...
/datum/action/innate/cult/create_rune/proc/turf_check(turf/T)
	if(!T)
		return FALSE
	if(isspaceturf(T))
		to_chat(owner, "<span_class='warning'>You cannot scribe runes in space!</span>")
		return FALSE
	if(locate(/obj/effect/rune) in T)
		to_chat(owner, "<span_class='cult'>There is already a rune here.</span>")
		return FALSE
	if(!is_station_level(T.z) && !is_mining_level(T.z))
		to_chat(owner, "<span_class='warning'>The veil is not weak enough here.</span>")
		return FALSE
	return TRUE
