...
/obj/structure/destructible/cult/examine(mob/user)
	..()
	to_chat(user, "<span_class='notice'>\The [src] is [anchored ? "":"not "]secured to the floor.</span>")
	if((iscultist(user) || isobserver(user)) && cooldowntime > world.time)
		to_chat(user, "<span_class='cult italic'>The magic in [src] is too weak, [p_they()] will be ready to use again in [DisplayTimeText(cooldowntime - world.time)].</span>")

/obj/structure/destructible/cult/examine_status(mob/user)
	if(iscultist(user) || isobserver(user))
		var/t_It = p_they(TRUE)
		var/t_is = p_are()
		return "<span_class='cult'>[t_It] [t_is] at <b>[round(obj_integrity * 100 / max_integrity)]%</b> stability.</span>"
	return ..()
...
/obj/structure/destructible/cult/attack_animal(mob/living/simple_animal/M)
	if(istype(M, /mob/living/simple_animal/hostile/construct/builder))
		if(obj_integrity < max_integrity)
			...
			M.visible_message("<span_class='danger'>[M] repairs \the <b>[src]</b>.</span>", \
				"<span_class='cult'>You repair <b>[src]</b>, leaving [p_they()] at <b>[round(obj_integrity * 100 / max_integrity)]%</b> stability.</span>")
		else
			to_chat(M, "<span_class='cult'>You cannot repair [src], as [p_theyre()] undamaged!</span>")
	else
		..()
...
/obj/structure/destructible/cult/attackby(obj/I, mob/user, params)
	if(istype(I, /obj/item/melee/cultblade/dagger) && iscultist(user))
		anchored = !anchored
		to_chat(user, "<span_class='notice'>You [anchored ? "":"un"]secure \the [src] [anchored ? "to":"from"] the floor.</span>")
		if(!anchored)
...
/obj/structure/destructible/cult/talisman
	...
	break_message = "<span_class='warning'>The altar shatters, leaving only the wailing of the damned!</span>"
...
/obj/structure/destructible/cult/talisman/attack_hand(mob/living/user)
	...
	if(.)
		return
	if(!iscultist(user))
		to_chat(user, "<span_class='warning'>You're pretty sure you know exactly what this is used for and you can't seem to touch it.</span>")
		return
	if(!anchored)
		to_chat(user, "<span_class='cultitalic'>You need to anchor [src] to the floor with your dagger first.</span>")
		return
	if(cooldowntime > world.time)
		to_chat(user, "<span_class='cult italic'>The magic in [src] is weak, it will be ready to use again in [DisplayTimeText(cooldowntime - world.time)].</span>")
		return
	...
	if(src && !QDELETED(src) && anchored && pickedtype && Adjacent(user) && !user.incapacitated() && iscultist(user) && cooldowntime <= world.time)
		...
		for(var/N in pickedtype)
			new N(get_turf(src))
			to_chat(user, "<span_class='cultitalic'>You kneel before the altar and your faith is rewarded with the [choice]!</span>")
...
/obj/structure/destructible/cult/forge
	...
	break_message = "<span_class='warning'>The force breaks apart into shards with a howling scream!</span>"
...
/obj/structure/destructible/cult/forge/attack_hand(mob/living/user)
	...
	if(.)
		return
	if(!iscultist(user))
		to_chat(user, "<span_class='warning'>The heat radiating from [src] pushes you back.</span>")
		return
	if(!anchored)
		to_chat(user, "<span_class='cultitalic'>You need to anchor [src] to the floor with your dagger first.</span>")
		return
	...
	if(src && !QDELETED(src) && anchored && pickedtype && Adjacent(user) && !user.incapacitated() && iscultist(user) && cooldowntime <= world.time)
		...
		for(var/N in pickedtype)
			new N(get_turf(src))
			to_chat(user, "<span_class='cultitalic'>You work the forge as dark knowledge guides your hands, creating the [choice]!</span>")
...
/obj/structure/destructible/cult/pylon
	...
	break_message = "<span_class='warning'>The blood-red crystal falls to the floor and shatters!</span>"
	var/heal_delay = 25
	var/last_heal = 0
...
/obj/structure/destructible/cult/tome
	...
	break_message = "<span_class='warning'>The books and tomes of the archives burn into ash as the desk shatters!</span>"
...
/obj/structure/destructible/cult/tome/attack_hand(mob/living/user)
	...
	if(.)
		return
	if(!iscultist(user))
		to_chat(user, "<span_class='warning'>These books won't open and it hurts to even try and read the covers.</span>")
		return
	if(!anchored)
		to_chat(user, "<span_class='cultitalic'>You need to anchor [src] to the floor with your dagger first.</span>")
		return
	...
	if(src && !QDELETED(src) && anchored && pickedtype.len && Adjacent(user) && !user.incapacitated() && iscultist(user) && cooldowntime <= world.time)
		...
		for(var/N in pickedtype)
			new N(get_turf(src))
			to_chat(user, "<span_class='cultitalic'>You summon the [choice] from the archives!</span>")
