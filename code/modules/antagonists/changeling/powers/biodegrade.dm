...
/datum/action/changeling/biodegrade/sting_action(mob/living/carbon/human/user)
	...
	if(!user.restrained() && isopenturf(user.loc))
		to_chat(user, "<span_class='warning'>We are already free!</span>")
		return 0
	...
	if(user.handcuffed)
		...
		if(!istype(O))
			return 0
		user.visible_message("<span_class='warning'>[user] vomits a glob of acid on [user.p_their()] [O]!</span>", \
			"<span_class='warning'>We vomit acidic ooze onto our restraints!</span>")
	...
	if(user.wear_suit && user.wear_suit.breakouttime && !used)
		...
		if(!istype(S))
			return 0
		user.visible_message("<span_class='warning'>[user] vomits a glob of acid across the front of [user.p_their()] [S]!</span>", \
			"<span_class='warning'>We vomit acidic ooze onto our straight jacket!</span>")
	...
	if(istype(user.loc, /obj/structure/closet) && !used)
		...
		if(!istype(C))
			return 0
		C.visible_message("<span_class='warning'>[C]'s hinges suddenly begin to melt and run!</span>")
		to_chat(user, "<span_class='warning'>We vomit acidic goop onto the interior of [C]!</span>")
	...
	if(istype(user.loc, /obj/structure/spider/cocoon) && !used)
		...
		if(!istype(C))
			return 0
		C.visible_message("<span_class='warning'>[src] shifts and starts to fall apart!</span>")
		to_chat(user, "<span_class='warning'>We secrete acidic enzymes from our skin and begin melting our cocoon...</span>")
...
/datum/action/changeling/biodegrade/proc/dissolve_handcuffs(mob/living/carbon/human/user, obj/O)
	if(O && user.handcuffed == O)
		user.visible_message("<span_class='warning'>[O] dissolve[O.gender==PLURAL?"":"s"] into a puddle of sizzling goop.</span>")
...
/datum/action/changeling/biodegrade/proc/dissolve_straightjacket(mob/living/carbon/human/user, obj/S)
	if(S && user.wear_suit == S)
		user.visible_message("<span_class='warning'>[S] dissolves into a puddle of sizzling goop.</span>")
...
/datum/action/changeling/biodegrade/proc/open_closet(mob/living/carbon/human/user, obj/structure/closet/C)
	if(C && user.loc == C)
		C.visible_message("<span_class='warning'>[C]'s door breaks and opens!</span>")
		new /obj/effect/decal/cleanable/greenglow(C.drop_location())
		C.welded = FALSE
		C.locked = FALSE
		C.broken = TRUE
		C.open()
		to_chat(user, "<span_class='warning'>We open the container restraining us!</span>")
...
/datum/action/changeling/biodegrade/proc/dissolve_cocoon(mob/living/carbon/human/user, obj/structure/spider/cocoon/C)
	if(C && user.loc == C)
		...
		to_chat(user, "<span_class='warning'>We dissolve the cocoon!</span>")
