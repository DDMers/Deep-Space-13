...
/datum/action/changeling/fakedeath/sting_action(mob/living/user)
	...
	if(revive_ready)
		...
		to_chat(user, "<span_class='notice'>We begin our stasis, preparing energy to arise once more.</span>")
		if(user.stat != DEAD)
...
/datum/action/changeling/fakedeath/proc/revive(mob/living/user)
	...
	if(missing.len)
		...
		user.visible_message("<span_class='warning'>[user]'s missing limbs \
			reform, making a loud, grotesque sound!</span>",
			"<span_class='userdanger'>Your limbs regrow, making a \
			loud, crunchy sound and giving you great pain!</span>",
			"<span_class='italics'>You hear organic matter ripping \
			and tearing!</span>")
		user.emote("scream")
	...
	UpdateButtonIcon()
	chemical_cost = 15
	to_chat(user, "<span_class='notice'>We have revived ourselves.</span>")

/datum/action/changeling/fakedeath/proc/ready_to_regenerate(mob/user)
	if(user && user.mind)
		var/datum/antagonist/changeling/C = user.mind.has_antag_datum(/datum/antagonist/changeling)
		if(C && C.purchasedpowers)
			to_chat(user, "<span_class='notice'>We are ready to revive.</span>")
			name = "Revive"
...
/datum/action/changeling/fakedeath/can_sting(mob/living/user)
	if(user.has_trait(TRAIT_DEATHCOMA, "changeling") && !revive_ready)
		to_chat(user, "<span_class='warning'>We are already reviving.</span>")
		return
