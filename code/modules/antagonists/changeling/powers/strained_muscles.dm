...
/datum/action/changeling/strained_muscles/sting_action(mob/living/carbon/user)
	...
	active = !active
	if(active)
		to_chat(user, "<span_class='notice'>Our muscles tense and strengthen.</span>")
	else
		...
		to_chat(user, "<span_class='notice'>Our muscles relax.</span>")
		if(stacks >= 10)
			to_chat(user, "<span_class='danger'>We collapse in exhaustion.</span>")
			user.Paralyze(60)
			user.emote("gasp")
...
/datum/action/changeling/strained_muscles/proc/muscle_loop(mob/living/carbon/user)
	while(active)
		...
		if(user.stat != CONSCIOUS || user.staminaloss >= 90)
			active = !active
			to_chat(user, "<span_class='notice'>Our muscles relax without the energy to strengthen them.</span>")
			user.Paralyze(40)
		...
		if(stacks == 11) //Warning message that the stacks are getting too high
			to_chat(user, "<span_class='warning'>Our legs are really starting to hurt...</span>")
