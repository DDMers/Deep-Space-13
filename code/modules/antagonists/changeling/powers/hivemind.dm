...
/datum/action/changeling/hivemind_upload/sting_action(var/mob/living/user)
	...
	if(names.len <= 0)
		to_chat(user, "<span_class='notice'>The airwaves already have all of our DNA.</span>")
		return
	...
	to_chat(user, "<span_class='notice'>We channel the DNA of [chosen_name] to the air.</span>")
	return TRUE
...
/datum/action/changeling/hivemind_download/sting_action(mob/user)
	...
	if(names.len <= 0)
		to_chat(user, "<span_class='notice'>There's no new DNA to absorb from the air.</span>")
		return
	...
	to_chat(user, "<span_class='notice'>We absorb the DNA of [S] from the air.</span>")
	return TRUE
