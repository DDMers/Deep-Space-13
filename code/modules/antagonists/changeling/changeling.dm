...
/datum/antagonist/changeling/proc/readapt()
	if(!ishuman(owner.current))
		to_chat(owner.current, "<span_class='danger'>We can't remove our evolutions in this form!</span>")
		return
	if(canrespec)
		to_chat(owner.current, "<span_class='notice'>We have removed our evolutions from this form, and are now ready to readapt.</span>")
		reset_powers()
		canrespec = 0
		SSblackbox.record_feedback("tally", "changeling_power_purchase", 1, "Readapt")
		return 1
	else
		to_chat(owner.current, "<span_class='danger'>You lack the power to readapt your evolutions!</span>")
		return 0
...
/datum/antagonist/changeling/proc/can_absorb_dna(mob/living/carbon/human/target, var/verbose=1)
	...
	if(stored_profiles.len)
		...
		if(prof.dna == user.dna && stored_profiles.len >= dna_max)//If our current DNA is the stalest, we gotta ditch it.
			if(verbose)
				to_chat(user, "<span_class='warning'>We have reached our capacity to store genetic information! We must transform before absorbing more.</span>")
			return
	if(!target)
		return
	if(NO_DNA_COPY in target.dna.species.species_traits)
		if(verbose)
			to_chat(user, "<span_class='warning'>[target] is not compatible with our biology.</span>")
		return
	if(target.has_trait(TRAIT_BADDNA))
		if(verbose)
			to_chat(user, "<span_class='warning'>DNA of [target] is ruined beyond usability!</span>")
		return
	if(target.has_trait(TRAIT_HUSK))
		if(verbose)
			to_chat(user, "<span_class='warning'>[target]'s body is ruined beyond usability!</span>")
		return
	if(!ishuman(target))//Absorbing monkeys is entirely possible, but it can cause issues with transforming. That's what lesser form is for anyway!
		if(verbose)
			to_chat(user, "<span_class='warning'>We could gain no benefit from absorbing a lesser creature.</span>")
		return
	if(has_dna(target.dna))
		if(verbose)
			to_chat(user, "<span_class='warning'>We already have this DNA in storage!</span>")
		return
	if(!target.has_dna())
		if(verbose)
			to_chat(user, "<span_class='warning'>[target] is not compatible with our biology.</span>")
...
/datum/antagonist/changeling/greet()
	if (you_are_greet)
		to_chat(owner.current, "<span_class='boldannounce'>You are [changelingID], a changeling! You have absorbed and taken the form of a human.</span>")
	to_chat(owner.current, "<span_class='boldannounce'>Use say \"[MODE_TOKEN_CHANGELING] message\" to communicate with your fellow changelings.</span>")
...
/datum/antagonist/changeling/farewell()
	to_chat(owner.current, "<span_class='userdanger'>You grow weak and lose your powers! You are no longer a changeling and are stuck in your current form!</span>")
...
/datum/antagonist/changeling/admin_add(datum/mind/new_owner,mob/admin)
	. = ..()
	to_chat(new_owner.current, "<span_class='boldannounce'>Our powers have awoken. A flash of memory returns to us...we are [changelingID], a changeling!</span>")
...
/datum/antagonist/changeling/proc/admin_restore_appearance(mob/admin)
	if(!stored_profiles.len || !iscarbon(owner.current))
		to_chat(admin, "<span_class='danger'>Resetting DNA failed!</span>")
	else
		var/mob/living/carbon/C = owner.current
	...
	if(objectives.len)
		...
		for(var/datum/objective/objective in objectives)
			if(objective.check_completion())
				parts += "<b>Objective #[count]</b>: [objective.explanation_text] <span_class='greentext'>Success!</b></span>"
			else
				parts += "<b>Objective #[count]</b>: [objective.explanation_text] <span_class='redtext'>Fail.</span>"
				changelingwin = 0
			count++

	if(changelingwin)
		parts += "<span_class='greentext'>The changeling was successful!</span>"
	else
		parts += "<span_class='redtext'>The changeling has failed.</span>"
