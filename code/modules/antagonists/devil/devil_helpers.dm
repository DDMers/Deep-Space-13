	...
	switch(devilInfo.bane)
			...
			if(ishuman(attacker))
				...
				if(H.w_uniform && istype(H.w_uniform, /obj/item/clothing/under))
					...
					if(U && whiteness[U.type])
						src.visible_message("<span_class='warning'>[src] seems to have been harmed by the purity of [attacker]'s clothes.</span>", "<span_class='notice'>Unsullied white clothing is disrupting your form.</span>")
						return whiteness[U.type] + 1
		if(BANE_TOOLBOX)
			if(istype(weapon, /obj/item/storage/toolbox))
				src.visible_message("<span_class='warning'>The [weapon] seems unusually robust this time.</span>", "<span_class='notice'>The [weapon] is your unmaking!</span>")
		...
		if(BANE_HARVEST)
			if(istype(weapon, /obj/item/reagent_containers/food/snacks/grown/))
				visible_message("<span_class='warning'>The spirits of the harvest aid in the exorcism.</span>", "<span_class='notice'>The harvest spirits are harming you.</span>")
				Paralyze(40)
				qdel(weapon)
				return 2
	return 1