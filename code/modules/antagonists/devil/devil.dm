...
/datum/antagonist/devil/proc/add_soul(datum/mind/soul)
	...
	to_chat(owner.current, "<span_class='warning'>You feel satiated as you received a new soul.</span>")
	update_hud()
	switch(SOULVALUE)
		if(0)
			to_chat(owner.current, "<span_class='warning'>Your hellish powers have been restored.</span>")
...
/datum/antagonist/devil/proc/remove_soul(datum/mind/soul)
	if(soulsOwned.Remove(soul))
		check_regression()
		to_chat(owner.current, "<span_class='warning'>You feel as though a soul has slipped from your grasp.</span>")
		update_hud()
...
/datum/antagonist/devil/proc/check_regression()
	...
	if(SOULVALUE < 0)
		give_appropriate_spells()
		to_chat(owner.current, "<span_class='warning'>As punishment for your failures, all of your powers except contract creation have been revoked.</span>")

/datum/antagonist/devil/proc/regress_humanoid()
	to_chat(owner.current, "<span_class='warning'>Your powers weaken, have more contracts be signed to regain power.</span>")
...
/datum/antagonist/devil/proc/regress_blood_lizard()
	...
	to_chat(D, "<span_class='warning'>Your powers weaken, have more contracts be signed to regain power.</span>")
...
/datum/antagonist/devil/proc/increase_blood_lizard()
	to_chat(owner.current, "<span_class='warning'>You feel as though your humanoid form is about to shed.  You will soon turn into a blood lizard.</span>")
	sleep(50)
	if(ishuman(owner.current))
		var/mob/living/carbon/human/H = owner.current
...
/datum/antagonist/devil/proc/increase_true_devil()
	to_chat(owner.current, "<span_class='warning'>You feel as though your current form is about to shed.  You will soon turn into a true devil.</span>")
	sleep(50)
...
/datum/antagonist/devil/proc/increase_arch_devil()
	if(!ascendable)
		return
	var/mob/living/carbon/true_devil/D = owner.current
	to_chat(D, "<span_class='warning'>You feel as though your form is about to ascend.</span>")
	sleep(50)
	if(!D)
		return
	D.visible_message("<span_class='warning'>[D]'s skin begins to erupt with spikes.</span>", \
		"<span_class='warning'>Your flesh begins creating a shield around yourself.</span>")
	sleep(100)
	if(!D)
		return
	D.visible_message("<span_class='warning'>The horns on [D]'s head slowly grow and elongate.</span>", \
		"<span_class='warning'>Your body continues to mutate. Your telepathic abilities grow.</span>")
	sleep(90)
	if(!D)
		return
	D.visible_message("<span_class='warning'>[D]'s body begins to violently stretch and contort.</span>", \
		"<span_class='warning'>You begin to rend apart the final barriers to ultimate power.</span>")
	sleep(40)
	if(!D)
		return
	...
	if(!D)
		return
	to_chat(D, "<i><b><span_class='big'>YES!!</span></b></i>")
	sleep(10)
	if(!D)
		return
	to_chat(D, "<i><b><span_class='reallybig'>YE--</span></b></i>")
	sleep(1)
	if(!D)
		return
	send_to_playing_players("<font size=5><span_class='danger'><b>\"SLOTH, WRATH, GLUTTONY, ACEDIA, ENVY, GREED, PRIDE! FIRES OF HELL AWAKEN!!\"</font></span>")
...
/datum/antagonist/devil/proc/beginResurrectionCheck(mob/living/body)
	if(SOULVALUE>0)
		to_chat(owner.current, "<span_class='userdanger'>Your body has been damaged to the point that you may no longer use it.  At the cost of some of your power, you will return to life soon.  Remain in your body.</span>")
		sleep(DEVILRESURRECTTIME)
		if (!body ||  body.stat == DEAD)
			if(SOULVALUE>0)
				if(check_banishment(body))
					to_chat(owner.current, "<span_class='userdanger'>Unfortunately, the mortals have finished a ritual that prevents your resurrection.</span>")
					return -1
				else
					to_chat(owner.current, "<span_class='userdanger'>WE LIVE AGAIN!</span>")
					return hellish_resurrection(body)
			else
				to_chat(owner.current, "<span_class='userdanger'>Unfortunately, the power that stemmed from your contracts has been extinguished.  You no longer have enough power to resurrect.</span>")
				return -1
		else
			to_chat(owner.current, "<span_class='danger'> You seem to have resurrected without your hellish powers.</span>")
	else
		to_chat(owner.current, "<span_class='userdanger'>Your hellish powers are too weak to resurrect yourself.</span>")
...
/datum/antagonist/devil/greet()
	to_chat(owner.current, "<span_class='warning'><b>You remember your link to the infernal.  You are [truename], an agent of hell, a devil.  And you were sent to the plane of creation for a reason.  A greater purpose.  Convince the crew to sin, and embroiden Hell's grasp.</b></span>")
	to_chat(owner.current, "<span_class='warning'><b>However, your infernal form is not without weaknesses.</b></span>")
	...
	to_chat(owner.current, "<span_class='warning'>Remember, the crew can research your weaknesses if they find out your devil name.</span><br>")
	.=..()
...
/datum/antagonist/devil/on_gain()
	...
	if(owner.assigned_role == "Clown" && ishuman(owner.current))
		...
		to_chat(S, "<span_class='notice'>Your infernal nature has allowed you to overcome your clownishness.</span>")
		S.dna.remove_mutation(CLOWNMUT)
	.=..()

/datum/antagonist/devil/on_removal()
	to_chat(owner.current, "<span_class='userdanger'>Your infernal link has been severed! You are no longer a devil!</span>")
	.=..()
