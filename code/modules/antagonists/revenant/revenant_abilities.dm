...
/mob/living/simple_animal/revenant/ClickOn(atom/A, params) //revenants can't interact with the world directly.
	...
	if(ishuman(A))
		if(A in drained_mobs)
			to_chat(src, "<span_class='revenwarning'>[A]'s soul is dead and empty.</span>" )
...
/mob/living/simple_animal/revenant/proc/Harvest(mob/living/carbon/human/target)
	if(!castcheck(0))
		return
	if(draining)
		to_chat(src, "<span_class='revenwarning'>You are already siphoning the essence of a soul!</span>")
		return
	if(!target.stat)
		to_chat(src, "<span_class='revennotice'>[target.p_their(TRUE)] soul is too strong to harvest.</span>")
		if(prob(10))
			to_chat(target, "You feel as if you are being watched.")
		return
	draining = TRUE
	essence_drained += rand(15, 20)
	to_chat(src, "<span_class='revennotice'>You search for the soul of [target].</span>")
	if(do_after(src, rand(10, 20), 0, target)) //did they get deleted in that second?
		if(target.ckey)
			to_chat(src, "<span_class='revennotice'>[target.p_their(TRUE)] soul burns with intelligence.</span>")
			essence_drained += rand(20, 30)
		if(target.stat != DEAD)
			to_chat(src, "<span_class='revennotice'>[target.p_their(TRUE)] soul blazes with life!</span>")
			essence_drained += rand(40, 50)
		else
			to_chat(src, "<span_class='revennotice'>[target.p_their(TRUE)] soul is weak and faltering.</span>")
		if(do_after(src, rand(15, 20), 0, target)) //did they get deleted NOW?
			switch(essence_drained)
				if(1 to 30)
					to_chat(src, "<span_class='revennotice'>[target] will not yield much essence. Still, every bit counts.</span>")
				if(30 to 70)
					to_chat(src, "<span_class='revennotice'>[target] will yield an average amount of essence.</span>")
				if(70 to 90)
					to_chat(src, "<span_class='revenboldnotice'>Such a feast! [target] will yield much essence to you.</span>")
				if(90 to INFINITY)
					to_chat(src, "<span_class='revenbignotice'>Ah, the perfect soul. [target] will yield massive amounts of essence to you.</span>")
			if(do_after(src, rand(15, 25), 0, target)) //how about now
				if(!target.stat)
					to_chat(src, "<span_class='revenwarning'>[target.p_theyre(TRUE)] now powerful enough to fight off your draining.</span>")
					to_chat(target, "<span_class='boldannounce'>You feel something tugging across your body before subsiding.</span>")
					draining = 0
					essence_drained = 0
					return //hey, wait a minute...
				to_chat(src, "<span_class='revenminor'>You begin siphoning essence from [target]'s soul.</span>")
				if(target.stat != DEAD)
					to_chat(target, "<span_class='warning'>You feel a horribly unpleasant draining sensation as your grip on life weakens...</span>")
				if(target.stat == SOFT_CRIT)
					target.Stun(46)
				reveal(46)
				stun(46)
				target.visible_message("<span_class='warning'>[target] suddenly rises slightly into the air, [target.p_their()] skin turning an ashy gray.</span>")
				if(target.anti_magic_check(FALSE, TRUE))
					to_chat(src, "<span_class='revenminor'>Something's wrong! [target] seems to be resisting the siphoning, leaving you vulnerable!</span>")
					target.visible_message("<span_class='warning'>[target] slumps onto the ground.</span>", \
											   "<span_class='revenwarning'>Violet lights, dancing in your vision, receding--</span>")
					draining = FALSE
					return
				...
				if(do_after(src, 46, 0, target)) //As one cannot prove the existance of ghosts, ghosts cannot prove the existance of the target they were draining.
					...
					if(essence_drained <= 90 && target.stat != DEAD)
						essence_regen_cap += 5
						to_chat(src, "<span_class='revenboldnotice'>The absorption of [target]'s living soul has increased your maximum essence level. Your new maximum essence is [essence_regen_cap].</span>")
					if(essence_drained > 90)
						essence_regen_cap += 15
						perfectsouls++
						to_chat(src, "<span_class='revenboldnotice'>The perfection of [target]'s soul has increased your maximum essence level. Your new maximum essence is [essence_regen_cap].</span>")
					to_chat(src, "<span_class='revennotice'>[target]'s soul has been considerably weakened and will yield no more essence for the time being.</span>")
					target.visible_message("<span_class='warning'>[target] slumps onto the ground.</span>", \
										   "<span_class='revenwarning'>Violets lights, dancing in your vision, getting clo--</span>")
					drained_mobs.Add(target)
					target.death(0)
				else
					to_chat(src, "<span_class='revenwarning'>[target ? "[target] has":"[target.p_theyve(TRUE)]"] been drawn out of your grasp. The link has been broken.</span>")
					if(target) //Wait, target is WHERE NOW?
						target.visible_message("<span_class='warning'>[target] slumps onto the ground.</span>", \
											   "<span_class='revenwarning'>Violets lights, dancing in your vision, receding--</span>")
				qdel(B)
			else
				to_chat(src, "<span_class='revenwarning'>You are not close enough to siphon [target ? "[target]'s":"[target.p_their()]"] soul. The link has been broken.</span>")
	draining = FALSE
	essence_drained = 0
...
/obj/effect/proc_holder/spell/targeted/night_vision/revenant
	...
	message = "<span_class='revennotice'>You toggle your night vision.</span>"
...
/obj/effect/proc_holder/spell/aoe_turf/revenant/proc/attempt_cast(mob/living/simple_animal/revenant/user = usr)
	...
	if(locked)
		if (!user.unlock(unlock_amount))
			...
			return FALSE
		name = "[initial(name)] ([cast_amount]E)"
		to_chat(user, "<span_class='revennotice'>You have unlocked [initial(name)]!</span>")
		panel = "Revenant Abilities"
		locked = FALSE
		charge_counter = charge_max
		return FALSE
...
/obj/effect/proc_holder/spell/aoe_turf/revenant/overload/proc/overload(turf/T, mob/user)
	for(var/obj/machinery/light/L in T)
		if(!L.on)
			return
		L.visible_message("<span_class='warning'><b>\The [L] suddenly flares brightly and begins to spark!</span>")
...
/obj/effect/proc_holder/spell/aoe_turf/revenant/malfunction/proc/malfunction(turf/T, mob/user)
	...
	for(var/mob/living/carbon/human/human in T)
		...
		if(human.anti_magic_check(FALSE, TRUE))
			continue
		to_chat(human, "<span_class='revenwarning'>You feel [pick("your sense of direction flicker out", "a stabbing pain in your head", "your mind fill with static")].</span>")
		...
		if(iscarbon(mob))
			if(ishuman(mob))
				...
				if(!blightfound)
					...
					to_chat(H, "<span_class='revenminor'>You feel [pick("suddenly sick", "a surge of nausea", "like your skin is <i>wrong</i>")].</span>")
			else
				if(mob.reagents)
