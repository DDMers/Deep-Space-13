...
/datum/disease/revblight/cure()
	if(affected_mob)
		...
		to_chat(affected_mob, "<span_class='notice'>You feel better.</span>")
	..()
...
/datum/disease/revblight/stage_act()
	if(!finalstage)
		if(!(affected_mob.mobility_flags & MOBILITY_STAND) && prob(stage*6))
			...
			return
		if(prob(stage*3))
			to_chat(affected_mob, "<span_class='revennotice'>You suddenly feel [pick("sick and tired", "disoriented", "tired and confused", "nauseated", "faint", "dizzy")]...</span>")
	...
	switch(stage)
		...
		if(5)
			if(!finalstage)
				finalstage = TRUE
				to_chat(affected_mob, "<span_class='revenbignotice'>You feel like [pick("nothing's worth it anymore", "nobody ever needed your help", "nothing you did mattered", "everything you tried to do was worthless")].</span>")
				affected_mob.adjustStaminaLoss(45)
				new /obj/effect/temp_visual/revenant(affected_mob.loc)
				if(affected_mob.dna && affected_mob.dna.species)
					affected_mob.dna.species.handle_mutant_bodyparts(affected_mob,"#1d2953")
					affected_mob.dna.species.handle_hair(affected_mob,"#1d2953")
				affected_mob.visible_message("<span_class='warning'>[affected_mob] looks terrifyingly gaunt...</span>", "<span_class='revennotice'>You suddenly feel like your skin is <i>wrong</i>...</span>")
