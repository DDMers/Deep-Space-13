...
/obj/item/overthrow_converter/proc/convert(mob/living/carbon/human/target, mob/living/carbon/human/user) // Should probably also delete any mindshield implant. Not sure.
	if(istype(target) && target.mind && user && user.mind)
		...
		if(!UO)
			to_chat(user, "<span_class='danger'>You don't know how to use this thing!</span>") // It needs a valid team to work, if you aren't an antag don't use this thing
			return FALSE
		if(TO)
			to_chat(user, "<span_class='notice'>[target.name] woke up already, the implant would be ineffective against him!</span>")
			return FALSE
...
/obj/item/overthrow_converter/attack(mob/living/carbon/human/M, mob/living/carbon/human/user)
	if(!istype(M) || !istype(user))
		return
	if(!uses)
		to_chat(user,"<span_class='warning'>The converter is empty!</span>")
		return
	if(M == user)
		to_chat(user,"<span_class='warning'>You cannot convert yourself!</span>")
		return
	if(M.has_trait(TRAIT_MINDSHIELD))
		to_chat(user, "<span_class='danger'>This mind is too strong to convert, try to remove whatever is protecting it first!</span>")
		return
	M.visible_message("<span_class='warning'>[user] is attempting to implant [M].</span>")
	if(do_mob(user, M, 50))
		if(convert(M,user))
			M.visible_message("[user] has implanted [M].", "<span_class='notice'>[user] implants you.</span>")
			uses--
			update_icon()
		else
			to_chat(user, "<span_class='warning'>[user] fails to implant [M].</span>")
