	...
	for(var/obj/effect/proc_holder/spell/aspell in user.mind.spell_list)
		if(initial(S.name) == initial(aspell.name)) // Not using directly in case it was learned from one spellbook then upgraded in another
			if(aspell.spell_level >= aspell.level_max)
				to_chat(user,  "<span_class='warning'>This spell cannot be improved further.</span>")
				return FALSE
			else
				...
				switch(aspell.spell_level)
					if(1)
						to_chat(user, "<span_class='notice'>You have improved [aspell.name] into Efficient [aspell.name].</span>")
						aspell.name = "Efficient [aspell.name]"
					if(2)
						to_chat(user, "<span_class='notice'>You have further improved [aspell.name] into Quickened [aspell.name].</span>")
						aspell.name = "Quickened [aspell.name]"
					if(3)
						to_chat(user, "<span_class='notice'>You have further improved [aspell.name] into Free [aspell.name].</span>")
						aspell.name = "Free [aspell.name]"
					if(4)
						to_chat(user, "<span_class='notice'>You have further improved [aspell.name] into Instant [aspell.name].</span>")
						aspell.name = "Instant [aspell.name]"
				if(aspell.spell_level >= aspell.level_max)
					to_chat(user, "<span_class='notice'>This spell cannot be strengthened any further.</span>")
				SSblackbox.record_feedback("nested tally", "wizard_spell_improved", 1, list("[name]", "[aspell.spell_level]"))
				return TRUE
	//No same spell found - just learn it
	SSblackbox.record_feedback("tally", "wizard_spell_learned", 1, name)
	user.mind.AddSpell(S)
	to_chat(user, "<span_class='notice'>You have learned [S.name].</span>")
	return TRUE
...
/datum/spellbook_entry/proc/Refund(mob/living/carbon/human/user,obj/item/spellbook/book) //return point value or -1 for failure
	...
	if(!(user in A.contents))
		to_chat(user, "<span_class='warning'>You can only refund spells at the wizard lair</span>")
		return -1
	if(!S)
		S = new spell_type()
	var/spell_levels = 0
...
/datum/spellbook_entry/summon/ghosts/Buy(mob/living/carbon/human/user, obj/item/spellbook/book)
	...
	active = TRUE
	to_chat(user, "<span_class='notice'>You have cast summon ghosts!</span>")
...
/datum/spellbook_entry/summon/guns/Buy(mob/living/carbon/human/user,obj/item/spellbook/book)
	...
	to_chat(user, "<span_class='notice'>You have cast summon guns!</span>")
	return TRUE
...
/datum/spellbook_entry/summon/magic/Buy(mob/living/carbon/human/user,obj/item/spellbook/book)
	...
	active = TRUE
	playsound(get_turf(user), 'sound/magic/castsummon.ogg', 50, 1)
	to_chat(user, "<span_class='notice'>You have cast summon magic!</span>")
	return TRUE
...
/datum/spellbook_entry/summon/events/Buy(mob/living/carbon/human/user,obj/item/spellbook/book)
	...
	times++
	playsound(get_turf(user), 'sound/magic/castsummon.ogg', 50, 1)
	to_chat(user, "<span_class='notice'>You have cast summon events.</span>")
	return TRUE
...
/datum/spellbook_entry/summon/curse_of_madness/Buy(mob/living/carbon/human/user, obj/item/spellbook/book)
	...
	to_chat(user, "<span_class='notice'>You have cast the curse of insanity!</span>")
...
/obj/item/spellbook/attackby(obj/item/O, mob/user, params)
	if(istype(O, /obj/item/antag_spawner/contract))
		...
		if(contract.used)
			to_chat(user, "<span_class='warning'>The contract has been used, you can't get your points back now!</span>")
		else
			to_chat(user, "<span_class='notice'>You feed the contract back into the spellbook, refunding your points.</span>")
			uses++
		...
		to_chat(user, "<span_class='notice'>On second thought, maybe summoning a demon is a bad idea. You refund your points.</span>")
		uses++
...
/obj/item/spellbook/attack_self(mob/user)
	if(!owner)
		to_chat(user, "<span_class='notice'>You bind the spellbook to yourself.</span>")
		owner = user
		return
	if(user != owner)
		to_chat(user, "<span_class='warning'>The [name] does not recognize you as its owner and refuses to open!</span>")
		return
