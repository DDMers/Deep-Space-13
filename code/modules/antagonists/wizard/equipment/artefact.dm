...
/obj/item/veilrender/attack_self(mob/user)
	if(charges > 0)
		...
		charges--
		user.visible_message("<span_class='boldannounce'>[src] hums with power as [user] deals a blow to [activate_descriptor] itself!</span>")
	else
		to_chat(user, "<span_class='danger'>The unearthly energies that powered the blade are now dormant.</span>")
...
/obj/effect/rend/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/nullrod))
		user.visible_message("<span_class='danger'>[user] seals \the [src] with \the [I].</span>")
		qdel(src)
		return
	else
		return ..()
...
/obj/singularity/wizard/attack_tk(mob/user)
	if(iscarbon(user))
		...
		to_chat(C, "<span_class='userdanger'>OH GOD! NONE OF IT IS REAL! NONE OF IT IS REEEEEEEEEEEEEEEEEEEEEEEEAL!</span>")
...
/obj/item/scrying/process()
	...
	if(current_owner && current_owner != holder)
		...
		to_chat(current_owner, "<span_class='notice'>Your otherworldly vision fades...</span>")
	...
	if(!current_owner)
		...
		to_chat(current_owner, "<span_class='notice'>You can see...everything!</span>")
...
/obj/item/scrying/attack_self(mob/user)
	visible_message("<span_class='danger'>[user] stares into [src], their eyes glazing over.</span>")
	user.ghostize(1)
...
/obj/item/necromantic_stone/attack(mob/living/carbon/human/M, mob/living/carbon/human/user)
	...
	if(M.stat != DEAD)
		to_chat(user, "<span_class='warning'>This artifact can only affect the dead!</span>")
		return
	...
	if(!M.mind || !M.client)
		to_chat(user, "<span_class='warning'>There is no soul connected to this body...</span>")
		return

	check_spooky()//clean out/refresh the list
	if(spooky_scaries.len >= 3 && !unlimited)
		to_chat(user, "<span_class='warning'>This artifact can only affect three undead at a time!</span>")
		return

	M.set_species(/datum/species/skeleton, icon_update=0)
	M.revive(full_heal = 1, admin_revive = 1)
	spooky_scaries |= M
	to_chat(M, "<span_class='userdanger'>You have been revived by </span><B>[user.real_name]!</B>")
	to_chat(M, "<span_class='userdanger'>[user.p_theyre(TRUE)] your master now, assist [user.p_them()] even if it costs you your new life!</span>")
...
/obj/item/voodoo/attackby(obj/item/I, mob/user, params)
	if(target && cooldown < world.time)
		if(I.is_hot())
			to_chat(target, "<span_class='userdanger'>You suddenly feel very hot</span>")
			target.adjust_bodytemperature(50)
			GiveHint(target)
		else if(is_pointed(I))
			to_chat(target, "<span_class='userdanger'>You feel a stabbing pain in [parse_zone(user.zone_selected)]!</span>")
			target.Paralyze(40)
			GiveHint(target)
		else if(istype(I, /obj/item/bikehorn))
			to_chat(target, "<span_class='userdanger'>HONK</span>")
...
/obj/item/voodoo/attack_self(mob/user)
	...
	if(user.zone_selected == BODY_ZONE_CHEST)
		if(voodoo_link)
			...
			to_chat(user, "<span_class='notice'>You remove the [voodoo_link] from the doll.</span>")
			voodoo_link = null
			update_targets()
			return
	...
	if(target && cooldown < world.time)
		switch(user.zone_selected)
			...
			if(BODY_ZONE_R_LEG,BODY_ZONE_L_LEG)
				to_chat(user, "<span_class='notice'>You move the doll's legs around.</span>")
			...
			if(BODY_ZONE_R_ARM,BODY_ZONE_L_ARM)
				...
				GiveHint(target)
			if(BODY_ZONE_HEAD)
				to_chat(user, "<span_class='notice'>You smack the doll's head with your hand.</span>")
				target.Dizzy(10)
				to_chat(target, "<span_class='warning'>You suddenly feel as if your head was hit with a hammer!</span>")
...
/obj/item/voodoo/proc/GiveHint(mob/victim,force=0)
	if(prob(50) || force)
		...
		to_chat(victim, "<span_class='notice'>You feel a dark presence from [way]</span>")
	if(prob(20) || force)
		var/area/A = get_area(src)
		to_chat(victim, "<span_class='notice'>You feel a dark presence from [A.name]</span>")

/obj/item/voodoo/suicide_act(mob/living/carbon/user)
    user.visible_message("<span_class='suicide'>[user] links the voodoo doll to [user.p_them()]self and sits on it, infinitely crushing [user.p_them()]self! It looks like [user.p_theyre()] trying to commit suicide!</span>")
    user.gib()
    return(BRUTELOSS)
