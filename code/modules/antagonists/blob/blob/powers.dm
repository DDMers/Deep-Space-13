/mob/camera/blob/proc/can_buy(cost = 15)
	if(blob_points < cost)
		to_chat(src, "<span_class='warning'>You cannot afford this, you need at least [cost] resources!</span>")
		return 0
	add_points(-cost)
	return 1
		...
		if(!pop_override)
			for(var/mob/living/M in range(7, src))
				if(ROLE_BLOB in M.faction)
					continue
				if(M.client)
					to_chat(src, "<span_class='warning'>There is someone too close to place your blob core!</span>")
					return 0
			for(var/mob/living/M in view(13, src))
				if(ROLE_BLOB in M.faction)
					continue
				if(M.client)
					to_chat(src, "<span_class='warning'>Someone could see your blob core from here!</span>")
					return 0
		var/turf/T = get_turf(src)
		if(T.density)
			to_chat(src, "<span_class='warning'>This spot is too dense to place a blob core on!</span>")
			return 0
		for(var/obj/O in T)
			if(istype(O, /obj/structure/blob))
				if(istype(O, /obj/structure/blob/normal))
					...
					to_chat(src, "<span_class='warning'>There is already a blob here!</span>")
					return 0
			else if(O.density)
				to_chat(src, "<span_class='warning'>This spot is too dense to place a blob core on!</span>")
				return 0
		if(!pop_override && world.time <= manualplace_min_time && world.time <= autoplace_max_time)
			to_chat(src, "<span_class='warning'>It is too early to place your blob core!</span>")
			return 0
...
/mob/camera/blob/proc/createSpecial(price, blobType, nearEquals, needsNode, turf/T)
	...
	if(!B)
		to_chat(src, "<span_class='warning'>There is no blob here!</span>")
		return
	if(!istype(B, /obj/structure/blob/normal))
		to_chat(src, "<span_class='warning'>Unable to use this blob, find a normal one.</span>")
		return
	if(needsNode && nodes_required)
		if(!(locate(/obj/structure/blob/node) in orange(3, T)) && !(locate(/obj/structure/blob/core) in orange(4, T)))
			to_chat(src, "<span_class='warning'>You need to place this blob closer to a node or core!</span>")
			return //handholdotron 2000
	if(nearEquals)
		for(var/obj/structure/blob/L in orange(nearEquals, T))
			if(L.type == blobType)
				to_chat(src, "<span_class='warning'>There is a similar blob nearby, move more than [nearEquals] tiles away from it!</span>")
				return
	if(!can_buy(price))
		return
...
/mob/camera/blob/verb/toggle_node_req()
	...
	if(nodes_required)
		to_chat(src, "<span_class='warning'>You now require a nearby node or core to place factory and resource blobs.</span>")
	else
		to_chat(src, "<span_class='warning'>You no longer require a nearby node or core to place factory and resource blobs.</span>")
...
/mob/camera/blob/proc/create_shield(turf/T)
	...
	if(S)
		...
		if(S.obj_integrity < S.max_integrity * 0.5)
			...
			to_chat(src, "<span_class='warning'>This shield blob is too damaged to be modified properly!</span>")
			return
		to_chat(src, "<span_class='warning'>You secrete a reflective ooze over the shield blob, allowing it to reflect projectiles at the cost of reduced integrity.</span>")
...
/mob/camera/blob/verb/create_blobbernaut()
	...
	if(!B)
		to_chat(src, "<span_class='warning'>You must be on a factory blob!</span>")
		return
	if(B.naut) //if it already made a blobbernaut, it can't do it again
		to_chat(src, "<span_class='warning'>This factory blob is already sustaining a blobbernaut.</span>")
		return
	if(B.obj_integrity < B.max_integrity * 0.5)
		to_chat(src, "<span_class='warning'>This factory blob is too damaged to sustain a blobbernaut.</span>")
		return
	if(!can_buy(40))
		return
	...
	if(LAZYLEN(candidates)) //if we got at least one candidate, they're a blobbernaut now.
		...
		B.update_icon()
		B.visible_message("<span_class='warning'><b>The blobbernaut [pick("rips", "tears", "shreds")] its way out of the factory blob!</b></span>")
		...
		to_chat(blobber, "You are powerful, hard to kill, and slowly regenerate near nodes and cores, <span_class='cultlarge'>but will slowly die if not near the blob</span> or if the factory that made you is killed.")
		...
		to_chat(src, "<span_class='warning'>You could not conjure a sentience for your blobbernaut. Your points have been refunded. Try again later.</span>")
		add_points(40)
...
/mob/camera/blob/verb/relocate_core()
	...
	if(!B)
		to_chat(src, "<span_class='warning'>You must be on a blob node!</span>")
		return
	if(!blob_core)
		to_chat(src, "<span_class='userdanger'>You have no core and are about to die! May you rest in peace.</span>")
		return
	var/area/A = get_area(T)
	if(isspaceturf(T) || A && !A.blob_allowed)
		to_chat(src, "<span_class='warning'>You cannot relocate your core here!</span>")
		return
	if(!can_buy(80))
		return
...
/mob/camera/blob/proc/remove_blob(turf/T)
	...
	if(!B)
		to_chat(src, "<span_class='warning'>There is no blob there!</span>")
		return
	if(B.point_return < 0)
		to_chat(src, "<span_class='warning'>Unable to remove this blob.</span>")
		return
	if(max_blob_points < B.point_return + blob_points)
		to_chat(src, "<span_class='warning'>You have too many resources to remove this blob!</span>")
		return
	if(B.point_return)
		add_points(B.point_return)
		to_chat(src, "<span_class='notice'>Gained [B.point_return] resources from removing \the [B].</span>")
	qdel(B)
...
/mob/camera/blob/proc/expand_blob(turf/T)
	...
	if(!possibleblobs.len)
		to_chat(src, "<span_class='warning'>There is no blob adjacent to the target tile!</span>")
		return
	if(can_buy(BLOB_SPREAD_COST))
		...
		if(B)
			if(attacksuccess) //if we successfully attacked a turf with a blob on it, only give an attack refund
				...
				to_chat(src, "<span_class='warning'>There is a blob there!</span>")
...
/mob/camera/blob/verb/blob_help()
	...
	if(!placed && autoplace_max_time <= world.time)
		to_chat(src, "<span_class='big'><font color=\"#EE4000\">You will automatically place your blob core in [DisplayTimeText(autoplace_max_time - world.time)].</font></span>")
		to_chat(src, "<span_class='big'><font color=\"#EE4000\">You [manualplace_min_time ? "will be able to":"can"] manually place your blob core by pressing the Place Blob Core button in the bottom right corner of the screen.</font></span>")
