				...
				if(mod_mode)
					if(authorized())
						...
						if(!printer.print_text(contents,"access report"))
							to_chat(usr, "<span_class='notice'>Hardware error: Printer was unable to print the file. It may be out of paper.</span>")
							return
						else
							computer.visible_message("<span_class='notice'>\The [computer] prints out paper.</span>")
				else
			...
			if(computer && authorized())
				if(params["name"])
					...
					if(temp_name)
						...
						computer.visible_message("<span_class='notice'>[computer] buzzes rudely.</span>")
					...
					if(is_centcom)
						...
						for(var/jobtype in typesof(/datum/job))
							...
							if(ckey(J.title) == ckey(t1))
								...
								break
						if(!jobdatum)
							to_chat(usr, "<span_class='warning'>No log exists for this job: [t1]</span>")
							return
