...
/datum/computer_file/program/proc/is_supported_by_hardware(hardware_flag = 0, loud = 0, mob/user = null)
	if(!(hardware_flag & usage_flags))
		if(loud && computer && user)
			to_chat(user, "<span_class='danger'>\The [computer] flashes an \"Hardware Error - Incompatible software\" warning.</span>")
		return 0
	return 1
...
/datum/computer_file/program/proc/can_run(mob/user, loud = 0, access_to_check, transfer = 0)
	...
	if(ishuman(user))
		...
		if(!I && !D)
			if(loud)
				to_chat(user, "<span_class='danger'>\The [computer] flashes an \"RFID Error - Unable to scan ID\" warning.</span>")
			return 0
		...
		if(I)
			...
			if(access_to_check in D.GetAccess())
				return 1
		if(loud)
			to_chat(user, "<span_class='danger'>\The [computer] flashes an \"Access Denied\" warning.</span>")
	return 0
