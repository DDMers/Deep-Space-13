/obj/item/modular_computer/proc/can_install_component(obj/item/computer_hardware/H, mob/living/user = null)
	...
	if(H.w_class > max_hardware_size)
		to_chat(user, "<span_class='warning'>This component is too large for \the [src]!</span>")
		return FALSE

	if(all_components[H.device_type])
		to_chat(user, "<span_class='warning'>This computer's hardware slot is already occupied by \the [all_components[H.device_type]].</span>")
		return FALSE
	return TRUE
...
/obj/item/modular_computer/proc/install_component(obj/item/computer_hardware/H, mob/living/user = null)
	...
	to_chat(user, "<span_class='notice'>You install \the [H] into \the [src].</span>")
	H.holder = src
	H.forceMove(src)
...
/obj/item/modular_computer/proc/uninstall_component(obj/item/computer_hardware/H, mob/living/user = null)
	...
	to_chat(user, "<span_class='notice'>You remove \the [H] from \the [src].</span>")
