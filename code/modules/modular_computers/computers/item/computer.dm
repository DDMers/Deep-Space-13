...
/obj/item/modular_computer/emag_act(mob/user)
	if(obj_flags & EMAGGED)
		to_chat(user, "<span_class='warning'>\The [src] was already emagged.</span>")
		return 0
	else
		obj_flags |= EMAGGED
		to_chat(user, "<span_class='notice'>You emag \the [src]. It's screen briefly shows a \"OVERRIDE ACCEPTED: New software downloads available.\" message.</span>")
		return 1

/obj/item/modular_computer/examine(mob/user)
	..()
	if(obj_integrity <= integrity_failure)
		to_chat(user, "<span_class='danger'>It is heavily damaged!</span>")
	else if(obj_integrity < max_integrity)
		to_chat(user, "<span_class='warning'>It is damaged.</span>")
...
/obj/item/modular_computer/proc/turn_on(mob/user)
	...
	if(obj_integrity <= integrity_failure)
		if(issynth)
			to_chat(user, "<span_class='warning'>You send an activation signal to \the [src], but it responds with an error code. It must be damaged.</span>")
		else
			to_chat(user, "<span_class='warning'>You press the power button, but the computer fails to boot up, displaying variety of errors before shutting down again.</span>")
		return
	...
	if(all_components[MC_CPU] && use_power()) // use_power() checks if the PC is powered
		if(issynth)
			to_chat(user, "<span_class='notice'>You send an activation signal to \the [src], turning it on.</span>")
		else
			to_chat(user, "<span_class='notice'>You press the power button and start up \the [src].</span>")
		enabled = 1
		update_icon()
		ui_interact(user)
	else // Unpowered
		if(issynth)
			to_chat(user, "<span_class='warning'>You send an activation signal to \the [src] but it does not respond.</span>")
		else
			to_chat(user, "<span_class='warning'>You press the power button but \the [src] does not respond.</span>")
...
/obj/item/modular_computer/proc/shutdown_computer(loud = 1)
	...
	for(var/datum/computer_file/program/P in idle_threads)
		...
		idle_threads.Remove(P)
	if(loud)
		physical.visible_message("<span_class='notice'>\The [src] shuts down.</span>")
	enabled = 0
	update_icon()
...
/obj/item/modular_computer/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(W.tool_behaviour == TOOL_WRENCH)
		if(all_components.len)
			to_chat(user, "<span_class='warning'>Remove all components from \the [src] before disassembling it.</span>")
			return
	...
	if(W.tool_behaviour == TOOL_WELDER)
		if(obj_integrity == max_integrity)
			to_chat(user, "<span_class='warning'>\The [src] does not require repairs.</span>")
			return

		if(!W.tool_start_check(user, amount=1))
			return

		to_chat(user, "<span_class='notice'>You begin repairing damage to \the [src]...</span>")
	...
	if(W.tool_behaviour == TOOL_SCREWDRIVER)
		if(!all_components.len)
			to_chat(user, "<span_class='warning'>This device doesn't have any components installed.</span>")
			return
