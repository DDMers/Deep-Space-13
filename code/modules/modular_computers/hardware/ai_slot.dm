...
/obj/item/computer_hardware/ai_slot/try_insert(obj/item/I, mob/living/user = null)
	...
	if(stored_card)
		to_chat(user, "<span_class='warning'>You try to insert \the [I] into \the [src], but the slot is occupied.</span>")
		return FALSE
...
/obj/item/computer_hardware/ai_slot/try_eject(slot=0,mob/living/user = null,forced = 0)
	if(!stored_card)
		to_chat(user, "<span_class='warning'>There is no card in \the [src].</span>")
		return FALSE

	if(locked && !forced)
		to_chat(user, "<span_class='warning'>Safeties prevent you from removing the card until reconstruction is complete...</span>")
		return FALSE

	if(stored_card)
		to_chat(user, "<span_class='notice'>You remove [stored_card] from [src].</span>")
		locked = FALSE
		if(user)
...
/obj/item/computer_hardware/ai_slot/attackby(obj/item/I, mob/living/user)
	...
	if(I.tool_behaviour == TOOL_SCREWDRIVER)
		to_chat(user, "<span_class='notice'>You press down on the manual eject button with \the [I].</span>")
		try_eject(,user,1)
		return