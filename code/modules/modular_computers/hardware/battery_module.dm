...
/obj/item/computer_hardware/battery/try_insert(obj/item/I, mob/living/user = null)
	...
	if(battery)
		to_chat(user, "<span_class='warning'>You try to connect \the [I] to \the [src], but its connectors are occupied.</span>")
		return FALSE

	if(I.w_class > holder.max_hardware_size)
		to_chat(user, "<span_class='warning'>This power cell is too large for \the [holder]!</span>")
		return FALSE
	...
	battery = I
	to_chat(user, "<span_class='notice'>You connect \the [I] to \the [src].</span>")
...
/obj/item/computer_hardware/battery/try_eject(slot=0, mob/living/user = null, forced = 0)
	if(!battery)
		to_chat(user, "<span_class='warning'>There is no power cell connected to \the [src].</span>")
		return FALSE
	else
		if(user)
		...
		to_chat(user, "<span_class='notice'>You detach \the [battery] from \the [src].</span>")
		battery = null
