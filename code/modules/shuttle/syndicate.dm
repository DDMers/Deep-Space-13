...
/obj/machinery/computer/shuttle/syndicate/Topic(href, href_list)
	if(href_list["move"])
		...
		if(board.challenge && world.time < SYNDICATE_CHALLENGE_TIMER)
			to_chat(usr, "<span_class='warning'>You've issued a combat challenge to the station! You've got to give them at least [DisplayTimeText(SYNDICATE_CHALLENGE_TIMER - world.time)] more to allow them to prepare.</span>")
			return 0
		board.moved = TRUE
	..()
...
/obj/machinery/computer/shuttle/syndicate/drop_pod/Topic(href, href_list)
	if(href_list["move"])
		if(!is_centcom_level(z))
			to_chat(usr, "<span_class='warning'>Pods are one way!</span>")
			return 0
	..()
