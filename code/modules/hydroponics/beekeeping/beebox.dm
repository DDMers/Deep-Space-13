...
/obj/structure/beebox/examine(mob/user)
	...
	if(!queen_bee)
		to_chat(user, "<span_class='warning'>There is no queen bee! There won't bee any honeycomb without a queen!</span>")

	var/half_bee = get_max_bees()*0.5
	if(half_bee && (bees.len >= half_bee))
		to_chat(user, "<span_class='notice'>This place is aBUZZ with activity... there are lots of bees!</span>")

	to_chat(user, "<span_class='notice'>[bee_resources]/100 resource supply.</span>")
	to_chat(user, "<span_class='notice'>[bee_resources]% towards a new honeycomb.</span>")
	to_chat(user, "<span_class='notice'>[bee_resources*2]% towards a new bee.</span>")

	if(honeycombs.len)
		var/plural = honeycombs.len > 1
		to_chat(user, "<span_class='notice'>There [plural? "are" : "is"] [honeycombs.len] uncollected honeycomb[plural ? "s":""] in the apiary.</span>")

	if(honeycombs.len >= get_max_honeycomb())
		to_chat(user, "<span_class='warning'>There's no room for more honeycomb!</span>")
...
/obj/structure/beebox/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/honey_frame))
		...
		if(honey_frames.len < BEEBOX_MAX_FRAMES)
			visible_message("<span_class='notice'>[user] adds a frame to the apiary.</span>")
			if(!user.transferItemToLoc(HF, src))
				return
			honey_frames += HF
		else
			to_chat(user, "<span_class='warning'>There's no room for any more frames in the apiary!</span>")
		return
	...
	if(istype(I, /obj/item/queen_bee))
		if(queen_bee)
			to_chat(user, "<span_class='warning'>This hive already has a queen!</span>")
			return
		...
		if(queen_bee)
			visible_message("<span_class='notice'>[user] sets [qb] down inside the apiary, making it their new home.</span>")
			var/relocated = 0
			for(var/b in bees)
				...
				if(B.reagent_incompatible(queen_bee))
					...
					relocated++
			if(relocated)
				to_chat(user, "<span_class='warning'>This queen has a different reagent to some of the bees who live here, those bees will not return to this apiary!</span>")

		else
			to_chat(user, "<span_class='warning'>The queen bee disappeared! Disappearing bees have been in the news lately...</span>")
...
/obj/structure/beebox/interact(mob/user)
	...
	if(!user.bee_friendly())
		...
		for(var/b in bees) //everyone who's ever lived here now instantly hates you, suck it assistant!
			...
			bees = TRUE
		if(bees)
			visible_message("<span_class='danger'>[user] disturbs the bees!</span>")
		else
			visible_message("<span_class='danger'>[user] disturbs the [name] to no effect!</span>")
	else
		...
		switch(option)
			if("Remove a Honey Frame")
				if(!honey_frames.len)
					to_chat(user, "<span_class='warning'>There are no honey frames to remove!</span>")
					return
				...
				if(HF)
					...
					visible_message("<span_class='notice'>[user] removes a frame from the apiary.</span>")
					...
					if(fallen)
						...
						visible_message("<span_class='notice'>[user] scrapes [multiple ? "[fallen]" : "a"] honeycomb[multiple ? "s" : ""] off of the frame.</span>")

			if("Remove the Queen Bee")
				if(!queen_bee || queen_bee.loc != src)
					to_chat(user, "<span_class='warning'>There is no queen bee to remove!</span>")
					return
				...
				visible_message("<span_class='notice'>[user] removes the queen from the apiary.</span>")
				queen_bee = null
