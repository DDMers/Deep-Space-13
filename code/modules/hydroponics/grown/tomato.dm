...
/obj/item/reagent_containers/food/snacks/grown/tomato/killer/attack(mob/M, mob/user, def_zone)
	if(awakening)
		to_chat(user, "<span_class='warning'>The tomato is twitching and shaking, preventing you from eating it.</span>")
		return
	..()
...
/obj/item/reagent_containers/food/snacks/grown/tomato/killer/attack_self(mob/user)
	if(awakening || isspaceturf(user.loc))
		return
	to_chat(user, "<span_class='notice'>You begin to awaken the Killer Tomato...</span>")
	awakening = 1
	...
	spawn(30)
		if(!QDELETED(src))
			...
			K.visible_message("<span_class='notice'>The Killer Tomato growls as it suddenly awakens.</span>")
			qdel(src)
