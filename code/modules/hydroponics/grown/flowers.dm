...
/obj/item/grown/novaflower/attack(mob/living/carbon/M, mob/user)
	if(!..())
		return
	if(isliving(M))
		to_chat(M, "<span_class='danger'>You are lit on fire from the intense heat of the [name]!</span>")
...
/obj/item/grown/novaflower/afterattack(atom/A as mob|obj, mob/user,proximity)
	...
	if(force > 0)
		...
		to_chat(usr, "<span_class='warning'>All the petals have fallen off the [name] from violent whacking!</span>")
		qdel(src)

/obj/item/grown/novaflower/pickup(mob/living/carbon/human/user)
	..()
	if(!user.gloves)
		to_chat(user, "<span_class='danger'>The [name] burns your bare hand!</span>")
