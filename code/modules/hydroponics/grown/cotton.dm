...
/obj/item/grown/cotton/attack_self(mob/user)
	user.show_message("<span_class='notice'>You pull some [cotton_name] out of the [name]!</span>", 1)
	var/seed_modifier = 0
	if(seed)
	...
	if(cotton.amount > old_cotton_amount)
		to_chat(user, "<span_class='notice'>You add the newly-formed [cotton_name] to the stack. It now contains [cotton.amount] [cotton_name].</span>")
	qdel(src)
