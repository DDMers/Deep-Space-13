...
/obj/item/grown/corncob/attackby(obj/item/grown/W, mob/user, params)
	if(W.is_sharp())
		to_chat(user, "<span_class='notice'>You use [W] to fashion a pipe out of the corn cob!</span>")
...
/obj/item/grown/snapcorn/attack_self(mob/user)
	..()
	to_chat(user, "<span_class='notice'>You pick a snap pop from the cob.</span>")
