...
/obj/item/grown/log/attackby(obj/item/W, mob/user, params)
	if(W.sharpness)
		user.show_message("<span_class='notice'>You make [plank_name] out of \the [src]!</span>", 1)
		var/seed_modifier = 0
		if(seed)
		...
		if(plank.amount > old_plank_amount)
			to_chat(user, "<span_class='notice'>You add the newly-formed [plank_name] to the stack. It now contains [plank.amount] [plank_name].</span>")
		qdel(src)

	if(CheckAccepted(W))
		var/obj/item/reagent_containers/food/snacks/grown/leaf = W
		if(leaf.dry)
			user.show_message("<span_class='notice'>You wrap \the [W] around the log, turning it into a torch!</span>")
			...
			to_chat(usr, "<span_class ='warning'>You must dry this first!</span>")
	else
		return ..()
	...
	if(istype(W, /obj/item/stack/rods) && !can_buckle && !grill)
		...
		switch(choice)
			if("Stake")
				...
				to_chat(user, "<span_class='italics'>You add a rod to \the [src].")
			...
			if("Grill")
				R.use(1)
				grill = TRUE
				to_chat(user, "<span_class='italics'>You add a grill to \the [src].")
...
/obj/structure/bonfire/attack_hand(mob/user)
	...
	if(.)
		return
	if(burning)
		to_chat(user, "<span_class='warning'>You need to extinguish [src] before removing the logs!</span>")
		return
