...
/obj/item/seeds/watermelon/suicide_act(mob/user)
	user.visible_message("<span_class='suicide'>[user] is swallowing [src]! It looks like [user.p_theyre()] trying to commit suicide!</span>")
	user.gib()
...
/obj/item/reagent_containers/food/snacks/grown/holymelon/proc/block_magic(mob/user, major)
	if(major)
		to_chat(user, "<span_class='warning'>[src] hums slightly, and seems to decay a bit.</span>")

/obj/item/reagent_containers/food/snacks/grown/holymelon/proc/expire(mob/user)
	to_chat(user, "<span_class='warning'>[src] rapidly turns into ash!</span>")
	qdel(src)
