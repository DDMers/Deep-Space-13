...
/obj/item/seeds/kudzu/suicide_act(mob/user)
	user.visible_message("<span_class='suicide'>[user] swallows the pack of kudzu seeds! It looks like [user.p_theyre()] trying to commit suicide!</span>")
	plant(user)
	return (BRUTELOSS)
...
/obj/item/seeds/kudzu/proc/plant(mob/user)
	...
	if(locate(/obj/structure/spacevine) in user.loc)
		to_chat(user, "<span_class='warning'>There is too much kudzu here to plant [src].</span>")
		return FALSE
...
/obj/item/seeds/kudzu/attack_self(mob/user)
	user.visible_message("<span_class='danger'>[user] begins throwing seeds on the ground...</span>")
	if(do_after(user, 50, needhand = TRUE, target = user.drop_location(), progress = TRUE))
		plant(user)
		to_chat(user, "<span_class='notice'>You plant the kudzu. You monster.</span>")
