...
/obj/machinery/biogenerator/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		to_chat(user, "<span_class='notice'>The status display reads: Productivity at <b>[productivity*100]%</b>.<br>Matter consumption reduced by <b>[(efficiency*25)-25]</b>%.<br>Machine can hold up to <b>[max_items]</b> pieces of produce.<span>")
...
/obj/machinery/biogenerator/attackby(obj/item/O, mob/user, params)
	...
	if(processing)
		to_chat(user, "<span_class='warning'>The biogenerator is currently processing.</span>")
		return
	...
	if(istype(O, /obj/item/reagent_containers/glass))
		...
		if(!panel_open)
			if(beaker)
				to_chat(user, "<span_class='warning'>A container is already loaded into the machine.</span>")
			else
				if(!user.transferItemToLoc(O, src))
					return
				beaker = O
				to_chat(user, "<span_class='notice'>You add the container to the machine.</span>")
				update_icon()
				updateUsrDialog()
		else
		...
		for(var/obj/item/reagent_containers/food/snacks/grown/G in contents)
			i++
		if(i >= max_items)
			to_chat(user, "<span_class='warning'>The biogenerator is already full! Activate it.</span>")
		else
			for(var/obj/item/reagent_containers/food/snacks/grown/G in PB.contents)
				...
				if(SEND_SIGNAL(PB, COMSIG_TRY_STORAGE_TAKE, G, src))
					i++
			if(i<max_items)
				to_chat(user, "<span_class='info'>You empty the plant bag into the biogenerator.</span>")
			else if(PB.contents.len == 0)
				to_chat(user, "<span_class='info'>You empty the plant bag into the biogenerator, filling it to its capacity.</span>")
			else
				to_chat(user, "<span_class='info'>You fill the biogenerator to its capacity.</span>")
		...
		for(var/obj/item/reagent_containers/food/snacks/grown/G in contents)
			i++
		if(i >= max_items)
			to_chat(user, "<span_class='warning'>The biogenerator is full! Activate it.</span>")
		else
			if(user.transferItemToLoc(O, src))
				to_chat(user, "<span_class='info'>You put [O.name] in [src.name]</span>")
		return TRUE //no afterattack
		...
		to_chat(user, "<span_class='warning'>You cannot put this in [src.name]!</span>")
...
/obj/machinery/biogenerator/proc/activate()
	...
	if (src.stat != NONE) //NOPOWER etc
		return
	if(processing)
		to_chat(usr, "<span_class='warning'>The biogenerator is in the process of working.</span>")
		return
	var/S = 0
