...
/obj/effect/sound_emitter/examine(mob/user)
	...
	if(!isobserver(user))
		return
	to_chat(user, "<span_class='boldnotice'>Sound File:</span> [sound_file ? sound_file : "None chosen"]")
	to_chat(user, "<span_class='boldnotice'>Mode:</span> [motus_operandi]</span>")
	to_chat(user, "<span_class='boldnotice'>Range:</span> [emitter_range]</span>")
...
/obj/effect/sound_emitter/AltClick(mob/user)
	if(check_rights_for(user.client, R_SOUND))
		activate(user)
		to_chat(user, "<span_class='notice'>Sound emitter activated.</span>")
...
/obj/effect/sound_emitter/Topic(href, href_list)
	...
	if(href_list["edit_label"])
		...
		if(!new_label)
			return
		maptext = new_label
		to_chat(user, "<span_class='notice'>Label set to [maptext].</span>")
	if(href_list["edit_sound_file"])
		var/new_file = input(user, "Choose a sound file.", "Sound Emitter") as null|sound
		if(!new_file)
			return
		sound_file = new_file
		to_chat(user, "<span_class='notice'>New sound file set to [sound_file].</span>")
	if(href_list["edit_volume"])
		...
		to_chat(user, "<span_class='notice'>Volume set to [sound_volume]%.</span>")
	if(href_list["edit_mode"])
		...
		to_chat(user, "<span_class='notice'>Mode set to [motus_operandi].</span>")
	if(href_list["edit_range"])
		...
		to_chat(user, "<span_class='notice'>Range set to [emitter_range].</span>")
	if(href_list["edit_radius"])
		...
		to_chat(user, "<span_class='notice'>Audible radius set to [play_radius].</span>")
	if(href_list["play"])
		activate(user)
