...
/obj/effect/fun_balloon/scatter/effect()
	for(var/mob/living/M in range(effect_range, get_turf(src)))
		...
		M.forceMove(T)
		to_chat(M, "<span_class='notice'>Pop!</span>")
...
/obj/effect/forcefield/arena_shuttle/Bumped(atom/movable/AM)
	...
	if(L.pulling && istype(L.pulling, /obj/item/bodypart/head))
		...
		L.forceMove(LA)
		L.hallucination = 0
		to_chat(L, "<span_class='reallybig redtext'>The battle is won. Your bloodlust subsides.</span>")
...
/obj/effect/forcefield/arena_shuttle_entrance/Bumped(atom/movable/AM)
	...
	to_chat(M, "<span_class='reallybig redtext'>You're trapped in a deadly arena! To escape, you'll need to drag a severed head to the escape portals.</span>")
	spawn()
