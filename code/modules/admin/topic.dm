...
/datum/admins/Topic(href, href_list)
	...
	if(href_list["ahelp"])
		...
		if(!M)
			to_chat(usr, "<span_class='danger'>ERROR: Mob not found.</span>")
			return
		...
		if(!C)
			to_chat(usr, "<span_class='danger'>ERROR: Client not found.</span>")
			return
		...
		if(!check_rights(R_ADMIN))
			return
		if (!SSticker.mode)
			to_chat(usr, "<span_class='danger'>Not until the round starts!</span>")
			return
	...
	if(href_list["editrightsbrowsermanage"])
		...
		switch(href_list["call_shuttle"])
			if("1")
				...
				message_admins("<span_class='adminnotice'>[key_name_admin(usr)] called the Emergency Shuttle to the station.</span>")
			...
			if("2")
				...
				switch(SSshuttle.emergency.mode)
					if(SHUTTLE_CALL)
						...
						message_admins("<span_class='adminnotice'>[key_name_admin(usr)] sent the Emergency Shuttle back.</span>")
					else
						SSshuttle.emergency.cancel()
						log_admin("[key_name(usr)] called the Emergency Shuttle.")
						message_admins("<span_class='adminnotice'>[key_name_admin(usr)] called the Emergency Shuttle to the station.</span>")
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] edited the Emergency Shuttle's timeleft to [timer] seconds.</span>")
		href_list["secrets"] = "check_antagonist"
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] toggled the round to [continuous[SSticker.mode.config_tag] ? "continue if all antagonists die" : "end with the antagonists"].</span>")
		check_antagonists()
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] toggled the round to [midround_antag[SSticker.mode.config_tag] ? "use" : "skip"] the midround antag system.</span>")
		check_antagonists()
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] edited the maximum midround antagonist time to [timer] minutes.</span>")
		check_antagonists()
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] edited the midround antagonist living crew ratio to [ratio]% alive.</span>")
		check_antagonists()
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] edited the midround antagonist system to [SSticker.mode.round_ends_with_antag_death ? "end the round" : "continue as extended"] upon failure.</span>")
		check_antagonists()
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] is considering ending the round.</span>")
		if(alert(usr, "This will end the round, are you SURE you want to do this?", "Confirmation", "Yes", "No") == "Yes")
			if(alert(usr, "Final Confirmation: End the round NOW?", "Confirmation", "Yes", "No") == "Yes")
				message_admins("<span_class='adminnotice'>[key_name_admin(usr)] has ended the round.</span>")
				SSticker.force_ending = 1 //Yeah there we go APC destroyed mission accomplished
				return
			else
				message_admins("<span_class='adminnotice'>[key_name_admin(usr)] decided against ending the round.</span>")
		else
			message_admins("<span_class='adminnotice'>[key_name_admin(usr)] decided against ending the round.</span>")
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] has used rudimentary transformation on [key_name_admin(M)]. Transforming to [href_list["simplemake"]].; deletemob=[delmob]</span>")
		...
		if(ismob(M))
			if(!check_if_greater_rights_than(M.client))
				to_chat(usr, "<span_class='danger'>Error: They have more rights than you do.</span>")
				return
			if(alert(usr, "Kick [key_name(M)]?", "Confirm", "Yes", "No") != "Yes")
				return
			if(!M)
				to_chat(usr, "<span_class='danger'>Error: [M] no longer exists!</span>")
				return
			if(!M.client)
				to_chat(usr, "<span_class='danger'>Error: [M] no longer has a client!</span>")
				return
			to_chat(M, "<span_class='danger'>You have been kicked from the server by [usr.client.holder.fakekey ? "an Administrator" : "[usr.client.key]"].</span>")
			log_admin("[key_name(usr)] kicked [key_name(M)].")
			message_admins("<span_class='adminnotice'>[key_name_admin(usr)] kicked [key_name_admin(M)].</span>")
			qdel(M.client)
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] set the mode as [GLOB.master_mode].</span>")
		to_chat(world, "<span_class='adminnotice'><b>The mode is now: [GLOB.master_mode]</b></span>")
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] set the forced secret mode as [GLOB.secret_force_mode].</span>")
		Game() // updates the main game menu
		HandleFSecret()
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] attempting to monkeyize [key_name_admin(H)].</span>")
		H.monkeyize()
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] attempting to humanize [key_name_admin(Mo)].</span>")
		Mo.humanize()
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] attempting to corgize [key_name_admin(H)].</span>")
		H.corgize()
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] forced [key_name_admin(M)] to say: [speech]</span>")
		...
		to_chat(M, "<span_class='adminnotice'>You have been sent to Prison!</span>")
		...
		if(!isobserver(M))
			to_chat(usr, "<span_class='notice'>You can only send ghost players back to the Lobby.</span>")
			return

		if(!M.client)
			to_chat(usr, "<span_class='warning'>[M] doesn't seem to have an active client.</span>")
			return
		...
		spawn(50)
			to_chat(L, "<span_class='adminnotice'>You have been sent to the Thunderdome.</span>")
		...
		message_admins("<span_class='danger'>Admin [key_name_admin(usr)] healed / revived [key_name_admin(L)]!</span>")
		...
		message_admins("<span_class='danger'>Admin [key_name_admin(usr)] AIized [key_name_admin(H)]!</span>")
		...
		to_chat(H, "<span_class='adminnotice'>Your prayers have been answered!! You received the <b>best cookie</b>!</span>")
		...
		if(SSticker.IsRoundInProgress())
			...
			if(alert("Are you sure you want to kick all [afkonly ? "AFK" : ""] clients from the lobby??","Message","Yes","Cancel") != "Yes")
				...
				return
			var/list/listkicked = kick_clients_in_lobby("<span_class='danger'>You were kicked from the lobby by [usr.client.holder.fakekey ? "an Administrator" : "[usr.client.key]"].</span>", afkonly)
		...
		if(!istype(error_viewer))
			to_chat(usr, "<span_class='warning'>That runtime viewer no longer exists.</span>")
			return
