/proc/create_message(type, target_key, admin_ckey, text, timestamp, server, secret, logged = 1, browse, expiry, note_severity)
	...
	if(logged)
		...
		admin_ticket_log(target_ckey, "<span class='notice'>[header]</span>")
	...
	while(query_get_message_output.NextRow())
		...
		switch(type)
			if("message")
				...
				qdel(query_message_read)
			if("watchlist entry")
				message_admins("<font color='red'><B>Notice: </B></font><span class='notice'>[key_name_admin(target_ckey)] has been on the watchlist since [timestamp] and has just connected - Reason: [text]</span>")
