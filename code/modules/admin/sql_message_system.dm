/proc/create_message(type, target_key, admin_ckey, text, timestamp, server, secret, logged = 1, browse, expiry, note_severity)
	...
	if(logged)
		...
		admin_ticket_log(target_ckey, "<span_class='notice'>[header]</span>")
...
/proc/get_message_output(type, target_ckey)
	...
	while(query_get_message_output.NextRow())
		...
		switch(type)
			if("message")
				output += "<font color='red' size='3'><b>Admin message left by <span_class='prefix'>[admin_key]</span> on [timestamp]</b></font>"
				...
				qdel(query_message_read)
			if("watchlist entry")
				message_admins("<font color='red'><B>Notice: </B></font><span_class='notice'>[key_name_admin(target_ckey)] has been on the watchlist since [timestamp] and has just connected - Reason: [text]</span>")
				send2irc_adminless_only("Watchlist", "[key_name(target_ckey)] is on the watchlist and has just connected - Reason: [text]")
			if("memo")
				output += "<span_class='memo'>Memo by <span_class='prefix'>[admin_key]</span> on [timestamp]"
				if(editor_key)
					output += "<br><span_class='memoedit'>Last edit by [editor_key] <A href='?_src_=holder;[HrefToken()];messageedits=[message_id]'>(Click here to see edit log)</A></span>"
