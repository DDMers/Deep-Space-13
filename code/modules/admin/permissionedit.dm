...
/datum/admins/proc/edit_rights_topic(list/href_list)
	...
	if(IsAdminAdvancedProcCall())
		to_chat(usr, "<span_class='admin prefix'>Admin Edit blocked: Advanced ProcCall detected.</span>")
		return
	...
	if(!CONFIG_GET(flag/admin_legacy_system) && CONFIG_GET(flag/protect_legacy_admins) && task == "rank")
		if(admin_ckey in GLOB.protected_admins)
			to_chat(usr, "<span_class='admin prefix'>Editing the rank of this admin is blocked by server configuration.</span>")
			return
	if(!CONFIG_GET(flag/admin_legacy_system) && CONFIG_GET(flag/protect_legacy_ranks) && task == "permissions")
		if(D.rank in GLOB.protected_ranks)
			to_chat(usr, "<span_class='admin prefix'>Editing the flags of this rank is blocked by server configuration.</span>")
			return
	if(CONFIG_GET(flag/load_legacy_ranks_only) && (task == "add" || task == "rank" || task == "permissions"))
		to_chat(usr, "<span_class='admin prefix'>Database rank loading is disabled, only temporary changes can be made to a rank's permissions and permanently creating a new rank is blocked.</span>")
		legacy_only = TRUE
	if(check_rights(R_DBRANKS, FALSE))
		if(!skip)
			if(!SSdbcore.Connect())
				to_chat(usr, "<span_class='danger'>Unable to connect to database, changes are temporary only.</span>")
				use_db = FALSE
			else
...
/datum/admins/proc/add_admin(admin_ckey, admin_key, use_db)
	...
	if(!admin_ckey && (. in GLOB.admin_datums+GLOB.deadmins))
		to_chat(usr, "<span_class='danger'>[admin_key] is already an admin.</span>")
		return FALSE
	if(use_db)
		. = sanitizeSQL(.)
		...
		if(query_admin_in_db.NextRow())
			qdel(query_admin_in_db)
			to_chat(usr, "<span_class='danger'>[admin_key] already listed in admin database. Check the Management tab if they don't appear in the list of admins.</span>")
			return FALSE
		qdel(query_admin_in_db)
...
/datum/admins/proc/remove_rank(admin_rank)
	...
	for(var/datum/admin_rank/R in GLOB.admin_ranks)
		if(R.name == admin_rank && (!(R.rights & usr.client.holder.rank.can_edit_rights) == R.rights))
			to_chat(usr, "<span_class='admin prefix'>You don't have edit rights to all the rights this rank has, rank deletion not permitted.</span>")
			return
	if(!CONFIG_GET(flag/admin_legacy_system) && CONFIG_GET(flag/protect_legacy_ranks) && (admin_rank in GLOB.protected_ranks))
		to_chat(usr, "<span_class='admin prefix'>Deletion of protected ranks is not permitted, it must be removed from admin_ranks.txt.</span>")
		return
	if(CONFIG_GET(flag/load_legacy_ranks_only))
		to_chat(usr, "<span_class='admin prefix'>Rank deletion not permitted while database rank loading is disabled.</span>")
		return
	...
	if(query_admins_with_rank.NextRow())
		qdel(query_admins_with_rank)
		to_chat(usr, "<span_class='danger'>Error: Rank deletion attempted while rank still used; Tell a coder, this shouldn't happen.</span>")
		return
	qdel(query_admins_with_rank)
...
/datum/admins/proc/sync_lastadminrank(admin_ckey, admin_key, datum/admins/D)
	...
	if(!query_sync_lastadminrank.warn_execute())
		...
		return
	qdel(query_sync_lastadminrank)
	to_chat(usr, "<span_class='admin'>Sync of [admin_key] successful.</span>")
