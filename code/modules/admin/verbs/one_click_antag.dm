...
/datum/admins/proc/makeClockCult()
	...
	if(candidates.len)
		...
		for(var/i = 0, i<numCultists, i++)
			H = pick(candidates)
			to_chat(H, "<span_class='heavy_brass'>The world before you suddenly glows a brilliant yellow. You hear the whooshing steam and clanking cogs of a billion billion machines, and all at once \
