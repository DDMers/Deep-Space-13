...
/client/proc/cmd_admin_subtle_message(mob/M in GLOB.mob_list)
	...
	msg = "<span_class='adminnotice'><b> SubtleMessage: [key_name_admin(usr)] -> [key_name_admin(M)] :</b> [msg]</span>"
	message_admins(msg)
	admin_ticket_log(M, msg)
...
/client/proc/admin_headset_message(mob/M in GLOB.mob_list, sender = null)
	...
	to_chat(H, "You hear something crackle in your ears for a moment before a voice speaks.  \"Please stand by for a message from [sender == "Syndicate" ? "your benefactor" : "Central Command"].  Message as follows[sender == "Syndicate" ? ", agent." : ":"] <span_class='bold'>[input].</span> Message ends.\"")
...
/client/proc/cmd_admin_mod_antag_rep(client/C in GLOB.clients, var/operation)
	...
	message_admins("<span_class='adminnotice'>[key_name_admin(usr)]: Modified [key_name(C)]'s antagonist reputation ([log_text])</span>")
...
/client/proc/cmd_admin_world_narrate()
	...
	message_admins("<span_class='adminnotice'>[key_name_admin(usr)] Sent a global narrate</span>")
...
/client/proc/cmd_admin_direct_narrate(mob/M)
	...
	msg = "<span_class='adminnotice'><b> DirectNarrate: [key_name(usr)] to ([M.name]/[M.key]):</b> [msg]<BR></span>"
	message_admins(msg)
	admin_ticket_log(M, msg)
...
/client/proc/cmd_admin_local_narrate(atom/A)
	...
	message_admins("<span_class='adminnotice'><b> LocalNarrate: [key_name_admin(usr)] at [ADMIN_VERBOSEJMP(A)]:</b> [msg]<BR></span>")
...
/client/proc/cmd_admin_godmode(mob/M in GLOB.mob_list)
	...
	to_chat(usr, "<span_class='adminnotice'>Toggled [(M.status_flags & GODMODE) ? "ON" : "OFF"]</span>")
...
/proc/create_xeno(ckey)
	...
	new_xeno.ckey = ckey
	var/msg = "<span_class='notice'>[key_name_admin(usr)] has spawned [ckey] as a filthy xeno [alien_caste].</span>"
	message_admins(msg)
...
/client/proc/respawn_character()
	...
	if(G_found.mind && !G_found.mind.active)	//mind isn't currently in use by someone/something
		...
		if(G_found.mind.assigned_role == ROLE_ALIEN)
			if(alert("This character appears to have been an alien. Would you like to respawn them as such?",,"Yes","No")=="Yes")
				...
				var/msg = "<span_class='adminnotice'>[key_name_admin(usr)] has respawned [new_xeno.key] as a filthy xeno.</span>"
				message_admins(msg)
				admin_ticket_log(new_xeno, msg)
		...
		//check if they were a monkey
			...
			if(alert("This character appears to have been a monkey. Would you like to respawn them as such?",,"Yes","No")=="Yes")
				...
				var/msg = "<span_class='adminnotice'>[key_name_admin(usr)] has respawned [new_monkey.key] as a filthy xeno.</span>"
				message_admins(msg)
	...
	*/
	...
	var/msg = "<span_class='adminnotice'>[admin] has respawned [player_key] as [new_character.real_name].</span>"
	message_admins(msg)
...
/client/proc/cmd_admin_rejuvenate(mob/living/M in GLOB.mob_list)
	...
	var/msg = "<span_class='danger'>Admin [key_name_admin(usr)] healed / revived [ADMIN_LOOKUPFLW(M)]!</span>"
	message_admins(msg)
	admin_ticket_log(M, msg)
...
/client/proc/cmd_admin_gib_self()
	...
	if(confirm == "Yes")
		...
		message_admins("<span_class='adminnotice'>[key_name_admin(usr)] used gibself.</span>")
...
/client/proc/admin_call_shuttle()
	...
	message_admins("<span_class='adminnotice'>[key_name_admin(usr)] admin-called the emergency shuttle.</span>")
	return
...
/client/proc/admin_cancel_shuttle()
	...
	message_admins("<span_class='adminnotice'>[key_name_admin(usr)] admin-recalled the emergency shuttle.</span>")
...
/client/proc/everyone_random()
	...
	if(notifyplayers == "Yes")
		to_chat(world, "<span_class='adminnotice'>Admin [usr.key] has forced the players to have completely random identities!</span>")
...
/client/proc/polymorph_all()
	...
	for(var/mob/living/M in mobs)
		...
		M.audible_message("<span_class='italics'>...wabbajack...wabbajack...</span>")
...
/client/proc/smite(mob/living/carbon/human/target as mob)
	...
	switch(punishment)
		if(ADMIN_PUNISHMENT_LIGHTNING)
			...
			to_chat(target, "<span_class='userdanger'>The gods have punished you for your sins!</span>")
		...
		if(ADMIN_PUNISHMENT_MAZING)
			if(!puzzle_imprison(target))
				to_chat(usr,"<span_class='warning'>Imprisonment failed!</span>")
				return
...
/client/proc/cmd_admin_check_player_exp()	//Allows admins to determine who the newer players are.
	...
	if(!CONFIG_GET(flag/use_exp_tracking))
		to_chat(usr, "<span_class='warning'>Tracking is disabled in the server configuration file.</span>")
		return
...
/obj/effect/temp_visual/target/proc/fall(list/flame_hit)
	...
	for(var/mob/living/L in T.contents)
		...
		if(islist(flame_hit) && !flame_hit[L])
			L.adjustFireLoss(40)
			to_chat(L, "<span_class='userdanger'>You're hit by the drake's fire breath!</span>")
			flame_hit[L] = TRUE
		else
...
/datum/admins/proc/toggle_exempt_status(client/C)
	...
	if(!C.set_db_player_flags())
		to_chat(usr, "<span_class='danger'>ERROR: Unable read player flags from database. Please check logs.</span>")
	...
	if(C.update_flag_db(DB_FLAG_EXEMPT, newstate))
		to_chat(usr, "<span_class='danger'>ERROR: Unable to update player flags. Please check logs.</span>")
	else
