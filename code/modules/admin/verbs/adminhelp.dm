...
/datum/admin_help_tickets/proc/BrowseTickets(state)
	...
	for(var/I in l2b)
		...
		dat += "<span_class='adminnotice'><span_class='adminhelp'>Ticket #[AH.id]</span>: <A href='?_src_=holder;[HrefToken()];ahelp=[REF(AH)];ahelp_action=ticket'>[AH.initiator_key_name]: [AH.name]</A></span><br>"
...
/datum/admin_help/New(msg, client/C, is_bwoink)
	...
	if(is_bwoink)
		...
		if(admin_number_present <= 0)
			to_chat(C, "<span_class='notice'>No active admins are online, your adminhelp was sent to the admin irc.</span>")
...
/datum/admin_help/proc/MessageNoRecipient(msg)
	...
	var/admin_msg = "<span_class='adminnotice'><span_class='adminhelp'>Ticket [TicketHref("#[id]", ref_src)]</span><b>: [LinkedReplyName(ref_src)] [FullMonty(ref_src)]:</b> <span_class='linkify'>[keywords_lookup(msg)]</span></span>"
	...
	to_chat(initiator, "<span_class='adminnotice'>PM to-<b>Admins</b>: <span_class='linkify'>[msg]</span></span>")

//Reopen a closed ticket
/datum/admin_help/proc/Reopen()
	if(state == AHELP_ACTIVE)
		to_chat(usr, "<span_class='warning'>This ticket is already open.</span>")
		return

	if(GLOB.ahelp_tickets.CKey2ActiveTicket(initiator_ckey))
		to_chat(usr, "<span_class='warning'>This user already has an active ticket, cannot reopen this one.</span>")
		return
	...
	var/msg = "<span_class='adminhelp'>Ticket [TicketHref("#[id]")] reopened by [key_name_admin(usr)].</span>"
	message_admins(msg)
...
/datum/admin_help/proc/Resolve(key_name = key_name_admin(usr), silent = FALSE)
	...
	to_chat(initiator, "<span_class='adminhelp'>Your ticket has been resolved by an admin. The Adminhelp verb will be returned to you shortly.</span>")
	if(!silent)
		SSblackbox.record_feedback("tally", "ahelp_stats", 1, "resolved")
...
/client/verb/adminhelp(msg as text)
	...
	if(prefs.muted & MUTE_ADMINHELP)
		to_chat(src, "<span_class='danger'>Error: Admin-PM: You cannot send adminhelps (Muted).</span>")
		return
	...
	if(current_ticket)
		if(alert(usr, "You already have a ticket open. Is this for the same issue?",,"Yes","No") != "No")
			if(current_ticket)
				...
				to_chat(usr, "<span_class='warning'>Ticket not found, creating new one...</span>")
		else
