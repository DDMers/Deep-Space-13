/client/proc/map_template_load()
	...
	if(alert(src,"Confirm location.","Template Confirm","Yes","No") == "Yes")
		if(template.load(T, centered = TRUE))
			message_admins("<span_class='adminnotice'>[key_name_admin(src)] has placed a map template ([template.name]) at [ADMIN_COORDJMP(T)]</span>")
		else
...
/client/proc/map_template_upload()
	...
	if(copytext("[map]",-4) != ".dmm")
		to_chat(src, "<span_class='warning'>Filename must end in '.dmm': [map]</span>")
		return
	...
	switch(alert(src, "What kind of map is this?", "Map type", "Normal", "Shuttle", "Cancel"))
		...
		if("Shuttle")
			...
			return
	if(!M.cached_map)
		to_chat(src, "<span_class='warning'>Map template '[map]' failed to parse properly.</span>")
		return
	...
	if(report)
		...
		to_chat(src, "<span_class='warning'>Map template '[map]' <a href='?src=[REF(report)];[HrefToken()];show=1'>failed validation</a>.</span>")
		if(report.loadable)
	...
	message_admins("<span_class='adminnotice'>[key_name_admin(src)] has uploaded a map template '[map]' ([M.width]x[M.height])[report_link].</span>")
	to_chat(src, "<span_class='notice'>Map template '[map]' ready to place ([M.width]x[M.height])</span>")
