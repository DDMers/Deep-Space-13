...
/datum/admins/proc/Secrets_topic(item,href_list)
	...
	switch(item)
		...
		if("tdomereset")
			...
			message_admins("<span_class='adminnotice'>[key_name_admin(usr)] reset the thunderdome to default with delete_mobs==[delete_mobs].</span>")
		...
		if("set_name")
			...
			message_admins("<span_class='adminnotice'>[key_name_admin(usr)] renamed the station to: [new_name].</span>")
		...
		if("reset_name")
			...
			message_admins("<span_class='adminnotice'>[key_name_admin(usr)] reset the station name.</span>")
			priority_announce("[command_name()] has renamed the station to \"[new_name]\".")
		...
		if("togglearrivals")
			...
			if(A)
				...
				to_chat(usr, "<span_class='admin'>There is no arrivals shuttle</span>")
		if("showailaws")
			if(!check_rights(R_ADMIN))
				return
			output_ai_laws()
		...
		if("power")
			...
			message_admins("<span_class='adminnotice'>[key_name_admin(usr)] made all areas powered</span>")
			power_restore()
		...
		if("unpower")
			...
			message_admins("<span_class='adminnotice'>[key_name_admin(usr)] made all areas unpowered</span>")
			power_failure()
		...
		if("quickpower")
			...
			message_admins("<span_class='adminnotice'>[key_name_admin(usr)] made all SMESs powered</span>")
			power_restore_quick()
		...
		if("traitor_all")
			...
			message_admins("<span_class='adminnotice'>[key_name_admin(usr)] used everyone is a traitor secret. Objective is [objective]</span>")
		...
		if("changebombcap")
			...
			message_admins("<span_class='boldannounce'>[key_name_admin(usr)] changed the bomb cap to [GLOB.MAX_EX_DEVESTATION_RANGE], [GLOB.MAX_EX_HEAVY_RANGE], [GLOB.MAX_EX_LIGHT_RANGE]</span>")
		...
		if("retardify")
			...
			for(var/mob/living/carbon/human/H in GLOB.player_list)
				to_chat(H, "<span_class='boldannounce'>You suddenly feel stupid.</span>")
