	...
	switch (action)
		...
		if ("add")
			...
			if (get_stickyban_from_ckey(ckey))
				to_chat(usr, "<span_class='adminnotice'>Error: Can not add a stickyban: User already has a current sticky ban</span>")
			...
			message_admins("<span_class='adminnotice'>[key_name_admin(usr)] has stickybanned [ckey].\nReason: [ban["message"]]</span>")
		...
		if ("remove")
			...
			if (!ban)
				to_chat(usr, "<span_class='adminnotice'>Error: No sticky ban for [ckey] found!</span>")
				return
			if (alert("Are you sure you want to remove the sticky ban on [ckey]?","Are you sure","Yes","No") == "No")
				return
			if (!get_stickyban_from_ckey(ckey))
				to_chat(usr, "<span_class='adminnotice'>Error: The ban disappeared.</span>")
				return
			world.SetConfig("ban",ckey, null)

			log_admin_private("[key_name(usr)] removed [ckey]'s stickyban")
			message_admins("<span_class='adminnotice'>[key_name_admin(usr)] removed [ckey]'s stickyban</span>")
		...
		if ("remove_alt")
			...
			if (!found)
				to_chat(usr, "<span_class='adminnotice'>Error: [alt] is not linked to [ckey]'s sticky ban!</span>")
				return
			...
			if (!found)
				to_chat(usr, "<span_class='adminnotice'>Error: [alt] link to [ckey]'s sticky ban disappeared.</span>")
				return

			world.SetConfig("ban",ckey,list2stickyban(ban))

			log_admin_private("[key_name(usr)] has disassociated [alt] from [ckey]'s sticky ban")
			message_admins("<span_class='adminnotice'>[key_name_admin(usr)] has disassociated [alt] from [ckey]'s sticky ban</span>")
		...
		if ("edit")
			...
			message_admins("<span_class='adminnotice'>[key_name_admin(usr)] has edited [ckey]'s sticky ban reason from [oldreason] to [reason]</span>")
		...
		if ("revert")
			...
			if (!cached_ban)
				to_chat(usr, "<span_class='adminnotice'>Error: No cached sticky ban for [ckey] found!</span>")
			world.SetConfig("ban",ckey,null)

			log_admin_private("[key_name(usr)] has reverted [ckey]'s sticky ban to its state at round start.")
			message_admins("<span_class='adminnotice'>[key_name_admin(usr)] has reverted [ckey]'s sticky ban to its state at round start.</span>")
