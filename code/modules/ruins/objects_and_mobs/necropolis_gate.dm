...
/obj/structure/necropolis_gate/attack_hand(mob/user)
	if(locked)
		to_chat(user, "<span_class='boldannounce'>It's [open ? "stuck open":"locked"].</span>")
		return
...
/obj/structure/necropolis_gate/proc/toggle_the_gate(mob/user, legion_damaged)
	...
	if(open)
		...
		visible_message("<span_class='boldwarning'>The door slams closed!</span>")
		sleep(1)
		...
		sleep(2)
		visible_message("<span_class='warning'>The door starts to grind open...</span>")
...
/obj/structure/necropolis_gate/legion_gate/attack_hand(mob/user)
	if(!open && !changing_openness)
		...
		if(safety == "Abort" || !in_range(src, user) || !src || open || changing_openness || user.incapacitated())
			return
		user.visible_message("<span_class='warning'>[user] knocks on [src]...</span>", "<span_class='boldannounce'>You tentatively knock on [src]...</span>")
...
/obj/structure/necropolis_gate/legion_gate/toggle_the_gate(mob/user, legion_damaged)
	...
	if(.)
		locked = TRUE
		var/turf/T = get_turf(src)
		visible_message("<span_class='userdanger'>Something horrible emerges from the Necropolis!</span>")
		if(legion_damaged)
		...
		for(var/mob/M in GLOB.player_list)
			if(M.z == z)
				to_chat(M, "<span_class='userdanger'>Discordant whispers flood your mind in a thousand voices. Each one speaks your name, over and over. Something horrible has been released.</span>")
