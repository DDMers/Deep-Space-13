...
/obj/structure/cursed_slot_machine/interact(mob/living/carbon/human/user)
	...
	if(user.stat)
		to_chat(user, "<span_class='userdanger'>No... just one more try...</span>")
		user.gib()
	else
		user.visible_message("<span_class='warning'>[user] pulls [src]'s lever with a glint in [user.p_their()] eyes!</span>", "<span_class='warning'>You feel a draining as you pull the lever, but you \
...
/obj/structure/cursed_slot_machine/proc/determine_victor(mob/living/user)
	...
	if(prob(win_prob))
		...
		if(user)
			to_chat(user, "<span_class='boldwarning'>You've hit jackpot. Laughter echoes around you as your reward appears in the machine's place.</span>")
		qdel(src)
	else
		if(user)
			to_chat(user, "<span_class='boldwarning'>Fucking machine! Must be rigged. Still... one more try couldn't hurt, right?</span>")
...
/obj/structure/cursed_money/proc/collapse()
	visible_message("<span_class='warning'>[src] falls in on itself, \
...
/obj/structure/cursed_money/attack_hand(mob/living/user)
	...
	if(.)
		return
	user.visible_message("<span_class='warning'>[user] opens the bag and \
		and removes a die. The bag then vanishes.</span>",
		"<span_class='boldwarning'>You open the bag...!</span>\n\
		<span_class='danger'>And see a bag full of dice. Confused, \
...
/obj/effect/gluttony/CanPass(atom/movable/mover, turf/target)//So bullets will fly over and stuff.
	if(ishuman(mover))
		...
		if(H.nutrition >= NUTRITION_LEVEL_FAT)
			H.visible_message("<span_class='warning'>[H] pushes through [src]!</span>", "<span_class='notice'>You've seen and eaten worse than this.</span>")
			return TRUE
		else
			to_chat(H, "<span_class='warning'>You're repulsed by even looking at [src]. Only a pig could force themselves to go through it.</span>")
...
/obj/structure/mirror/magic/pride/curse(mob/user)
	user.visible_message("<span_class='danger'><B>The ground splits beneath [user] as [user.p_their()] hand leaves the mirror!</B></span>", \
	"<span_class='notice'>Perfect. Much better! Now <i>nobody</i> will be able to resist yo-</span>")
...
/obj/item/kitchen/knife/envy/afterattack(atom/movable/AM, mob/living/carbon/human/user, proximity)
	...
	if(ishuman(AM))
		...
		if(user.real_name != H.dna.real_name)
			...
			user.domutcheck()
			user.visible_message("<span_class='warning'>[user]'s appearance shifts into [H]'s!</span>", \
			"<span_class='boldannounce'>[H.p_they(TRUE)] think[H.p_s()] [H.p_theyre()] <i>sooo</i> much better than you. Not anymore, [H.p_they()] won't.</span>")
