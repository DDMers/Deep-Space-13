...
/obj/item/hilbertshotel/attack(mob/living/M, mob/living/user)
    if(M.mind)
        to_chat(user, "<span_class='notice'>You invite [M] to the hotel.</span>")
        promptAndCheckIn(M)
    else
        to_chat(user, "<span_class='warning'>[M] is not intelligent enough to understand how to use this device!</span>")
...
/obj/item/hilbertshotel/proc/promptAndCheckIn(mob/user)
    ...
    if(chosenRoomNumber > SHORT_REAL_LIMIT)
        to_chat(user, "<span_class='warning'>You have to check out the first [SHORT_REAL_LIMIT] rooms before you can go to a higher numbered one!</span>")
        return
    if((chosenRoomNumber < 1) || (chosenRoomNumber != round(chosenRoomNumber)))
        to_chat(user, "<span_class='warning'>That is not a valid room number!</span>")
        return
    if(ismob(loc))
...
/obj/item/hilbertshotel/proc/linkTurfs(var/datum/turf_reservation/currentReservation, var/currentRoomnumber)
    ...
    for(var/turf/closed/indestructible/hoteldoor/door in currentArea)
        ...
        door.desc = "The door to this hotel room. The placard reads 'Room [currentRoomnumber]'. Strange, this door doesnt even seem openable. The doorknob, however, seems to buzz with unusual energy...<br /><span_class='info'>Alt-Click to look through the peephole.</span>"
                ...
                for(var/j=0, j<hotelRoomTemp.height, j++)
                    for(var/atom/movable/A in locate(room.bottom_left_coords[1] + i, room.bottom_left_coords[2] + j, room.bottom_left_coords[3]))
                        if(ismob(A))
                            var/mob/M = A
                            if(M.mind)
                                to_chat(M, "<span_class='warning'>As the sphere breaks apart, you're suddenly ejected into the depths of space!")
...
/turf/closed/indestructible/hoteldoor/proc/promptExit(mob/living/user)
    ...
    if(!user.mind)
        return
    if(!parentSphere)
        to_chat(user, "<span_class='warning'>The door seems to be malfunctioning and refuses to operate!</span>")
        return
...
/turf/closed/indestructible/hoteldoor/AltClick(mob/user)
    . = ..()
    if(get_dist(get_turf(src), get_turf(user)) <= 1)
        to_chat(user, "<span_class='notice'>You peak through the door's bluespace peephole...</span>")
...
/datum/action/peepholeCancel/Trigger()
    . = ..()
    to_chat(owner, "<span_class='warning'>You move away from the peephole.</span>")
...
/obj/item/analyzer/hilbertsanalyzer/afterattack(atom/target, mob/user, proximity)
    ...
    if(istype(target, /obj/item/hilbertshotel))
        if(!proximity)
            to_chat(user, "<span_class='warning'>It's to far away to scan!</span>")
            return
