...
/obj/item/stack/marker_beacon/examine(mob/user)
	..()
	to_chat(user, "<span_class='notice'>Use in-hand to place a [singular_name].</span>")
	to_chat(user, "<span_class='notice'>Alt-click to select a color. Current color is [picked_color].</span>")
...
/obj/item/stack/marker_beacon/attack_self(mob/user)
	if(!isturf(user.loc))
		to_chat(user, "<span_class='warning'>You need more space to place a [singular_name] here.</span>")
		return
	if(locate(/obj/structure/marker_beacon) in user.loc)
		to_chat(user, "<span_class='warning'>There is already a [singular_name] here.</span>")
		return
	if(use(1))
		to_chat(user, "<span_class='notice'>You activate and anchor [amount ? "a":"the"] [singular_name] in place.</span>")
...
/obj/structure/marker_beacon/attack_hand(mob/living/user)
	...
	if(.)
		return
	to_chat(user, "<span_class='notice'>You start picking [src] up...</span>")
...
/obj/structure/marker_beacon/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/light_eater))
		...
		visible_message("<span_class='danger'>[src] is disintegrated by [I]!</span>")
