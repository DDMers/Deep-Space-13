...
/obj/item/wormhole_jaunter/attack_self(mob/user)
	user.visible_message("<span_class='notice'>[user.name] activates the [src.name]!</span>")
...
/obj/item/wormhole_jaunter/proc/turf_check(mob/user)
	...
	if(!device_turf || is_centcom_level(device_turf.z) || is_reserved_level(device_turf.z))
		to_chat(user, "<span_class='notice'>You're having difficulties getting the [src.name] to work.</span>")
		return FALSE
	return TRUE
...
/obj/item/wormhole_jaunter/proc/activate(mob/user, adjacent)
	...
	if(!L.len)
		to_chat(user, "<span_class='notice'>The [src.name] found no beacons in the world to anchor a wormhole to.</span>")
		return
...
/obj/item/wormhole_jaunter/emp_act(power)
	...
	if(istype(M))
		...
		if(triggered)
			M.visible_message("<span_class='warning'>[src] overloads and activates!</span>")
