...
/obj/item/survivalcapsule/attack_self()
	...
	get_template()
	if(!used)
		loc.visible_message("<span_class='warning'>\The [src] begins to shake. Stand back!</span>")
		used = TRUE
		sleep(50)
		...
		switch(status)
			if(SHELTER_DEPLOY_BAD_AREA)
				src.loc.visible_message("<span_class='warning'>\The [src] will not function in this area.</span>")
			if(SHELTER_DEPLOY_BAD_TURFS, SHELTER_DEPLOY_ANCHORED_OBJECTS)
				var/width = template.width
				var/height = template.height
				src.loc.visible_message("<span_class='warning'>\The [src] doesn't have room to deploy! You need to clear a [width]x[height] area!</span>")
...
/obj/item/gps/computer/wrench_act(mob/living/user, obj/item/I)
	...
	user.visible_message("<span_class='warning'>[user] disassembles [src].</span>",
		"<span_class='notice'>You start to disassemble [src]...</span>", "You hear clanking and banging noises.")
