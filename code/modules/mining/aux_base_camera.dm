...
/obj/item/construction/rcd/internal //Base console's internal RCD. Roundstart consoles are filled, rebuilt cosoles start empty.
	...
	no_ammo_message = "<span_class='warning'>Internal matter exhausted. Please add additional materials.</span>"
	delay_mod = 0.5
...
//Check a loction to see if it is inside the aux base at the station. Camera visbility checks omitted so as to not hinder construction.
	...
	if(!istype(build_area, /area/shuttle/auxillary_base))
		to_chat(owner, "<span_class='warning'>You can only build within the mining base!</span>")
		return FALSE

	if(!is_station_level(build_target.z))
		to_chat(owner, "<span_class='warning'>The mining base has launched and can no longer be modified.</span>")
		return FALSE
...
datum/action/innate/aux_base/place_fan/Activate()
	...
	if(!B.fans_remaining)
		to_chat(owner, "<span_class='warning'>[B] is out of fans!</span>")
		return
	...
	if(fan_turf.density)
		to_chat(owner, "<span_class='warning'>Fans may only be placed on a floor.</span>")
		return

	new /obj/structure/fans/tiny(fan_turf)
	B.fans_remaining--
	to_chat(owner, "<span_class='notice'>Tiny fan placed. [B.fans_remaining] remaining.</span>")
...
datum/action/innate/aux_base/install_turret/Activate()
	...
	if(!B.turret_stock)
		to_chat(owner, "<span_class='warning'>Unable to construct additional turrets.</span>")
		return

	var/turf/turret_turf = get_turf(remote_eye)

	if(is_blocked_turf(turret_turf))
		to_chat(owner, "<span_class='warning'>Location is obstructed by something. Please clear the location and try again.</span>")
		return
	...
	B.turret_stock--
	to_chat(owner, "<span_class='notice'>Turret installation complete!</span>")
