...
/obj/machinery/computer/auxillary_base/ui_interact(mob/user)
	...
	if(LAZYLEN(turrets))
		...
		for(var/PDT in turrets)
			...
			var/status
			if(T.stat & BROKEN)
				status = "<span_class='bad'>ERROR</span>"
			else if(!T.on)
				status = "Disabled"
			else if(T.raised)
				status = "<span_class='average'><b>Firing</b></span>"
			else
				status = "<span_class='good'>All Clear</span>"
...
/obj/machinery/computer/auxillary_base/Topic(href, href_list)
	...
	if(href_list["move"])
		if(!is_station_level(z) && shuttleId == "colony_drop")
			to_chat(usr, "<span_class='warning'>You can't move the base again!</span>")
			return
		var/shuttle_error = SSshuttle.moveShuttle(shuttleId, href_list["move"], 1)
		if(launch_warning)
			say("<span_class='danger'>Launch sequence activated! Prepare for drop!!</span>")
	...
	if(href_list["random"] && !possible_destinations)
		...
		//Find a suitable mining turf. Reduces chance of landing in a bad area
			to_chat(usr, "<span_class='warning'>Landing zone scan failed. Please try again.</span>")
			updateUsrDialog()
			return
		if(set_landing_zone(LZ, usr) != ZONE_SET)
			to_chat(usr, "<span_class='warning'>Landing zone unsuitable. Please recalculate.</span>")
			updateUsrDialog()
			return
...
/obj/machinery/computer/auxillary_base/proc/set_landing_zone(turf/T, mob/user, no_restrictions)
	...
	if(!base_dock) //Not all maps have an Aux base. This object is useless in that case.
		to_chat(user, "<span_class='warning'>This station is not equipped with an auxillary base. Please contact your Nanotrasen contractor.</span>")
		return
	if(!no_restrictions)
...
//Serves as a nice mechanic to people get ready for the launch.
	...
	to_chat(user, "<span_class='notice'>Landing zone set.</span>")
	return ZONE_SET
...
/obj/item/assault_pod/mining/attack_self(mob/living/user)
	...
	to_chat(user, "<span_class='notice'>You begin setting the landing zone parameters...</span>")
	setting = TRUE
	...
	for (var/obj/machinery/computer/auxillary_base/A in GLOB.machines)
		if(is_station_level(A.z))
			...
			break
	if(!AB)
		to_chat(user, "<span_class='warning'>No auxillary base console detected.</span>")
		return
	...
	switch(AB.set_landing_zone(T, user, no_restrictions))
		if(ZONE_SET)
			qdel(src)
		if(BAD_ZLEVEL)
			to_chat(user, "<span_class='warning'>This uplink can only be used in a designed mining zone.</span>")
		if(BAD_AREA)
			to_chat(user, "<span_class='warning'>Unable to acquire a targeting lock. Find an area clear of structures or entirely within one.</span>")
		if(BAD_COORDS)
			to_chat(user, "<span_class='warning'>Location is too close to the edge of the station's scanning range. Move several paces away and try again.</span>")
		if(BAD_TURF)
			to_chat(user, "<span_class='warning'>The landing zone contains turfs unsuitable for a base. Make sure you've removed all walls and dangerous terrain from the landing zone.</span>")
...
/obj/structure/mining_shuttle_beacon/attack_hand(mob/user)
	...
	if(.)
		return
	if(anchored)
		to_chat(user, "<span_class='warning'>Landing zone already set.</span>")
		return

	if(anti_spam_cd)
		to_chat(user, "<span_class='warning'>[src] is currently recalibrating. Please wait.</span>")
		return
	...
	if(!is_mining_level(landing_spot.z))
		to_chat(user, "<span_class='warning'>This device is only to be used in a mining zone.</span>")
		return
	...
	if(!aux_base_console) //Needs to be near the base to serve as its dock and configure it to control the mining shuttle.
		to_chat(user, "<span_class='warning'>The auxillary base's console must be within [console_range] meters in order to interface.</span>")
		return
...
//Mining shuttles may not be created equal, so we find the map's shuttle dock and size accordingly.
	for(var/S in SSshuttle.stationary)
		...
		if(SM.id == "mining_home" || SM.id == "mining_away")
			...
			break
	if(!Mport)
		to_chat(user, "<span_class='warning'>This station is not equipped with an appropriate mining shuttle. Please contact Nanotrasen Support.</span>")
		return
	...
	if(!mining_shuttle) //Not having a mining shuttle is a map issue
		to_chat(user, "<span_class='warning'>No mining shuttle signal detected. Please contact Nanotrasen Support.</span>")
	...
	for(var/i in 1 to landing_turfs.len) //You land NEAR the base, not IN it.
		...
		if(!L) //This happens at map edges
			to_chat(user, "<span_class='warning'>Unable to secure a valid docking zone. Please try again in an open area near, but not within the aux. mining base.</span>")
			SSshuttle.stationary.Remove(Mport)
			qdel(Mport)
			return
		if(istype(get_area(L), /area/shuttle/auxillary_base))
			to_chat(user, "<span_class='warning'>The mining shuttle must not land within the mining base itself.</span>")
			SSshuttle.stationary.Remove(Mport)
			qdel(Mport)
			return
	...
	to_chat(user, "<span_class='notice'>Mining shuttle calibration successful! Shuttle interface available at base console.</span>")
