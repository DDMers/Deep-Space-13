...
/mob/living/simple_animal/hostile/mining_drone/examine(mob/user)
	...
	if(health < maxHealth)
		if(health >= maxHealth * 0.5)
			to_chat(user, "<span_class='warning'>[t_He] look[t_s] slightly dented.</span>")
		else
			to_chat(user, "<span_class='boldwarning'>[t_He] look[t_s] severely dented!</span>")
	to_chat(user, "<span_class='notice'>Using a mining scanner on [t_him] will instruct [t_him] to drop stored ore. <b>[max(0, LAZYLEN(contents) - 1)] Stored Ore</b>\n\
	...
	if(stored_gun && stored_gun.max_mod_capacity)
		...
		for(var/A in stored_gun.get_modkits())
			...
			to_chat(user, "<span_class='notice'>There is \a [M] installed, using <b>[M.cost]%</b> capacity.</span>")

/mob/living/simple_animal/hostile/mining_drone/welder_act(mob/living/user, obj/item/I)
	. = TRUE
	if(mode == MINEDRONE_ATTACK)
		to_chat(user, "<span_class='info'>[src] can't be repaired while in attack mode!</span>")
		return

	if(maxHealth == health)
		to_chat(user, "<span_class='info'>[src] is at full integrity.</span>")
		return

	if(I.use_tool(src, user, 0, volume=40))
		adjustBruteLoss(-15)
		to_chat(user, "<span_class='info'>You repair some of the armor on [src].</span>")

/mob/living/simple_animal/hostile/mining_drone/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/mining_scanner) || istype(I, /obj/item/t_scanner/adv_mining_scanner))
		to_chat(user, "<span_class='info'>You instruct [src] to drop any collected ore.</span>")
		DropOre()
		return
	...
	if(M.a_intent == INTENT_HELP)
		...
		switch(mode)
			if(MINEDRONE_COLLECT)
				to_chat(M, "<span_class='info'>[src] has been set to search and store loose ore.</span>")
			if(MINEDRONE_ATTACK)
				to_chat(M, "<span_class='info'>[src] has been set to attack hostile wildlife.</span>")
		return
...
/mob/living/simple_animal/hostile/mining_drone/proc/SetCollectBehavior()
	...
	icon_state = "mining_drone"
	to_chat(src, "<span_class='info'>You are set to collect mode. You can now collect loose ore.</span>")
...
/mob/living/simple_animal/hostile/mining_drone/proc/SetOffenseBehavior()
	...
	to_chat(src, "<span_class='info'>You are set to attack mode. You can now attack from range.</span>")
...
/mob/living/simple_animal/hostile/mining_drone/proc/DropOre(message = 1)
	if(!contents.len)
		if(message)
			to_chat(src, "<span_class='notice'>You attempt to dump your stored ore, but you have none.</span>")
		return
	if(message)
		to_chat(src, "<span_class='notice'>You dump your stored ore.</span>")
...
/datum/action/innate/minedrone/toggle_meson_vision/Activate()
	...
	to_chat(user, "<span_class='notice'>You toggle your meson vision [(user.sight & SEE_TURFS) ? "on" : "off"].</span>")
...
/datum/action/innate/minedrone/toggle_light/Activate()
	...
	to_chat(user, "<span_class='notice'>You toggle your light [user.light_on ? "on" : "off"].</span>")
