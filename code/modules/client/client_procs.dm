...
/client/Topic(href, href_list, hsrc)
	...
	if(href_list["asset_cache_confirm_arrival"])
		...
		if (job && job <= last_asset_job && !(job in completed_asset_jobs))
			...
			to_chat(src, "<span_class='danger'>An error has been detected in how your client is receiving resources. Attempting to correct.... (If you keep seeing these messages you might want to close byond and reconnect)</span>")
	...
	if (!holder && mtl)
		...
		if (topiclimiter[MINUTE_COUNT] > mtl)
			...
			to_chat(src, "<span_class='danger'>[msg]</span>")
			return
	...
	if (!holder && stl)
		...
		if (topiclimiter[SECOND_COUNT] > stl)
			to_chat(src, "<span_class='danger'>Your previous action was ignored because you've done too many in a second</span>")
			return
...
/client/proc/handle_spam_prevention(message, mute_type)
	if(CONFIG_GET(flag/automute_on) && !holder && last_message == message)
		...
		if(src.last_message_count >= SPAM_TRIGGER_AUTOMUTE)
			to_chat(src, "<span_class='danger'>You have exceeded the spam filter limit for identical messages. An auto-mute was applied.</span>")
			cmd_admin_mute(src, mute_type, 1)
			return 1
		if(src.last_message_count >= SPAM_TRIGGER_WARNING)
			to_chat(src, "<span_class='danger'>You are nearing the spam filter limit for identical messages.</span>")
			return 0
	else
...
/client/New(TopicData)
	...
	if(CONFIG_GET(flag/log_access))
		for(var/I in GLOB.clients)
			...
			if(C.key && (C.key != key) )
				...
				if(matches)
					if(C)
						message_admins("<font color='red'><B>Notice: </B><span_class='notice'>[key_name_admin(src)] has the same [matches] as [key_name_admin(C)].</span>")
						log_access("Notice: [key_name(src)] has the same [matches] as [key_name(C)].")
					else
						message_admins("<font color='red'><B>Notice: </B><span_class='notice'>[key_name_admin(src)] has the same [matches] as [key_name_admin(C)] (no longer logged in). </span>")
	...
	if (byond_version >= 512)
		if (!byond_build || byond_build < 1386)
			message_admins("<span_class='adminnotice'>[key_name(src)] has been detected as spoofing their byond version. Connection rejected.</span>")
		...
		if (num2text(byond_build) in GLOB.blacklisted_builds)
			...
			to_chat(src, "<span_class='userdanger'>Your version of byond is blacklisted.</span>")
			to_chat(src, "<span_class='danger'>Byond build [byond_build] ([byond_version].[byond_build]) has been blacklisted for the following reason: [GLOB.blacklisted_builds[num2text(byond_build)]].</span>")
			to_chat(src, "<span_class='danger'>Please download a new version of byond. If [byond_build] is the latest, you can go to <a href=\"https://secure.byond.com/download/build\">BYOND's website</a> to download other versions.</span>")
			if(connecting_admin)
	...
	if (byond_version < cev)		//Out of date client.
		to_chat(src, "<span_class='danger'><b>Your version of BYOND is too old:</b></span>")
		...
		if(CONFIG_GET(flag/client_warn_popup))
			...
			to_chat(src, "<span_class='danger'><b>Your version of byond may be getting out of date:</b></span>")
	...
	if(prefs.lastchangelog != GLOB.changelog_hash) //bolds the changelog button on the interface so we know there are updates.
		to_chat(src, "<span_class='info'>You have unread updates in the changelog.</span>")
	...
	if(!winexists(src, "asset_cache_browser")) // The client is using a custom skin, tell them.
		to_chat(src, "<span_class='warning'>Unable to access asset cache browser, if you are using a custom skin file, please allow DS to download the updated version, if you are not, then make a bug report. This is not a critical issue but can cause issues with resource downloading, as it is impossible to know when extra resources arrived to you.</span>")
...
/client/proc/set_client_age_from_db(connectiontopic)
	...
	if(!query_client_in_db.NextRow())
		if (CONFIG_GET(flag/panic_bunker) && !holder && !GLOB.deadmins[ckey])
			...
			message_admins("<span_class='adminnotice'>Failed Login: [key] - New account attempting to connect during panic bunker</span>")
			to_chat(src, "Sorry but the server is currently not accepting connections from never before seen players.")
			var/list/connectiontopic_a = params2list(connectiontopic)
			var/list/panic_addr = CONFIG_GET(string/panic_server_address)
			if(panic_addr && !connectiontopic_a["redirect"])
				var/panic_name = CONFIG_GET(string/panic_server_name)
				to_chat(src, "<span_class='notice'>Sending you to [panic_name ? panic_name : panic_addr].</span>")
	...
	if (oldcid)
		if (!topic || !topic["token"] || !tokens[ckey] || topic["token"] != tokens[ckey])
			if (!cidcheck_spoofckeys[ckey])
				message_admins("<span_class='adminnotice'>[key_name(src)] appears to have attempted to spoof a cid randomizer check.</span>")
		...
		if (oldcid != computer_id && computer_id != lastcid) //IT CHANGED!!!
			...
			to_chat(src, "<span_class='userdanger'>Connection Error:</span>")
			to_chat(src, "<span_class='danger'>Invalid ComputerID(spoofed). Please remove the ComputerID spoofer from your byond installation and try again.</span>")

			if (!cidcheck_failedckeys[ckey])
				message_admins("<span_class='adminnotice'>[key_name(src)] has been detected as using a cid randomizer. Connection rejected.</span>")
			...
			if (cidcheck_failedckeys[ckey])
				message_admins("<span_class='adminnotice'>[key_name_admin(src)] has been allowed to connect after showing they removed their cid randomizer</span>")
			...
			if (cidcheck_spoofckeys[ckey])
				message_admins("<span_class='adminnotice'>[key_name_admin(src)] has been allowed to connect after appearing to have attempted to spoof a cid randomizer check because it <i>appears</i> they aren't spoofing one this time</span>")
...
/client/proc/check_ip_intel()
	...
	if (CONFIG_GET(string/ipintel_email))
		...
		if (res.intel >= CONFIG_GET(number/ipintel_rating_bad))
			message_admins("<span_class='adminnotice'>Proxy Detection: [key_name_admin(src)] IP intel rated [res.intel*100]% likely to be a Proxy/VPN.</span>")
		ip_intel = res.intel
...
/client/Click(atom/object, atom/location, control, params)
	...
	if (!holder && scl)
		...
		if (clicklimiter[SECOND_COUNT] > scl)
			to_chat(src, "<span_class='danger'>Your previous click was ignored because you've done too many in a second</span>")
			return
