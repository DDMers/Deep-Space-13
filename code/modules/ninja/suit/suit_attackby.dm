...
/obj/item/clothing/suit/space/space_ninja/attackby(obj/item/I, mob/U, params)
	...
	if(istype(I, /obj/item/reagent_containers/glass))//If it's a glass beaker.
		if(I.reagents.has_reagent("radium", a_transfer) && a_boost < a_maxamount)
			...
			a_boost++;
			to_chat(U, "<span_class='notice'>There are now [a_boost] adrenaline boosts remaining.</span>")
			return
		if(I.reagents.has_reagent("smoke_powder", a_transfer) && s_bombs < s_maxamount)
			I.reagents.remove_reagent("smoke_powder", a_transfer)
			s_bombs++;
			to_chat(U, "<span_class='notice'>There are now [s_bombs] smoke bombs remaining.</span>")
			return
		...
		if(CELL.maxcharge > cell.maxcharge && n_gloves && n_gloves.candrain)
			to_chat(U, "<span_class='notice'>Higher maximum capacity detected.\nUpgrading...</span>")
			if (n_gloves && n_gloves.candrain && do_after(U,s_delay, target = src))
				...
				cell = CELL
				to_chat(U, "<span_class='notice'>Upgrade complete. Maximum capacity: <b>[round(cell.maxcharge/100)]</b>%</span>")
			else
				to_chat(U, "<span_class='danger'>Procedure interrupted. Protocol terminated.</span>")
		return
		...
		if(has_research)//If it has something on it.
			...
			if(do_after(U,s_delay, target = src))
				...
				to_chat(U, "<span_class='notice'>Data analyzed and updated. Disk erased.</span>")
			else
				to_chat(U, "<span_class='userdanger'>ERROR</span>: Procedure interrupted. Process terminated.")
		else
			to_chat(U, "<span_class='notice'>No research information detected.</span>")
		return
	return ..()
