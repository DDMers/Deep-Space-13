...
/obj/item/clothing/suit/space/space_ninja/proc/ninja_sword_recall()
	...
	if(!energyKatana)
		to_chat(H, "<span_class='warning'>Could not locate Energy Katana!</span>")
		return
	...
	if(!ninjacost(cost))
		...
		if(inview) //If we can see the katana, throw it towards ourselves, damaging people as we go.
			...
			H.visible_message("<span_class='danger'>\the [energyKatana] flies towards [H]!</span>","<span_class='warning'>You hold out your hand and \the [energyKatana] flies towards you!</span>")
