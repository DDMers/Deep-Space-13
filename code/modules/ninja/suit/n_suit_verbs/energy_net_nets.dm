...
/obj/structure/energy_net/Destroy()
	if(!success)
		if(!QDELETED(affecting))
			affecting.visible_message("[affecting.name] was recovered from the energy net!", "You were recovered from the energy net!", "<span_class='italics'>You hear a grunt.</span>")
		if(!QDELETED(master))//As long as they still exist.
			to_chat(master, "<span_class='userdanger'>ERROR</span>: unable to initiate transport protocol. Procedure terminated.")
	return ..()
...
/obj/structure/energy_net/process()
	...
	to_chat(affecting, "<span_class='danger'>You appear in a strange place!</span>")

	if(!QDELETED(master))//As long as they still exist.
		to_chat(master, "<span_class='notice'><b>SUCCESS</b>: transport procedure of [affecting] complete.</span>")
