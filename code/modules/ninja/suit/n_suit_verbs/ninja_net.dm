...
/obj/item/clothing/suit/space/space_ninja/proc/ninjanet()
	...
	if(!C.client)//Monkeys without a client can still step_to() and bypass the net. Also, netting inactive people is lame.
		to_chat(H, "<span_class='warning'>[C.p_they(TRUE)] will bring no honor to your Clan!</span>")
		return
	if(locate(/obj/structure/energy_net) in get_turf(C))//Check if they are already being affected by an energy net.
		to_chat(H, "<span_class='warning'>[C.p_they(TRUE)] are already trapped inside an energy net!</span>")
		return
	for(var/turf/T in getline(get_turf(H), get_turf(C)))
		if(T.density)//Don't want them shooting nets through walls. It's kind of cheesy.
			to_chat(H, "<span_class='warning'>You may not use an energy net through solid obstacles!</span>")
			return
	if(!ninjacost(200,N_STEALTH_CANCEL))
		...
		E.affecting = C
		E.master = H
		H.visible_message("<span_class='danger'>[H] caught [C] with an energy net!</span>","<span_class='notice'>You caught [C] with an energy net!</span>")
