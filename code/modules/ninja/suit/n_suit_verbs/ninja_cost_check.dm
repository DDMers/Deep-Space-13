...
/obj/item/clothing/suit/space/space_ninja/proc/ninjacost(cost = 0, specificCheck = 0)
	...
	if(cost && cell.charge < actualCost)
		to_chat(H, "<span_class='danger'>Not enough energy.</span>")
		return 1
	else
	...
	switch(specificCheck)
		...
		if(N_SMOKE_BOMB)
			if(!s_bombs)
				to_chat(H, "<span_class='danger'>There are no more smoke bombs remaining.</span>")
				return 1
		if(N_ADRENALINE)
			if(!a_boost)
				to_chat(H, "<span_class='danger'>You do not have any more adrenaline boosters.</span>")
				return 1
