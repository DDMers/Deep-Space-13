...
/obj/item/projectile/proc/on_hit(atom/target, blocked = FALSE)
	...
	if(blocked != 100) // not completely blocked
		...
		if(suppressed)
			...
			to_chat(L, "<span_class='userdanger'>You're shot by \a [src][organ_hit_text]!</span>")
		else
			if(hitsound)
				var/volume = vol_by_damage()
				playsound(loc, hitsound, volume, 1, -1)
			L.visible_message("<span_class='danger'>[L] is hit by \a [src][organ_hit_text]!</span>", \
					"<span_class='userdanger'>[L] is hit by \a [src][organ_hit_text]!</span>", null, COMBAT_MESSAGE_RANGE)
		L.on_hit(src)
