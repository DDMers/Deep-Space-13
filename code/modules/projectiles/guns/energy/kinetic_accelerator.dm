...
/obj/item/gun/energy/kinetic_accelerator/crowbar_act(mob/living/user, obj/item/I)
	. = TRUE
	if(modkits.len)
		to_chat(user, "<span_class='notice'>You pry the modifications out.</span>")
		I.play_tool_sound(src, 100)
		for(var/obj/item/borg/upgrade/modkit/M in modkits)
			M.uninstall(src)
	else
		to_chat(user, "<span_class='notice'>There are no modifications currently installed.</span>")
...
/obj/item/gun/energy/kinetic_accelerator/proc/reload()
	...
	if(!suppressed)
		...
		to_chat(loc, "<span_class='warning'>[src] silently charges up.</span>")
	update_icon()
	overheat = FALSE
...
/obj/item/borg/upgrade/modkit/examine(mob/user)
	..()
	to_chat(user, "<span_class='notice'>Occupies <b>[cost]%</b> of mod capacity.</span>")
...
/obj/item/borg/upgrade/modkit/proc/install(obj/item/gun/energy/kinetic_accelerator/KA, mob/user)
	...
	if(minebot_upgrade)
		if(minebot_exclusive && !istype(KA.loc, /mob/living/simple_animal/hostile/mining_drone))
			to_chat(user, "<span_class='notice'>The modkit you're trying to install is only rated for minebot use.</span>")
			return FALSE
	else if(istype(KA.loc, /mob/living/simple_animal/hostile/mining_drone))
		to_chat(user, "<span_class='notice'>The modkit you're trying to install is not rated for minebot use.</span>")
		return FALSE
	if(denied_type)
	...
	if(KA.get_remaining_mod_capacity() >= cost)
		if(.)
			if(!user.transferItemToLoc(src, KA))
				return
			to_chat(user, "<span_class='notice'>You install the modkit.</span>")
			playsound(loc, 'sound/items/screwdriver.ogg', 100, 1)
			KA.modkits += src
		else
			to_chat(user, "<span_class='notice'>The modkit you're trying to install would conflict with an already installed modkit. Use a crowbar to remove existing modkits.</span>")
	else
		to_chat(user, "<span_class='notice'>You don't have room(<b>[KA.get_remaining_mod_capacity()]%</b> remaining, [cost]% needed) to install this modkit. Use a crowbar to remove existing modkits.</span>")
		. = FALSE
...
/obj/item/borg/upgrade/modkit/aoe/projectile_strike(obj/item/projectile/kinetic/K, turf/target_turf, atom/target, obj/item/gun/energy/kinetic_accelerator/KA)
	...
	if(modifier)
		for(var/mob/living/L in range(1, target_turf) - K.firer - target)
			...
			to_chat(L, "<span_class='userdanger'>You're struck by a [K.name]!</span>")
