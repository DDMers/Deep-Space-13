...
/obj/item/gun/energy/proc/select_fire(mob/living/user)
	...
	fire_delay = shot.delay
	if (shot.select_name)
		to_chat(user, "<span_class='notice'>[src] is now set to [shot.select_name].</span>")
	chambered = null
	recharge_newshot(TRUE)
	update_icon(TRUE)
	return
...
/obj/item/gun/energy/suicide_act(mob/living/user)
	if (istype(user) && can_shoot() && can_trigger_gun(user) && user.get_bodypart(BODY_ZONE_HEAD))
		...
		if(user.is_holding(src))
			user.visible_message("<span_class='suicide'>[user] melts [user.p_their()] face off with [src]!</span>")
		...
		user.visible_message("<span_class='suicide'>[user] is pretending to melt [user.p_their()] face off with [src]! It looks like [user.p_theyre()] trying to commit suicide!</b></span>")
...
/obj/item/gun/energy/ignition_effect(atom/A, mob/living/user)
	if(!can_shoot() || !ammo_type[select])
		...
		if(!BB)
			...
			user.visible_message("<span_class='danger'>[user] tries to light [user.p_their()] [A.name] with [src], but it doesn't do anything. Dumbass.</span>")
			playsound(user, E.fire_sound, 50, 1)
			playsound(user, BB.hitsound, 50, 1)
			cell.use(E.e_cost)
			. = ""
		else if(BB.damage_type != BURN)
			user.visible_message("<span_class='danger'>[user] tries to light [user.p_their()] [A.name] with [src], but only succeeds in utterly destroying it. Dumbass.</span>")
			playsound(user, E.fire_sound, 50, 1)
			playsound(user, BB.hitsound, 50, 1)
			cell.use(E.e_cost)
			qdel(A)
			...
			playsound(user, BB.hitsound, 50, 1)
			cell.use(E.e_cost)
			. = "<span_class='danger'>[user] casually lights their [A.name] with [src]. Damn.</span>"
