	...
	if((istype(I, /obj/item/grenade)))
		if(grenades.len < max_grenades)
			if(!user.transferItemToLoc(I, src))
				return
			grenades += I
			to_chat(user, "<span_class='notice'>You put the grenade in the grenade launcher.</span>")
			to_chat(user, "<span_class='notice'>[grenades.len] / [max_grenades] Grenades.</span>")
		else
			to_chat(usr, "<span_class='danger'>The grenade launcher cannot hold more grenades.</span>")
...
/obj/item/gun/grenadelauncher/process_fire(atom/target, mob/living/user, message = TRUE, params = null, zone_override = "", bonus_spread = 0)
	user.visible_message("<span_class='danger'>[user] fired a grenade!</span>", \
						"<span_class='danger'>You fire the grenade launcher!</span>")
