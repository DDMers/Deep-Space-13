...
/obj/item/gun/blastcannon/attack_self(mob/user)
	if(bomb)
		...
		user.visible_message("<span_class='warning'>[user] detaches [bomb] from [src].</span>")
		bomb = null
	update_icon()
	return ..()
...
/obj/item/gun/blastcannon/attackby(obj/O, mob/user)
	if(istype(O, /obj/item/transfer_valve))
		...
		if(!T.tank_one || !T.tank_two)
			to_chat(user, "<span_class='warning'>What good would an incomplete bomb do?</span>")
			return FALSE
		if(!user.transferItemToLoc(T, src))
			to_chat(user, "<span_class='warning'>[T] seems to be stuck to your hand!</span>")
			return FALSE
		user.visible_message("<span_class='warning'>[user] attaches [T] to [src]!</span>")
		bomb = T
		update_icon()
		return TRUE
	return ..()
...
/obj/item/gun/blastcannon/afterattack(atom/target, mob/user, flag, params)
	...
	var/light = power
	user.visible_message("<span_class='danger'>[user] opens [bomb] on [user.p_their()] [name] and fires a blast wave at [target]!</span>","<span_class='danger'>You open [bomb] on your [name] and fire a blast wave at [target]!</span>")
