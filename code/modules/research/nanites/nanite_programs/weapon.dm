...
/datum/nanite_program/flesh_eating/active_effect()
	...
	if(prob(3))
		to_chat(host_mob, "<span_class='warning'>You feel a stab of pain from somewhere inside you.</span>")
...
/datum/nanite_program/poison/active_effect()
	...
	if(prob(2))
		to_chat(host_mob, "<span_class='warning'>You feel nauseous.</span>")
		if(iscarbon(host_mob))
			var/mob/living/carbon/C = host_mob
			C.vomit(20)
...
/datum/nanite_program/meltdown/enable_passive_effect()
	. = ..()
	to_chat(host_mob, "<span_class='userdanger'>Your blood is burning!</span>")
	nanites.safety_threshold = 0

/datum/nanite_program/meltdown/disable_passive_effect()
	. = ..()
	to_chat(host_mob, "<span_class='warning'>Your blood cools down, and the pain gradually fades.</span>")
...
/datum/nanite_program/triggered/explosive/trigger()
	if(!..())
		return
	host_mob.visible_message("<span_class='warning'>[host_mob] starts emitting a high-pitched buzzing, and [host_mob.p_their()] skin begins to glow...</span>",\
							"<span_class='userdanger'>You start emitting a high-pitched buzzing, and your skin begins to glow...</span>")
