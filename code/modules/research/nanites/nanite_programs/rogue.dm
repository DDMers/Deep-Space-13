...
/datum/nanite_program/necrotic/active_effect()
	...
	if(prob(1))
		to_chat(host_mob, "<span_class='warning'>You feel a mild ache from somewhere inside you.</span>")
...
/datum/nanite_program/toxic/active_effect()
	...
	if(prob(1))
		to_chat(host_mob, "<span_class='warning'>You feel a bit sick.</span>")
...
/datum/nanite_program/suffocating/active_effect()
	...
	if(prob(1))
		to_chat(host_mob, "<span_class='warning'>You feel short of breath.</span>")
...
/datum/nanite_program/skin_decay/active_effect()
	...
	if(prob(5)) //itching
		...
		host_mob.visible_message("[can_scratch ? "<span_class='warning'>[host_mob] scratches [host_mob.p_their()] [bodypart.name].</span>" : ""]",\
		"<span_class='warning'>Your [bodypart.name] itches. [can_scratch ? " You scratch it." : ""]</span>")
...
/datum/nanite_program/nerve_decay/active_effect()
	if(prob(5))
		to_chat(host_mob, "<span_class='warning'>You feel unbalanced!</span>")
		host_mob.confused += 10
	else if(prob(4))
		to_chat(host_mob, "<span_class='warning'>You can't feel your hands!</span>")
		host_mob.drop_all_held_items()
	else if(prob(4))
		to_chat(host_mob, "<span_class='warning'>You can't feel your legs!</span>")
