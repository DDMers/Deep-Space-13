...
/obj/item/nanite_remote/examine(mob/user)
	. = ..()
	if(locked)
		to_chat(user, "<span_class='notice'>Alt-click to unlock.</span>")
...
/obj/item/nanite_remote/AltClick(mob/user)
	...
	if(locked)
		if(allowed(user))
			to_chat(user, "<span_class='notice'>You unlock [src].</span>")
			locked = FALSE
			update_icon()
		else
...
/obj/item/nanite_remote/emag_act(mob/user)
	if(obj_flags & EMAGGED)
		return
	to_chat(user, "<span_class='warning'>You override [src]'s ID lock.</span>")
	obj_flags |= EMAGGED
	if(locked)
		locked = FALSE
		update_icon()
...
/obj/item/nanite_remote/afterattack(atom/target, mob/user, etc)
	switch(mode)
		if(REMOTE_MODE_OFF)
			return
		if(REMOTE_MODE_SELF)
			to_chat(user, "<span_class='notice'>You activate [src], signaling the nanites in your bloodstream.<span>")
			signal_mob(user, code, key_name(user))
		if(REMOTE_MODE_TARGET)
			if(isliving(target) && (get_dist(target, get_turf(src)) <= 7))
				to_chat(user, "<span_class='notice'>You activate [src], signaling the nanites inside [target].<span>")
				signal_mob(target, code, key_name(user))
		if(REMOTE_MODE_AOE)
			to_chat(user, "<span_class='notice'>You activate [src], signaling the nanites inside every host around you.<span>")
			for(var/mob/living/L in view(user, 7))
				signal_mob(L, code, key_name(user))
		if(REMOTE_MODE_RELAY)
			to_chat(user, "<span_class='notice'>You activate [src], signaling all connected relay nanites.<span>")
