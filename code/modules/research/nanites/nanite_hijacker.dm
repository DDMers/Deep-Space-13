...
/obj/item/nanite_hijacker/examine(mob/user)
	. = ..()
	if(disk)
		to_chat(user, "<span_class='notice'>Alt-click [src] to eject the disk.</span>")
...
/obj/item/nanite_hijacker/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/disk/nanite_program))
		...
		if(user.transferItemToLoc(N, src))
			to_chat(user, "<span_class='notice'>You insert [N] into [src]</span>")
			disk = N
			program = N.program
	else
		..()
...
/obj/item/nanite_hijacker/afterattack(atom/target, mob/user, etc)
	...
	if(isliving(target))
		...
		switch(success)
			if(NONE)
				to_chat(user, "<span_class='notice'>You don't detect any nanites in [target].</span>")
			if(COMPONENT_PROGRAM_INSTALLED)
				to_chat(user, "<span_class='notice'>You insert the currently loaded program into [target]'s nanites.</span>")
			if(COMPONENT_PROGRAM_NOT_INSTALLED)
				to_chat(user, "<span_class='warning'>You try to insert the currently loaded program into [target]'s nanites, but the installation fails.</span>")
