...
/obj/item/slimecross/charged/grey/do_effect(mob/user)
	...
	user.visible_message("<span_class='notice'>[src] distills into a potion!</span>")
	..()
...
/obj/item/slimecross/charged/purple/do_effect(mob/user)
	...
	user.visible_message("<span_class='notice'>[src] sparks, and floods with a regenerative solution!</span>")
	..()
...
/obj/item/slimecross/charged/metal/do_effect(mob/user)
	...
	user.visible_message("<span_class='notice'>[src] grows into a plethora of metals!</span>")
	..()
...
/obj/item/slimecross/charged/yellow/do_effect(mob/user)
	...
	user.visible_message("<span_class='notice'>[src] sparks violently, and swells with electric power!</span>")
	..()
...
/obj/item/slimecross/charged/darkpurple/do_effect(mob/user)
	...
	user.visible_message("<span_class='notice'>[src] produces a large amount of plasma!</span>")
	..()
...
/obj/item/slimecross/charged/silver/do_effect(mob/user)
	...
	user.visible_message("<span_class='notice'>[src] produces a party's worth of cake and drinks!</span>")
	..()
...
/obj/item/slimecross/charged/bluespace/do_effect(mob/user)
	...
	user.visible_message("<span_class='notice'>[src] produces several sheets of polycrystal!</span>")
	..()
...
/obj/item/slimecross/charged/sepia/do_effect(mob/user)
	...
	user.visible_message("<span_class='notice'>[src] flickers in a strange, ethereal manner, and produces a camera!</span>")
	..()
...
/obj/item/slimecross/charged/pyrite/do_effect(mob/user)
	...
	user.visible_message("<span_class='warning'>[src] solidifies with a horrifying banana stench!</span>")
	..()
...
/obj/item/slimecross/charged/green/do_effect(mob/user)
	...
	if(!istype(H))
		to_chat(user, "<span_class='warning'>You must be a humanoid to use this!</span>")
		return
	var/racechoice = input(H, "Choose your slime subspecies.", "Slime Selection") as null|anything in subtypesof(/datum/species/jelly)
	if(!racechoice)
		to_chat(user, "<span_class='notice'>You decide not to become a slime for now.</span>")
		return
	if(!user.canUseTopic(src, BE_CLOSE))
		return
	H.set_species(racechoice, icon_update=1)
	H.visible_message("<span_class='warning'>[H] suddenly shifts form as [src] dissolves into [H.p_their()] skin!</span>")
	..()
...
/obj/item/slimecross/charged/gold/do_effect(mob/user)
	user.visible_message("<span_class='warning'>[src] starts shuddering violently!</span>")
...
/obj/item/slimecross/charged/gold/process()
	visible_message("<span_class='warning'>[src] lets off a spark, and produces a living creature!</span>")
	new /obj/effect/particle_effect/sparks(get_turf(src))
	playsound(get_turf(src), "sparks", 50, 1)
	create_random_mob(get_turf(src), HOSTILE_SPAWN)
	spawned++
	if(spawned >= max_spawn)
		visible_message("<span_class='warning'>[src] collapses into a puddle of goo.</span>")
		qdel(src)
...
/obj/item/slimecross/charged/black/do_effect(mob/user)
	var/mob/living/carbon/human/H = user
	if(!istype(H))
		to_chat(user, "<span_class='warning'>You have to be able to have a species to get your species changed.</span>")
		return
	...
	if(changed)
		...
		to_chat(H, "<span_class='danger'>You feel very different!</span>")
	..()
...
/obj/item/slimecross/charged/adamantine/do_effect(mob/user)
	user.visible_message("<span_class='notice'>[src] produces a fully formed golem shell!</span>")
...
/obj/item/slimecross/charged/rainbow/do_effect(mob/user)
	user.visible_message("<span_class='warning'>[src] swells and splits into three new slimes!</span>")
	for(var/i in 1 to 3)
