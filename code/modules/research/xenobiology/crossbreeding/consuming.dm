...
/obj/item/slimecross/consuming/attackby(obj/item/O, mob/user)
	if(istype(O,/obj/item/reagent_containers/food/snacks))
		if(last_produced + cooldown > world.time)
			to_chat(user, "<span_class='warning'>[src] is still digesting after its last meal!<span>")
			return
		var/datum/reagent/N = O.reagents.has_reagent("nutriment")
		if(N)
			nutriment_eaten += N.volume
			to_chat(user, "<span_class='notice'>[src] opens up and swallows [O] whole!</span>")
			qdel(O)
			playsound(src, 'sound/items/eatfood.ogg', 20, 1)
		else
			to_chat(user, "<span_class='warning'>[src] burbles unhappily at the offering.</span>")
		if(nutriment_eaten >= nutriment_required)
			nutriment_eaten = 0
			user.visible_message("<span_class='notice'>[src] swells up and produces a small pile of cookies!</span>")
...
/obj/item/slime_cookie/attack(mob/living/M, mob/user)
	var/fed = FALSE
	if(M == user)
		M.visible_message("<span_class='notice'>[user] eats [src]!</span>", "<span_class='notice'>You eat [src].</span>")
		fed = TRUE
	else
		M.visible_message("<span_class='danger'>[user] tries to force [M] to eat [src]!</span>", "<span_class='userdanger'>[user] tries to force you to eat [src]!</span>")
		if(do_after(user, 20, target = M))
			fed = TRUE
			M.visible_message("<span_class='danger'>[user] forces [M] to eat [src]!</span>", "<span_class='warning'>[user] forces you to eat [src].</span>")
	if(fed)
...
/obj/item/slime_cookie/cerulean/do_effect(mob/living/M, mob/user)
	if(prob(50))
		to_chat(M, "<span_class='notice'>A piece of [src] breaks off while you chew, and falls to the ground.</span>")
