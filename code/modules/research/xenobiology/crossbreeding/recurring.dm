...
/obj/item/slimecross/recurring/Initialize()
	...
	visible_message("<span_class='notice'>[src] wraps a layer of goo around itself!</span>")
	extract.name = name
	extract.desc = desc
	extract.icon = icon
...
/obj/item/slimecross/recurring/process()
	if(cooldown > 0)
		...
		extract.visible_message("<span_class='warning'>The light inside [extract] flickers and dies out.</span>")
