...
/obj/item/slimecross/stabilized/gold/attack_self(mob/user)
	...
	if(choice == "Familiar Location")
		to_chat(user, "<span_class='notice'>You prod [src], and it shudders slightly.</span>")
		START_PROCESSING(SSobj, src)
	if(choice == "Familiar Species")
		to_chat(user, "<span_class='notice'>You squeeze [src], and a shape seems to shift around inside.</span>")
		generate_mobtype()
		START_PROCESSING(SSobj, src)
	if(choice == "Familiar Sentience")
		to_chat(user, "<span_class='notice'>You poke [src], and it lets out a glowing pulse.</span>")
		saved_mind = null
		START_PROCESSING(SSobj, src)
	if(choice == "Familiar Name")
		...
		if(newname)
			mob_name = newname
		to_chat(user, "<span_class='notice'>You speak softly into [src], and it shakes slightly in response.</span>")
		START_PROCESSING(SSobj, src)
...
/obj/item/slimecross/stabilized/rainbow/attackby(obj/item/O, mob/user)
	...
	if(istype(regen) && !regencore)
		to_chat(user, "<span_class='notice'>You place [O] in [src], prepping the extract for automatic application!</span>")
		regencore = regen
		regen.forceMove(src)
		return
	return ..()
