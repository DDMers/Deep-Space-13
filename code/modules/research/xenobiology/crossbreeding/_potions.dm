...
/obj/item/slimepotion/extract_cloner/afterattack(obj/item/target, mob/user , proximity)
	...
	if(istype(target, /obj/item/slimecross))
		to_chat(user, "<span_class='warning'>[target] is too complex for the potion to clone!</span>")
		return
	...
	C.Uses = S.Uses
	to_chat(user, "<span_class='notice'>You pour the potion onto [target], and the fluid solidifies into a copy of it!</span>")
	qdel(src)
	return
...
/obj/item/slimepotion/peacepotion/attack(mob/living/M, mob/user)
	if(!isliving(M) || M.stat == DEAD)
		to_chat(user, "<span_class='warning'>The pacification potion only works on the living.</span>")
		return ..()
	if(M != user)
		M.visible_message("<span_class='danger'>[user] starts to feed [M] a pacification potion!</span>",
			"<span_class='userdanger'>[user] starts to feed you a pacification!</span>")
	else
		M.visible_message("<span_class='danger'>[user] starts to drink the pacification potion!</span>",
			"<span_class='danger'>You start to drink the pacification potion!</span>")

	if(!do_after(user, 100, target = M))
		return
	if(M != user)
		to_chat(user, "<span_class='notice'>You feed [M] the pacification potion!</span>")
	else
		to_chat(user, "<span_class='warning'>You drink the pacification potion!</span>")
	if(isanimal(M))
...
/obj/item/slimepotion/lovepotion/attack(mob/living/M, mob/user)
	if(!isliving(M) || M.stat == DEAD)
		to_chat(user, "<span_class='warning'>The love potion only works on living things, sicko!</span>")
		return ..()
	if(user == M)
		to_chat(user, "<span_class='warning'>You can't drink the love potion. What are you, a narcissist?</span>")
		return ..()
	if(M.has_status_effect(STATUS_EFFECT_INLOVE))
		to_chat(user, "<span_class='warning'>[M] is already lovestruck!</span>")
		return ..()

	M.visible_message("<span_class='danger'>[user] starts to feed [M] a love potion!</span>",
		"<span_class='userdanger'>[user] starts to feed you a love potion!</span>")

	if(!do_after(user, 50, target = M))
		return
	to_chat(user, "<span_class='notice'>You feed [M] the love potion!</span>")
	to_chat(M, "<span_class='notice'>You develop feelings for [user], and anyone [user.p_they()] like.</span>")
	if(M.mind)
...
/obj/item/slimepotion/spaceproof/afterattack(obj/item/clothing/C, mob/user, proximity)
	...
	if(C.min_cold_protection_temperature == SPACE_SUIT_MIN_TEMP_PROTECT && C.clothing_flags & STOPSPRESSUREDAMAGE)
		to_chat(user, "<span_class='warning'>The [C] is already pressure-resistant!</span>")
		return ..()
	to_chat(user, "<span_class='notice'>You slather the blue gunk over the [C], making it airtight.</span>")
...
/obj/item/slimepotion/lavaproof/afterattack(obj/item/C, mob/user, proximity)
	...
	if(!proximity)
		return ..()
	if(!istype(C))
		to_chat(user, "<span_class='warning'>You can't coat this with lavaproofing fluid!</span>")
		return ..()
	to_chat(user, "<span_class='notice'>You slather the red gunk over the [C], making it lavaproof.</span>")
...
/obj/item/slimepotion/slime_reviver/attack(mob/living/simple_animal/slime/M, mob/user)
	if(!isslime(M))
		...
		return ..()
	if(M.stat != DEAD)
		to_chat(user, "<span_class='warning'>The slime is still alive!</span>")
		return
	if(M.maxHealth <= 0)
		to_chat(user, "<span_class='warning'>The slime is too unstable to return!</span>")
	M.revive(full_heal = 1)
	M.stat = CONSCIOUS
	M.visible_message("<span_class='notice'>[M] is filled with renewed vigor and blinks awake!</span>")
...
/obj/item/slimepotion/slime/chargedstabilizer/attack(mob/living/simple_animal/slime/M, mob/user)
	...
	to_chat(user, "<span_class='notice'>You feed the slime the omnistabilizer. It will not mutate this cycle!</span>")
	M.mutation_chance = 0
	qdel(src)
