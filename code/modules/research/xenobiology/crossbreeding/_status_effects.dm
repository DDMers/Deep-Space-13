...
/datum/status_effect/rainbow_protection/on_apply()
	...
	owner.visible_message("<span_class='warning'>[owner] shines with a brilliant rainbow light.</span>",
		"<span_class='notice'>You feel protected by an unknown force!</span>")
...
/datum/status_effect/rainbow_protection/on_remove()
	...
	owner.visible_message("<span_class='notice'>[owner] stops glowing, the rainbow light fading away.</span>",
		"<span_class='warning'>You no longer feel protected...</span>")
...
/datum/status_effect/slimeskin/on_apply()
	...
	owner.visible_message("<span_class='warning'>[owner] is suddenly covered in a strange, blue-ish gel!</span>",
		"<span_class='notice'>You are covered in a thick, rubbery gel.</span>")
	return ..()
...
/datum/status_effect/slimeskin/on_remove()
	...
	if(ishuman(owner))
		var/mob/living/carbon/human/H = owner
		H.physiology.damage_resistance -= 10
	owner.visible_message("<span_class='warning'>[owner]'s gel coating liquefies and dissolves away.</span>",
		"<span_class='notice'>Your gel second-skin dissolves!</span>")
...
/datum/status_effect/slimerecall/on_apply()
	...
	to_chat(owner, "<span_class='danger'>You feel a sudden tug from an unknown force, and feel a pull to bluespace!</span>")
	to_chat(owner, "<span_class='notice'>Resist if you wish avoid the force!</span>")
...
/datum/status_effect/slimerecall/on_remove()
	...
	if(interrupted || !ismob(target))
		to_chat(owner, "<span_class='warning'>The bluespace tug fades away, and you feel that the force has passed you by.</span>")
		return
	owner.visible_message("<span_class='warning'>[owner] disappears in a flurry of sparks!</span>",
		"<span_class='warning'>The unknown force snatches briefly you from reality, and deposits you next to [target]!</span>")
...
/datum/status_effect/lovecookie/tick()
	...
	if(length(huggables))
		...
		owner.visible_message("<span_class='notice'>[owner] hugs [hugged]!</span>", "<span_class='notice'>You hug [hugged]!</span>")
...
/datum/status_effect/stabilized/grey/tick()
	for(var/mob/living/simple_animal/slime/S in range(1, get_turf(owner)))
		if(!(owner in S.Friends))
			to_chat(owner, "<span_class='notice'>[linked_extract] pulses gently as it communicates with [S]</span>")
			S.Friends[owner] = 1
	return ..()
...
/datum/status_effect/stabilized/purple/tick()
	...
	if(owner.getToxLoss() > 0)
		...
		is_healing = TRUE
	if(is_healing)
		examine_text = "<span_class='warning'>SUBJECTPRONOUN is regenerating slowly, purplish goo filling in small injuries!</span>"
...
/datum/status_effect/stabilized/metal/tick()
	if(cooldown > 0)
		...
		if(sheets.len > 0)
			...
			S.amount++
			to_chat(owner, "<span_class='notice'>[linked_extract] adds a layer of slime to [S], which metamorphosizes into another sheet of material!</span>")
	return ..()
...
/datum/status_effect/stabilized/yellow
	...
	var/cooldown = 10
	var/max_cooldown = 10
	examine_text = "<span_class='warning'>Nearby electronics seem just a little more charged wherever SUBJECTPRONOUN goes.</span>"
...
/datum/status_effect/stabilized/yellow/tick()
	...
	if(batteries.len)
		...
		to_chat(owner, "<span_class='notice'>[linked_extract] discharges some energy into a device you have.</span>")
	return ..()
...
/datum/status_effect/stabilized/darkpurple
	...
	examine_text = "<span_class='notice'>Their fingertips burn brightly!</span>"
...
/datum/status_effect/stabilized/darkpurple/tick()
	...
	if(istype(F))
		if(F.cooked_type)
			to_chat(owner, "<span_class='warning'>[linked_extract] flares up brightly, and your hands alone are enough cook [F]!</span>")
...
/datum/status_effect/stabilized/darkblue/tick()
	if(owner.fire_stacks > 0 && prob(80))
		...
		if(owner.fire_stacks <= 0)
			to_chat(owner, "<span_class='notice'>[linked_extract] coats you in a watery goo, extinguishing the flames.</span>")
	...
	if(istype(O, /obj/item/reagent_containers/food/snacks/monkeycube))
		to_chat(owner, "<span_class='warning'>[linked_extract] kept your hands wet! It makes [O] expand!</span>")
	...
	// Dehydrated carp
		...
		to_chat(owner, "<span_class='warning'>[linked_extract] kept your hands wet! It wets [O]!</span>")
...
/datum/status_effect/stabilized/bluespace/tick()
	...
	if(healthcheck && (healthcheck - owner.health) > 5)
		owner.visible_message("<span_class='warning'>[linked_extract] notices the sudden change in [owner]'s physical health, and activates!</span>")
		...
		if(do_teleport(owner, F, range, channel = TELEPORT_CHANNEL_BLUESPACE))
			to_chat(owner, "<span_class='notice'>[linked_extract] will take some time to re-align you on the bluespace axis.</span>")
			do_sparks(5,FALSE,owner)
...
/datum/status_effect/stabilized/cerulean/tick()
	if(owner.stat == DEAD)
		if(clone && clone.stat != DEAD)
			owner.visible_message("<span_class='warning'>[owner] blazes with brilliant light, [linked_extract] whisking [owner.p_their()] soul away.</span>",
				"<span_class='notice'>You feel a warm glow from [linked_extract], and you open your eyes... elsewhere.</span>")
			if(owner.mind)
				owner.mind.transfer_to(clone)
			clone = null
			qdel(linked_extract)
		if(!clone || clone.stat == DEAD)
			to_chat(owner, "<span_class='notice'>[linked_extract] desperately tries to move your soul to a living body, but can't find one!</span>")
			qdel(linked_extract)
	..()

/datum/status_effect/stabilized/cerulean/on_remove()
	if(clone)
		clone.visible_message("<span_class='warning'>[clone] dissolves into a puddle of goo!</span>")
		clone.unequip_everything()
		qdel(clone)
...
/datum/status_effect/stabilized/green/on_apply()
	to_chat(owner, "<span_class='warning'>You feel different...</span>")
	if(ishuman(owner))
		var/mob/living/carbon/human/H = owner
...
/datum/status_effect/stabilized/green/tick() //Only occasionally give examiners a warning.
	if(prob(50))
		examine_text = "<span_class='warning'>SUBJECTPRONOUN looks a bit green and gooey...</span>"
	else
		examine_text = null
	return ..()

/datum/status_effect/stabilized/green/on_remove()
	to_chat(owner, "<span_class='notice'>You feel more like yourself.</span>")
	if(ishuman(owner))
		var/mob/living/carbon/human/H = owner
...
/datum/status_effect/stabilized/pink/tick()
	...
	if(HasFaction && owner.has_status_effect(/datum/status_effect/brokenpeace))
		...
		to_chat(owner, "<span_class='userdanger'>The peace has been broken! Hostile creatures will now react to you!</span>")
	if(!HasFaction && !owner.has_status_effect(/datum/status_effect/brokenpeace))
		to_chat(owner, "<span_class='notice'>[linked_extract] pulses, generating a fragile aura of peace.</span>")
		owner.faction |= faction_name
	return ..()
...
/datum/status_effect/stabilized/oil
	id = "stabilizedoil"
	colour = "oil"
	examine_text = "<span_class='warning'>SUBJECTPRONOUN smells of sulfer and oil!</span>"
...
/datum/status_effect/stabilized/black/tick()
	if(owner.pulling && isliving(owner.pulling) && owner.grab_state == GRAB_KILL)
		...
		if(M.stat == DEAD)
			return
		if(!messagedelivered)
			to_chat(owner,"<span_class='notice'>You feel your hands melt around [M]'s neck and start to drain [M.p_them()] of life.</span>")
			to_chat(owner.pulling, "<span_class='userdanger'>[owner]'s hands melt around your neck, and you can feel your life starting to drain away!</span>")
			messagedelivered = TRUE
		examine_text = "<span_class='warning'>SUBJECTPRONOUN is draining health from [owner.pulling]!</span>"
...
/datum/status_effect/stabilized/adamantine
	...
	colour = "adamantine"
	examine_text = "<span_class='warning'>SUBJECTPRONOUN has a strange metallic coating on their skin.</span>"
	...
	if(owner.health <= 0)
		...
		if(istype(X))
			if(X.regencore)
				...
				X.regencore = null
				owner.visible_message("<span_class='warning'>[owner] flashes a rainbow of colors, and [owner.p_their()] skin is coated in a milky regenerative goo!</span>")
				qdel(src)
				qdel(linked_extract)
	return ..()
