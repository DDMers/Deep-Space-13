...
/obj/structure/light_prism/attack_hand(mob/user)
	to_chat(user, "<span_class='notice'>You dispel [src]</span>")
	qdel(src)
...
/datum/action/item_action/place_light_prism/Trigger()
	...
	if(locate(/obj/structure/light_prism) in get_turf(owner))
		to_chat(owner, "<span_class='warning'>There isn't enough ambient energy to fabricate another light prism here.</span>")
		return
	if(istype(glasses))
		if(!glasses.glasses_color)
			to_chat(owner, "<span_class='warning'>The lens is oddly opaque...</span>")
			return
		to_chat(owner, "<span_class='notice'>You channel nearby light into a glowing, ethereal prism.</span>")
...
/obj/item/clothing/head/peaceflower/attack_hand(mob/user)
	if(iscarbon(user))
		...
		if(src == C.head)
			to_chat(user, "<span_class='warning'>You feel at peace. <b style='color:pink'>Why would you want anything else?</b></span>")
			return
	return ..()
