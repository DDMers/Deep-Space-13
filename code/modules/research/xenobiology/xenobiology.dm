...
/obj/item/slime_extract/attackby(obj/item/O, mob/user)
	if(istype(O, /obj/item/slimepotion/enhancer))
		if(Uses >= 5 || recurring)
			to_chat(user, "<span_class='warning'>You cannot enhance this extract further!</span>")
			return ..()
		if(O.type == /obj/item/slimepotion/enhancer) //Seriously, why is this defined here...?
			to_chat(user, "<span_class='notice'>You apply the enhancer to the slime extract. It may now be reused one more time.</span>")
			Uses++
		if(O.type == /obj/item/slimepotion/enhancer/max)
			to_chat(user, "<span_class='notice'>You dump the maximizer on the slime extract. It can now be used a total of 5 times!</span>")
			Uses = 5
		qdel(O)
	..()
...
/obj/item/slime_extract/proc/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	to_chat(user, "<span_class='notice'>Nothing happened... This slime extract cannot be activated this way.</span>")
	return 0
...
/obj/item/slime_extract/attack(mob/living/simple_animal/slime/M, mob/user)
	if(!isslime(M))
		return ..()
	if(M.stat)
		to_chat(user, "<span_class='warning'>The slime is dead!</span>")
		return
	if(!M.is_adult)
		to_chat(user, "<span_class='warning'>The slime must be an adult to cross its core!</span>")
		return
	if(M.effectmod && M.effectmod != effectmod)
		to_chat(user, "<span_class='warning'>The slime is already being crossed with a different extract!</span>")
		return
	...
	M.applied++
	qdel(src)
	to_chat(user, "<span_class='notice'>You feed the slime [src], [M.applied == 1 ? "starting to mutate its core." : "further mutating its core."]</span>")
...
/obj/item/slime_extract/grey/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			...
			to_chat(user, "<span_class='notice'>You spit out a monkey cube.</span>")
			return 120
		if(SLIME_ACTIVATE_MAJOR)
			to_chat(user, "<span_class='notice'>Your [name] starts pulsing...</span>")
			if(do_after(user, 40, target = user))
				var/mob/living/simple_animal/slime/S = new(get_turf(user), "grey")
				playsound(user, 'sound/effects/splat.ogg', 50, 1)
				to_chat(user, "<span_class='notice'>You spit out [S].</span>")
				return 350
			else
				return 0
...
/obj/item/slime_extract/gold/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			user.visible_message("<span_class='warning'>[user] starts shaking!</span>","<span_class='notice'>Your [name] starts pulsing gently...</span>")
			if(do_after(user, 40, target = user))
				var/mob/living/simple_animal/S = create_random_mob(user.drop_location(), FRIENDLY_SPAWN)
				S.faction |= "neutral"
				playsound(user, 'sound/effects/splat.ogg', 50, 1)
				user.visible_message("<span_class='warning'>[user] spits out [S]!</span>", "<span_class='notice'>You spit out [S]!</span>")
				return 300

		if(SLIME_ACTIVATE_MAJOR)
			user.visible_message("<span_class='warning'>[user] starts shaking violently!</span>","<span_class='warning'>Your [name] starts pulsing violently...</span>")
			if(do_after(user, 50, target = user))
				...
				if(user.a_intent != INTENT_HARM)
					...
					S.faction |= "slime"
				playsound(user, 'sound/effects/splat.ogg', 50, 1)
				user.visible_message("<span_class='warning'>[user] spits out [S]!</span>", "<span_class='warning'>You spit out [S]!</span>")
				return 600
...
/obj/item/slime_extract/silver/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			...
			playsound(user, 'sound/effects/splat.ogg', 50, 1)
			user.visible_message("<span_class='warning'>[user] spits out [O]!</span>", "<span_class='notice'>You spit out [O]!</span>")
			return 200
		if(SLIME_ACTIVATE_MAJOR)
...
/obj/item/slime_extract/purple/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			...
			to_chat(user, "<span_class='notice'>You activate [src], and your body is refilled with fresh slime jelly!</span>")
			return 150

		if(SLIME_ACTIVATE_MAJOR)
			to_chat(user, "<span_class='notice'>You activate [src], and it releases regenerative chemicals!</span>")
...
/obj/item/slime_extract/darkpurple/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		...
		if(SLIME_ACTIVATE_MAJOR)
			...
			to_chat(user, "<span_class='warning'>You activate [src], and a cloud of plasma bursts out of your skin!</span>")
			return 900
...
/obj/item/slime_extract/orange/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			to_chat(user, "<span_class='notice'>You activate [src]. You start feeling hot!</span>")
		...
		if(SLIME_ACTIVATE_MAJOR)
			...
			to_chat(user, "<span_class='warning'>You activate [src], and a cloud of smoke bursts out of your skin!</span>")
			return 450
	...
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			if(species.glow_intensity != LUMINESCENT_DEFAULT_GLOW)
				to_chat(user, "<span_class='warning'>Your glow is already enhanced!</span>")
				return
			...
			to_chat(user, "<span_class='notice'>You start glowing brighter.</span>")

		if(SLIME_ACTIVATE_MAJOR)
			user.visible_message("<span_class='warning'>[user]'s skin starts flashing intermittently...</span>", "<span_class='warning'>Your skin starts flashing intermittently...</span>")
			if(do_after(user, 25, target = user))
				empulse(user, 1, 2)
				user.visible_message("<span_class='warning'>[user]'s skin flashes!</span>", "<span_class='warning'>Your skin flashes as you emit an electromagnetic pulse!</span>")
				return 600
...
/obj/item/slime_extract/red/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			to_chat(user, "<span_class='notice'>You activate [src]. You start feeling fast!</span>")
			user.reagents.add_reagent("ephedrine",5)
			return 450

		if(SLIME_ACTIVATE_MAJOR)
			user.visible_message("<span_class='warning'>[user]'s skin flashes red for a moment...</span>", "<span_class='warning'>Your skin flashes red as you emit rage-inducing pheromones...</span>")
			for(var/mob/living/simple_animal/slime/slime in viewers(get_turf(user), null))
				slime.rabid = TRUE
				slime.visible_message("<span_class='danger'>The [slime] is driven into a frenzy!</span>")
			return 600
...
/obj/item/slime_extract/blue/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			to_chat(user, "<span_class='notice'>You activate [src]. Your genome feels more stable!</span>")
		...
		if(SLIME_ACTIVATE_MAJOR)
			...
			user.visible_message("<span_class='danger'>Foam spews out from [user]'s skin!</span>", "<span_class='warning'>You activate [src], and foam bursts out of your skin!</span>")
			return 600
...
/obj/item/slime_extract/darkblue/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			to_chat(user, "<span_class='notice'>You activate [src]. You start feeling colder!</span>")
			user.ExtinguishMob()
		...
		if(SLIME_ACTIVATE_MAJOR)
			...
			to_chat(user, "<span_class='warning'>You activate [src], and icy air bursts out of your skin!</span>")
			return 900
	...
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			if(user.gender != MALE && user.gender != FEMALE)
				to_chat(user, "<span_class='warning'>You can't swap your gender!</span>")
				return

			if(user.gender == MALE)
				user.gender = FEMALE
				user.visible_message("<span_class='boldnotice'>[user] suddenly looks more feminine!</span>", "<span_class='boldwarning'>You suddenly feel more feminine!</span>")
			else
				user.gender = MALE
				user.visible_message("<span_class='boldnotice'>[user] suddenly looks more masculine!</span>", "<span_class='boldwarning'>You suddenly feel more masculine!</span>")
			return 100

		if(SLIME_ACTIVATE_MAJOR)
			user.visible_message("<span_class='warning'>[user]'s skin starts flashing hypnotically...</span>", "<span_class='notice'>Your skin starts forming odd patterns, pacifying creatures around you.</span>")
...
/obj/item/slime_extract/green/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			to_chat(user, "<span_class='warning'>You feel yourself reverting to human form...</span>")
			if(do_after(user, 120, target = user))
				to_chat(user, "<span_class='warning'>You feel human again!</span>")
				user.set_species(/datum/species/human)
				return
			to_chat(user, "<span_class='notice'>You stop the transformation.</span>")

		if(SLIME_ACTIVATE_MAJOR)
			to_chat(user, "<span_class='warning'>You feel yourself radically changing your slime type...</span>")
			if(do_after(user, 120, target = user))
				to_chat(user, "<span_class='warning'>You feel different!</span>")
...
/obj/item/slime_extract/black/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			to_chat(user, "<span_class='userdanger'>You feel something <i>wrong</i> inside you...</span>")
			user.ForceContractDisease(new /datum/disease/transformation/slime(), FALSE, TRUE)
			return 100

		if(SLIME_ACTIVATE_MAJOR)
			to_chat(user, "<span_class='warning'>You feel your own light turning dark...</span>")
			if(do_after(user, 120, target = user))
				to_chat(user, "<span_class='warning'>You feel a longing for darkness.</span>")
				user.set_species(pick(/datum/species/shadow))
				return
			to_chat(user, "<span_class='notice'>You stop feeding [src].</span>")
...
/obj/item/slime_extract/oil/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			to_chat(user, "<span_class='warning'>You vomit slippery oil.</span>")
			playsound(user, 'sound/effects/splat.ogg', 50, 1)
			new /obj/effect/decal/cleanable/oil/slippery(get_turf(user))
			return 450

		if(SLIME_ACTIVATE_MAJOR)
			user.visible_message("<span_class='warning'>[user]'s skin starts pulsing and glowing ominously...</span>", "<span_class='userdanger'>You feel unstable...</span>")
			if(do_after(user, 60, target = user))
				to_chat(user, "<span_class='userdanger'>You explode!</span>")
				explosion(get_turf(user), 1 ,3, 6)
				user.gib()
				return
			to_chat(user, "<span_class='notice'>You stop feeding [src], and the feeling passes.</span>")
...
/obj/item/slime_extract/adamantine/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			if(species.armor > 0)
				to_chat(user, "<span_class='warning'>Your skin is already hardened!</span>")
				return
			to_chat(user, "<span_class='notice'>You feel your skin harden and become more resistant.</span>")
			species.armor += 25
			addtimer(CALLBACK(src, .proc/reset_armor, species), 1200)
			return 450

		if(SLIME_ACTIVATE_MAJOR)
			to_chat(user, "<span_class='warning'>You feel your body rapidly crystallizing...</span>")
			if(do_after(user, 120, target = user))
				to_chat(user, "<span_class='warning'>You feel solid.</span>")
				user.set_species(pick(/datum/species/golem/adamantine))
				return
			to_chat(user, "<span_class='notice'>You stop feeding [src], and your body returns to its slimelike state.</span>")
...
/obj/item/slime_extract/bluespace/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			to_chat(user, "<span_class='warning'>You feel your body vibrating...</span>")
			if(do_after(user, 25, target = user))
				to_chat(user, "<span_class='warning'>You teleport!</span>")
				do_teleport(user, get_turf(user), 6, asoundin = 'sound/weapons/emitter2.ogg', channel = TELEPORT_CHANNEL_BLUESPACE)
				return 300

		if(SLIME_ACTIVATE_MAJOR)
			if(!teleport_ready)
				to_chat(user, "<span_class='notice'>You feel yourself anchoring to this spot...</span>")
				...
				if(teleport_x && teleport_y && teleport_z)
					...
					to_chat(user, "<span_class='notice'>You snap back to your anchor point!</span>")
...
/obj/item/slime_extract/cerulean/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			...
			to_chat(user, "<span_class='notice'>You feel like you don't need to breathe!</span>")
			return 150
		...
		if(SLIME_ACTIVATE_MAJOR)
			...
			if(istype(T))
				...
				to_chat(user, "<span_class='warning'>You activate [src], and fresh air bursts out of your skin!</span>")
				return 600
...
/obj/item/slime_extract/sepia/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		...
		if(SLIME_ACTIVATE_MAJOR)
			to_chat(user, "<span_class='warning'>You feel time slow down...</span>")
...
/obj/item/slime_extract/rainbow/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			...
			to_chat(user, "<span_class='notice'>You feel different...</span>")
			return 100
...
/obj/item/slimepotion/afterattack(obj/item/reagent_containers/target, mob/user , proximity)
	. = ..()
	if (istype(target))
		to_chat(user, "<span_class='notice'>You cannot transfer [src] to [target]! It appears the potion must be given directly to a slime to absorb.</span>" )
		return
...
/obj/item/slimepotion/slime/docility/attack(mob/living/simple_animal/slime/M, mob/user)
	if(!isslime(M))
		to_chat(user, "<span_class='warning'>The potion only works on slimes!</span>")
		return ..()
	if(M.stat)
		to_chat(user, "<span_class='warning'>The slime is dead!</span>")
		return
	...
	M.docile = 1
	M.set_nutrition(700)
	to_chat(M, "<span_class='warning'>You absorb the potion and feel your intense desire to feed melt away.</span>")
	to_chat(user, "<span_class='notice'>You feed the slime the potion, removing its hunger and calming it.</span>")
...
/obj/item/slimepotion/slime/sentience/attack(mob/living/M, mob/user)
	...
	if(!isanimal(M) || M.ckey) //only works on animals that aren't player controlled
		to_chat(user, "<span_class='warning'>[M] is already too intelligent for this to work!</span>")
		return
	if(M.stat)
		to_chat(user, "<span_class='warning'>[M] is dead!</span>")
		return
	var/mob/living/simple_animal/SM = M
	if(SM.sentience_type != sentience_type)
		to_chat(user, "<span_class='warning'>[src] won't work on [SM].</span>")
		return

	to_chat(user, "<span_class='notice'>You offer [src] to [SM]...</span>")
	being_used = TRUE
	...
	if(LAZYLEN(candidates))
		...
		SM.sentience_act()
		to_chat(SM, "<span_class='warning'>All at once it makes sense: you know what you are and who you are! Self awareness is yours!</span>")
		to_chat(SM, "<span_class='userdanger'>You are grateful to be self aware and owe [user.real_name] a great debt. Serve [user.real_name], and assist [user.p_them()] in completing [user.p_their()] goals at any cost.</span>")
		if(SM.flags_1 & HOLOGRAM_1) //Check to see if it's a holodeck creature
			to_chat(SM, "<span_class='userdanger'>You also become depressingly aware that you are not a real creature, but instead a holoform. Your existence is limited to the parameters of the holodeck.</span>")
		to_chat(user, "<span_class='notice'>[SM] accepts [src] and suddenly becomes attentive and aware. It worked!</span>")
		SM.copy_known_languages_from(user, FALSE)
		after_success(user, SM)
		qdel(src)
	else
		to_chat(user, "<span_class='notice'>[SM] looks interested for a moment, but then looks back down. Maybe you should try again later.</span>")
		being_used = FALSE
		..()
...
/obj/item/slimepotion/transference/afterattack(mob/living/M, mob/user)
	...
	if(!isanimal(M) || M.ckey) //much like sentience, these will not work on something that is already player controlled
		to_chat(user, "<span_class='warning'>[M] already has a higher consciousness!</span>")
		return ..()
	if(M.stat)
		to_chat(user, "<span_class='warning'>[M] is dead!</span>")
		return ..()
	...
	if(SM.sentience_type != animal_type)
		to_chat(user, "<span_class='warning'>You cannot transfer your consciousness to [SM].</span>" )
		return ..()
	...
	if(jb)
		to_chat(user, "<span_class='warning'>Your mind goes blank as you attempt to use the potion.</span>")
		return
	...
	to_chat(user, "<span_class='notice'>You drink the potion then place your hands on [SM]...</span>")
	...
	user.death()
	to_chat(SM, "<span_class='notice'>In a quick flash, you feel your consciousness flow into [SM]!</span>")
	to_chat(SM, "<span_class='warning'>You are now [SM]. Your allegiances, alliances, and role is still the same as it was prior to consciousness transfer!</span>")
...
/obj/item/slimepotion/slime/steroid/attack(mob/living/simple_animal/slime/M, mob/user)
	if(!isslime(M))//If target is not a slime.
		to_chat(user, "<span_class='warning'>The steroid only works on baby slimes!</span>")
		return ..()
	if(M.is_adult) //Can't steroidify adults
		to_chat(user, "<span_class='warning'>Only baby slimes can use the steroid!</span>")
		return
	if(M.stat)
		to_chat(user, "<span_class='warning'>The slime is dead!</span>")
		return
	if(M.cores >= 5)
		to_chat(user, "<span_class='warning'>The slime already has the maximum amount of extract!</span>")
		return

	to_chat(user, "<span_class='notice'>You feed the slime the steroid. It will now produce one more extract.</span>")
	M.cores++
	qdel(src)
...
/obj/item/slimepotion/slime/stabilizer/attack(mob/living/simple_animal/slime/M, mob/user)
	if(!isslime(M))
		to_chat(user, "<span_class='warning'>The stabilizer only works on slimes!</span>")
		return ..()
	if(M.stat)
		to_chat(user, "<span_class='warning'>The slime is dead!</span>")
		return
	if(M.mutation_chance == 0)
		to_chat(user, "<span_class='warning'>The slime already has no chance of mutating!</span>")
		return

	to_chat(user, "<span_class='notice'>You feed the slime the stabilizer. It is now less likely to mutate.</span>")
...
/obj/item/slimepotion/slime/mutator/attack(mob/living/simple_animal/slime/M, mob/user)
	if(!isslime(M))
		to_chat(user, "<span_class='warning'>The mutator only works on slimes!</span>")
		return ..()
	if(M.stat)
		to_chat(user, "<span_class='warning'>The slime is dead!</span>")
		return
	if(M.mutator_used)
		to_chat(user, "<span_class='warning'>This slime has already consumed a mutator, any more would be far too unstable!</span>")
		return
	if(M.mutation_chance == 100)
		to_chat(user, "<span_class='warning'>The slime is already guaranteed to mutate!</span>")
		return

	to_chat(user, "<span_class='notice'>You feed the slime the mutator. It is now more likely to mutate.</span>")
...
/obj/item/slimepotion/speed/afterattack(obj/C, mob/user)
	. = ..()
	if(!istype(C))
		to_chat(user, "<span_class='warning'>The potion can only be used on items or vehicles!</span>")
		return
	if(isitem(C))
		var/obj/item/I = C
		if(I.slowdown <= 0 || I.obj_flags & IMMUTABLE_SLOW)
			to_chat(user, "<span_class='warning'>The [C] can't be made any faster!</span>")
			return ..()
		I.slowdown = 0
	...
	to_chat(user, "<span_class='notice'>You slather the red gunk over the [C], making it faster.</span>")
...
/obj/item/slimepotion/fireproof/afterattack(obj/item/clothing/C, mob/user)
	...
	if(!uses)
		...
		return
	if(!istype(C))
		to_chat(user, "<span_class='warning'>The potion can only be used on clothing!</span>")
		return
	if(C.max_heat_protection_temperature >= FIRE_IMMUNITY_MAX_TEMP_PROTECT)
		to_chat(user, "<span_class='warning'>The [C] is already fireproof!</span>")
		return ..()
	to_chat(user, "<span_class='notice'>You slather the blue gunk over the [C], fireproofing it.</span>")
...
/obj/item/slimepotion/genderchange/attack(mob/living/L, mob/user)
	if(!istype(L) || L.stat == DEAD)
		to_chat(user, "<span_class='warning'>The potion can only be used on living things!</span>")
		return

	if(L.gender != MALE && L.gender != FEMALE)
		to_chat(user, "<span_class='warning'>The potion can only be used on gendered things!</span>")
		return

	if(L.gender == MALE)
		L.gender = FEMALE
		L.visible_message("<span_class='boldnotice'>[L] suddenly looks more feminine!</span>", "<span_class='boldwarning'>You suddenly feel more feminine!</span>")
	else
		L.gender = MALE
		L.visible_message("<span_class='boldnotice'>[L] suddenly looks more masculine!</span>", "<span_class='boldwarning'>You suddenly feel more masculine!</span>")
	L.regenerate_icons()
	qdel(src)
...
/obj/item/slimepotion/slime/renaming/attack(mob/living/M, mob/user)
	...
	if(!M.ckey) //only works on animals that aren't player controlled
		to_chat(user, "<span_class='warning'>[M] is not self aware, and cannot pick its own name.</span>")
		return

	being_used = TRUE

	to_chat(user, "<span_class='notice'>You offer [src] to [user]...</span>")
	...
	M.visible_message("<span_class='notice'><span_class='name'>[M]</span> has a new name, <span_class='name'>[new_name]</span>.</span>", "<span_class='notice'>Your old name of <span_class='name'>[M.real_name]</span> fades away, and your new name <span_class='name'>[new_name]</span> anchors itself in your mind.</span>")
...
/obj/item/slimepotion/slime/slimeradio/attack(mob/living/M, mob/user)
	if(!ismob(M))
		return
	if(!isanimal(M))
		to_chat(user, "<span_class='warning'>[M] is too complex for the potion!</span>")
		return
	if(M.stat)
		to_chat(user, "<span_class='warning'>[M] is dead!</span>")
		return

	to_chat(user, "<span_class='notice'>You feed the potion to [M].</span>")
	to_chat(M, "<span_class='notice'>Your mind tingles as you are fed the potion. You can hear radio waves now!</span>")
