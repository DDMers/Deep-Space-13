...
/obj/machinery/rnd/destructive_analyzer/Insert_Item(obj/item/O, mob/user)
	if(user.a_intent != INTENT_HARM)
		...
		if(!user.transferItemToLoc(O, src))
			to_chat(user, "<span_class='warning'>\The [O] is stuck to your hand, you cannot put it in the [src.name]!</span>")
			return
		busy = TRUE
		loaded_item = O
		to_chat(user, "<span_class='notice'>You add the [O.name] to the [src.name]!</span>")
