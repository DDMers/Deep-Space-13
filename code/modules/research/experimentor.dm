...
/obj/machinery/rnd/experimentor/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		to_chat(user, "<span_class='notice'>The status display reads: Malfunction probability reduced by <b>[badThingCoeff]%</b>.<br>Cooldown interval between experiments at <b>[resetTime*0.1]</b> seconds.<span>")
...
/obj/machinery/rnd/experimentor/Insert_Item(obj/item/O, mob/user)
	if(user.a_intent != INTENT_HARM)
		...
		if(!user.transferItemToLoc(O, src))
			return
		loaded_item = O
		to_chat(user, "<span_class='notice'>You add [O] to the machine.</span>")
...
/obj/machinery/rnd/experimentor/Topic(href, href_list)
	...
	if(scantype == "search")
		...
		if(recentlyExperimented)
			to_chat(usr, "<span_class='warning'>[src] has been used too recently!</span>")
		else if(!loaded_item)
			to_chat(usr, "<span_class='warning'>[src] is not currently loaded!</span>")
			...
			to_chat(usr, "<span_class='danger'>Interface failure detected in [src]. Please try again.</span>")
		else
			var/dotype
...
/obj/machinery/rnd/experimentor/proc/ejectItem(delete=FALSE)
	if(loaded_item)
		if(cloneMode)
			visible_message("<span_class='notice'>A duplicate [loaded_item] pops out!</span>")
...
/obj/machinery/rnd/experimentor/proc/experiment(exp,obj/item/exp_on)
	...
	if(exp == SCANTYPE_POKE)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			visible_message("<span_class='danger'>[src] malfunctions and destroys [exp_on], lashing its arms out at nearby people!</span>")
			...
			visible_message("<span_class='warning'>[src] malfunctions!</span>")
			exp = SCANTYPE_OBLITERATE
		else if(prob(EFFECT_PROB_MEDIUM-badThingCoeff))
			visible_message("<span_class='danger'>[src] malfunctions, throwing the [exp_on]!</span>")
	...
	if(exp == SCANTYPE_IRRADIATE)
		visible_message("<span_class='danger'>[src] reflects radioactive rays at [exp_on]!</span>")
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			visible_message("<span_class='danger'>[src] malfunctions, melting [exp_on] and leaking radiation!</span>")
			radiation_pulse(src, 500)
			ejectItem(TRUE)
		else if(prob(EFFECT_PROB_LOW-badThingCoeff))
			visible_message("<span_class='warning'>[src] malfunctions, spewing toxic waste!</span>")
			...
			visible_message("<span_class='warning'>[src] malfunctions, transforming [savedName] into [loaded_item]!</span>")
	...
	////////////////////////////////////////////////////////////////////////////////////////////////
	if(exp == SCANTYPE_GAS)
		visible_message("<span_class='warning'>[src] fills its chamber with gas, [exp_on] included.</span>")
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			visible_message("[exp_on] achieves the perfect mix!")
			new /obj/item/stack/sheet/mineral/plasma(get_turf(pick(oview(1,src))))
		else if(prob(EFFECT_PROB_VERYLOW-badThingCoeff))
			visible_message("<span_class='danger'>[src] destroys [exp_on], leaking dangerous gas!</span>")
			...
			visible_message("<span_class='danger'>[src]'s chemical chamber has sprung a leak!</span>")
			...
			visible_message("<span_class='warning'>[src] melts [exp_on], ionizing the air around it!</span>")
			empulse(loc, 4, 6)
	...
	if(exp == SCANTYPE_HEAT)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			visible_message("<span_class='warning'>[src]'s emergency coolant system gives off a small ding!</span>")
			...
			if(MT)
				visible_message("<span_class='danger'>[src] dangerously overheats, launching a flaming fuel orb!</span>")
			...
			visible_message("<span_class='danger'>[src] malfunctions, melting [exp_on] and releasing a burst of flame!</span>")
			explosion(loc, -1, 0, 0, 0, 0, flame_range = 2)
			investigate_log("Experimentor started a fire.", INVESTIGATE_EXPERIMENTOR)
			ejectItem(TRUE)
		else if(prob(EFFECT_PROB_MEDIUM-badThingCoeff))
			visible_message("<span_class='warning'>[src] malfunctions, melting [exp_on] and leaking hot air!</span>")
			...
			visible_message("<span_class='warning'>[src] malfunctions, activating its emergency coolant systems!</span>")
			throwSmoke(loc)
			for(var/mob/living/m in oview(1, src))
	...
	if(exp == SCANTYPE_COLD)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			visible_message("<span_class='danger'>[src] malfunctions, shattering [exp_on] and releasing a dangerous cloud of coolant!</span>")
			var/datum/reagents/R = new/datum/reagents(50)
			R.my_atom = src
			...
			visible_message("<span_class='warning'>[src] malfunctions, shattering [exp_on] and leaking cold air!</span>")
			var/datum/gas_mixture/env = loc.return_air()
			var/transfer_moles = 0.25 * env.total_moles()
			var/datum/gas_mixture/removed = env.remove(transfer_moles)
			if(removed)
			...
			visible_message("<span_class='warning'>[src] malfunctions, releasing a flurry of chilly air as [exp_on] pops out!</span>")
			var/datum/effect_system/smoke_spread/smoke = new
			smoke.set_up(0, loc)
			smoke.start()
			ejectItem()
	...
	if(exp == SCANTYPE_OBLITERATE)
		visible_message("<span_class='warning'>[exp_on] activates the crushing mechanism, [exp_on] is destroyed!</span>")
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			visible_message("<span_class='warning'>[src]'s crushing mechanism slowly and smoothly descends, flattening the [exp_on]!</span>")
			new /obj/item/stack/sheet/plasteel(get_turf(pick(oview(1,src))))
		else if(prob(EFFECT_PROB_VERYLOW-badThingCoeff))
			visible_message("<span_class='danger'>[src]'s crusher goes way too many levels too high, crushing right through space-time!</span>")
			...
			visible_message("<span_class='danger'>[src]'s crusher goes one level too high, crushing right into space-time!</span>")
			playsound(src, 'sound/effects/supermatter.ogg', 50, 1, -3)
	...
	if(exp == FAIL)
		...
		visible_message("<span_class='warning'>[exp_on] [a], and [b], the experiment was a failure.</span>")
	...
	if(exp == SCANTYPE_DISCOVER)
		...
		R.reveal()
		investigate_log("Experimentor has revealed a relic with <span_class='danger'>[R.realProc]</span> effect.", INVESTIGATE_EXPERIMENTOR)
		ejectItem()
	...
	if(prob(EFFECT_PROB_VERYLOW-badThingCoeff) && loaded_item)
		...
		if(globalMalf < 15)
			visible_message("<span_class='warning'>[src]'s onboard detection system has malfunctioned!</span>")
		...
		if(globalMalf > 16 && globalMalf < 35)
			visible_message("<span_class='warning'>[src] melts [exp_on], ian-izing the air around it!</span>")
			throwSmoke(loc)
			if(trackedIan)
		...
		if(globalMalf > 36 && globalMalf < 50)
			visible_message("<span_class='warning'>Experimentor draws the life essence of those nearby!</span>")
			for(var/mob/living/m in view(4,src))
				to_chat(m, "<span_class='danger'>You feel your flesh being torn from you, mists of blood drifting to [src]!</span>")
		...
		if(globalMalf > 51 && globalMalf < 75)
			visible_message("<span_class='warning'>[src] encounters a run-time error!</span>")
			throwSmoke(loc)
			if(trackedRuntime)
			...
			ejectItem(TRUE)
		if(globalMalf > 76)
			visible_message("<span_class='warning'>[src] begins to smoke and hiss, shaking violently!</span>")
			use_power(500000)
...
/obj/item/relic/attack_self(mob/user)
	if(revealed)
		if(cooldown)
			to_chat(user, "<span_class='warning'>[src] does not react!</span>")
			return
		else if(loc == user)
			cooldown = TRUE
		...
		to_chat(user, "<span_class='notice'>You aren't quite sure what to do with this yet.</span>")
...
/obj/item/relic/proc/petSpray(mob/user)
	var/message = "<span_class='danger'>[src] begins to shake, and in the distance the sound of rampaging animals arises!</span>"
	...
	if(prob(60))
		to_chat(user, "<span_class='warning'>[src] falls apart!</span>")
		qdel(src)
...
/obj/item/relic/proc/explode(mob/user)
	to_chat(user, "<span_class='danger'>[src] begins to heat up!</span>")
	addtimer(CALLBACK(src, .proc/do_explode, user), rand(35, 100))

/obj/item/relic/proc/do_explode(mob/user)
	if(loc == user)
		visible_message("<span_class='notice'>\The [src]'s top opens, releasing a powerful blast!</span>")
...
/obj/item/relic/proc/teleport(mob/user)
	to_chat(user, "<span_class='notice'>[src] begins to vibrate!</span>")
...
/obj/item/relic/proc/do_the_teleport(mob/user)
	...
	if(loc == user && !is_centcom_level(userturf.z)) //Because Nuke Ops bringing this back on their shuttle, then looting the ERT area is 2fun4you!
		visible_message("<span_class='notice'>[src] twists and bends, relocating itself!</span>")
		throwSmoke(userturf)
