...
/obj/item/camera/siliconcam/ai_camera/after_picture(mob/user, datum/picture/picture, proximity_flag)
	...
	stored[picture] = TRUE
	to_chat(usr, "<span_class='unconscious'>Image recorded</span>")
...
/obj/item/camera/siliconcam/robot_camera/after_picture(mob/user, datum/picture/picture, proximity_flag)
	...
	if(istype(C) && istype(C.connected_ai))
		...
		to_chat(usr, "<span_class='unconscious'>Image recorded and saved to remote database</span>")
	else
		var/number = stored.len
		picture.picture_name = "Image [number] (taken by [loc.name])"
		stored[picture] = TRUE
		to_chat(usr, "<span_class='unconscious'>Image recorded and saved to local storage. Upload will happen automatically if unit is lawsynced.</span>")
...
/obj/item/camera/siliconcam/robot_camera/proc/borgprint(mob/user)
	...
	if(!istype(C) || C.toner < 20)
		to_chat(user, "<span_class='warning'>Insufficent toner to print image.</span>")
		return
	var/datum/picture/selection = selectpicture(user)
	if(!istype(selection))
		to_chat(user, "<span_class='warning'>Invalid Image.</span>")
		return
	...
	to_chat(usr, "<span_class='notice'>You print a photograph.</span>")
