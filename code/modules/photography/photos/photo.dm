...
/obj/item/photo/suicide_act(mob/living/carbon/user)
	user.visible_message("<span_class='suicide'>[user] is taking one last look at \the [src]! It looks like [user.p_theyre()] giving in to death!</span>")//when you wanna look at photo of waifu one last time before you die...
	if (user.gender == MALE)
...
/obj/item/photo/examine(mob/user)
	...
	if(in_range(src, user))
		...
		to_chat(user, "<span_class='warning'>You need to get closer to get a good look at this photo!</span>")

/obj/item/photo/proc/show(mob/user)
	if(!istype(picture) || !picture.picture_image)
		to_chat(user, "<span_class='warning'>[src] seems to be blank...</span>")
		return
