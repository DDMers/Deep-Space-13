...
/obj/machinery/airalarm/examine(mob/user)
	...
	switch(buildstage)
		if(0)
			to_chat(user, "<span_class='notice'>It is missing air alarm electronics.</span>")
		if(1)
			to_chat(user, "<span_class='notice'>It is missing wiring.</span>")
		if(2)
			to_chat(user, "<span_class='notice'>Alt-click to [locked ? "unlock" : "lock"] the interface.</span>")
	...
	switch(buildstage)
		if(2)
			if(W.tool_behaviour == TOOL_WIRECUTTER && panel_open && wires.is_all_cut())
				...
				to_chat(user, "<span_class='notice'>You cut the final wires.</span>")
		...
		if(1)
			if(W.tool_behaviour == TOOL_CROWBAR)
				...
				if (W.use_tool(src, user, 20))
					if (buildstage == 1)
						to_chat(user, "<span_class='notice'>You remove the air alarm electronics.</span>")
			...
			if(istype(W, /obj/item/stack/cable_coil))
				...
				if(cable.get_amount() < 5)
					to_chat(user, "<span_class='warning'>You need five lengths of cable to wire the air alarm!</span>")
					return
				user.visible_message("[user.name] wires the air alarm.", \
									"<span_class='notice'>You start wiring the air alarm...</span>")
				if (do_after(user, 20, target = src))
					if (cable.get_amount() >= 5 && buildstage == 1)
						cable.use(5)
						to_chat(user, "<span_class='notice'>You wire the air alarm.</span>")
						wires.repair()
						aidisabled = 0
						locked = FALSE
						mode = 1
		...
		if(0)
			...
			if(istype(W, /obj/item/electroadaptive_pseudocircuit))
				...
				"<span_class='notice'>You adapt an air alarm circuit and slot it into the assembly.</span>")
				buildstage = 1
				update_icon()
				return

			if(W.tool_behaviour == TOOL_WRENCH)
				to_chat(user, "<span_class='notice'>You detach \the [src] from the wall.</span>")
				W.play_tool_sound(src)
...
/obj/machinery/airalarm/proc/togglelock(mob/living/user)
	if(stat & (NOPOWER|BROKEN))
		to_chat(user, "<span_class='warning'>It does nothing!</span>")
	else
		if(src.allowed(usr) && !wires.is_cut(WIRE_IDSCAN))
			locked = !locked
			to_chat(user, "<span_class='notice'>You [ locked ? "lock" : "unlock"] the air alarm interface.</span>")
			updateUsrDialog()
		else
...
/obj/machinery/airalarm/emag_act(mob/user)
	if(obj_flags & EMAGGED)
		return
	obj_flags |= EMAGGED
	visible_message("<span_class='warning'>Sparks fly out of [src]!</span>", "<span_class='notice'>You emag [src], disabling its safeties.</span>")
