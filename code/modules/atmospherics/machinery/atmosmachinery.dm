...
/obj/machinery/atmospherics/examine(mob/user)
	...
	if(is_type_in_list(src, GLOB.ventcrawl_machinery) && isliving(user))
		...
		if(L.ventcrawler)
			to_chat(L, "<span_class='notice'>Alt-click to crawl through it.</span>")
...
/obj/machinery/atmospherics/wrench_act(mob/living/user, obj/item/I)
	...
	if (level==1 && isturf(T) && T.intact)
		to_chat(user, "<span_class='warning'>You must remove the plating first!</span>")
		return TRUE
	...
	to_chat(user, "<span_class='notice'>You begin to unfasten \the [src]...</span>")

	if (internal_pressure > 2*ONE_ATMOSPHERE)
		to_chat(user, "<span_class='warning'>As you begin unwrenching \the [src] a gush of air blows in your face... maybe you should reconsider?</span>")
		unsafe_wrenching = TRUE //Oh dear oh dear

	if(I.use_tool(src, user, 20, volume=50))
		user.visible_message( \
			"[user] unfastens \the [src].", \
			"<span_class='notice'>You unfasten \the [src].</span>", \
			"<span_class='italics'>You hear ratchet.</span>")
		investigate_log("was <span_class='warning'>REMOVED</span> by [key_name(usr)]", INVESTIGATE_ATMOS)
...
/obj/machinery/atmospherics/proc/unsafe_pressure_release(mob/user, pressures = null)
	...
	user.visible_message("<span_class='danger'>[user] is sent flying by pressure!</span>","<span_class='userdanger'>The pressure sends you flying!</span>")
	...
	if(target_move)
		if(target_move.can_crawl_through())
			if(is_type_in_typecache(target_move, GLOB.ventcrawl_machinery))
				...
				user.visible_message("<span_class='notice'>You hear something squeezing through the ducts...</span>", "<span_class='notice'>You climb out the ventilation system.")
			else
