...
/obj/machinery/atmospherics/components/binary/circulator/multitool_act(mob/living/user, obj/item/I)
	if(generator)
		disconnectFromGenerator()
	mode = !mode
	to_chat(user, "<span_class='notice'>You set [src] to [mode?"cold":"hot"] mode.</span>")
	return TRUE
...
/obj/machinery/atmospherics/components/binary/circulator/verb/circulator_flip()
	...
	if(anchored)
		to_chat(usr, "<span_class='danger'>[src] is anchored!</span>")
		return

	flipped = !flipped
	to_chat(usr, "<span_class='notice'>You flip [src].</span>")
	update_icon()
