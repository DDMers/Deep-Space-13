...
/datum/uplink_purchase_log/proc/generate_render(show_key = TRUE)
	...
	for(var/hash in purchase_log)
		var/datum/uplink_purchase_entry/UPE = purchase_log[hash]
		. += "<span_class='tooltip_container'>\[[UPE.icon_b64][show_key?"([owner])":""]<span_class='tooltip_hover'><b>[UPE.name]</b><br>[UPE.spent_cost ? "[UPE.spent_cost] TC" : "[UPE.base_cost] TC<br>(Surplus)"]<br>[UPE.desc]</span>[(UPE.amount_purchased > 1) ? "x[UPE.amount_purchased]" : ""]\]</span>"
