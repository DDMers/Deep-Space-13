	...
	switch(action)
		if("vr_connect")
			...
			if(human_occupant && human_occupant.mind && usr == occupant)
				to_chat(occupant, "<span_class='warning'>Transferring to virtual reality...</span>")
				if(vr_human && vr_human.stat == CONSCIOUS && !vr_human.real_mind)
					...
					to_chat(vr_human, "<span_class='notice'>Transfer successful! You are now playing as [vr_human] in VR!</span>")
				else
					if(allow_creating_vr_humans)
						to_chat(occupant, "<span_class='warning'>Virtual avatar not found, attempting to create one...</span>")
						...
						if(T)
							...
							to_chat(occupant, "<span_class='warning'>Virtual world misconfigured, aborting transfer</span>")
					else
						to_chat(occupant, "<span_class='warning'>The virtual world does not support the creation of new virtual avatars, aborting transfer</span>")
			return TRUE
		if("delete_avatar")
			if(!occupant || usr == occupant)
				...
				to_chat(usr, "<span_class='warning'>The VR Sleeper's safeties prevent you from doing that.</span>")
			return TRUE
		if("toggle_open")
			if(state_open)
				close_machine()
