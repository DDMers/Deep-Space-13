...
/obj/item/reagent_containers/food/snacks/egg/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/toy/crayon))
		...
		if(!(clr in list("blue", "green", "mime", "orange", "purple", "rainbow", "red", "yellow")))
			to_chat(usr, "<span_class='notice'>[src] refuses to take on this colour!</span>")
			return

		to_chat(usr, "<span_class='notice'>You colour [src] with [W].</span>")
		icon_state = "egg-[clr]"
		item_color = clr
	else
		..()
...
/obj/item/reagent_containers/food/snacks/omelette/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/kitchen/fork))
		...
		if(F.forkload)
			to_chat(user, "<span_class='warning'>You already have omelette on your fork!</span>")
		else
			F.icon_state = "forkloaded"
			user.visible_message("[user] takes a piece of omelette with [user.p_their()] fork!", \
				"<span_class='notice'>You take a piece of omelette with your fork.</span>")
