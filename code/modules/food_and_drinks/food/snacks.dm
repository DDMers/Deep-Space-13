...
/obj/item/reagent_containers/food/snacks/attack(mob/living/M, mob/living/user, def_zone)
	...
	if(!reagents.total_volume)						//Shouldn't be needed but it checks to see if it has anything left in it.
		to_chat(user, "<span_class='notice'>None of [src] left, oh no!</span>")
		qdel(src)
		return FALSE
	if(iscarbon(M))
		...
		if(M == user)								//If you're eating it yourself.
			if(junkiness && M.satiety < -150 && M.nutrition > NUTRITION_LEVEL_STARVING + 50 && !user.has_trait(TRAIT_VORACIOUS))
				to_chat(M, "<span_class='notice'>You don't feel like eating any more junk food at the moment.</span>")
				return FALSE
			else if(fullness <= 50)
				user.visible_message("<span_class='notice'>[user] hungrily [eatverb]s \the [src], gobbling it down!</span>", "<span_class='notice'>You hungrily [eatverb] \the [src], gobbling it down!</span>")
			else if(fullness > 50 && fullness < 150)
				user.visible_message("<span_class='notice'>[user] hungrily [eatverb]s \the [src].</span>", "<span_class='notice'>You hungrily [eatverb] \the [src].</span>")
			else if(fullness > 150 && fullness < 500)
				user.visible_message("<span_class='notice'>[user] [eatverb]s \the [src].</span>", "<span_class='notice'>You [eatverb] \the [src].</span>")
			else if(fullness > 500 && fullness < 600)
				user.visible_message("<span_class='notice'>[user] unwillingly [eatverb]s a bit of \the [src].</span>", "<span_class='notice'>You unwillingly [eatverb] a bit of \the [src].</span>")
			else if(fullness > (600 * (1 + M.overeatduration / 2000)))	// The more you eat - the more you can eat
				user.visible_message("<span_class='warning'>[user] cannot force any more of \the [src] to go down [user.p_their()] throat!</span>", "<span_class='warning'>You cannot force any more of \the [src] to go down your throat!</span>")
				return FALSE
			...
			if(!isbrain(M))		//If you're feeding it to someone else.
				if(fullness <= (600 * (1 + M.overeatduration / 1000)))
					M.visible_message("<span_class='danger'>[user] attempts to feed [M] [src].</span>", \
										"<span_class='userdanger'>[user] attempts to feed [M] [src].</span>")
				else
					M.visible_message("<span_class='warning'>[user] cannot force any more of [src] down [M]'s throat!</span>", \
										"<span_class='warning'>[user] cannot force any more of [src] down [M]'s throat!</span>")
					return FALSE

				if(!do_mob(user, M))
					return
				log_combat(user, M, "fed", reagents.log_list())
				M.visible_message("<span_class='danger'>[user] forces [M] to eat [src].</span>", \
									"<span_class='userdanger'>[user] forces [M] to eat [src].</span>")

			else
				to_chat(user, "<span_class='warning'>[M] doesn't seem to have a mouth!</span>")
				return
	...
	if(istype(W, /obj/item/reagent_containers/food/snacks))
		...
		if(custom_food_type && ispath(custom_food_type))
			if(S.w_class > WEIGHT_CLASS_SMALL)
				to_chat(user, "<span_class='warning'>[S] is too big for [src]!</span>")
				return 0
			if(!S.customfoodfilling || istype(W, /obj/item/reagent_containers/food/snacks/customizable) || istype(W, /obj/item/reagent_containers/food/snacks/pizzaslice/custom) || istype(W, /obj/item/reagent_containers/food/snacks/cakeslice/custom))
				to_chat(user, "<span_class='warning'>[src] can't be filled with [S]!</span>")
				return 0
			if(contents.len >= 20)
...
/obj/item/reagent_containers/food/snacks/proc/slice(accuracy, obj/item/W, mob/user)
	...
	if ( \
		...
		to_chat(user, "<span_class='warning'>You cannot slice [src] here! You need a table or at least a tray.</span>")
		return FALSE
	...
	if (accuracy >= IS_SHARP_ACCURATE)
		user.visible_message( \
			"[user] slices [src].", \
			"<span_class='notice'>You slice [src].</span>" \
		)
	else
		user.visible_message( \
			"[user] inaccurately slices [src] with [W]!", \
			"<span_class='notice'>You inaccurately slice [src] with your [W]!</span>" \
		)
...
/obj/item/reagent_containers/food/snacks/store/attackby(obj/item/W, mob/user, params)
	...
	if(W.w_class <= WEIGHT_CLASS_SMALL & !istype(W, /obj/item/reagent_containers/food/snacks)) //can't slip snacks inside, they're used for custom foods.
		...
		if(contents.len >= 20)
			...
			return 0
		to_chat(user, "<span_class='notice'>You slip [W] inside [src].</span>")
