...
/obj/item/reagent_containers/food/drinks/attack(mob/living/M, mob/user, def_zone)
	...
	if (!is_drainable())
		to_chat(user, "<span_class='warning'>[src]'s lid hasn't been opened!</span>")
		return 0

	if(M == user)
		user.visible_message("<span_class='notice'>[user] swallows a gulp of [src].</span>", "<span_class='notice'>You swallow a gulp of [src].</span>")
		if(M.has_trait(TRAIT_VORACIOUS))
			M.changeNext_move(CLICK_CD_MELEE * 0.5) //chug! chug! chug!

	else
		M.visible_message("<span_class='danger'>[user] attempts to feed the contents of [src] to [M].</span>", "<span_class='userdanger'>[user] attempts to feed the contents of [src] to [M].</span>")
		if(!do_mob(user, M))
			return
		if(!reagents || !reagents.total_volume)
			return // The drink might be empty after the delay, such as by spam-feeding
		M.visible_message("<span_class='danger'>[user] feeds the contents of [src] to [M].</span>", "<span_class='userdanger'>[user] feeds the contents of [src] to [M].</span>")
...
/obj/item/reagent_containers/food/drinks/afterattack(obj/target, mob/user , proximity)
	...
	if(target.is_refillable() && is_drainable()) //Something like a glass. Player probably wants to transfer TO it.
		...
		to_chat(user, "<span_class='notice'>You transfer [trans] units of the solution to [target].</span>")
		...
		if (!is_refillable())
			to_chat(user, "<span_class='warning'>[src]'s tab isn't open!</span>")
			return
...
/obj/item/reagent_containers/food/drinks/soda_cans/suicide_act(mob/living/carbon/human/H)
	if(!reagents.total_volume)
		H.visible_message("<span_class='warning'>[H] is trying to take a big sip from [src]... The can is empty!</span>")
		return SHAME
	if(!is_drainable())
		open_soda()
		sleep(10)
	H.visible_message("<span_class='suicide'>[H] takes a big sip from [src]! It looks like [H.p_theyre()] trying to commit suicide!</span>")
...
/obj/item/reagent_containers/food/drinks/soda_cans/attack(mob/M, mob/user)
	if(M == user && !src.reagents.total_volume && user.a_intent == INTENT_HARM && user.zone_selected == BODY_ZONE_HEAD)
		user.visible_message("<span_class='warning'>[user] crushes the can of [src] on [user.p_their()] forehead!</span>", "<span_class='notice'>You crush the can of [src] on your forehead.</span>")
