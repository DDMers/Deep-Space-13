...
/obj/machinery/processor/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		to_chat(user, "<span_class='notice'>The status display reads: Outputting <b>[rating_amount]</b> item(s) at <b>[rating_speed*100]%</b> speed.<span>")
...
/obj/machinery/processor/attackby(obj/item/O, mob/user, params)
	if(processing)
		to_chat(user, "<span_class='warning'>[src] is in the process of processing!</span>")
		return TRUE
	...
	if(istype(O, /obj/item/storage/bag/tray))
		...
		if(loaded)
			to_chat(user, "<span_class='notice'>You insert [loaded] items into [src].</span>")
		return
	...
	if(P)
		...
		if(user.a_intent != INTENT_HARM)
			to_chat(user, "<span_class='warning'>That probably won't blend!</span>")
			return 1
		else
			return ..()
...
/obj/machinery/processor/interact(mob/user)
	...
	if(user.a_intent == INTENT_GRAB && ismob(user.pulling) && select_recipe(user.pulling))
		...
		visible_message("<span_class='warner'>[user] stuffs [pushed_mob] into [src]!</span>")
	...
	user.visible_message("[user] turns on [src].", \
		...
		"<span_class='italics'>You hear a food processor.</span>")
...
/obj/machinery/processor/slime/process_food(datum/food_processor_process/recipe, atom/movable/what)
	...
	if (istype(S))
		...
		if(S.stat != DEAD)
			...
			S.visible_message("<span_class='notice'>[C] crawls free of the processor!</span>")
			return
