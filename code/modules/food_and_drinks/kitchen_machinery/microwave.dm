...
/obj/machinery/microwave/examine(mob/user)
	. = ..()
	if(!operating)
		to_chat(user, "<span_class='notice'>Alt-click [src] to turn it on.</span>")

	if(!in_range(user, src) && !issilicon(user) && !isobserver(user))
		to_chat(user, "<span_class='warning'>You're too far away to examine [src]'s contents and display!</span>")
		return
	if(operating)
		to_chat(user, "<span_class='notice'>\The [src] is operating.</span>")
		return

	if(length(ingredients))
		if(issilicon(user))
			to_chat(user, "<span_class='notice'>\The [src] camera shows:</span>")
		else
		...
		for(var/O in items_counts)
			to_chat(user, "<span_class='notice'>- [items_counts[O]]x [O].</span>")
	else
		to_chat(user, "<span_class='notice'>\The [src] is empty.</span>")

	if(!(stat & (NOPOWER|BROKEN)))
		to_chat(user, "<span_class='notice'>The status display reads:</span>")
		to_chat(user, "<span_class='notice'>- Capacity: <b>[max_n_of_items]</b> items.<span>")
		to_chat(user, "<span_class='notice'>- Cook time reduced by <b>[(efficiency - 1) * 25]%</b>.<span>")
...
/obj/machinery/microwave/attackby(obj/item/O, mob/user, params)
	...
	if(broken > 0)
		if(broken == 2 && O.tool_behaviour == TOOL_WIRECUTTER) // If it's broken and they're using a screwdriver
			user.visible_message("[user] starts to fix part of \the [src].", "<span_class='notice'>You start to fix part of \the [src]...</span>")
			if(O.use_tool(src, user, 20))
				user.visible_message("[user] fixes part of \the [src].", "<span_class='notice'>You fix part of \the [src].</span>")
				broken = 1 // Fix it a bit
		else if(broken == 1 && O.tool_behaviour == TOOL_WELDER) // If it's broken and they're doing the wrench
			user.visible_message("[user] starts to fix part of \the [src].", "<span_class='notice'>You start to fix part of \the [src]...</span>")
			if(O.use_tool(src, user, 20))
				user.visible_message("[user] fixes \the [src].", "<span_class='notice'>You fix \the [src].</span>")
				broken = 0
				update_icon()
				return FALSE //to use some fuel
		else
			to_chat(user, "<span_class='warning'>It's broken!</span>")
			return TRUE
		return
	...
	if(istype(O, /obj/item/reagent_containers/spray))
		...
		if(clean_spray.reagents.has_reagent("cleaner", clean_spray.amount_per_transfer_from_this))
			...
			user.visible_message("[user] has cleaned \the [src].", "<span_class='notice'>You clean \the [src].</span>")
			dirty = 0
			update_icon()
		else
			to_chat(user, "<span_class='warning'>You need more space cleaner!</span>")
		return TRUE

	if(istype(O, /obj/item/soap))
		var/obj/item/soap/P = O
		user.visible_message("[user] starts to clean \the [src].", "<span_class='notice'>You start to clean \the [src]...</span>")
	...
	if(dirty == 100) // The microwave is all dirty so can't be used!
		to_chat(user, "<span_class='warning'>\The [src] is dirty!</span>")
		return TRUE
	...
	if(istype(O, /obj/item/storage/bag/tray))
		...
		for(var/obj/item/reagent_containers/food/snacks/S in T.contents)
			if(ingredients.len >= max_n_of_items)
				to_chat(user, "<span_class='warning'>\The [src] is full, you can't put anything in!</span>")
				return TRUE
			if(SEND_SIGNAL(T, COMSIG_TRY_STORAGE_TAKE, S, src))
				loaded++
				ingredients += S
		if(loaded)
			to_chat(user, "<span_class='notice'>You insert [loaded] items into \the [src].</span>")
		return
	...
	if(O.w_class <= WEIGHT_CLASS_NORMAL && !istype(O, /obj/item/storage) && user.a_intent == INTENT_HELP)
		...
		if(!user.transferItemToLoc(O, src))
			to_chat(user, "<span_class='warning'>\The [O] is stuck to your hand!</span>")
			return FALSE

		ingredients += O
		user.visible_message("[user] has added \a [O] to \the [src].", "<span_class='notice'>You add [O] to \the [src].</span>")
		return
...
/obj/machinery/microwave/proc/turn_on()
	visible_message("\The [src] turns on.", "<span_class='italics'>You hear a microwave humming.</span>")
	operating = TRUE
...
/obj/machinery/microwave/proc/spark()
	visible_message("<span_class='warning'>Sparks fly around [src]!</span>")
...
/obj/machinery/microwave/proc/muck_finish()
	visible_message("<span_class='warning'>\The [src] gets covered in muck!</span>")
