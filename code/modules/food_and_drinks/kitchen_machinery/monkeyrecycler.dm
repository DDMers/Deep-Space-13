...
/obj/machinery/monkey_recycler/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		to_chat(user, "<span_class='notice'>The status display reads: Producing <b>[cube_production]</b> cubes for every monkey inserted.<span>")
...
/obj/machinery/monkey_recycler/proc/stuff_monkey_in(mob/living/carbon/monkey/target, mob/living/user)
	...
	if(target.stat == CONSCIOUS)
		to_chat(user, "<span_class='warning'>The monkey is struggling far too much to put it in the recycler.</span>")
		return
	if(target.buckled || target.has_buckled_mobs())
		to_chat(user, "<span_class='warning'>The monkey is attached to something.</span>")
		return
	qdel(target)
	to_chat(user, "<span_class='notice'>You stuff the monkey into the machine.</span>")
	...
	addtimer(CALLBACK(GLOBAL_PROC, /proc/to_chat, user, "<span_class='notice'>The machine now has [stored_matter] monkey\s worth of material stored.</span>"))

/obj/machinery/monkey_recycler/interact(mob/user)
	if(stored_matter >= 1)
		to_chat(user, "<span_class='notice'>The machine hisses loudly as it condenses the ground monkey meat. After a moment, it dispenses a brand new monkey cube.</span>")
		playsound(src.loc, 'sound/machines/hiss.ogg', 50, 1)
		for(var/i in 1 to FLOOR(stored_matter, 1))
			new /obj/item/reagent_containers/food/snacks/monkeycube(src.loc)
			stored_matter--
		to_chat(user, "<span_class='notice'>The machine's display flashes that it has [stored_matter] monkeys worth of material left.</span>")
	else
		to_chat(user, "<span_class='danger'>The machine needs at least 1 monkey worth of material to produce a monkey cube. It currently has [stored_matter].</span>")

/obj/machinery/monkey_recycler/multitool_act(mob/living/user, obj/item/multitool/I)
	if(istype(I))
		to_chat(user, "<span_class='notice'>You log [src] in the multitool's buffer.</span>")
		I.buffer = src
		return TRUE
