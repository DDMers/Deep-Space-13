...
/obj/machinery/gibber/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		to_chat(user, "<span_class='notice'>The status display reads: Outputting <b>[meat_produced]</b> meat slab(s) after <b>[gibtime*0.1]</b> seconds of processing.<span>")
		for(var/obj/item/stock_parts/manipulator/M in component_parts)
			if(M.rating >= 2)
				to_chat(user, "<span_class='notice'>Gibber has been upgraded to process inorganic materials.<span>")
...
/obj/machinery/gibber/attack_hand(mob/user)
	...
	if(stat & (NOPOWER|BROKEN))
		return
	if(operating)
		to_chat(user, "<span_class='danger'>It's locked and running.</span>")
		return

	if(!anchored)
		to_chat(user, "<span_class='notice'>[src] cannot be used unless bolted to the ground.</span>")
		return

	if(user.pulling && user.a_intent == INTENT_GRAB && isliving(user.pulling))
		var/mob/living/L = user.pulling
		if(!iscarbon(L))
			to_chat(user, "<span_class='danger'>This item is not suitable for the gibber!</span>")
			return
		var/mob/living/carbon/C = L
		if(C.buckled ||C.has_buckled_mobs())
			to_chat(user, "<span_class='warning'>[C] is attached to something!</span>")
			return

		if(!ignore_clothing)
			for(var/obj/item/I in C.held_items + C.get_equipped_items())
				if(!(I.item_flags & NODROP))
					to_chat(user, "<span_class='danger'>Subject may not have abiotic items on.</span>")
					return

		user.visible_message("<span_class='danger'>[user] starts to put [C] into the gibber!</span>")

		add_fingerprint(user)

		if(do_after(user, gibtime, target = src))
			if(C && user.pulling == C && !C.buckled && !C.has_buckled_mobs() && !occupant)
				user.visible_message("<span_class='danger'>[user] stuffs [C] into the gibber!</span>")
				C.forceMove(src)
				occupant = C
				update_icon()
	else
...
/obj/machinery/gibber/proc/startgibbing(mob/user)
	if(src.operating)
		return
	if(!src.occupant)
		visible_message("<span_class='italics'>You hear a loud metallic grinding sound.</span>")
		return
	use_power(1000)
	visible_message("<span_class='italics'>You hear a loud squelchy grinding sound.</span>")
