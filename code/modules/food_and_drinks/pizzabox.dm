...
/obj/item/pizzabox/attack_self(mob/user)
	...
	if(open && !bomb_defused)
		audible_message("<span_class='warning'>[icon2html(src, hearers(src))] *beep*</span>")
		bomb_active = TRUE
...
/obj/item/pizzabox/attack_hand(mob/user)
	...
	if(open)
		if(pizza)
			...
			to_chat(user, "<span_class='notice'>You take [pizza] out of [src].</span>")
			pizza = null
			update_icon()
		else if(bomb)
			if(wires.is_all_cut() && bomb_defused)
				user.put_in_hands(bomb)
				to_chat(user, "<span_class='notice'>You carefully remove the [bomb] from [src].</span>")
				bomb = null
				update_icon()
				return
			else
				...
				to_chat(user, "<span_class='warning'>You trap [src] with [bomb].</span>")
				update_icon()
	else if(boxes.len)
		var/obj/item/pizzabox/topbox = boxes[boxes.len]
		boxes -= topbox
		user.put_in_hands(topbox)
		to_chat(user, "<span_class='notice'>You remove the topmost [name] from the stack.</span>")
		topbox.update_icon()
		update_icon()
...
/obj/item/pizzabox/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/pizzabox))
		...
		if(!open && !newbox.open)
			...
			boxes += add
			newbox.boxes.Cut()
			to_chat(user, "<span_class='notice'>You put [newbox] on top of [src]!</span>")
			newbox.update_icon()
			update_icon()
			user.regenerate_icons()
			if(boxes.len >= 5)
				if(prob(10 * boxes.len))
					to_chat(user, "<span_class='danger'>You can't keep holding the stack!</span>")
					disperse_pizzas()
				else
					to_chat(user, "<span_class='warning'>The stack is getting a little high...</span>")
			return
		else
			to_chat(user, "<span_class='notice'>Close [open ? src : newbox] first!</span>")
		...
		if(open)
			if(pizza)
				to_chat(user, "<span_class='warning'>[src] already has \a [pizza.name]!</span>")
				return
		...
		if(open && !bomb)
			...
			bomb = I
			to_chat(user, "<span_class='notice'>You put [I] in [src]. Sneeki breeki...</span>")
			update_icon()
			return
		else if(bomb)
			to_chat(user, "<span_class='notice'>[src] already has a bomb in it!</span>")
	else if(istype(I, /obj/item/pen))
		if(!open)
			if(!user.is_literate())
				to_chat(user, "<span_class='notice'>You scribble illegibly on [src]!</span>")
				return
			var/obj/item/pizzabox/box = boxes.len ? boxes[boxes.len] : src
			box.boxtag += stripped_input(user, "Write on [box]'s tag:", box, "", 30)
			if(!user.canUseTopic(src, BE_CLOSE))
				return
			to_chat(user, "<span_class='notice'>You write with [I] on [src].</span>")
			update_icon()
			return
		...
		to_chat(user, "<span_class='warning'>That's not a pizza!</span>")
	..()
...
/obj/item/pizzabox/proc/disperse_pizzas()
	visible_message("<span_class='warning'>The pizzas fall everywhere!</span>")
	for(var/V in boxes)
		var/obj/item/pizzabox/P = V
...
/obj/item/pizzabox/infinite/examine(mob/user)
	..()
	if(isobserver(user))
		to_chat(user, "<span_class='deadsay'>This pizza box is anomalous, and will produce infinite pizza.</span>")
