...
/datum/round_event_control/Topic(href, href_list)
	...
	if(href_list["cancel"])
		if(!triggering)
			to_chat(usr, "<span_class='admin'>You are too late to cancel that event</span>")
			return
		triggering = FALSE
...
/datum/round_event_control/proc/runEvent()
	...
	if (alert_observers)
		deadchat_broadcast("<span_class='deadsay'><b>[name]</b> has just been[random ? " randomly" : ""] triggered!</span>") //STOP ASSUMING IT'S BADMINS!
	return E
