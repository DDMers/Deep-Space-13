...
/obj/item/papercutter/suicide_act(mob/user)
	if(storedcutter)
		user.visible_message("<span_class='suicide'>[user] is beheading [user.p_them()]self with [src.name]! It looks like [user.p_theyre()] trying to commit suicide!</span>")
		if(iscarbon(user))
		...
		user.visible_message("<span_class='suicide'>[user] repeatedly bashes [src.name] against [user.p_their()] head! It looks like [user.p_theyre()] trying to commit suicide!</span>")
...
/obj/item/papercutter/attackby(obj/item/P, mob/user, params)
	if(istype(P, /obj/item/paper) && !storedpaper)
		...
		to_chat(user, "<span_class='notice'>You place [P] in [src].</span>")
		storedpaper = P
		update_icon()
		return
	if(istype(P, /obj/item/hatchet/cutterblade) && !storedcutter)
		if(!user.transferItemToLoc(P, src))
			return
		to_chat(user, "<span_class='notice'>You replace [src]'s [P].</span>")
		P.forceMove(src)
		storedcutter = P
		update_icon()
		return
	if(P.tool_behaviour == TOOL_SCREWDRIVER && storedcutter)
		...
		to_chat(user, "<span_class='notice'>[storedcutter] has been [cuttersecured ? "unsecured" : "secured"].</span>")
...
/obj/item/papercutter/attack_hand(mob/user)
	...
	if(!storedcutter)
		to_chat(user, "<span_class='notice'>The cutting blade is gone! You can't use [src] now.</span>")
		return

	if(!cuttersecured)
		to_chat(user, "<span_class='notice'>You remove [src]'s [storedcutter].</span>")
	...
	if(storedpaper)
		...
		to_chat(user, "<span_class='notice'>You neatly cut [storedpaper].</span>")
		storedpaper = null
		qdel(storedpaper)
