...
/obj/structure/filingcabinet/attackby(obj/item/P, mob/user, params)
	if(istype(P, /obj/item/paper) || istype(P, /obj/item/folder) || istype(P, /obj/item/photo) || istype(P, /obj/item/documents))
		if(!user.transferItemToLoc(P, src))
			return
		to_chat(user, "<span_class='notice'>You put [P] in [src].</span>")
		icon_state = "[initial(icon_state)]-open"
		sleep(5)
		icon_state = initial(icon_state)
		updateUsrDialog()
	else if(P.tool_behaviour == TOOL_WRENCH)
		to_chat(user, "<span_class='notice'>You begin to [anchored ? "unwrench" : "wrench"] [src].</span>")
		if(P.use_tool(src, user, 20, volume=50))
			to_chat(user, "<span_class='notice'>You successfully [anchored ? "unwrench" : "wrench"] [src].</span>")
			anchored = !anchored
	else if(user.a_intent != INTENT_HARM)
		to_chat(user, "<span_class='warning'>You can't put [P] in [src]!</span>")
	else
		return ..()
...
/obj/structure/filingcabinet/attack_self_tk(mob/user)
	if(contents.len)
		if(prob(40 + contents.len * 5))
			...
			if(prob(25))
				step_rand(I)
			to_chat(user, "<span_class='notice'>You pull \a [I] out of [src] at random.</span>")
			return
	to_chat(user, "<span_class='notice'>You find nothing in [src].</span>")
...
/obj/structure/filingcabinet/employment/interact(mob/user)
	if(!cooldown)
		...
		to_chat(user, "<span_class='warning'>[src] is jammed, give it a few seconds.</span>")
	..()
