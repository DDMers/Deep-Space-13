...
/obj/item/clipboard/suicide_act(mob/living/carbon/user)
	user.visible_message("<span_class='suicide'>[user] begins putting [user.p_their()] head into the clip of \the [src]! It looks like [user.p_theyre()] trying to commit suicide!</span>")
...
/obj/item/clipboard/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/paper))
		if(!user.transferItemToLoc(W, src))
			return
		toppaper = W
		to_chat(user, "<span_class='notice'>You clip the paper onto \the [src].</span>")
		update_icon()
	else if(toppaper)
...
/obj/item/clipboard/Topic(href, href_list)
	...
	if(usr.contents.Find(src))
			...
			if(!haspen)
				...
				if(istype(held, /obj/item/pen))
					...
					if(!usr.transferItemToLoc(W, src))
						return
					haspen = W
					to_chat(usr, "<span_class='notice'>You slot [W] into [src].</span>")
		...
		if(href_list["top"])
			...
			if(istype(P))
				toppaper = P
				to_chat(usr, "<span_class='notice'>You move [P.name] to the top.</span>")
