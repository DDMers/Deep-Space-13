...
/obj/item/paper/contract/employment/attack(mob/living/M, mob/living/carbon/human/user)
	...
	if(M.mind == target && !M.owns_soul())
		if(user.mind && (user.mind.assigned_role == "Lawyer"))
			...
			deconvert = prob (5)
	if(deconvert)
		M.visible_message("<span_class='notice'>[user] reminds [M] that [M]'s soul was already purchased by Nanotrasen!</span>")
		to_chat(M, "<span_class='boldnotice'>You feel that your soul has returned to its rightful owner, Nanotrasen.</span>")
		M.return_soul()
	else
...
/obj/item/paper/contract/infernal/suicide_act(mob/user)
	if(signed && (user == target.current) && istype(user, /mob/living/carbon/human/))
		...
		H.visible_message("<span_class='suicide'>[H] holds up a contract claiming [user.p_their()] soul, then immediately catches fire.  It looks like [user.p_theyre()] trying to commit suicide!</span>")
...
/obj/item/paper/contract/infernal/attackby(obj/item/P, mob/living/carbon/human/user, params)
	...
	if(istype(P, /obj/item/pen) || istype(P, /obj/item/toy/crayon))
		...
		to_chat(user, "<span_class='notice'>You stamp the paper with your rubber stamp, however the ink ignites as you release the stamp.</span>")
	else if(P.is_hot())
		user.visible_message("<span_class='danger'>[user] brings [P] next to [src], but [src] does not catch fire!</span>", "<span_class='danger'>[src] refuses to ignite!</span>")
	else
		return ..()
...
/obj/item/paper/contract/infernal/attack(mob/M, mob/living/user)
	...
	if(M == user && target == M.mind && M.mind.soulOwner != owner && attempt_signature(user, 1))
		user.visible_message("<span_class='danger'>[user] slices [user.p_their()] wrist with [src], and scrawls [user.p_their()] name in blood.</span>", "<span_class='danger'>You slice your wrist open and scrawl your name in blood.</span>")
...
/obj/item/paper/contract/infernal/proc/attempt_signature(mob/living/carbon/human/user, blood = 0)
	if(!user.IsAdvancedToolUser() || !user.is_literate())
		...
		return 0
	if(user.mind != target)
		to_chat(user, "<span_class='notice'>Your signature simply slides off the sheet, it seems this contract is not meant for you to sign.</span>")
		return 0
	if(user.mind.soulOwner == owner)
		to_chat(user, "<span_class='notice'>This devil already owns your soul, you may not sell it to [owner.p_them()] again.</span>")
		return 0
	if(signed)
		to_chat(user, "<span_class='notice'>This contract has already been signed.  It may not be signed again.</span>")
		return 0
	if(!user.mind.hasSoul)
		to_chat(user, "<span_class='notice'>You do not possess a soul.</span>")
		return 0
	if(user.has_trait(TRAIT_DUMB))
		to_chat(user, "<span_class='notice'>You quickly scrawl 'your name' on the contract.</span>")
		signIncorrectly()
		return 0
	if (contractType == CONTRACT_REVIVE)
		to_chat(user, "<span_class='notice'>You are already alive, this contract would do nothing.</span>")
		return 0
	else
		to_chat(user, "<span_class='notice'>You quickly scrawl your name on the contract</span>")
		if(fulfillContract(target.current, blood)<=0)
			to_chat(user, "<span_class='notice'>But it seemed to have no effect, perhaps even Hell itself cannot grant this boon?</span>")
		return 1
...
/obj/item/paper/contract/infernal/revive/attack(mob/M, mob/living/user)
	if (target == M.mind && M.stat == DEAD && M.mind.soulOwner == M.mind)
		if (cooldown)
			to_chat(user, "<span_class='notice'>Give [M] a chance to think through the contract, don't rush [M.p_them()].</span>")
			return 0
		cooldown = TRUE
		...
		if(response == "Yes")
			...
			user.visible_message("<span_class='notice'>With a sudden blaze, [H] stands back up.</span>")
			H.fakefire()
...
/obj/item/paper/contract/infernal/proc/fulfillContract(mob/living/carbon/human/user = target.current, blood = FALSE)
	...
	to_chat(user, "<span_class='notice'>A profound emptiness washes over you as you lose ownership of your soul.</span>")
	to_chat(user, "<span_class='boldnotice'>This does NOT make you an antagonist if you were not already.</span>")
	return TRUE
