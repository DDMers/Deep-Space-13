...
/obj/item/paperplane/suicide_act(mob/living/user)
	user.Stun(200)
	user.visible_message("<span_class='suicide'>[user] jams [src] in [user.p_their()] nose. It looks like [user.p_theyre()] trying to commit suicide!</span>")
...
/obj/item/paperplane/attack_self(mob/user)
	to_chat(user, "<span_class='notice'>You unfold [src].</span>")
...
/obj/item/paperplane/attackby(obj/item/P, mob/living/carbon/human/user, params)
	...
	if(istype(P, /obj/item/pen) || istype(P, /obj/item/toy/crayon))
		to_chat(user, "<span_class='notice'>You should unfold [src] before changing it.</span>")
		return
		...
		if(user.has_trait(TRAIT_CLUMSY) && prob(10))
			user.visible_message("<span_class='warning'>[user] accidentally ignites [user.p_them()]self!</span>", \
				"<span_class='userdanger'>You miss [src] and accidentally light yourself on fire!</span>")
...
/obj/item/paperplane/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(prob(hit_probability))
		if(H.is_eyes_covered())
			return
		visible_message("<span_class='danger'>\The [src] hits [H] in the eye!</span>")
...
/obj/item/paper/examine(mob/user)
	..()
	to_chat(user, "<span_class='notice'>Alt-click [src] to fold it into a paper plane.</span>")

/obj/item/paper/AltClick(mob/living/carbon/user, obj/item/I)
	if(!istype(user) || !user.canUseTopic(src, BE_CLOSE, ismonkey(user)))
		return
	to_chat(user, "<span_class='notice'>You fold [src] into the shape of a plane!</span>")
