...
/obj/machinery/paystand/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/card/id))
		if(W == my_card)
			locked = !locked
			to_chat(user, "<span_class='notice'>You [src.locked ? "lock" : "unlock"] the bolts on the paystand.</span>")
			return
		if(!my_card)
		...
		if(vbucks.registered_account)
			...
			if(momsdebitcard < 1)
				to_chat(user, "<span_class='warning'>ERROR: Invalid amount designated.</span>")
				return
			if(vbucks.registered_account.adjust_money(-momsdebitcard))
				...
				to_chat(user, "<span_class='warning'>ERROR: Account has insufficient funds to make transaction.</span>")
				return
		else
			to_chat(user, "<span_class='warning'>ERROR: No bank account assigned to identification card.</span>")
			return
	if(istype(W, /obj/item/holochip))
		...
		if(H.spend(cashmoney, FALSE))
			...
			to_chat(user, "<span_class='warning'>ERROR: Insufficient funds to make transaction.</span>")
			return
	...
	if(istype(W, /obj/item/assembly/signaler))
		...
		if(S.secured)
			to_chat(user, "<span_class='warning'>The signaler needs to be in attachable mode to add it to the paystand!</span>")
			return
		if(!my_card)
			to_chat(user, "<span_class='warning'>ERROR: No identification card has been assigned to this paystand yet!</span>")
			return
		if(!signaler)
			...
			to_chat(user, "<span_class='warning'>A signaler is already attached to this unit!</span>")
...
/obj/machinery/paystand/default_unfasten_wrench(mob/user, obj/item/I, time = 20)
	if(locked)
		to_chat(user, "<span_class='warning'>The anchored bolts on this paystand are currently locked!</span>")
		return
	. = ..()
