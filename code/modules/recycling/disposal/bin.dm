...
/obj/machinery/disposal/attackby(obj/item/I, mob/user, params)
	...
	if(!pressure_charging && !full_pressure && !flush)
		if(I.tool_behaviour == TOOL_SCREWDRIVER)
			...
			to_chat(user, "<span_class='notice'>You [panel_open ? "remove":"attach"] the screws around the power connection.</span>")
			return
		else if(I.tool_behaviour == TOOL_WELDER && panel_open)
			if(!I.tool_start_check(user, amount=0))
				return

			to_chat(user, "<span_class='notice'>You start slicing the floorweld off \the [src]...</span>")
			if(I.use_tool(src, user, 20, volume=100) && panel_open)
				to_chat(user, "<span_class='notice'>You slice the floorweld off \the [src].</span>")
				deconstruct()
			return
...
/obj/machinery/disposal/proc/place_item_in_disposal(obj/item/I, mob/user)
	I.forceMove(src)
	user.visible_message("[user.name] places \the [I] into \the [src].", "<span_class='notice'>You place \the [I] into \the [src].</span>")
...
/obj/machinery/disposal/proc/stuff_mob_in(mob/living/target, mob/living/user)
	...
	if(target.mob_size > MOB_SIZE_HUMAN)
		to_chat(user, "<span_class='warning'>[target] doesn't fit inside [src]!</span>")
		return
	add_fingerprint(user)
	if(user == target)
		user.visible_message("[user] starts climbing into [src].", "<span_class='notice'>You start climbing into [src]...</span>")
	else
		target.visible_message("<span_class='danger'>[user] starts putting [target] into [src].</span>", "<span_class='userdanger'>[user] starts putting you into [src]!</span>")
	if(do_mob(user, target, 20))
		if (!loc)
			return
		target.forceMove(src)
		if(user == target)
			user.visible_message("[user] climbs into [src].", "<span_class='notice'>You climb into [src].</span>")
		else
			target.visible_message("<span_class='danger'>[user] has placed [target] in [src].</span>", "<span_class='userdanger'>[user] has placed [target] in [src].</span>")
...
/obj/machinery/disposal/bin/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/storage/bag/trash))	//Not doing component overrides because this is a specific type.
		...
		to_chat(user, "<span_class='warning'>You empty the bag.</span>")
...
/obj/machinery/disposal/bin/hitby(atom/movable/AM, skipcatch, hitpush, blocked, datum/thrownthing/throwingdatum)
	if(isitem(AM) && AM.CanEnterDisposals())
		if(prob(75))
			AM.forceMove(src)
			visible_message("<span_class='notice'>[AM] lands in [src].</span>")
			update_icon()
		else
			visible_message("<span_class='notice'>[AM] bounces off of [src]'s rim!</span>")
			return ..()
	else
		return ..()
...
/obj/machinery/disposal/deliveryChute/Bumped(atom/movable/AM) //Go straight into the chute
	...
	if(isobj(AM))
		...
		if(prob(2)) // to prevent mobs being stuck in infinite loops
			to_chat(M, "<span_class='warning'>You hit the edge of the chute.</span>")
			return
		M.forceMove(src)
	flush()
