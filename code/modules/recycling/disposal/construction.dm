...
/obj/structure/disposalconstruct/proc/can_be_rotated(mob/user,rotation_type)
	if(anchored)
		to_chat(user, "<span_class='warning'>You must unfasten the pipe before rotating it!</span>")
		return FALSE
	return TRUE
...
/obj/structure/disposalconstruct/wrench_act(mob/living/user, obj/item/I)
	if(anchored)
		anchored = FALSE
		density = FALSE
		to_chat(user, "<span_class='notice'>You detach the [pipename] from the underfloor.</span>")
	else
		var/ispipe = is_pipe() // Indicates if we should change the level of this pipe

		var/turf/T = get_turf(src)
		if(T.intact && isfloorturf(T))
			to_chat(user, "<span_class='warning'>You can only attach the [pipename] if the floor plating is removed!</span>")
			return TRUE

		if(!ispipe && iswallturf(T))
			to_chat(user, "<span_class='warning'>You can't build [pipename]s on walls, only disposal pipes!</span>")
			return TRUE
		...
		if(ispipe)
			...
			for(var/obj/structure/disposalpipe/CP in T)
				...
				if(istype(CP, /obj/structure/disposalpipe/broken))
					pdir = CP.dir
				if(pdir & dpdir)
					to_chat(user, "<span_class='warning'>There is already a disposal pipe at that location!</span>")
					return TRUE
			...
			if(!found_trunk)
				to_chat(user, "<span_class='warning'>The [pipename] requires a trunk underneath it in order to work!</span>")
				return TRUE

		anchored = TRUE
		density = initial(pipe_type.density)
		to_chat(user, "<span_class='notice'>You attach the [pipename] to the underfloor.</span>")
...
/obj/structure/disposalconstruct/welder_act(mob/living/user, obj/item/I)
	if(anchored)
		...
		to_chat(user, "<span_class='notice'>You start welding the [pipename] in place...</span>")
		if(I.use_tool(src, user, 8, volume=50))
			to_chat(user, "<span_class='notice'>The [pipename] has been welded in place.</span>")
			var/obj/O = new pipe_type(loc, src)
			transfer_fingerprints_to(O)

	else
		to_chat(user, "<span_class='warning'>You need to attach it to the plating first!</span>")
	return TRUE
