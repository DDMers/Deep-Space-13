...
/obj/item/melee/transforming/energy/sword/bananium/suicide_act(mob/user)
	...
	user.visible_message("<span_class='suicide'>[user] is [pick("slitting [user.p_their()] stomach open with", "falling on")] [src]! It looks like [user.p_theyre()] trying to commit seppuku, but the blade slips off of [user.p_them()] harmlessly!</span>")
	GET_COMPONENT(slipper, /datum/component/slippery)
...
/obj/item/grown/bananapeel/bombanana/suicide_act(mob/user)
	user.visible_message("<span_class='suicide'>[user] is deliberately slipping on the [src.name]! It looks like \he's trying to commit suicide.</span>")
