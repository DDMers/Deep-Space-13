...
/datum/action/innate/mecha/mech_defence_mode/Activate(forced_state = null)
	...
	if(chassis.defence_mode)
		...
		chassis.occupant_message("<span_class='notice'>You enable [chassis] defence mode.</span>")
	else
		chassis.deflect_chance = initial(chassis.deflect_chance)
		chassis.occupant_message("<span_class='danger'>You disable [chassis] defence mode.</span>")
...
/datum/action/innate/mecha/mech_overload_mode/Activate(forced_state = null)
	...
	if(chassis.leg_overload_mode)
		...
		chassis.occupant_message("<span_class='danger'>You enable leg actuators overload.</span>")
	else
		chassis.leg_overload_mode = 0
		chassis.bumpsmash = 0
		chassis.step_in = initial(chassis.step_in)
		chassis.step_energy_drain = chassis.normal_step_energy_drain
		chassis.occupant_message("<span_class='notice'>You disable leg actuators overload.</span>")
	UpdateButtonIcon()
...
/datum/action/innate/mecha/mech_zoom/Activate()
	...
	if(owner.client)
		...
		if(chassis.zoom_mode)
			...
			owner.client.check_view() //Deep Space 13 - Widescreen mode //world.view - default mob view size
		UpdateButtonIcon()
