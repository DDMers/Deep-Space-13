...
/obj/item/mecha_parts/mecha_equipment/hydraulic_clamp/action(atom/target)
	...
	if(isobj(target))
		...
		if(!O.anchored)
			if(cargo_holder.cargo.len < cargo_holder.cargo_capacity)
				...
				if(do_after_cooldown(target))
					...
					O.forceMove(chassis)
					O.anchored = FALSE
					occupant_message("<span_class='notice'>[target] successfully loaded.</span>")
					log_message("Loaded [O]. Cargo compartment capacity: [cargo_holder.cargo_capacity - cargo_holder.cargo.len]", LOG_MECHA)
				else
					O.anchored = initial(O.anchored)
			else
				occupant_message("<span_class='warning'>Not enough room in cargo compartment!</span>")
		else
			occupant_message("<span_class='warning'>[target] is firmly secured!</span>")
		...
		if(chassis.occupant.a_intent == INTENT_HARM)
			...
			M.updatehealth()
			target.visible_message("<span_class='danger'>[chassis] squeezes [target].</span>", \
								"<span_class='userdanger'>[chassis] squeezes [target].</span>",\
								"<span_class='italics'>You hear something crack.</span>")
	...
	if(isobj(target))
		...
		if(chassis.occupant.a_intent == INTENT_HARM)
			if(real_clamp)
				...
				M.adjustOxyLoss(round(dam_force/2))
				M.updatehealth()
				target.visible_message("<span_class='danger'>[chassis] destroys [target] in an unholy fury.</span>", \
									"<span_class='userdanger'>[chassis] destroys [target] in an unholy fury.</span>")
				log_combat(chassis.occupant, M, "attacked", "[name]", "(INTENT: [uppertext(chassis.occupant.a_intent)]) (DAMTYE: [uppertext(damtype)])")
			else
				target.visible_message("<span_class='danger'>[chassis] destroys [target] in an unholy fury.</span>", \
									"<span_class='userdanger'>[chassis] destroys [target] in an unholy fury.</span>")
			...
			if(real_clamp)
				...
				if(play_sound)
					...
					target.visible_message("<span_class='danger'>[chassis] rips [target]'s arms off.</span>", \
								   "<span_class='userdanger'>[chassis] rips [target]'s arms off.</span>")
...
/obj/item/mecha_parts/mecha_equipment/extinguisher/action(atom/target) //copypasted from extinguisher. TODO: Rewrite from scratch.
	...
	if(istype(target, /obj/structure/reagent_dispensers/watertank) && get_dist(chassis,target) <= 1)
		...
		occupant_message("<span_class='notice'>Extinguisher refilled.</span>")
...
/obj/item/mecha_parts/mecha_equipment/cable_layer/action(var/obj/item/stack/cable_coil/target)
	...
	if(istype(target) && target.amount)
		...
		if(to_load)
			...
			target.use(to_load)
			occupant_message("<span_class='notice'>[to_load] meters of cable successfully loaded.</span>")
			send_byjax(chassis.occupant,"exosuit.browser","[REF(src)]",src.get_equip_info())
		else
			occupant_message("<span_class='warning'>Reel is full.</span>")
	else
		occupant_message("<span_class='warning'>Unable to load [target] - no cable found.</span>")
