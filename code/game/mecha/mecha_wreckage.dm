...
/obj/structure/mecha_wreckage/examine(mob/user)
	..()
	if(AI)
		to_chat(user, "<span_class='notice'>The AI recovery beacon is active.</span>")

/obj/structure/mecha_wreckage/attackby(obj/item/I, mob/user, params)
	if(I.tool_behaviour == TOOL_WELDER)
		if(salvage_num <= 0 || !length(welder_salvage))
			to_chat(user, "<span_class='warning'>You don't see anything that can be cut with [I]!</span>")
			return
		...
		if(type)
			...
			user.visible_message("[user] cuts [N] from [src].", "<span_class='notice'>You cut [N] from [src].</span>")
			if(istype(N, /obj/item/mecha_parts/part))
				welder_salvage -= type
			salvage_num--
		else
			to_chat(user, "<span_class='warning'>You fail to salvage anything valuable from [src]!</span>")
		return
		...
		if(crowbar_salvage && crowbar_salvage.len)
			...
			if(S)
				...
				crowbar_salvage -= S
				user.visible_message("[user] pries [S] from [src].", "<span_class='notice'>You pry [S] from [src].</span>")
			return
		else
			to_chat(user, "<span_class='warning'>You don't see anything that can be pried with [I]!</span>")
 ...
 //Proc called on the wreck by the AI card.
	if(interaction == AI_TRANS_TO_CARD) //AIs can only be transferred in one direction, from the wreck to the card.
		if(!AI) //No AI in the wreck
			to_chat(user, "<span_class='warning'>No AI backups found.</span>")
			return
		...
		to_chat(user, "<span_class='boldnotice'>Backup files recovered</span>: [AI.name] ([rand(1000,9999)].exe) salvaged from [name] and stored within local memory.")
