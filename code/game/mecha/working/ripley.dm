...
/obj/mecha/working/ripley/Topic(href, href_list)
	...
	if(href_list["drop_from_cargo"])
		...
		if(O)
			occupant_message("<span_class='notice'>You unload [O].</span>")
...
/obj/mecha/working/ripley/relay_container_resist(mob/living/user, obj/O)
	to_chat(user, "<span_class='notice'>You lean on the back of [O] and start pushing so it falls out of [src].</span>")
	if(do_after(user, 300, target = O))
		if(!user || user.stat != CONSCIOUS || user.loc != src || O.loc != src )
			return
		to_chat(user, "<span_class='notice'>You successfully pushed [O] out of [src]!</span>")
		O.forceMove(drop_location())
		cargo -= O
	else
		if(user.loc == src) //so we don't get the message if we resisted multiple times and succeeded.
			to_chat(user, "<span_class='warning'>You fail to push [O] out of [src]!</span>")
