...
/obj/structure/life_candle/attack_hand(mob/user)
	...
	if(user.mind in linked_minds)
		user.visible_message("<span_class='notice'>[user] reaches out and pinches the flame of [src].</span>", "<span_class='warning'>You sever the connection between yourself and [src].</span>")
		linked_minds -= user.mind
	else
		user.visible_message("<span_class='notice'>[user] touches [src]. It seems to respond to [user.p_their()] presence!</span>", "<span_class='warning'>You create a connection between you and [src].</span>")
		linked_minds |= user.mind
