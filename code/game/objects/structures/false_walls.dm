...
/obj/structure/falsewall/attackby(obj/item/W, mob/user, params)
	if(opening)
		to_chat(user, "<span_class='warning'>You must wait until the door has stopped moving!</span>")
		return
	...
	if(W.tool_behaviour == TOOL_SCREWDRIVER)
		if(density)
			var/turf/T = get_turf(src)
			if(T.density)
				to_chat(user, "<span_class='warning'>[src] is blocked!</span>")
				return
			if(!isfloorturf(T))
				to_chat(user, "<span_class='warning'>[src] bolts must be tightened on the floor!</span>")
				return
			user.visible_message("<span_class='notice'>[user] tightens some bolts on the wall.</span>", "<span_class='notice'>You tighten the bolts on the wall.</span>")
			ChangeToWall()
		else
			to_chat(user, "<span_class='warning'>You can't reach, close it first!</span>")
...
/obj/structure/falsewall/proc/dismantle(mob/user, disassembled=TRUE, obj/item/tool = null)
	user.visible_message("[user] dismantles the false wall.", "<span_class='notice'>You dismantle the false wall.</span>")
	if(tool)
