...
/obj/structure/easel/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/canvas))
		...
		C.layer = layer+0.1
		user.visible_message("<span_class='notice'>[user] puts \the [C] on \the [src].</span>","<span_class='notice'>You place \the [C] on \the [src].</span>")
	else
		return ..()
...
/obj/item/canvas/attack_self(mob/user)
	...
	if(blank)
		...
		icon = blank
		user.visible_message("<span_class='notice'>[user] cleans the canvas.</span>","<span_class='notice'>You clean the canvas.</span>")
