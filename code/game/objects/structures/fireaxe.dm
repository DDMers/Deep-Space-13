...
/obj/structure/fireaxecabinet/attackby(obj/item/I, mob/user, params)
	if(iscyborg(user) || I.tool_behaviour == TOOL_MULTITOOL)
		...
		if(G.get_amount() < 2)
			to_chat(user, "<span_class='warning'>You need two glass sheets to fix [src]!</span>")
			return
		...
		if(istype(I, /obj/item/twohanded/fireaxe) && !fireaxe)
			...
			if(F.wielded)
				to_chat(user, "<span_class='warning'>Unwield the [F.name] first.</span>")
				return
			if(!user.transferItemToLoc(F, src))
				return
			fireaxe = F
			to_chat(user, "<span_class='caution'>You place the [F.name] back in the [name].</span>")
			update_icon()
			return
		else if(!broken)
			toggle_open()
...
/obj/structure/fireaxecabinet/attack_hand(mob/user)
	...
	if(open || broken)
		if(fireaxe)
			...
			fireaxe = null
			to_chat(user, "<span_class='caution'>You take the fire axe from the [name].</span>")
			src.add_fingerprint(user)
			update_icon()
			return
	if(locked)
		to_chat(user, "<span_class='warning'>The [name] won't budge!</span>")
		return
	else
		open = !open
		update_icon()
...
/obj/structure/fireaxecabinet/proc/toggle_lock(mob/user)
	to_chat(user, "<span_class = 'caution'> Resetting circuitry...</span>")
	playsound(src, 'sound/machines/locktoggle.ogg', 50, 1)
	if(do_after(user, 20, target = src))
		to_chat(user, "<span_class='caution'>You [locked ? "disable" : "re-enable"] the locking modules.</span>")
		locked = !locked
		update_icon()
...
/obj/structure/fireaxecabinet/verb/toggle_open()
	...
	if(locked)
		to_chat(usr, "<span_class='warning'>The [name] won't budge!</span>")
		return
	else
		open = !open
		update_icon()
