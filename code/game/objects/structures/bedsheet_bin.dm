...
/obj/item/bedsheet/attack_self(mob/user)
	...
	if(layer == initial(layer))
		...
		to_chat(user, "<span_class='notice'>You cover yourself with [src].</span>")
	else
		layer = initial(layer)
		to_chat(user, "<span_class='notice'>You smooth [src] out beneath you.</span>")
...
/obj/item/bedsheet/attackby(obj/item/I, mob/user, params)
	if(I.tool_behaviour == TOOL_WIRECUTTER || I.is_sharp())
		...
		qdel(src)
		to_chat(user, "<span_class='notice'>You tear [src] up.</span>")
	else
		return ..()
...
/obj/structure/bedsheetbin/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/bedsheet))
		...
		if(!user.transferItemToLoc(I, src))
			to_chat(user, "<span_class='warning'>\The [I] is stuck to your hand, you cannot hide it among the sheets!</span>")
			return
		hidden = I
		to_chat(user, "<span_class='notice'>You hide [I] among the sheets.</span>")
...
/obj/structure/bedsheetbin/attack_hand(mob/user)
	...
	if(amount >= 1)
		...
		user.put_in_hands(B)
		to_chat(user, "<span_class='notice'>You take [B] out of [src].</span>")
		update_icon()

		if(hidden)
			hidden.forceMove(drop_location())
			to_chat(user, "<span_class='notice'>[hidden] falls out of [B]!</span>")
			hidden = null
...
/obj/structure/bedsheetbin/attack_tk(mob/user)
	if(amount >= 1)
		...
		B.forceMove(drop_location())
		to_chat(user, "<span_class='notice'>You telekinetically remove [B] from [src].</span>")
		update_icon()
