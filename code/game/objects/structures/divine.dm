...
/obj/structure/sacrificealtar/attack_hand(mob/living/user)
	...
	if(!L)
		return
	to_chat(user, "<span_class='notice'>You attempt to sacrifice [L] by invoking the sacrificial ritual.</span>")
	L.gib()
...
/obj/structure/healingfountain/attack_hand(mob/living/user)
	...
	if(last_process + time_between_uses > world.time)
		to_chat(user, "<span_class='notice'>The fountain appears to be empty.</span>")
		return
	last_process = world.time
	to_chat(user, "<span_class='notice'>The water feels warm and soothing as you touch it. The fountain immediately dries up shortly afterwards.</span>")
