...
/obj/structure/sign/barsign/attack_hand(mob/user)
	...
	if(.)
		return
	if(!allowed(user))
		to_chat(user, "<span_class='info'>Access denied.</span>")
		return
	if(broken)
		to_chat(user, "<span_class ='danger'>The controls seem unresponsive.</span>")
		return
	pick_sign(user)
...
/obj/structure/sign/barsign/attackby(obj/item/I, mob/user)
	if(I.tool_behaviour == TOOL_SCREWDRIVER)
		...
		if(!broken)
			to_chat(user, "<span_class='warning'>This sign is functioning properly!</span>")
			return

		if(C.use(2))
			to_chat(user, "<span_class='notice'>You replace the burnt wiring.</span>")
			broken = FALSE
		else
			to_chat(user, "<span_class='warning'>You need at least two lengths of cable!</span>")
	else
		return ..()
...
/obj/structure/sign/barsign/emag_act(mob/user)
	if(broken)
		to_chat(user, "<span_class='warning'>Nothing interesting happens!</span>")
		return
	to_chat(user, "<span_class='notice'>You load an illegal barsign into the memory buffer...</span>")
	sleep(10 SECONDS)
