...
/obj/structure/sign/attackby(obj/item/I, mob/user, params)
	if(I.tool_behaviour == TOOL_WRENCH && buildable_sign)
		user.visible_message("<span_class='notice'>[user] starts removing [src]...</span>", \
							 "<span_class='notice'>You start unfastening [src].</span>")
		I.play_tool_sound(src)
		if(I.use_tool(src, user, 40))
			playsound(src, 'sound/items/deconstruct.ogg', 50, 1)
			user.visible_message("<span_class='notice'>[user] unfastens [src].</span>", \
								 "<span_class='notice'>You unfasten [src].</span>")
...
/obj/item/sign_backing/afterattack(atom/target, mob/user, proximity)
	...
	if(isturf(target) && proximity)
		var/turf/T = target
		user.visible_message("<span_class='notice'>[user] fastens [src] to [T].</span>", \
							 "<span_class='notice'>You attach the sign to [T].</span>")
