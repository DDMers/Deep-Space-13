...
/obj/structure/mineral_door/proc/pickaxe_door(mob/living/user, obj/item/I) //override if the door isn't supposed to be a minable mineral.
	...
	if(I.tool_behaviour != TOOL_MINING)
		return
	. = TRUE
	to_chat(user, "<span_class='notice'>You start digging [src]...</span>")
	if(I.use_tool(src, user, 40, volume=50))
		to_chat(user, "<span_class='notice'>You finish digging.</span>")
		deconstruct(TRUE)

/obj/structure/mineral_door/welder_act(mob/living/user, obj/item/I) //override if the door is supposed to be flammable.
	. = TRUE
	if(anchored)
		to_chat(user, "<span_class='warning'>[src] is still firmly secured to the ground!</span>")
		return

	user.visible_message("[user] starts to weld apart [src]!", "<span_class='notice'>You start welding apart [src].</span>")
	if(!I.use_tool(src, user, 60, 5, 50))
		to_chat(user, "<span_class='warning'>You failed to weld apart [src]!/span>")
		return

	user.visible_message("[user] welded [src] into pieces!", "<span_class='notice'>You welded apart [src]!</span>")
	deconstruct(TRUE)
...
/obj/structure/mineral_door/proc/crowbar_door(mob/living/user, obj/item/I) //if the door is flammable, call this in crowbar_act() so we can still decon it
	...
	user.visible_message("[user] starts to pry apart [src]!", "<span_class='notice'>You start prying apart [src].</span>")
	if(!I.use_tool(src, user, 60, volume = 50))
		to_chat(user, "<span_class='warning'>You failed to pry apart [src]!/span>")
		return

	user.visible_message("[user] pried [src] into pieces!", "<span_class='notice'>You pried apart [src]!</span>")
	deconstruct(TRUE)
...
/obj/structure/mineral_door/paperframe/attackby(obj/item/I, mob/living/user)
	...
	if((user.a_intent != INTENT_HARM) && istype(I, /obj/item/paper) && (obj_integrity < max_integrity))
		user.visible_message("[user] starts to patch the holes in [src].", "<span_class='notice'>You start patching some of the holes in [src]!</span>")
		if(do_after(user, 20, TRUE, src))
			obj_integrity = min(obj_integrity+4,max_integrity)
			qdel(I)
			user.visible_message("[user] patches some of the holes in [src].", "<span_class='notice'>You patch some of the holes in [src]!</span>")
			return TRUE
