...
/obj/structure/flora/tree/attackby(obj/item/W, mob/user, params)
	if(log_amount && (!(flags_1 & NODECONSTRUCT_1)))
		if(W.sharpness && W.force > 0)
			...
			user.visible_message("<span_class='notice'>[user] begins to cut down [src] with [W].</span>","<span_class='notice'>You begin to cut down [src] with [W].</span>", "You hear the sound of sawing.")
			if(do_after(user, 1000/W.force, target = src)) //5 seconds with 20 force, 8 seconds with a hatchet, 20 seconds with a shard.
				user.visible_message("<span_class='notice'>[user] fells [src] with the [W].</span>","<span_class='notice'>You fell [src] with the [W].</span>", "You hear the sound of a tree falling.")
...
/obj/structure/flora/tree/pine/xmas/presents/attack_hand(mob/living/user)
	...
	if(took_presents[user.ckey] && !unlimited)
		to_chat(user, "<span_class='warning'>There are no presents with your name on.</span>")
		return
	to_chat(user, "<span_class='warning'>After a bit of rummaging, you locate a gift with your name on it!</span>")
