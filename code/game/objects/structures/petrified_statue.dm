...
/obj/structure/statue/petrified/New(loc, mob/living/L, statue_timer)
	...
	if(L)
		...
		L.visible_message("<span_class='warning'>[L]'s skin rapidly turns to marble!</span>", "<span_class='userdanger'>Your body freezes up! Can't... move... can't...  think...</span>")
		L.forceMove(src)
	...
	if(istype(src.loc, /mob/living/simple_animal/hostile/statue))
		...
		if(S.mind)
			if(petrified_mob)
				...
				to_chat(petrified_mob, "<span_class='notice'>You slowly come back to your senses. You are in control of yourself again!</span>")
		qdel(S)
...
/obj/structure/statue/petrified/deconstruct(disassembled = TRUE)
	if(!disassembled)
		if(petrified_mob)
			petrified_mob.dust()
	visible_message("<span_class='danger'>[src] shatters!.</span>")
	qdel(src)
