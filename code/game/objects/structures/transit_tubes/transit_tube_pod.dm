...
/obj/structure/transit_tube_pod/attackby(obj/item/I, mob/user, params)
	if(I.tool_behaviour == TOOL_CROWBAR)
		if(!moving)
			...
			if(contents.len)
				user.visible_message("[user] empties \the [src].", "<span_class='notice'>You empty \the [src].</span>")
				empty_pod()
			else
...
/obj/structure/transit_tube_pod/deconstruct(disassembled = TRUE, mob/user)
	if(!(flags_1 & NODECONSTRUCT_1))
		...
		if(user)
			...
			user.visible_message("[user] removes [src].", "<span_class='notice'>You remove [src].</span>")
...
/obj/structure/transit_tube_pod/container_resist(mob/living/user)
	...
	if(!moving)
		...
		to_chat(user, "<span_class='notice'>You start trying to escape from the pod...</span>")
		if(do_after(user, 600, target = src))
			to_chat(user, "<span_class='notice'>You manage to open the pod.</span>")
			empty_pod()
