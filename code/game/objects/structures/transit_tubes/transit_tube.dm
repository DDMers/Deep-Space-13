	...
	if(W.tool_behaviour == TOOL_WRENCH)
		if(tube_construction)
			for(var/obj/structure/transit_tube_pod/pod in src.loc)
				to_chat(user, "<span_class='warning'>Remove the pod first!</span>")
				return
			user.visible_message("[user] starts to detach \the [src].", "<span_class='notice'>You start to detach the [name]...</span>")
			if(W.use_tool(src, user, time_to_unwrench, volume=50))
				to_chat(user, "<span_class='notice'>You detach the [name].</span>")
