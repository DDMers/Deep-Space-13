...
/obj/structure/transit_tube/station/MouseDrop_T(obj/structure/c_transit_tube_pod/R, mob/user)
	...
	user.visible_message("[user] inserts [R].", "<span_class='notice'>You insert [R].</span>")
	qdel(R)
			...
			if(open_status == STATION_TUBE_OPEN)
				...
				if(user.grab_state >= GRAB_AGGRESSIVE)
					if(GM.buckled || GM.has_buckled_mobs())
						to_chat(user, "<span_class='warning'>[GM] is attached to something!</span>")
						return
					for(var/obj/structure/transit_tube_pod/pod in loc)
						pod.visible_message("<span_class='warning'>[user] starts putting [GM] into the [pod]!</span>")
				...
				if(!pod.moving && (pod.dir in tube_dirs))
					if(open_status == STATION_TUBE_CLOSED)
						...
						if(pod.contents.len && user.loc != pod)
							user.visible_message("[user] starts emptying [pod]'s contents onto the floor.", "<span_class='notice'>You start emptying [pod]'s contents onto the floor...</span>")
