...
/obj/structure/noticeboard/attackby(obj/item/O, mob/user, params)
	if(istype(O, /obj/item/paper) || istype(O, /obj/item/photo))
		if(!allowed(user))
			to_chat(user, "<span_class='info'>You are not authorized to add notices</span>")
			return
		if(notices < 5)
			...
			notices++
			icon_state = "nboard0[notices]"
			to_chat(user, "<span_class='notice'>You pin the [O] to the noticeboard.</span>")
		else
			to_chat(user, "<span_class='notice'>The notice board is full</span>")
	else
		return ..()
	...
	if(href_list["write"])
		...
		if(istype(P) && P.loc == src)
			...
			if(I)
				...
				to_chat(usr, "<span_class='notice'>You'll need something to write with!</span>")
