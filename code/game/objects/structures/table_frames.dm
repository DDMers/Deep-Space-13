...
/obj/structure/table_frame/attackby(obj/item/I, mob/user, params)
	if(I.tool_behaviour == TOOL_WRENCH)
		...
		if(P.get_amount() < 1)
			to_chat(user, "<span_class='warning'>You need one plasteel sheet to do this!</span>")
			return
		...
		if(M.get_amount() < 1)
			to_chat(user, "<span_class='warning'>You need one metal sheet to do this!</span>")
			return
		to_chat(user, "<span_class='notice'>You start adding [M] to [src]...</span>")
		if(do_after(user, 20, target = src) && M.use(1))
			make_new_table(/obj/structure/table)
	else if(istype(I, /obj/item/stack/sheet/glass))
		var/obj/item/stack/sheet/glass/G = I
		if(G.get_amount() < 1)
			to_chat(user, "<span_class='warning'>You need one glass sheet to do this!</span>")
			return
		...
		if(S.get_amount() < 1)
			to_chat(user, "<span_class='warning'>You need one silver sheet to do this!</span>")
			return
		to_chat(user, "<span_class='notice'>You start adding [S] to [src]...</span>")
		...
		if(C.get_amount() < 1)
			to_chat(user, "<span_class='warning'>You need one  black carpet sheet to do this!</span>")
			return
		to_chat(user, "<span_class='notice'>You start adding [C] to [src]...</span>")
		if(do_after(user, 20, target = src) && C.use(1))
			make_new_table(/obj/structure/table/wood/fancy/black)
	else if(istype(I, /obj/item/stack/tile/carpet))
		var/obj/item/stack/tile/carpet/C = I
		if(C.get_amount() < 1)
			to_chat(user, "<span_class='warning'>You need one carpet sheet to do this!</span>")
			return
		to_chat(user, "<span_class='notice'>You start adding [C] to [src]...</span>")
		if(do_after(user, 20, target = src) && C.use(1))
		...
		if(B.get_amount() < 1)
			to_chat(user, "<span_class='warning'>You need one bronze sheet to do this!</span>")
			return
		to_chat(user, "<span_class='notice'>You start adding [B] to [src]...</span>")
...
/obj/structure/table_frame/wood/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/stack/sheet/mineral/wood))
		...
		if(W.get_amount() < 1)
			to_chat(user, "<span_class='warning'>You need one wood sheet to do this!</span>")
			return
		to_chat(user, "<span_class='notice'>You start adding [W] to [src]...</span>")
...
/obj/structure/table_frame/brass/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/stack/tile/brass))
		...
		if(W.get_amount() < 1)
			to_chat(user, "<span_class='warning'>You need one brass sheet to do this!</span>")
			return
		to_chat(user, "<span_class='notice'>You start adding [W] to [src]...</span>")
		if(do_after(user, 20, target = src) && W.use(1))
