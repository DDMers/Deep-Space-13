...
/obj/structure/plasticflaps/examine(mob/user)
	. = ..()
	if(anchored)
		to_chat(user, "<span_class='notice'>[src] are <b>screwed</b> to the floor.</span>")
	else
		to_chat(user, "<span_class='notice'>[src] are no longer <i>screwed</i> to the floor, and the flaps can be <b>cut</b> apart.</span>")
...
/obj/structure/plasticflaps/screwdriver_act(mob/living/user, obj/item/W)
	...
	user.visible_message("<span_class='warning'>[user] [action] the floor.</span>", "<span_class='notice'>You start to [uraction] the floor...</span>", "You hear rustling noises.")
	if(W.use_tool(src, user, 100, volume=100, extra_checks = CALLBACK(src, .proc/check_anchored_state, anchored)))
		setAnchored(!anchored)
		to_chat(user, "<span_class='notice'> You [anchored ? "unscrew" : "screw"] [src] from the floor.</span>")
		return TRUE
	else
		return TRUE
...
/obj/structure/plasticflaps/wirecutter_act(mob/living/user, obj/item/W)
	if(!anchored)
		user.visible_message("<span_class='warning'>[user] cuts apart [src].</span>", "<span_class='notice'>You start to cut apart [src].</span>", "You hear cutting.")
