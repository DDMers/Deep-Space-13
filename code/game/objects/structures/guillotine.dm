		...
		if (GUILLOTINE_BLADE_RAISED)
			if (LAZYLEN(buckled_mobs))
				if (user.a_intent == INTENT_HARM)
					user.visible_message("<span_class='warning'>[user] begins to pull the lever!</span>",
						                 "<span_class='warning'>You begin to the pull the lever.</span>")
		...
		if (blade_status == GUILLOTINE_BLADE_RAISED)
			if (blade_sharpness < GUILLOTINE_BLADE_MAX_SHARP)
				...
				if(do_after(user, 7, target = src))
					blade_status = GUILLOTINE_BLADE_RAISED
					user.visible_message("<span_class='notice'>[user] sharpens the large blade of the guillotine.</span>",
						                 "<span_class='notice'>You sharpen the large blade of the guillotine.</span>")
					blade_sharpness += 1
				...
				to_chat(user, "<span_class='warning'>The blade is sharp enough!</span>")
				return
		else
			to_chat(user, "<span_class='warning'>You need to raise the blade in order to sharpen it!</span>")
			return
	else
		return ..()
...
/obj/structure/guillotine/buckle_mob(mob/living/M, force = FALSE, check_loc = TRUE)
	if (!anchored)
		to_chat(usr, "<span_class='warning'>The [src] needs to be wrenched to the floor!</span>")
		return FALSE

	if (!istype(M, /mob/living/carbon/human))
		to_chat(usr, "<span_class='warning'>It doesn't look like [M.p_they()] can fit into this properly!</span>")
		return FALSE // Can't decapitate non-humans

	if (blade_status != GUILLOTINE_BLADE_RAISED)
		to_chat(usr, "<span_class='warning'>You need to raise the blade before buckling someone in!</span>")
		return FALSE
...
/obj/structure/guillotine/can_be_unfasten_wrench(mob/user, silent)
	if (LAZYLEN(buckled_mobs))
		if (!silent)
			to_chat(user, "<span_class='warning'>Can't unfasten, someone's strapped in!</span>")
