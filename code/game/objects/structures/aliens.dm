...
/obj/structure/alien/egg/attack_hand(mob/living/user)
	...
	if(user.getorgan(/obj/item/organ/alien/plasmavessel))
		switch(status)
			if(BURST)
				to_chat(user, "<span_class='notice'>You clear the hatched egg.</span>")
				playsound(loc, 'sound/effects/attackblob.ogg', 100, 1)
				qdel(src)
				return
			if(GROWING)
				to_chat(user, "<span_class='notice'>The child is not developed yet.</span>")
				return
			if(GROWN)
				to_chat(user, "<span_class='notice'>You retrieve the child.</span>")
				Burst(kill=FALSE)
				return
	else
		to_chat(user, "<span_class='notice'>It feels slimy.</span>")
