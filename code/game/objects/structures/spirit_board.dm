...
/obj/structure/spirit_board/proc/spirit_board_pick_letter(mob/M)
	...
	var/msg = "<span_class='notice'>The planchette slowly moves... and stops at the letter \"[planchette]\".</span>"
...
/obj/structure/spirit_board/proc/spirit_board_checks(mob/M)
	...
	if(light_amount > 0.2)
		to_chat(M, "<span_class='warning'>It's too bright here to use [src.name]!</span>")
		return 0
	...
	for(var/mob/living/L in orange(1,src))
		if(L.ckey && L.client)
			if((world.time - L.client.inactivity) < (world.time - 300) || L.stat != CONSCIOUS || L.restrained())//no playing with braindeads or corpses or handcuffed dudes.
				to_chat(M, "<span_class='warning'>[L] doesn't seem to be paying attention...</span>")
			else
				users_in_range++

	if(users_in_range < 2)
		to_chat(M, "<span_class='warning'>There aren't enough people to use the [src.name]!</span>")
		return 0
