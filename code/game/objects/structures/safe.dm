...
/obj/structure/safe/proc/check_unlocked(mob/user, canhear)
	...
	if(user && canhear)
		if(tumbler_1_pos == tumbler_1_open)
			to_chat(user, "<span_class='italics'>You hear a [pick("tonk", "krunk", "plunk")] from [src].</span>")
		if(tumbler_2_pos == tumbler_2_open)
			to_chat(user, "<span_class='italics'>You hear a [pick("tink", "krink", "plink")] from [src].</span>")
...
/obj/structure/safe/Topic(href, href_list)
	...
	if(href_list["open"])
		if(check_unlocked())
			...
			to_chat(user, "<span_class='warning'>You can't [open ? "close" : "open"] [src], the lock is engaged!</span>")
			return
	...
	if(href_list["decrement"])
		...
		if(dial == tumbler_1_pos + 1 || dial == tumbler_1_pos - 71)
			...
			if(canhear)
				to_chat(user, "<span_class='italics'>You hear a [pick("clack", "scrape", "clank")] from [src].</span>")
			if(tumbler_1_pos == tumbler_2_pos + 37 || tumbler_1_pos == tumbler_2_pos - 35)
				tumbler_2_pos = decrement(tumbler_2_pos)
				if(canhear)
					to_chat(user, "<span_class='italics'>You hear a [pick("click", "chink", "clink")] from [src].</span>")
	...
	if(open)
		...
		if(I.w_class + space <= maxspace)
			...
			if(!user.transferItemToLoc(I, src))
				to_chat(user, "<span_class='warning'>\The [I] is stuck to your hand, you cannot put it in the safe!</span>")
				return
			to_chat(user, "<span_class='notice'>You put [I] in [src].</span>")
			updateUsrDialog()
			return
		else
			to_chat(user, "<span_class='notice'>[I] won't fit in [src].</span>")
			return
	else if(istype(I, /obj/item/clothing/neck/stethoscope))
		to_chat(user, "<span_class='warning'>Hold [I] in one of your hands while you manipulate the dial!</span>")
	else
		return ..()
