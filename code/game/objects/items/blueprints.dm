...
/obj/item/areaeditor/blueprints/Topic(href, href_list)
	...
	if(href_list["view_blueprints"])
		set_viewer(usr, "<span_class='notice'>You flip the blueprints over to view the complex information diagram.</span>")
	if(href_list["hide_blueprints"])
		clear_viewer(usr,"<span_class='notice'>You flip the blueprints over to view the simple information diagram.</span>")
...
/obj/item/areaeditor/proc/edit_area()
	...
	if(!str || !length(str) || str==prevname) //cancel
		return
	if(length(str) > 50)
		to_chat(usr, "<span_class='warning'>The given name is too long.  The area's name is unchanged.</span>")
		return

	rename_area(A, str)

	to_chat(usr, "<span_class='notice'>You rename the '[prevname]' to '[str]'.</span>")
