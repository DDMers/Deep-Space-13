...
/obj/item/a_gift/suicide_act(mob/user)
	user.visible_message("<span_class='suicide'>[user] peeks inside [src] and cries [user.p_them()]self to death! It looks like [user.p_they()] [user.p_were()] on the naughty list...</span>")
	return (BRUTELOSS)

/obj/item/a_gift/examine(mob/M)
	. = ..()
	if(M.has_trait(TRAIT_PRESENT_VISION) || isobserver(M))
		to_chat(M, "<span_class='notice'>It contains \a [initial(contains_type.name)].</span>")

/obj/item/a_gift/attack_self(mob/M)
	if(M.has_trait(TRAIT_CANNOT_OPEN_PRESENTS))
		to_chat(M, "<span_class='warning'>You're supposed to be spreading gifts, not opening them yourself!</span>")
		return

	qdel(src)

	var/obj/item/I = new contains_type(get_turf(M))
	M.visible_message("<span_class='notice'>[M] unwraps \the [src], finding \a [I] inside!</span>")
