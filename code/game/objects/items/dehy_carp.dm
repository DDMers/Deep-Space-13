...
/obj/item/toy/plush/carpplushie/dehy_carp/attack_self(mob/user)
	...
	if(!owned)
		to_chat(user, "<span_class='notice'>You pet [src]. You swear it looks up at you.</span>")
		owner = user
		owned = 1
	else
		return ..()
...
/obj/item/toy/plush/carpplushie/dehy_carp/proc/Swell()
	desc = "It's growing!"
	visible_message("<span_class='notice'>[src] swells up!</span>")
	...
	if (!owner || owner.faction != M.faction)
		visible_message("<span_class='warning'>You have a bad feeling about this.</span>") //welcome to the hostile carp enjoy your die
	else
		visible_message("<span_class='notice'>The newly grown [M.name] looks up at you with friendly eyes.</span>")
	qdel(src)
