...
/obj/item/aiModule/proc/install(datum/ai_laws/law_datum, mob/user)
	if(!bypass_law_amt_check && (!laws.len || laws[1] == "")) //So we don't loop trough an empty list and end up with runtimes.
		to_chat(user, "<span_class='warning'>ERROR: No laws found on board.</span>")
		return
	...
	if(law_datum)
		...
		if(tot_laws > CONFIG_GET(number/silicon_max_law_amount) && !bypass_law_amt_check)//allows certain boards to avoid this check, eg: reset
			to_chat(user, "<span_class='caution'>Not enough memory allocated to [law_datum.owner ? law_datum.owner : "the AI core"]'s law processor to handle this amount of laws.</span>")
	...
	if(law_datum.owner)
		to_chat(user, "<span_class='notice'>Upload complete. [law_datum.owner]'s laws have been modified.</span>")
		law_datum.owner.law_change_counter++
	else
		to_chat(user, "<span_class='notice'>Upload complete.</span>")
...
/obj/item/aiModule/proc/transmitInstructions(datum/ai_laws/law_datum, mob/sender, overflow = FALSE)
	if(law_datum.owner)
		to_chat(law_datum.owner, "<span_class='userdanger'>[sender] has uploaded a change to the laws you must follow using a [name].</span>")
...
/obj/item/aiModule/remove/attack_self(mob/user)
	...
	if(lawpos == null)
		return
	if(lawpos <= 0)
		to_chat(user, "<span_class='warning'>Error: The law number of [lawpos] is invalid.</span>")
		lawpos = 1
		return
	to_chat(user, "<span_class='notice'>Law [lawpos] selected.</span>")
	..()
...
/obj/item/aiModule/remove/install(datum/ai_laws/law_datum, mob/user)
	if(lawpos > (law_datum.get_law_amount(list(LAW_INHERENT = 1, LAW_SUPPLIED = 1))))
		to_chat(user, "<span_class='warning'>There is no law [lawpos] to delete!</span>")
		return
	..()
...
/obj/item/aiModule/core/full/overthrow/install(datum/ai_laws/law_datum, mob/user)
	...
	if(!O)
		to_chat(user, "<span_class='warning'>It appears that to install this module, you require a password you do not know.</span>") // This is the best fluff i could come up in my mind
		return
	var/mob/living/silicon/ai/AI = law_datum.owner
	if(!AI)
		return
	...
	if(T)
		...
		if(AI)
			to_chat(AI, "<span_class='userdanger'>You feel your circuits being scrambled! You serve another overthrow team now!</span>") // to make it clearer for the AI
	T = target_mind.add_antag_datum(/datum/antagonist/overthrow, O.team)
	if(AI)
		to_chat(AI, "<span_class='warning'>You serve the [T.team] team now! Assist them in completing the team shared objectives, which you can see in your notes.</span>")
	..()
...
//	..()    //We don't want this module reporting to the AI who dun it. --NEO
	if(law_datum.owner)
		to_chat(law_datum.owner, "<span_class='warning'>BZZZZT</span>")
		if(!overflow)
...
/obj/item/aiModule/toyAI/attack_self(mob/user)
	...
	to_chat(user, "<span_class='notice'>You press the button on [src].</span>")
	playsound(user, 'sound/machines/click.ogg', 20, 1)
	src.loc.visible_message("<span_class='warning'>[icon2html(src, viewers(loc))] [laws[1]]</span>")
