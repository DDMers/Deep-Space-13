...
/obj/item/gun/energy/chrono_gun/proc/field_connect(obj/structure/chrono_field/F)
	...
	if(F.gun)
		if(isliving(user) && F.captured)
			to_chat(user, "<span_class='alert'><b>FAIL: <i>[F.captured]</i> already has an existing connection.</b></span>")
		field_disconnect(F)
	else
		...
		F.gun = src
		if(isliving(user) && F.captured)
			to_chat(user, "<span_class='notice'>Connection established with target: <b>[F.captured]</b></span>")
...
/obj/item/gun/energy/chrono_gun/proc/field_disconnect(obj/structure/chrono_field/F)
	if(F && field == F)
		...
		if(F.gun == src)
			F.gun = null
		if(isliving(user) && F.captured)
			to_chat(user, "<span_class='alert'>Disconnected from target: <b>[F.captured]</b></span>")
	field = null
	startpos = null
...
/obj/structure/chrono_field/Initialize(mapload, mob/living/target, obj/item/gun/energy/chrono_gun/G)
	if(target && isliving(target) && G)
		...
		desc = initial(desc) + "<br><span_class='info'>It appears to contain [target.name].</span>"
...
/obj/structure/chrono_field/process()
	if(captured)
		if(tickstokill > initial(tickstokill))
			...
			to_chat(captured, "<span_class='boldnotice'>As the last essence of your being is erased from time, you are taken back to your most enjoyable memory. You feel happy...</span>")
