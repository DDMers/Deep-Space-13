...
/obj/item/melee/transforming/energy/suicide_act(mob/user)
	...
	user.visible_message("<span_class='suicide'>[user] is [pick("slitting [user.p_their()] stomach open with", "falling on")] [src]! It looks like [user.p_theyre()] trying to commit seppuku!</span>")
...
/obj/item/melee/transforming/energy/axe/suicide_act(mob/user)
	user.visible_message("<span_class='suicide'>[user] swings [src] towards [user.p_their()] head! It looks like [user.p_theyre()] trying to commit suicide!</span>")
	return (BRUTELOSS|FIRELOSS)
...
/obj/item/melee/transforming/energy/sword/cyborg/attack(mob/M, var/mob/living/silicon/robot/R)
	if(R.cell)
		...
		if(active && !(C.use(hitcost)))
			attack_self(R)
			to_chat(R, "<span_class='notice'>It's out of charge!</span>")
			return
		return ..()
