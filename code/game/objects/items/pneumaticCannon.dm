...
/obj/item/pneumatic_cannon/examine(mob/user)
	...
	if(!in_range(user, src))
		out += "<span_class='notice'>You'll need to get closer to see any more.</span>"
		return
	for(var/obj/item/I in loadedItems)
		out += "<span_class='info'>[icon2html(I, user)] It has \a [I] loaded.</span>"
		CHECK_TICK
	if(tank)
		out += "<span_class='notice'>[icon2html(tank, user)] It has \a [tank] mounted onto it.</span>"
...
/obj/item/pneumatic_cannon/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/tank/internals))
		if(!tank)
			...
			if(IT.volume <= 3)
				to_chat(user, "<span_class='warning'>\The [IT] is too small for \the [src].</span>")
				return
			updateTank(W, 0, user)
	else if(W.type == type)
		to_chat(user, "<span_class='warning'>You're fairly certain that putting a pneumatic cannon inside another pneumatic cannon would cause a spacetime disruption.</span>")
		...
		switch(pressureSetting)
			...
			if(3)
				pressureSetting = 1
		to_chat(user, "<span_class='notice'>You tweak \the [src]'s pressure output to [pressureSetting].</span>")
	else if(W.tool_behaviour == TOOL_SCREWDRIVER)
		if(tank)
			updateTank(tank, 1, user)
	else if(loadedWeightClass >= maxWeightClass)
		to_chat(user, "<span_class='warning'>\The [src] can't hold any more items!</span>")
	else if(isitem(W))
		var/obj/item/IW = W
		load_item(IW, user)
...
/obj/item/pneumatic_cannon/proc/can_load_item(obj/item/I, mob/user)
	...
	if(allowed_typecache && !is_type_in_typecache(I, allowed_typecache))
		if(user)
			to_chat(user, "<span_class='warning'>[I] won't fit into [src]!</span>")
		return
	if((loadedWeightClass + I.w_class) > maxWeightClass)	//Only make messages if there's a user
		if(user)
			to_chat(user, "<span_class='warning'>\The [I] won't fit into \the [src]!</span>")
		return FALSE
	if(I.w_class > w_class)
		if(user)
			to_chat(user, "<span_class='warning'>\The [I] is too large to fit into \the [src]!</span>")
		return FALSE
	return TRUE
...
/obj/item/pneumatic_cannon/proc/load_item(obj/item/I, mob/user)
	...
	if(user)		//Only use transfer proc if there's a user, otherwise just set loc.
		if(!user.transferItemToLoc(I, src))
			return FALSE
		to_chat(user, "<span_class='notice'>You load \the [I] into \the [src].</span>")
	else
		I.forceMove(src)
	loadedItems += I
...
/obj/item/pneumatic_cannon/proc/Fire(mob/living/user, var/atom/target)
	...
	if(!loadedItems || !loadedWeightClass)
		to_chat(user, "<span_class='warning'>\The [src] has nothing loaded.</span>")
		return
	if(!tank && checktank)
		to_chat(user, "<span_class='warning'>\The [src] can't fire without a source of gas.</span>")
		return
	if(tank && !tank.air_contents.remove(gasPerThrow * pressureSetting))
		to_chat(user, "<span_class='warning'>\The [src] lets out a weak hiss and doesn't react!</span>")
		return
	if(user.has_trait(TRAIT_CLUMSY) && prob(75) && clumsyCheck && iscarbon(user))
		var/mob/living/carbon/C = user
		C.visible_message("<span_class='warning'>[C] loses [C.p_their()] grip on [src], causing it to go off!</span>", "<span_class='userdanger'>[src] slips out of your hands and goes off!</span>")
		...
		discharge = 1
	if(!discharge)
		user.visible_message("<span_class='danger'>[user] fires \the [src]!</span>", \
				    		 "<span_class='danger'>You fire \the [src]!</span>")
	...
	if(pressureSetting >= 3 && iscarbon(user))
		var/mob/living/carbon/C = user
		C.visible_message("<span_class='warning'>[C] is thrown down by the force of the cannon!</span>", "<span_class='userdanger'>[src] slams into your shoulder, knocking you down!")
		C.Paralyze(60)
...
/obj/item/pneumatic_cannon/proc/updateTank(obj/item/tank/internals/thetank, removing = 0, mob/living/carbon/human/user)
	if(removing)
		if(!tank)
			return
		to_chat(user, "<span_class='notice'>You detach \the [thetank] from \the [src].</span>")
		tank.forceMove(user.drop_location())
		user.put_in_hands(tank)
		tank = null
	if(!removing)
		if(tank)
			to_chat(user, "<span_class='warning'>\The [src] already has a tank.</span>")
			return
		if(!user.transferItemToLoc(thetank, src))
			return
		to_chat(user, "<span_class='notice'>You hook \the [thetank] up to \the [src].</span>")
		tank = thetank
	update_icons()
