...
/obj/item/borg/stun/attack(mob/living/M, mob/living/user)
	...
	M.visible_message("<span_class='danger'>[user] has prodded [M] with [src]!</span>", \
					"<span_class='userdanger'>[user] has prodded you with [src]!</span>")
	...
	switch(mode)
		if(0)
			if(M.health >= 0)
				if(user.zone_selected == BODY_ZONE_HEAD)
					user.visible_message("<span_class='notice'>[user] playfully boops [M] on the head!</span>", \
									"<span_class='notice'>You playfully boop [M] on the head!</span>")
					user.do_attack_animation(M, ATTACK_EFFECT_BOOP)
					playsound(loc, 'sound/weapons/tap.ogg', 50, 1, -1)
				else if(ishuman(M))
					if(!(user.mobility_flags & MOBILITY_STAND))
						user.visible_message("<span_class='notice'>[user] shakes [M] trying to get [M.p_them()] up!</span>", \
										"<span_class='notice'>You shake [M] trying to get [M.p_them()] up!</span>")
					else
						user.visible_message("<span_class='notice'>[user] hugs [M] to make [M.p_them()] feel better!</span>", \
								"<span_class='notice'>You hug [M] to make [M.p_them()] feel better!</span>")
					if(M.resting)
						M.set_resting(FALSE, TRUE)
				else
					user.visible_message("<span_class='notice'>[user] pets [M]!</span>", \
							"<span_class='notice'>You pet [M]!</span>")
			...
			if(M.health >= 0)
				if(ishuman(M))
					if(!(M.mobility_flags & MOBILITY_STAND))
						...
						user.visible_message("<span_class='warning'>[user] bops [M] on the head!</span>", \
										"<span_class='warning'>You bop [M] on the head!</span>")
						user.do_attack_animation(M, ATTACK_EFFECT_PUNCH)
					else
						user.visible_message("<span_class='warning'>[user] hugs [M] in a firm bear-hug! [M] looks uncomfortable...</span>", \
								"<span_class='warning'>You hug [M] firmly to make [M.p_them()] feel better! [M] looks uncomfortable...</span>")
					if(M.resting)
						M.set_resting(FALSE, TRUE)
				else
					user.visible_message("<span_class='warning'>[user] bops [M] on the head!</span>", \
			...
			if(scooldown < world.time)
				if(M.health >= 0)
					if(ishuman(M)||ismonkey(M))
						...
						user.visible_message("<span_class='userdanger'>[user] electrocutes [M] with [user.p_their()] touch!</span>", \
							"<span_class='danger'>You electrocute [M] with your touch!</span>")
						M.update_mobility()
					else
						if(!iscyborg(M))
							M.adjustFireLoss(10)
							user.visible_message("<span_class='userdanger'>[user] shocks [M]!</span>", \
								"<span_class='danger'>You shock [M]!</span>")
						else
							user.visible_message("<span_class='userdanger'>[user] shocks [M]. It does not seem to have an effect</span>", \
								"<span_class='danger'>You shock [M] to no effect.</span>")
		...
		if(3)
			if(ccooldown < world.time)
				if(M.health >= 0)
					if(ishuman(M))
						user.visible_message("<span_class='userdanger'>[user] crushes [M] in [user.p_their()] grip!</span>", \
							"<span_class='danger'>You crush [M] in your grip!</span>")
					else
						user.visible_message("<span_class='userdanger'>[user] crushes [M]!</span>", \
								"<span_class='danger'>You crush [M]!</span>")
...
/obj/item/borg/charger/attack_self(mob/user)
	if(mode == "draw")
		...
		mode = "draw"
	to_chat(user, "<span_class='notice'>You toggle [src] to \"[mode]\" mode.</span>")
	update_icon()
	...
	if(mode == "draw")
		if(is_type_in_list(target, charge_machines))
			...
			if((M.stat & (NOPOWER|BROKEN)) || !M.anchored)
				to_chat(user, "<span_class='warning'>[M] is unpowered!</span>")
				return

			to_chat(user, "<span_class='notice'>You connect to [M]'s power line...</span>")
			...
			to_chat(user, "<span_class='notice'>You stop charging yourself.</span>")
			...
			if(!cell)
				to_chat(user, "<span_class='warning'>[target] has no power cell!</span>")
				return

			if(istype(target, /obj/item/gun/energy))
				var/obj/item/gun/energy/E = target
				if(!E.can_charge)
					to_chat(user, "<span_class='warning'>[target] has no power port!</span>")
					return

			if(!cell.charge)
				to_chat(user, "<span_class='warning'>[target] has no power!</span>")


			to_chat(user, "<span_class='notice'>You connect to [target]'s power port...</span>")
		...
		if(cell.charge >= cell.maxcharge)
			to_chat(user, "<span_class='warning'>[target] is already charged!</span>")
		...
		to_chat(user, "<span_class='notice'>You stop charging [target].</span>")
...
/obj/item/harmalarm/attack_self(mob/user)
	...
	if(safety == TRUE)
		user.visible_message("<font color='red' size='2'>[user] blares out a near-deafening siren from its speakers!</font>", \
			"<span_class='userdanger'>The siren pierces your hearing and confuses you!</span>", \
			"<span_class='danger'>The siren pierces your hearing!</span>")
		...
		if(iscyborg(user))
			var/mob/living/silicon/robot/R = user
			to_chat(R.connected_ai, "<br><span_class='notice'>NOTICE - Peacekeeping 'HARM ALARM' used by: [user]</span><br>")
...
/obj/item/borg/lollipop/proc/dispense(atom/A, mob/user)
	if(candy <= 0)
		to_chat(user, "<span_class='warning'>No treats left in storage!</span>")
		return FALSE
	...
	if(into_hands)
		user.visible_message("<span_class='notice'>[user] dispenses a treat into the hands of [A].</span>", "<span_class='notice'>You dispense a treat into the hands of [A].</span>", "<span_class='italics'>You hear a click.</span>")
	else
		user.visible_message("<span_class='notice'>[user] dispenses a treat.</span>", "<span_class='notice'>You dispense a treat.</span>", "<span_class='italics'>You hear a click.</span>")
...
/obj/item/borg/lollipop/proc/shootL(atom/target, mob/living/user, params)
	if(candy <= 0)
		to_chat(user, "<span_class='warning'>Not enough lollipops left!</span>")
		return FALSE
	candy--
	...
	user.visible_message("<span_class='warning'>[user] blasts a flying lollipop at [target]!</span>")
	check_amount()

/obj/item/borg/lollipop/proc/shootG(atom/target, mob/living/user, params)	//Most certainly a good idea.
	if(candy <= 0)
		to_chat(user, "<span_class='warning'>Not enough gumballs left!</span>")
		return FALSE
	candy--
	...
	A.fire_casing(target, user, params, 0, 0, null, 0)
	user.visible_message("<span_class='warning'>[user] shoots a high-velocity gumball at [target]!</span>")
	check_amount()
...
/obj/item/borg/lollipop/afterattack(atom/target, mob/living/user, proximity, click_params)
	...
	if(iscyborg(user))
		var/mob/living/silicon/robot/R = user
		if(!R.cell.use(12))
			to_chat(user, "<span_class='warning'>Not enough power.</span>")
			return FALSE
		if(R.emagged)
...
/obj/item/borg/lollipop/attack_self(mob/living/user)
	switch(mode)
		if(DISPENSE_LOLLIPOP_MODE)
			...
			to_chat(user, "<span_class='notice'>Module is now throwing lollipops.</span>")
		if(THROW_LOLLIPOP_MODE)
			mode = THROW_GUMBALL_MODE
			to_chat(user, "<span_class='notice'>Module is now blasting gumballs.</span>")
		if(THROW_GUMBALL_MODE)
			mode = DISPENSE_ICECREAM_MODE
			to_chat(user, "<span_class='notice'>Module is now dispensing ice cream.</span>")
		if(DISPENSE_ICECREAM_MODE)
			mode = DISPENSE_LOLLIPOP_MODE
			to_chat(user, "<span_class='notice'>Module is now dispensing lollipops.</span>")
	..()
...
/obj/item/borg/projectile_dampen
	...
	desc = "A device that projects a dampening field that weakens kinetic energy above a certain threshold. <span_class='boldnotice'>Projects a field that drains power per second while active, that will weaken and slow damaging projectiles inside its field.</span> Still being a prototype, it tends to induce a charge on ungrounded metallic surfaces."
	icon = 'icons/obj/device.dmi'
	icon_state = "shield"
	var/maxenergy = 1500
	var/energy = 1500
...
/obj/item/borg/projectile_dampen/attack_self(mob/user)
	if(cycle_delay > world.time)
		to_chat(user, "<span_class='boldwarning'>[src] is still recycling its projectors!</span>")
		return
	...
	if(!active)
		if(!user.has_buckled_mobs())
			...
			to_chat(user, "<span_class='warning'>[src]'s safety cutoff prevents you from activating it due to living beings being ontop of you!</span>")
	else
		deactivate_field()
	update_icon()
	to_chat(user, "<span_class='boldnotice'>You [active? "activate":"deactivate"] [src].</span>")
...
/obj/item/borg/projectile_dampen/proc/deactivate_field()
	QDEL_NULL(dampening_field)
	visible_message("<span_class='warning'>\The [src] shuts off!</span>")
...
/obj/item/borg/projectile_dampen/proc/process_usage()
	...
	if(energy <= 0)
		deactivate_field()
		visible_message("<span_class='warning'>[src] blinks \"ENERGY DEPLETED\".</span>")
