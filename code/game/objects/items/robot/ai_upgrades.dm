...
/obj/item/malf_upgrade/afterattack(mob/living/silicon/ai/AI, mob/user)
	...
	if(AI.malf_picker)
		...
		to_chat(AI, "<span_class='userdanger'>[user] has attempted to upgrade you with combat software that you already possess. You gain 50 points to spend on Malfunction Modules instead.</span>")
	else
		to_chat(AI, "<span_class='userdanger'>[user] has upgraded you with combat software!</span>")
		to_chat(AI, "<span_class='userdanger'>Your current laws and objectives remain unchanged.</span>") //this unlocks malf powers, but does not give the license to plasma flood
		AI.add_malf_picker()
		log_game("[key_name(user)] has upgraded [key_name(AI)] with a [src].")
		message_admins("[ADMIN_LOOKUPFLW(user)] has upgraded [ADMIN_LOOKUPFLW(AI)] with a [src].")
	to_chat(user, "<span_class='notice'>You upgrade [AI]. [src] is consumed in the process.</span>")
	qdel(src)
...
/obj/item/surveillance_upgrade/afterattack(mob/living/silicon/ai/AI, mob/user)
	...
	if(AI.eyeobj)
		...
		to_chat(AI, "<span_class='userdanger'>[user] has upgraded you with surveillance software!</span>")
