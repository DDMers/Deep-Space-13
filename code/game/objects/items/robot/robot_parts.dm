	...
	if(l_leg || r_leg || chest || l_arm || r_arm || head)
		if(I.use_tool(src, user, 5, volume=50))
			...
			if(head)
				...
				head.flash2 = null
				head = null
			to_chat(user, "<span_class='notice'>You disassemble the cyborg shell.</span>")
	else
		to_chat(user, "<span_class='notice'>There is nothing to remove from the endoskeleton.</span>")
	update_icon()
...
/obj/item/robot_suit/screwdriver_act(mob/living/user, obj/item/I) //Swaps the power cell if you're holding a new one in your other hand.
	...
	if(!chest) //can't remove a cell if there's no chest to remove it from.
		to_chat(user, "<span_class='notice'>[src] has no attached torso.</span>")
		return
	...
	if(!temp_cell) //if we're not holding a cell
		...
		swap_failed = TRUE
		to_chat(user, "<span_class='warning'>[temp_cell] is stuck to your hand, you can't put it in [src]!</span>")
	...
	if(swap_failed) //we didn't transfer any new items.
		if(chest.cell) //old cell ejected, nothing inserted.
			to_chat(user, "<span_class='notice'>You remove [chest.cell] from [src].</span>")
			chest.cell = null
		else
			to_chat(user, "<span_class='notice'>The power cell slot in [src]'s torso is empty.</span>")
		return

	to_chat(user, "<span_class='notice'>You [chest.cell ? "replace [src]'s [chest.cell.name] with [temp_cell]" : "insert [temp_cell] into [src]"].</span>")
	...
	if(istype(W, /obj/item/stack/sheet/metal))
		...
		if(!l_arm && !r_arm && !l_leg && !r_leg && !chest && !head)
			if (M.use(1))
				...
				to_chat(user, "<span_class='notice'>You arm the robot frame.</span>")
				var/holding_this = user.get_inactive_held_item()==src
				qdel(src)
				if (holding_this)
					user.put_in_inactive_hand(B)
			else
				to_chat(user, "<span_class='warning'>You need one sheet of metal to start building ED-209!</span>")
				return
		...
		if(CH.wired && CH.cell)
			...
			to_chat(user, "<span_class='warning'>You need to attach wires to it first!</span>")
		else
			to_chat(user, "<span_class='warning'>You need to attach a cell to it first!</span>")
		...
		for(var/X in HD.contents)
			if(istype(X, /obj/item/organ))
				to_chat(user, "<span_class='warning'>There are organs inside [HD]!</span>")
				return
		if(head)
			return
		if(HD.flash2 && HD.flash1)
			...
			to_chat(user, "<span_class='warning'>You need to attach a flash to it first!</span>")

	else if (W.tool_behaviour == TOOL_MULTITOOL)
		if(check_completion())
			Interact(user)
		else
			to_chat(user, "<span_class='warning'>The endoskeleton must be assembled before debugging can begin!</span>")

	else if(istype(W, /obj/item/mmi))
		var/obj/item/mmi/M = W
		if(check_completion())
			if(!chest.cell)
				to_chat(user, "<span_class='warning'>The endoskeleton still needs a power cell!</span>")
				return
			if(!isturf(loc))
				to_chat(user, "<span_class='warning'>You can't put [M] in, the frame has to be standing on the ground to be perfectly precise!</span>")
				return
			if(!M.brainmob)
				to_chat(user, "<span_class='warning'>Sticking an empty [M.name] into the frame would sort of defeat the purpose!</span>")
				return

			var/mob/living/brain/BM = M.brainmob
			if(!BM.key || !BM.mind)
				to_chat(user, "<span_class='warning'>The MMI indicates that their mind is completely unresponsive; there's no point!</span>")
				return

			if(!BM.client) //braindead
				to_chat(user, "<span_class='warning'>The MMI indicates that their mind is currently inactive; it might change!</span>")
				return

			if(BM.stat == DEAD || BM.suiciding || (M.brain && (M.brain.brain_death || M.brain.suicided)))
				to_chat(user, "<span_class='warning'>Sticking a dead brain into the frame would sort of defeat the purpose!</span>")
				return

			if(M.brain?.damaged_brain)
				to_chat(user, "<span_class='warning'>The MMI indicates that the brain is damaged!</span>")
				return

			if(is_banned_from(BM.ckey, "Cyborg") || QDELETED(src) || QDELETED(BM) || QDELETED(user) || QDELETED(M) || !Adjacent(user))
				if(!QDELETED(M))
					to_chat(user, "<span_class='warning'>This [M.name] does not seem to fit!</span>")
				return
			...
			if(O.mind && O.mind.special_role)
				...
				to_chat(O, "<span_class='userdanger'>You have been robotized!</span>")
				to_chat(O, "<span_class='danger'>You must obey your silicon laws and master AI above all else. Your objectives will consider you to be dead.</span>")
			...
			if(!locomotion)
				O.lockcharge = TRUE
				O.update_mobility()
				to_chat(O, "<span_class='warning'>Error: Servo motors unresponsive.</span>")

		else
			to_chat(user, "<span_class='warning'>The MMI must go in after everything else!</span>")

	else if(istype(W, /obj/item/borg/upgrade/ai))
		var/obj/item/borg/upgrade/ai/M = W
		if(check_completion())
			if(!isturf(loc))
				to_chat(user, "<span_class='warning'>You cannot install[M], the frame has to be standing on the ground to be perfectly precise!</span>")
				return
			if(!user.temporarilyRemoveItemFromInventory(M))
				to_chat(user, "<span_class='warning'>[M] is stuck to your hand!</span>")
				return
			qdel(M)
		...
		to_chat(user, "<span_class='warning'>You need to use a multitool to name [src]!</span>")
	else
		return ..()
...
/obj/item/robot_suit/Topic(href, href_list)
	...
	if(!item_in_hand || item_in_hand.tool_behaviour != TOOL_MULTITOOL)
		to_chat(living_user, "<span_class='warning'>You need a multitool!</span>")
		return
	...
	if(href_list["Name"])
		...
		if(!forced_ai)
			to_chat(usr, "<span_class='error'>No active AIs detected.</span>")
