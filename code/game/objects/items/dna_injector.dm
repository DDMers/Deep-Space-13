...
/obj/item/dnainjector/proc/inject(mob/living/carbon/M, mob/user)
	if(M.has_dna() && !M.has_trait(TRAIT_RADIMMUNE) && !M.has_trait(TRAIT_BADDNA))
		...
		for(var/HM in add_mutations)
			if(HM == RACEMUT)
				message_admins("[ADMIN_LOOKUPFLW(user)] injected [key_name_admin(M)] with the [name] <span_class='danger'>(MONKEY)</span>")
				log_msg += " (MONKEY)"
...
/obj/item/dnainjector/attack(mob/target, mob/user)
	if(!user.IsAdvancedToolUser())
		...
		return
	if(used)
		to_chat(user, "<span_class='warning'>This injector is used up!</span>")
		return
	if(ishuman(target))
	...
	if(target != user)
		target.visible_message("<span_class='danger'>[user] is trying to inject [target] with [src]!</span>", "<span_class='userdanger'>[user] is trying to inject [target] with [src]!</span>")
		if(!do_mob(user, target) || used)
			return
		target.visible_message("<span_class='danger'>[user] injects [target] with the syringe with [src]!", \
						"<span_class='userdanger'>[user] injects [target] with the syringe with [src]!</span>")

	else
		to_chat(user, "<span_class='notice'>You inject yourself with [src].</span>")

	log_combat(user, target, "injected", src)

	if(!inject(target, user))	//Now we actually do the heavy lifting.
		to_chat(user, "<span_class='notice'>It appears that [target] does not have compatible DNA.</span>")
...
/obj/item/dnainjector/timed/inject(mob/living/carbon/M, mob/user)
	if(M.stat == DEAD)	//prevents dead people from having their DNA changed
		to_chat(user, "<span_class='notice'>You can't modify [M]'s DNA while [M.p_theyre()] dead.</span>")
		return FALSE
