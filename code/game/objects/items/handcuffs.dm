...
/obj/item/restraints/handcuffs/attack(mob/living/carbon/C, mob/living/user)
	...
	if(iscarbon(user) && (user.has_trait(TRAIT_CLUMSY) && prob(50)))
		to_chat(user, "<span_class='warning'>Uh... how do those things work?!</span>")
	...
	if(!C.handcuffed)
		if(C.get_num_arms(FALSE) >= 2 || C.get_arm_ignore())
			C.visible_message("<span_class='danger'>[user] is trying to put [src.name] on [C]!</span>", \
								"<span_class='userdanger'>[user] is trying to put [src.name] on [C]!</span>")
			...
			if(do_mob(user, C, 30) && (C.get_num_arms(FALSE) >= 2 || C.get_arm_ignore()))
				if(iscyborg(user))
					...
					apply_cuffs(C, user)
				to_chat(user, "<span_class='notice'>You handcuff [C].</span>")
				SSblackbox.record_feedback("tally", "handcuffs", 1, type)

				log_combat(user, C, "handcuffed")
			else
				to_chat(user, "<span_class='warning'>You fail to handcuff [C]!</span>")
		else
...
/obj/item/restraints/handcuffs/cable/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/stack/rods))
		...
		if (R.use(1))
			...
			user.put_in_hands(W)
			to_chat(user, "<span_class='notice'>You wrap the cable restraint around the top of the rod.</span>")
			qdel(src)
		else
			to_chat(user, "<span_class='warning'>You need one rod to make a wired rod!</span>")
			return
	else if(istype(I, /obj/item/stack/sheet/metal))
		var/obj/item/stack/sheet/metal/M = I
		if(M.get_amount() < 6)
			to_chat(user, "<span_class='warning'>You need at least six metal sheets to make good enough weights!</span>")
			return
		to_chat(user, "<span_class='notice'>You begin to apply [I] to [src]...</span>")
		if(do_after(user, 35, target = src))
			...
			M.use(6)
			user.put_in_hands(S)
			to_chat(user, "<span_class='notice'>You make some weights out of [I] and tie them to [src].</span>")
			remove_item_from_storage(user)
			qdel(src)
	else
		return ..()
...
/obj/item/restraints/legcuffs/beartrap/suicide_act(mob/user)
	user.visible_message("<span_class='suicide'>[user] is sticking [user.p_their()] head in the [src.name]! It looks like [user.p_theyre()] trying to commit suicide!</span>")
...
/obj/item/restraints/legcuffs/beartrap/attack_self(mob/user)
	...
	if(ishuman(user) && !user.stat && !user.restrained())
		armed = !armed
		icon_state = "[initial(icon_state)][armed]"
		to_chat(user, "<span_class='notice'>[src] is now [armed ? "armed" : "disarmed"]</span>")
	...
	if(armed && isturf(src.loc))
		if(isliving(AM))
			...
			if(snap)
				...
				L.visible_message("<span_class='danger'>[L] triggers \the [src].</span>", \
						"<span_class='userdanger'>You trigger \the [src]!</span>")
...
/obj/item/restraints/legcuffs/bola/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(!C.legcuffed && C.get_num_legs(FALSE) >= 2)
		visible_message("<span_class='danger'>\The [src] ensnares [C]!</span>")
		C.legcuffed = src
		forceMove(C)
		C.update_inv_legcuffed()
		SSblackbox.record_feedback("tally", "handcuffs", 1, type)
		to_chat(C, "<span_class='userdanger'>\The [src] ensnares you!</span>")
