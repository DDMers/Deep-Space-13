...
/obj/item/tank/proc/toggle_internals(mob/user)
	...
	if(H.internal == src)
		to_chat(H, "<span_class='notice'>You close [src] valve.</span>")
		H.internal = null
		H.update_internals_hud_icon(0)
	else
		if(!H.getorganslot(ORGAN_SLOT_BREATHING_TUBE))
			if(!H.wear_mask)
				to_chat(H, "<span_class='warning'>You need a mask!</span>")
				return
			if(H.wear_mask.mask_adjusted)
				H.wear_mask.adjustmask(H)
			if(!(H.wear_mask.clothing_flags & MASKINTERNALS))
				to_chat(H, "<span_class='warning'>[H.wear_mask] can't use [src]!</span>")
				return

		if(H.internal)
			to_chat(H, "<span_class='notice'>You switch your internals to [src].</span>")
		else
			to_chat(H, "<span_class='notice'>You open [src] valve.</span>")
		H.internal = src
...
/obj/item/tank/examine(mob/user)
	...
	if(!in_range(src, user) && !isobserver(user))
		if(icon == src)
			to_chat(user, "<span_class='notice'>If you want any more information you'll need to get closer.</span>")
		return

	to_chat(user, "<span_class='notice'>The pressure gauge reads [round(src.air_contents.return_pressure(),0.01)] kPa.</span>")
	...
	to_chat(user, "<span_class='notice'>It feels [descriptive].</span>")
...
/obj/item/tank/suicide_act(mob/user)
	var/mob/living/carbon/human/H = user
	user.visible_message("<span_class='suicide'>[user] is putting [src]'s valve to [user.p_their()] lips! It looks like [user.p_theyre()] trying to commit suicide!</span>")
