...
/obj/item/watertank/proc/toggle_mister(mob/living/user)
	...
	if(user.get_item_by_slot(user.getBackSlot()) != src)
		to_chat(user, "<span_class='warning'>The watertank must be worn properly to use!</span>")
		return
	...
	if(noz in src)
		...
		if(!user.put_in_hands(noz))
			to_chat(user, "<span_class='warning'>You need a free hand to hold the mister!</span>")
			return
	else
...
/obj/item/reagent_containers/spray/mister/doMove(atom/destination)
	if(destination && (destination != tank.loc || !ismob(destination)))
		if (loc != tank)
			to_chat(tank.loc, "<span_class='notice'>The mister snaps back onto the watertank.</span>")
		destination = tank
	..()
...
/obj/item/reagent_containers/spray/mister/janitor/attack_self(var/mob/user)
	...
	to_chat(user, "<span_class='notice'>You [amount_per_transfer_from_this == 10 ? "remove" : "fix"] the nozzle. You'll now use [amount_per_transfer_from_this] units per spray.</span>")
...
/obj/item/extinguisher/mini/nozzle/doMove(atom/destination)
	if(destination && (destination != tank.loc || !ismob(destination)))
		if(loc != tank)
			to_chat(tank.loc, "<span_class='notice'>The nozzle snaps back onto the tank!</span>")
		destination = tank
	..()
...
/obj/item/extinguisher/mini/nozzle/afterattack(atom/target, mob/user)
	...
	if(nozzle_mode == RESIN_LAUNCHER)
		...
		if(R.total_volume < 100)
			to_chat(user, "<span_class='warning'>You need at least 100 units of water to use the resin launcher!</span>")
			return
		if(resin_cooldown)
			to_chat(user, "<span_class='warning'>Resin launcher is still recharging...</span>")
			return
		resin_cooldown = TRUE
		R.remove_any(100)
	...
	if(nozzle_mode == RESIN_FOAM)
		...
		for(var/S in target)
			if(istype(S, /obj/effect/particle_effect/foam/metal/resin) || istype(S, /obj/structure/foamedmetal/resin))
				to_chat(user, "<span_class='warning'>There's already resin here!</span>")
				return
		if(metal_synthesis_cooldown < 5)
			...
			to_chat(user, "<span_class='warning'>Resin foam mix is still being synthesized...</span>")
			return
...
/obj/item/reagent_containers/chemtank/proc/toggle_injection()
	...
	if (user.get_item_by_slot(SLOT_BACK) != src)
		to_chat(user, "<span_class='warning'>The chemtank needs to be on your back before you can activate it!</span>")
		return
	if(on)
		turn_off()
	else
...
/obj/item/reagent_containers/chemtank/proc/turn_on()
	...
	if(ismob(loc))
		to_chat(loc, "<span_class='notice'>[src] turns on.</span>")

/obj/item/reagent_containers/chemtank/proc/turn_off()
	on = FALSE
	STOP_PROCESSING(SSobj, src)
	if(ismob(loc))
		to_chat(loc, "<span_class='notice'>[src] turns off.</span>")
