...
/obj/item/tank/jetpack/ui_action_click(mob/user, action)
	if(istype(action, /datum/action/item_action/toggle_jetpack))
		...
		if(on)
			...
			to_chat(user, "<span_class='notice'>You turn the jetpack stabilization [stabilizers ? "on" : "off"].</span>")
	else
...
/obj/item/tank/jetpack/proc/cycle(mob/user)
	...
	if(!on)
		turn_on(user)
		to_chat(user, "<span_class='notice'>You turn the jetpack on.</span>")
	else
		turn_off(user)
		to_chat(user, "<span_class='notice'>You turn the jetpack off.</span>")
...
/obj/item/tank/jetpack/suicide_act(mob/user)
	if (istype(user, /mob/living/carbon/human/))
		...
		H.visible_message("<span_class='suicide'>[user] is suffocating [user.p_them()]self with [src]! It looks like [user.p_they()] didn't read what that jetpack says!</span>")
		return (OXYLOSS)
	else
		..()
...
/obj/item/tank/jetpack/improvised/allow_thrust(num, mob/living/user)
	...
	if((num < 0.005 || air_contents.total_moles() < num))
		...
		return
	if(rand(0,250) == 0)
		to_chat(user, "<span_class='notice'>You feel your jetpack's engines cut out.</span>")
		turn_off(user)
		return
...
/obj/item/tank/jetpack/suit/cycle(mob/user)
	if(!istype(loc, /obj/item/clothing/suit/space/hardsuit))
		to_chat(user, "<span_class='warning'>\The [src] must be connected to a hardsuit!</span>")
		return

	var/mob/living/carbon/human/H = user
	if(!istype(H.s_store, /obj/item/tank/internals))
		to_chat(user, "<span_class='warning'>You need a tank in your suit storage!</span>")
		return
	..()
