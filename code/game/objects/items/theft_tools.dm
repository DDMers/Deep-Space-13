...
/obj/item/nuke_core/suicide_act(mob/user)
	user.visible_message("<span_class='suicide'>[user] is rubbing [src] against [user.p_them()]self! It looks like [user.p_theyre()] trying to commit suicide!</span>")
	return (TOXLOSS)
...
/obj/item/nuke_core_container/proc/load(obj/item/nuke_core/ncore, mob/user)
	...
	to_chat(user, "<span_class='warning'>Container is sealing...</span>")
...
/obj/item/nuke_core_container/proc/seal()
	if(istype(core))
		...
		if(ismob(loc))
			to_chat(loc, "<span_class='warning'>[src] is permanently sealed, [core]'s radiation is contained.</span>")

/obj/item/nuke_core_container/attackby(obj/item/nuke_core/core, mob/user)
	if(istype(core))
		if(!user.temporarilyRemoveItemFromInventory(core))
			to_chat(user, "<span_class='warning'>The [core] is stuck to your hand!</span>")
			return
		else
			load(core, user)
	else
...
/obj/item/nuke_core/supermatter_sliver/attackby(obj/item/W, mob/living/user, params)
	if(istype(W, /obj/item/hemostat/supermatter))
		...
		if (tongs.sliver)
			to_chat(user, "<span_class='notice'>\The [tongs] is already holding a supermatter sliver!</span>")
			return FALSE
		forceMove(tongs)
		tongs.sliver = src
		tongs.update_icon()
		to_chat(user, "<span_class='notice'>You carefully pick up [src] with [tongs].</span>")
	else if(istype(W, /obj/item/scalpel/supermatter) || istype(W, /obj/item/nuke_core_container/supermatter/)) // we don't want it to dust
		return
	else
		to_chat(user, "<span_class='notice'>As it touches \the [src], both \the [src] and \the [W] burst into dust!</span>")
...
/obj/item/nuke_core/supermatter_sliver/pickup(mob/living/user)
	...
	if(!iscarbon(user))
		return FALSE
	var/mob/ded = user
	to_chat(user, "<span_class='warning'>You reach for the supermatter sliver with your hands. That was dumb.</span>")
...
/obj/item/nuke_core_container/supermatter/seal()
	if(istype(sliver))
		...
		playsound(src, 'sound/items/Deconstruct.ogg', 60, 1)
		if(ismob(loc))
			to_chat(loc, "<span_class='warning'>[src] is permanently sealed, [sliver] is safely contained.</span>")
...
/obj/item/hemostat/supermatter/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum) // no instakill supermatter javelins
	if(sliver)
		...
		to_chat(usr, "<span_class='notice'>\The [sliver] falls out of \the [src] as you throw them.</span>")
		sliver = null
		update_icon()
	..()
...
/obj/item/hemostat/supermatter/proc/Consume(atom/movable/AM, mob/user)
	if(ismob(AM))
		...
		qdel(AM)
	if (user)
		user.visible_message("<span_class='danger'>As [user] touches [AM] with \a [src], silence fills the room...</span>",\
			"<span_class='userdanger'>You touch [AM] with [src], and everything suddenly goes silent.</span>\n<span_class='notice'>[AM] and [sliver] flash into dust, and soon as you can register this, you do as well.</span>",\
