...
/obj/item/etherealballdeployer/attack_self(mob/living/carbon/user)
	.=..()
	to_chat(user, "<span_class='notice'>You deploy the Ethereal Disco Ball.</span>")
...
/obj/structure/etherealball/attack_hand(mob/living/carbon/human/user)
	...
	if(TurnedOn)
		TurnOff()
		to_chat(user, "<span_class='notice'>You turn the disco ball off!</span>")
	else
		TurnOn()
		to_chat(user, "<span_class='notice'>You turn the disco ball on!</span>")

/obj/structure/etherealball/AltClick(mob/living/carbon/human/user)
	. = ..()
	if(anchored)
		to_chat(user, "<span_class='notice'>You unlock the disco ball.</span>")
		anchored = FALSE
	else
		to_chat(user, "<span_class='notice'>You lock the disco ball.</span>")
		anchored = TRUE
