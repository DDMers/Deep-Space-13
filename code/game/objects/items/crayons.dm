...
/obj/item/toy/crayon/suicide_act(mob/user)
	user.visible_message("<span_class='suicide'>[user] is jamming [src] up [user.p_their()] nose and into [user.p_their()] brain. It looks like [user.p_theyre()] trying to commit suicide!</span>")
...
/obj/item/toy/crayon/proc/check_empty(mob/user, amount = 1, requires_full = TRUE)
	...
	if(charges == -1)
		...
		to_chat(user, "<span_class='warning'>There is no more of [src] left!</span>")
		if(self_contained)
			qdel(src)
		. = TRUE
	else if(charges_left < amount && requires_full)
		to_chat(user, "<span_class='warning'>There is not enough of [src] left!</span>")
		. = TRUE
...
/obj/item/toy/crayon/spraycan/AltClick(mob/user)
	if(user.canUseTopic(src, BE_CLOSE, ismonkey(user)))
		if(has_cap)
			is_capped = !is_capped
			to_chat(user, "<span_class='notice'>The cap on [src] is now [is_capped ? "on" : "off"].</span>")
			update_icon()
...
/obj/item/toy/crayon/afterattack(atom/target, mob/user, proximity, params)
	...
	if(!instant)
		to_chat(user, "<span_class='notice'>You start drawing a [temp] on the	[target.name]...</span>")

	if(pre_noise)
		audible_message("<span_class='notice'>You hear spraying.</span>")
		...
		switch(paint_mode)
			...
			if(PAINT_LARGE_HORIZONTAL)
				...
				if(isValidSurface(left) && isValidSurface(right))
					...
					to_chat(user, "<span_class='warning'>There isn't enough space to paint!</span>")
					return

	if(!instant)
		to_chat(user, "<span_class='notice'>You finish drawing \the [temp].</span>")
	else
		to_chat(user, "<span_class='notice'>You spray a [temp] on \the [target.name]</span>")
...
/obj/item/toy/crayon/spraycan/suicide_act(mob/user)
	...
	if(is_capped || !actually_paints)
		user.visible_message("<span_class='suicide'>[user] shakes up [src] with a rattle and lifts it to [user.p_their()] mouth, but nothing happens!</span>")
		user.say("MEDIOCRE!!", forced="spraycan suicide")
		return SHAME
	else
		user.visible_message("<span_class='suicide'>[user] shakes up [src] with a rattle and lifts it to [user.p_their()] mouth, spraying paint across [user.p_their()] teeth!</span>")
...
/obj/item/toy/crayon/spraycan/examine(mob/user)
	...
	to_chat(user, "<span_class='notice'>Alt-click [src] to [ is_capped ? "take the cap off" : "put the cap on"].</span>")
...
/obj/item/toy/crayon/spraycan/afterattack(atom/target, mob/user, proximity)
	...
	if(iscarbon(target))
		...
		user.visible_message("<span_class='danger'>[user] sprays [src] into the face of [target]!</span>")
		to_chat(target, "<span_class='userdanger'>[user] sprays [src] into your face!</span>")
...
/obj/item/toy/crayon/spraycan/borg/afterattack(atom/target,mob/user,proximity)
	var/diff = ..()
	if(!iscyborg(user))
		to_chat(user, "<span_class='notice'>How did you get this?</span>")
		qdel(src)
		return FALSE
