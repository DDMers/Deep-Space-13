...
/obj/item/wallframe/proc/try_build(turf/on_wall, mob/user)
	...
	if(!isfloorturf(T))
		to_chat(user, "<span_class='warning'>You cannot place [src] on this spot!</span>")
		return
	if(A.always_unpowered)
		to_chat(user, "<span_class='warning'>You cannot place [src] in this area!</span>")
		return
	if(gotwallitem(T, ndir, inverse*2))
		to_chat(user, "<span_class='warning'>There's already an item on this wall!</span>")
		return
...
/obj/item/wallframe/proc/attach(turf/on_wall, mob/user)
	if(result_path)
		...
		user.visible_message("[user.name] attaches [src] to the wall.",
			"<span_class='notice'>You attach [src] to the wall.</span>",
			"<span_class='italics'>You hear clicking.</span>")
		var/ndir = get_dir(on_wall,user)
		if(inverse)
			ndir = turn(ndir, 180)
...
/obj/item/wallframe/apc/try_build(turf/on_wall, user)
	...
	var/area/A = get_area(T)
	if(A.get_apc())
		to_chat(user, "<span_class='warning'>This area already has an APC!</span>")
	...
	for(var/obj/machinery/power/terminal/E in T)
		if(E.master)
			to_chat(user, "<span_class='warning'>There is another network terminal here!</span>")
			return
		else
			new /obj/item/stack/cable_coil(T, 10)
			to_chat(user, "<span_class='notice'>You cut the cables and disassemble the unused power terminal.</span>")
			qdel(E)
	return TRUE
