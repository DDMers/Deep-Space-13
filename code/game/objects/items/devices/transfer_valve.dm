...
/obj/item/transfer_valve/attackby(obj/item/item, mob/user, params)
	if(istype(item, /obj/item/tank))
		if(tank_one && tank_two)
			to_chat(user, "<span_class='warning'>There are already two tanks attached, remove one first!</span>")
			return
		...
		if(!tank_one)
			if(!user.transferItemToLoc(item, src))
				return
			tank_one = item
			to_chat(user, "<span_class='notice'>You attach the tank to the transfer valve.</span>")
		else if(!tank_two)
			if(!user.transferItemToLoc(item, src))
				return
			tank_two = item
...
//TODO: Have this take an assemblyholder
		...
		if(A.secured)
			to_chat(user, "<span_class='notice'>The device is secured.</span>")
			return
		if(attached_device)
			to_chat(user, "<span_class='warning'>There is already a device attached to the valve, remove it first!</span>")
			return
		if(!user.transferItemToLoc(item, src))
			return
		attached_device = A
		to_chat(user, "<span_class='notice'>You attach the [item] to the valve controls and secure it.</span>")
		A.holder = src
