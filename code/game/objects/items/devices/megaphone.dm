...
/obj/item/megaphone/suicide_act(mob/living/carbon/user)
	user.visible_message("<span_class='suicide'>[user] is uttering [user.p_their()] last words into \the [src]! It looks like [user.p_theyre()] trying to commit suicide!</span>")
...
/obj/item/megaphone/get_held_item_speechspans(mob/living/carbon/user)
	if(spamcheck > world.time)
		to_chat(user, "<span_class='warning'>\The [src] needs to recharge!</span>")
	else
...
/obj/item/megaphone/emag_act(mob/user)
	if(obj_flags & EMAGGED)
		return
	to_chat(user, "<span_class='warning'>You overload \the [src]'s voice synthesizer.</span>")
	obj_flags |= EMAGGED
