...
/obj/item/desynchronizer/attack_self(mob/living/user)
	if(world.time < next_use)
		to_chat(user, "<span_class='warning'>[src] is still recharging.</span>")
		return
	if(!sync_holder)
		desync(user)
	else
...
/obj/item/desynchronizer/examine(mob/user)
	..()
	if(world.time < next_use)
		to_chat(user, "<span_class='warning'>Time left to recharge: [DisplayTimeText(next_use - world.time)]</span>")
	to_chat(user, "<span_class='notice'>Alt-click to customize the duration. Current duration: [DisplayTimeText(duration)] seconds.</span>")
	to_chat(user, "<span_class='notice'>Can be used again to interrupt the effect early. The recharge time is the same as the time spent in desync.</span>")
...
/obj/item/desynchronizer/AltClick(mob/living/user)
	...
	if(new_duration)
		...
		to_chat(user, "<span_class='notice'>You set the duration to [DisplayTimeText(duration)].</span>")
...
/obj/item/desynchronizer/proc/desync(mob/living/user)
	...
	to_chat(user, "<span_class='notice'>You activate [src], desynchronizing yourself from the present. You can still see your surroundings, but you feel eerily dissociated from reality.</span>")
