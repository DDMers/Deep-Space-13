	...
	if(I.tool_behaviour == TOOL_SCREWDRIVER)
		if(mode == DISCONNECTED)
			...
			if(isturf(T) && !T.intact)
				...
				if(!attached)
					...
					user.visible_message( \
						...
						"<span_class='notice'>You attach \the [src] to the cable.</span>",
						"<span_class='italics'>You hear some wires being connected to something.</span>")
			else
				to_chat(user, "<span_class='warning'>This device must be placed over an exposed, powered cable node!</span>")
		else
			...
			user.visible_message( \
				...
				"<span_class='notice'>You detach \the [src] from the cable.</span>",
				"<span_class='italics'>You hear some wires being disconnected from something.</span>")
	else
		return ..()
	...
	switch(mode)
		...
		if(CLAMPED_OFF)
			user.visible_message( \
				...
				"<span_class='italics'>You hear a click.</span>")
