...
/obj/item/reverse_bear_trap/process()
	...
	if(!time_left || !isliving(loc))
		...
		soundloop.stop()
		soundloop2.stop()
		to_chat(loc, "<span_class='userdanger'>*ding*</span>")
		...
		if(C.get_item_by_slot(SLOT_HEAD) == src)
			if((item_flags & NODROP) && !struggling)
				...
				switch(time_left)
					...
					if(50 to 60)
						fear_string = ""
				C.visible_message("<span_class='danger'>[C] fiddles with and pulls at [src]...</span>", \
				"<span_class='danger'>You [fear_string] try to pull at [src]...</span>", "<i>You hear clicking and ticking.</i>")
				if(!do_after(user, 20, target = src))
					struggling = FALSE
					return
				if(!prob(escape_chance))
					to_chat(user, "<span_class='warning'>It doesn't budge!</span>")
					escape_chance++
				else
					user.visible_message("<span_class='warning'>The lock on [user]'s [name] pops open!</span>", \
					"<span_class='userdanger'>You force open the padlock!</span>", "<i>You hear a single, pronounced click!</i>")
					item_flags &= ~NODROP
				struggling = FALSE
			else
				..()
...
/obj/item/reverse_bear_trap/attack(mob/living/target, mob/living/user)
	if(target.get_item_by_slot(SLOT_HEAD))
		to_chat(user, "<span_class='warning'>Remove [target.p_their()] headgear first!</span>")
		return
	target.visible_message("<span_class='warning'>[user] starts forcing [src] onto [target]'s head!</span>", \
	"<span_class='userdanger'>[target] starts forcing [src] onto your head!</span>", "<i>You hear clanking.</i>")
	to_chat(user, "<span_class='danger'>You start forcing [src] onto [target]'s head...</span>")
	if(!do_after(user, 30, target = target) || target.get_item_by_slot(SLOT_HEAD))
		return
	target.visible_message("<span_class='warning'>[user] forces and locks [src] onto [target]'s head!</span>", \
	"<span_class='userdanger'>[target] locks [src] onto your head!</span>", "<i>You hear a click, and then a timer ticking down.</i>")
	to_chat(user, "<span_class='danger'>You force [src] onto [target]'s head and click the padlock shut.</span>")
...
/obj/item/reverse_bear_trap/proc/snap()
	...
	if(!istype(H) || H.get_item_by_slot(SLOT_HEAD) != src)
		visible_message("<span_class='warning'>[src]'s jaws snap open with an ear-piercing crack!</span>")
		playsound(src, 'sound/effects/snap.ogg', 75, TRUE)
	else
		var/mob/living/carbon/human/jill = loc
		jill.visible_message("<span_class='boldwarning'>[src] goes off in [jill]'s mouth, ripping [jill.p_their()] head apart!</span>", "<span_class='userdanger'>[src] goes off!</span>")
		jill.emote("scream")
