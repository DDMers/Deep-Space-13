...
/obj/item/batterer/attack_self(mob/living/carbon/user, flag = 0, emp = 0)
	...
	if(times_used >= max_uses)
		to_chat(user, "<span_class='danger'>The mind batterer has been burnt out!</span>")
		return
	...
	for(var/mob/living/carbon/human/M in urange(10, user, 1))
		if(prob(50))
			...
			to_chat(M, "<span_class='userdanger'>You feel a tremendous, paralyzing wave flood your mind.</span>")

		else
			to_chat(M, "<span_class='userdanger'>You feel a sudden, electric jolt travel through your head.</span>")

	playsound(src.loc, 'sound/misc/interference.ogg', 50, 1)
	to_chat(user, "<span_class='notice'>You trigger [src].</span>")
	times_used += 1
	if(times_used >= max_uses)
...
/obj/item/healthanalyzer/rad_laser/attack(mob/living/M, mob/living/user)
	...
	if(!used)
		...
		to_chat(user, "<span_class='warning'>Successfully irradiated [M].</span>")
		...
		to_chat(user, "<span_class='warning'>The radioactive microlaser is still recharging.</span>")
...
/obj/item/shadowcloak/proc/Activate(mob/living/carbon/human/user)
	if(!user)
		return
	to_chat(user, "<span_class='notice'>You activate [src].</span>")
	src.user = user
...
/obj/item/shadowcloak/proc/Deactivate()
	to_chat(user, "<span_class='notice'>You deactivate [src].</span>")
...
/obj/item/jammer/attack_self(mob/user)
	to_chat(user,"<span_class='notice'>You [active ? "deactivate" : "activate"] [src].</span>")
	active = !active
	if(active)
