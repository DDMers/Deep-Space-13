...
/obj/item/camera_bug/check_eye(mob/user)
	...
	if(T_user.z != T_current.z || !current.can_use())
		to_chat(user, "<span_class='danger'>[src] has lost the signal.</span>")
		current = null
		user.unset_machine()
		return 0
	return 1
	...
	if("view" in href_list)
		...
		if(C && istype(C))
			if(!same_z_level(C))
				return
			if(!C.can_use())
				to_chat(usr, "<span_class='warning'>Something's wrong with that camera!  You can't get a feed.</span>")
				return
			current = C
			spawn(6)
...
/obj/item/camera_bug/proc/same_z_level(var/obj/machinery/camera/C)
	...
	if(!T_bug || T_cam.z != T_bug.z)
		to_chat(usr, "<span_class='warning'>You can't get a signal!</span>")
		return FALSE
	return TRUE
