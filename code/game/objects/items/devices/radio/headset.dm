...
/obj/item/radio/headset/suicide_act(mob/living/carbon/user)
	user.visible_message("<span_class='suicide'>[user] begins putting \the [src]'s antenna up [user.p_their()] nose! It looks like [user.p_theyre()] trying to give [user.p_them()]self cancer!</span>")
	return TOXLOSS
...
/obj/item/radio/headset/examine(mob/user)
	...
	if(item_flags & IN_INVENTORY && loc == user)
		...
		to_chat(user, "<span_class='notice'>A small screen on the headset displays the following available frequencies:\n[english_list(avail_chans)].")

		if(command)
			to_chat(user, "<span_class='info'>Alt-click to toggle the high-volume mode.</span>")
	else
		to_chat(user, "<span_class='notice'>A small screen on the headset flashes, it's too small to read without holding or wearing the headset.</span>")
...
/obj/item/radio/headset/attackby(obj/item/W, mob/user, params)
	...
	if(W.tool_behaviour == TOOL_SCREWDRIVER)
		if(keyslot || keyslot2)
			...
			recalculateChannels()
			to_chat(user, "<span_class='notice'>You pop out the encryption keys in the headset.</span>")

		else
			to_chat(user, "<span_class='warning'>This headset doesn't have any unique encryption keys!  How useless...</span>")

	else if(istype(W, /obj/item/encryptionkey))
		if(keyslot && keyslot2)
			to_chat(user, "<span_class='warning'>The headset can't hold another key!</span>")
			return
...
/obj/item/radio/headset/AltClick(mob/living/user)
	...
	if (command)
		...
		to_chat(user, "<span_class='notice'>You toggle high-volume mode [use_command ? "on" : "off"].</span>")
