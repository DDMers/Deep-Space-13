...
/obj/item/taperecorder/proc/eject(mob/user)
	if(mytape)
		to_chat(user, "<span_class='notice'>You remove [mytape] from [src].</span>")
		stop()
...
/obj/item/taperecorder/verb/record()
	...
	if(mytape.used_capacity < mytape.max_capacity)
		to_chat(usr, "<span_class='notice'>Recording started.</span>")
		recording = 1
		update_icon()
		mytape.timestamp += mytape.used_capacity
		...
		to_chat(usr, "<span_class='notice'>The tape is full.</span>")
...
/obj/item/taperecorder/verb/stop()
	...
	if(recording)
		...
		to_chat(usr, "<span_class='notice'>Recording stopped.</span>")
		return
	else if(playing)
		playing = 0
...
/obj/item/taperecorder/verb/play()
	...
	playing = 1
	update_icon()
	to_chat(usr, "<span_class='notice'>Playing started.</span>")
	var/used = mytape.used_capacity	//to stop runtimes when you eject the tape
	var/max = mytape.max_capacity
...
/obj/item/taperecorder/verb/print_transcript()
	...
	if(!mytape)
		return
	if(!canprint)
		to_chat(usr, "<span_class='notice'>The recorder can't print that fast!</span>")
		return
	if(recording || playing)
		return

	to_chat(usr, "<span_class='notice'>Transcript printed.</span>")
...
/obj/item/tape/attack_self(mob/user)
	if(!ruined)
		to_chat(user, "<span_class='notice'>You pull out all the tape!</span>")
		ruin()
...
/obj/item/tape/attackby(obj/item/I, mob/user, params)
	if(ruined && I.tool_behaviour == TOOL_SCREWDRIVER || istype(I, /obj/item/pen))
		to_chat(user, "<span_class='notice'>You start winding the tape back in...</span>")
		if(I.use_tool(src, user, 120))
			to_chat(user, "<span_class='notice'>You wound the tape back in.</span>")
			fix()
