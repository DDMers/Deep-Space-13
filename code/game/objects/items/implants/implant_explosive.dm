...
/obj/item/implant/explosive/activate(cause)
	...
	medium = round(medium)
	weak = round(weak)
	to_chat(imp_in, "<span_class='notice'>You activate your [name].</span>")
	active = TRUE
...
/obj/item/implant/explosive/proc/timed_explosion()
	imp_in.visible_message("<span_class='warning'>[imp_in] starts beeping ominously!</span>")
	playsound(loc, 'sound/items/timer.ogg', 30, 0)
	sleep(delay*0.25)
	if(imp_in && !imp_in.stat)
		imp_in.visible_message("<span_class='warning'>[imp_in] doubles over in pain!</span>")
		imp_in.Paralyze(140)
	playsound(loc, 'sound/items/timer.ogg', 30, 0)
	sleep(delay*0.25)
	playsound(loc, 'sound/items/timer.ogg', 30, 0)
