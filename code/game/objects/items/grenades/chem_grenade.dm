...
/obj/item/grenade/chem_grenade/examine(mob/user)
	...
	if(user.can_see_reagents())
		if(beakers.len)
			to_chat(user, "<span_class='notice'>You scan the grenade and detect the following reagents:</span>")
			for(var/obj/item/reagent_containers/glass/G in beakers)
				for(var/datum/reagent/R in G.reagents.reagent_list)
					to_chat(user, "<span_class='notice'>[R.volume] units of [R.name] in the [G.name].</span>")
			if(beakers.len == 1)
				to_chat(user, "<span_class='notice'>You detect no second beaker in the grenade.</span>")
		else
			to_chat(user, "<span_class='notice'>You scan the grenade, but detect nothing.</span>")
	else if(stage != READY && beakers.len)
		if(beakers.len == 2 && beakers[1].name == beakers[2].name)
			to_chat(user, "<span_class='notice'>You see two [beakers[1].name]s inside the grenade.</span>")
		else
			for(var/obj/item/reagent_containers/glass/G in beakers)
				to_chat(user, "<span_class='notice'>You see a [G.name] inside the grenade.</span>")
...
/obj/item/grenade/chem_grenade/attackby(obj/item/I, mob/user, params)
	if(I.tool_behaviour == TOOL_SCREWDRIVER)
		if(stage == WIRED)
			if(beakers.len)
				stage_change(READY)
				to_chat(user, "<span_class='notice'>You lock the [initial(name)] assembly.</span>")
				I.play_tool_sound(src, 25)
			else
				to_chat(user, "<span_class='warning'>You need to add at least one beaker before locking the [initial(name)] assembly!</span>")
		else if(stage == READY && !nadeassembly)
			det_time = det_time == 50 ? 30 : 50	//toggle between 30 and 50
			to_chat(user, "<span_class='notice'>You modify the time delay. It's set for [DisplayTimeText(det_time)].</span>")
		else if(stage == EMPTY)
			to_chat(user, "<span_class='warning'>You need to add an activation mechanism!</span>")

	else if(stage == WIRED && is_type_in_list(I, allowed_containers))
		. = 1 //no afterattack
		if(beakers.len == 2)
			to_chat(user, "<span_class='warning'>[src] can not hold more containers!</span>")
			return
		else
			if(I.reagents.total_volume)
				if(!user.transferItemToLoc(I, src))
					return
				to_chat(user, "<span_class='notice'>You add [I] to the [initial(name)] assembly.</span>")
				beakers += I
		...
		stage_change(WIRED)
		to_chat(user, "<span_class='notice'>You add [A] to the [initial(name)] assembly.</span>")
		...
		if (C.use(1))
			...
			stage_change(WIRED)
			to_chat(user, "<span_class='notice'>You rig the [initial(name)] assembly.</span>")
		else
			to_chat(user, "<span_class='warning'>You need one length of coil to wire the assembly!</span>")
			return

	else if(stage == READY && I.tool_behaviour == TOOL_WIRECUTTER && !active)
		stage_change(WIRED)
		to_chat(user, "<span_class='notice'>You unlock the [initial(name)] assembly.</span>")
		...
		if(beakers.len)
			...
			beakers = list()
			to_chat(user, "<span_class='notice'>You open the [initial(name)] assembly and remove the payload.</span>")
		...
		stage_change(EMPTY)
		to_chat(user, "<span_class='notice'>You remove the activation mechanism from the [initial(name)] assembly.</span>")
	else
		return ..()
	...
	if(I.tool_behaviour == TOOL_MULTITOOL)
		switch(unit_spread)
			...
			if(25 to 99)
				...
				unit_spread = 5
		to_chat(user, "<span_class='notice'> You set the time release to [unit_spread] units per detonation.</span>")
		return
	..()
