...
/obj/item/hand_tele/proc/try_dispel_portal(atom/target, mob/user)
	if(is_parent_of_portal(target))
		qdel(target)
		to_chat(user, "<span_class='notice'>You dispel [target] with \the [src]!</span>")
		return TRUE
	return FALSE
...
/obj/item/hand_tele/attack_self(mob/user)
	...
	if(!current_location || current_area.noteleport || is_away_level(current_location.z) || !isturf(user.loc))//If turf was not found or they're on z level 2 or >7 which does not currently exist. or if user is not located on a turf
		to_chat(user, "<span_class='notice'>\The [src] is malfunctioning.</span>")
		return
	var/list/L = list(  )
	...
	if(active_portal_pairs.len >= max_portal_pairs)
		user.show_message("<span_class='notice'>\The [src] is recharging!</span>")
		return
	var/atom/T = L[t1]
	...
	if(!current_location || current_area.noteleport || is_away_level(current_location.z) || !isturf(user.loc))//If turf was not found or they're on z level 2 or >7 which does not currently exist. or if user is not located on a turf
		to_chat(user, "<span_class='notice'>\The [src] is malfunctioning.</span>")
		return
	user.show_message("<span_class='notice'>Locked In.</span>", 2)
...
/obj/item/hand_tele/suicide_act(mob/user)
	if(iscarbon(user))
		user.visible_message("<span_class='suicide'>[user] is creating a weak portal and sticking [user.p_their()] head through! It looks like [user.p_theyre()] trying to commit suicide!</span>")
		...
		if(head)
			...
			itemUser.visible_message("<span_class='suicide'>The portal snaps closed taking [user]'s head with it!</span>")
		else
			itemUser.visible_message("<span_class='suicide'>[user] looks even further depressed as they realize they do not have a head...and suddenly dies of shame!</span>")
		return (BRUTELOSS)
