	...
	if(SEND_SIGNAL(src, COMSIG_IS_STORAGE_LOCKED))
		if (W.tool_behaviour == TOOL_SCREWDRIVER)
			if (W.use_tool(src, user, 20))
				open =! open
				to_chat(user, "<span_class='notice'>You [open ? "open" : "close"] the service panel.</span>")
			return
		if (W.tool_behaviour == TOOL_WIRECUTTER)
			to_chat(user, "<span_class='danger'>[src] is protected from this sort of tampering, yet it appears the internal memory wires can still be <b>pulsed</b>.</span>")
		if ((W.tool_behaviour == TOOL_MULTITOOL) && (!l_hacking))
			if(open == 1)
				to_chat(user, "<span_class='danger'>Now attempting to reset internal memory, please hold.</span>")
				l_hacking = 1
				if (W.use_tool(src, user, 400))
					to_chat(user, "<span_class='danger'>Internal memory reset - lock has been disengaged.</span>")
					l_set = 0
					l_hacking = 0
				else
					l_hacking = 0
			else
				to_chat(user, "<span_class='notice'>You must <b>unscrew</b> the service panel before you can pulse the wiring.</span>")
			return
