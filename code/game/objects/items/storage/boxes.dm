...
/obj/item/storage/box/suicide_act(mob/living/carbon/user)
	...
	if(myhead)
		user.visible_message("<span_class='suicide'>[user] puts [user.p_their()] head into \the [src], and begins closing it! It looks like [user.p_theyre()] trying to commit suicide!</span>")
		myhead.dismember()
		...
		return BRUTELOSS
	user.visible_message("<span_class='suicide'>[user] beating [user.p_them()]self with \the [src]! It looks like [user.p_theyre()] trying to commit suicide!</span>")
	return BRUTELOSS
...
/obj/item/storage/box/attack_self(mob/user)
	...
	if(!foldable)
		return
	if(contents.len)
		to_chat(user, "<span_class='warning'>You can't fold this box with items still inside!</span>")
		return
	if(!ispath(foldable))
		return

	to_chat(user, "<span_class='notice'>You fold [src] flat.</span>")
...
/obj/item/storage/box/mousetraps
	...
	desc = "<span_class='alert'>Keep out of reach of children.</span>"
...
/obj/item/storage/box/hug/suicide_act(mob/user)
	user.visible_message("<span_class='suicide'>[user] clamps the box of hugs on [user.p_their()] jugular! Guess it wasn't such a hugbox after all..</span>")
	return (BRUTELOSS)
...
/obj/item/storage/box/hug/attack_self(mob/user)
	...
	user.visible_message("<span_class='notice'>[user] hugs \the [src].</span>","<span_class='notice'>You hug \the [src].</span>")
...
/obj/item/storage/box/clown/attackby(obj/item/I, mob/user, params)
	if((istype(I, /obj/item/bodypart/l_arm/robot)) || (istype(I, /obj/item/bodypart/r_arm/robot)))
		...
		if(!user.temporarilyRemoveItemFromInventory(I))
			return
		qdel(I)
		to_chat(user, "<span_class='notice'>You add some wheels to the [src]! You've got a honkbot assembly now! Honk!</span>")
...
/obj/item/storage/box/papersack/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/pen))
		...
		if(contents.len)
			to_chat(user, "<span_class='warning'>You can't modify [src] with items still inside!</span>")
			return
		...
		if(get_dist(usr, src) > 1)
			to_chat(usr, "<span_class='warning'>You have moved too far away!</span>")
			return
		var/choice = designs.Find(switchDesign)
		if(design == designs[choice] || designs[choice] == "Cancel")
			return 0
		to_chat(usr, "<span_class='notice'>You make some modifications to [src] using your pen.</span>")
		...
		if(!contents.len)
			if(item_state == "paperbag_None")
				user.show_message("<span_class='notice'>You cut eyeholes into [src].</span>", 1)
				new /obj/item/clothing/head/papersack(user.loc)
				qdel(src)
				return 0
			else if(item_state == "paperbag_SmileyFace")
				user.show_message("<span_class='notice'>You cut eyeholes into [src] and modify the design.</span>", 1)
