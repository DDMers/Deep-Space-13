...
/obj/item/target/welder_act(mob/living/user, obj/item/I)
	if(I.use_tool(src, user, 0, volume=40))
		removeOverlays()
		to_chat(user, "<span_class='notice'>You slice off [src]'s uneven chunks of aluminium and scorch marks.</span>")
	return TRUE
...
/obj/item/target/bullet_act(obj/item/projectile/P)
	...
	if(C.GetPixel(p_x, p_y) && P.original == src && overlays.len <= 35) // if the located pixel isn't blank (null)
		hp -= P.damage
		if(hp <= 0)
			visible_message("<span_class='danger'>[src] breaks into tiny pieces and collapses!</span>")
			qdel(src)
