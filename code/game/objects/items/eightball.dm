...
/obj/item/toy/eightball/attack_self(mob/user)
	...
	if(on_cooldown)
		to_chat(user, "<span_class='warning'>[src] was shaken recently, it needs time to settle.</span>")
		return

	user.visible_message("<span_class='notice'>[user] starts shaking [src].</span>", "<span_class='notice'>You start shaking [src].</span>", "<span_class='italics'>You hear shaking and sloshing.</span>")
...
/obj/item/toy/eightball/haunted/attack_ghost(mob/user)
	if(!shaking)
		to_chat(user, "<span_class='warning'>[src] is not currently being shaken.</span>")
		return
	interact(user)
	return ..()
