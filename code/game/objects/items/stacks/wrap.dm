...
/obj/item/stack/packageWrap/suicide_act(mob/living/user)
	user.visible_message("<span_class='suicide'>[user] begins wrapping [user.p_them()]self in \the [src]! It looks like [user.p_theyre()] trying to commit suicide!</span>")
	if(use(3))
...
/obj/item/stack/packageWrap/afterattack(obj/target, mob/user, proximity)
	...
	if(isitem(target))
		...
		if(!O.delivery_icon) //no delivery icon means unwrappable closet (e.g. body bags)
			to_chat(user, "<span_class='warning'>You can't wrap this!</span>")
			return
		if(use(3))
		...
		to_chat(user, "<span_class='warning'>The object you are trying to wrap is unsuitable for the sorting machinery!</span>")
		return

	user.visible_message("<span_class='notice'>[user] wraps [target].</span>")
