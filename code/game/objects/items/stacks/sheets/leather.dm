...
/obj/item/stack/sheet/animalhide/attackby(obj/item/W, mob/user, params)
	if(W.is_sharp())
		...
		user.visible_message("[user] starts cutting hair off \the [src].", "<span_class='notice'>You start cutting the hair off \the [src]...</span>", "<span_class='italics'>You hear the sound of a knife rubbing against flesh.</span>")
		if(do_after(user, 50, target = src))
			to_chat(user, "<span_class='notice'>You cut the hair from this [src.singular_name].</span>")
