...
/obj/item/storage/pill_bottle/dice/suicide_act(mob/user)
	user.visible_message("<span_class='suicide'>[user] is gambling with death! It looks like [user.p_theyre()] trying to commit suicide!</span>")
	return (OXYLOSS)
...
/obj/item/dice/proc/diceroll(mob/user)
	...
	if(user != null) //Dice was rolled in someone's hand
		user.visible_message("[user] has thrown [src]. It lands on [result]. [comment]", \
							 "<span_class='notice'>You throw [src]. It lands on [result]. [comment]</span>", \
							 "<span_class='italics'>You hear [src] rolling, it sounds like a [fake_result].</span>")
	else if(!src.throwing) //Dice was thrown and is coming to rest
		visible_message("<span_class='notice'>[src] rolls to a stop, landing on [result]. [comment]</span>")
