	...
	if(awakened)
		switch(bloodthirst)
			if(HIS_GRACE_SATIATED to HIS_GRACE_PECKISH)
				to_chat(user, "<span_class='his_grace'>[src] isn't very hungry. Not yet.</span>")
			if(HIS_GRACE_PECKISH to HIS_GRACE_HUNGRY)
				to_chat(user, "<span_class='his_grace'>[src] would like a snack.</span>")
			if(HIS_GRACE_HUNGRY to HIS_GRACE_FAMISHED)
				to_chat(user, "<span_class='his_grace'>[src] is quite hungry now.</span>")
			if(HIS_GRACE_FAMISHED to HIS_GRACE_STARVING)
				to_chat(user, "<span_class='his_grace'>[src] is openly salivating at the sight of you. Be careful.</span>")
			if(HIS_GRACE_STARVING to HIS_GRACE_CONSUME_OWNER)
				to_chat(user, "<span_class='his_grace bold'>You walk a fine line. [src] is very close to devouring you.</span>")
			if(HIS_GRACE_CONSUME_OWNER to HIS_GRACE_FALL_ASLEEP)
				to_chat(user, "<span_class='his_grace bold'>[src] is shaking violently and staring directly at you.</span>")
	else
		to_chat(user, "<span_class='his_grace'>[src] is latched closed.</span>")
...
/obj/item/his_grace/relaymove(mob/living/user) //Allows changelings, etc. to climb out of Him after they revive, provided He isn't active
	if(!awakened)
		...
		user.visible_message("<span_class='warning'>[user] scrambles out of [src]!</span>", "<span_class='notice'>You climb out of [src]!</span>")
...
/obj/item/his_grace/process()
	...
	if(istype(master) && (src in master.held_items))
		switch(bloodthirst)
			if(HIS_GRACE_CONSUME_OWNER to HIS_GRACE_FALL_ASLEEP)
				master.visible_message("<span_class='boldwarning'>[src] turns on [master]!</span>", "<span_class='his_grace big bold'>[src] turns on you!</span>")
	...
	if(Adjacent(L))
		if(!L.stat)
			L.visible_message("<span_class='warning'>[src] lunges at [L]!</span>", "<span_class='his_grace big bold'>[src] lunges at you!</span>")
...
/obj/item/his_grace/proc/awaken(mob/user) //Good morning, Mr. Grace.
	if(awakened)
		return
	awakened = TRUE
	user.visible_message("<span_class='boldwarning'>[src] begins to rattle. He thirsts.</span>", "<span_class='his_grace'>You flick [src]'s latch up. You hope this is a good idea.</span>")
	name = "His Grace"
...
/obj/item/his_grace/proc/drowse() //Good night, Mr. Grace.
	...
	T.visible_message("<span_class='boldwarning'>[src] slowly stops rattling and falls still, His latch snapping shut.</span>")
...
/obj/item/his_grace/proc/consume(mob/living/meal) //Here's your dinner, Mr. Grace.
	if(!meal)
		return
	var/victims = 0
	meal.visible_message("<span_class='warning'>[src] swings open and devours [meal]!</span>", "<span_class='his_grace big bold'>[src] consumes you!</span>")
	...
	switch(bloodthirst)
		if(HIS_GRACE_CONSUME_OWNER to HIS_GRACE_FALL_ASLEEP)
			if(HIS_GRACE_CONSUME_OWNER > prev_bloodthirst)
				master.visible_message("<span_class='userdanger'>[src] enters a frenzy!</span>")
		if(HIS_GRACE_STARVING to HIS_GRACE_CONSUME_OWNER)
			item_flags |= NODROP
			if(HIS_GRACE_STARVING > prev_bloodthirst)
				master.visible_message("<span_class='boldwarning'>[src] is starving!</span>", "<span_class='his_grace big'>[src]'s bloodlust overcomes you. [src] must be fed, or you will become His meal.\
				[force_bonus < 15 ? " And still, His power grows.":""]</span>")
				force_bonus = max(force_bonus, 15)
		if(HIS_GRACE_FAMISHED to HIS_GRACE_STARVING)
			item_flags |= NODROP
			if(HIS_GRACE_FAMISHED > prev_bloodthirst)
				master.visible_message("<span_class='warning'>[src] is very hungry!</span>", "<span_class='his_grace big'>Spines sink into your hand. [src] must feed immediately.\
				[force_bonus < 10 ? " His power grows.":""]</span>")
				force_bonus = max(force_bonus, 10)
			if(prev_bloodthirst >= HIS_GRACE_STARVING)
				master.visible_message("<span_class='warning'>[src] is now only very hungry!</span>", "<span_class='his_grace big'>Your bloodlust recedes.</span>")
		if(HIS_GRACE_HUNGRY to HIS_GRACE_FAMISHED)
			if(HIS_GRACE_HUNGRY > prev_bloodthirst)
				master.visible_message("<span_class='warning'>[src] is getting hungry.</span>", "<span_class='his_grace big'>You feel [src]'s hunger within you.\
				[force_bonus < 5 ? " His power grows.":""]</span>")
				force_bonus = max(force_bonus, 5)
			if(prev_bloodthirst >= HIS_GRACE_FAMISHED)
				master.visible_message("<span_class='warning'>[src] is now only somewhat hungry.</span>", "<span_class='his_grace'>[src]'s hunger recedes a little...</span>")
		if(HIS_GRACE_PECKISH to HIS_GRACE_HUNGRY)
			if(HIS_GRACE_PECKISH > prev_bloodthirst)
				master.visible_message("<span_class='warning'>[src] is feeling snackish.</span>", "<span_class='his_grace'>[src] begins to hunger.</span>")
			if(prev_bloodthirst >= HIS_GRACE_HUNGRY)
				master.visible_message("<span_class='warning'>[src] is now only a little peckish.</span>", "<span_class='his_grace big'>[src]'s hunger recedes somewhat...</span>")
		if(HIS_GRACE_SATIATED to HIS_GRACE_PECKISH)
			if(prev_bloodthirst >= HIS_GRACE_PECKISH)
				master.visible_message("<span_class='warning'>[src] is satiated.</span>", "<span_class='his_grace big'>[src]'s hunger recedes...</span>")
...
/obj/item/his_grace/proc/ascend()
	...
	if(istype(master))
		master.visible_message("<span_class='his_grace big bold'>Gods will be watching.</span>")
