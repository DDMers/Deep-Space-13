...
/obj/structure/sign/poster/attackby(obj/item/I, mob/user, params)
	if(I.tool_behaviour == TOOL_WIRECUTTER)
		...
		if(ruined)
			to_chat(user, "<span_class='notice'>You remove the remnants of the poster.</span>")
			qdel(src)
		else
			to_chat(user, "<span_class='notice'>You carefully remove the poster from the wall.</span>")
...
/turf/closed/wall/proc/place_poster(obj/item/poster/P, mob/user)
	if(!P.poster_structure)
		to_chat(user, "<span_class='warning'>[P] has no poster... inside it? Inform a coder!</span>")
		return
	...
	for(var/obj/O in contents) //Let's see if it already has a poster on it or too much stuff
		if(istype(O, /obj/structure/sign/poster))
			to_chat(user, "<span_class='warning'>The wall is far too cluttered to place a poster!</span>")
			return
		stuff_on_wall++
		if(stuff_on_wall == 3)
			to_chat(user, "<span_class='warning'>The wall is far too cluttered to place a poster!</span>")
	...
	to_chat(user, "<span_class='notice'>You start placing the poster on the wall...</span>"	)
	...
	if(do_after(user, PLACE_SPEED, target=src))
		...
		if(iswallturf(src) && user && user.loc == temp_loc)	//Let's check if everything is still there
			to_chat(user, "<span_class='notice'>You place the poster!</span>")
			return

	to_chat(user, "<span_class='notice'>The poster falls down!</span>")
