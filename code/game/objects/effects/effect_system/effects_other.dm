...
/datum/effect_system/reagents_explosion/start()
	if(explosion_message)
		location.visible_message("<span_class='danger'>The solution violently explodes!</span>", \
								"<span_class='italics'>You hear an explosion!</span>")
	if (amount < 1)
		...
		for(var/mob/living/L in viewers(1, location))
			if(prob(50 * amount))
				to_chat(L, "<span_class='danger'>The explosion knocks you down.</span>")
