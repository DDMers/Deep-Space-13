	...
	if(isliving(AM))
		...
		if(L.m_intent != MOVE_INTENT_WALK && prob(40))
			...
			if(L.acid_act(10, acid_used, "feet"))
				...
				to_chat(L, "<span_class='userdanger'>[src] burns you!</span>")
...
/obj/effect/acid/alien/process()
	...
	if(.)
		...
		if(target_strength <= 0)
			target.visible_message("<span_class='warning'>[target] collapses under its own weight into a puddle of goop and undigested debris!</span>")
			target.acid_melt()
			qdel(src)
		else
			...
			switch(target_strength)
				if(24)
					visible_message("<span_class='warning'>[target] is holding up against the acid!</span>")
				if(16)
					visible_message("<span_class='warning'>[target] is being melted by the acid!</span>")
				if(8)
					visible_message("<span_class='warning'>[target] is struggling to withstand the acid!</span>")
				if(4)
					visible_message("<span_class='warning'>[target] begins to crumble under the acid!</span>")
