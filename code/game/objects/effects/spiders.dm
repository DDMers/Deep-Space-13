...
/obj/structure/spider/stickyweb/CanPass(atom/movable/mover, turf/target)
	if(istype(mover, /mob/living/simple_animal/hostile/poison/giant_spider))
		...
		if(istype(mover.pulledby, /mob/living/simple_animal/hostile/poison/giant_spider))
			return TRUE
		if(prob(50))
			to_chat(mover, "<span_class='danger'>You get stuck in \the [src] for a moment.</span>")
			return FALSE
...
/obj/structure/spider/spiderling/process()
		...
		if(get_dist(src, entry_vent) <= 1)
			...
			if(prob(50))
				visible_message("<B>[src] scrambles into the ventilation ducts!</B>", \
								"<span_class='italics'>You hear something scampering through the ventilation ducts.</span>")
			...
			spawn(rand(20,60))
				...
				spawn(travel_time)
					...
					if(prob(50))
						audible_message("<span_class='italics'>You hear something scampering through the ventilation ducts.</span>")
					sleep(travel_time)
	...
	//=================
		...
		if(nearby.len)
			...
			if(prob(40))
				src.visible_message("<span_class='notice'>\The [src] skitters[pick(" away"," around","")].</span>")
	else if(prob(10))
		//ventcrawl!
...
/obj/structure/spider/cocoon/container_resist(mob/living/user)
	...
	to_chat(user, "<span_class='notice'>You struggle against the tight bonds... (This will take about [DisplayTimeText(breakout_time)].)</span>")
...
/obj/structure/spider/cocoon/Destroy()
	...
	src.visible_message("<span_class='warning'>\The [src] splits open.</span>")
