...
/obj/effect/decal/cleanable/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/reagent_containers/glass) || istype(W, /obj/item/reagent_containers/food/drinks))
		if(src.reagents && W.reagents)
			...
			if(!src.reagents.total_volume)
				to_chat(user, "<span_class='notice'>[src] isn't thick enough to scoop up!</span>")
				return
			if(W.reagents.total_volume >= W.reagents.maximum_volume)
				to_chat(user, "<span_class='notice'>[W] is full!</span>")
				return
			to_chat(user, "<span_class='notice'>You scoop up [src] into [W]!</span>")
	...
	if(W.is_hot()) //todo: make heating a reagent holder proc
		if(istype(W, /obj/item/clothing/mask/cigarette))
			...
			to_chat(user, "<span_class='notice'>You heat [name] with [W]!</span>")
	else
		return ..()
