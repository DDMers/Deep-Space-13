	...
	switch(build_step)
		if(PTURRET_UNSECURED)	//first step
			if(I.tool_behaviour == TOOL_WRENCH && !anchored)
				...
				to_chat(user, "<span_class='notice'>You secure the external bolts.</span>")
				setAnchored(TRUE)
				...
				to_chat(user, "<span_class='notice'>You dismantle the turret construction.</span>")
		...
		if(PTURRET_BOLTED)
			if(istype(I, /obj/item/stack/sheet/metal))
				...
				if(M.use(2))
					to_chat(user, "<span_class='notice'>You add some metal armor to the interior frame.</span>")
					build_step = PTURRET_START_INTERNAL_ARMOUR
					icon_state = "turret_frame2"
				else
					to_chat(user, "<span_class='warning'>You need two sheets of metal to continue construction!</span>")
				return

			else if(I.tool_behaviour == TOOL_WRENCH)
				I.play_tool_sound(src, 75)
				to_chat(user, "<span_class='notice'>You unfasten the external bolts.</span>")
				setAnchored(FALSE)
		...
		if(PTURRET_START_INTERNAL_ARMOUR)
			if(I.tool_behaviour == TOOL_WRENCH)
				I.play_tool_sound(src, 100)
				to_chat(user, "<span_class='notice'>You bolt the metal armor into place.</span>")
				...
				to_chat(user, "<span_class='notice'>You start to remove the turret's interior metal armor...</span>")

				if(I.use_tool(src, user, 20, volume=50, amount=5)) //uses up 5 fuel
					build_step = PTURRET_BOLTED
					to_chat(user, "<span_class='notice'>You remove the turret's interior metal armor.</span>")
		...
		if(PTURRET_INTERNAL_ARMOUR_ON)
			if(istype(I, /obj/item/gun/energy)) //the gun installation part
				...
				if(!user.transferItemToLoc(E, src))
					return
				installed_gun = E
				to_chat(user, "<span_class='notice'>You add [I] to the turret.</span>")
				build_step = PTURRET_GUN_EQUIPPED
				return

			else if(I.tool_behaviour == TOOL_WRENCH)
				I.play_tool_sound(src, 100)
				to_chat(user, "<span_class='notice'>You remove the turret's metal armor bolts.</span>")
				build_step = PTURRET_START_INTERNAL_ARMOUR
				return
		...
		if(PTURRET_GUN_EQUIPPED)
			if(isprox(I))
				...
				if(!user.temporarilyRemoveItemFromInventory(I))
					return
				to_chat(user, "<span_class='notice'>You add the proximity sensor to the turret.</span>")
				qdel(I)
				return
		...
		if(PTURRET_SENSORS_ON)
			if(I.tool_behaviour == TOOL_SCREWDRIVER)
				...
				to_chat(user, "<span_class='notice'>You close the internal access hatch.</span>")
				return
		...
		if(PTURRET_CLOSED)
			if(istype(I, /obj/item/stack/sheet/metal))
				var/obj/item/stack/sheet/metal/M = I
				if(M.use(2))
					to_chat(user, "<span_class='notice'>You add some metal armor to the exterior frame.</span>")
				...
				I.play_tool_sound(src, 100)
				build_step = PTURRET_SENSORS_ON
				to_chat(user, "<span_class='notice'>You open the internal access hatch.</span>")
				return
		...
		if(PTURRET_START_EXTERNAL_ARMOUR)
			if(I.tool_behaviour == TOOL_WELDER)
				...
				to_chat(user, "<span_class='notice'>You begin to weld the turret's armor down...</span>")
				if(I.use_tool(src, user, 30, volume=50, amount=5))
					build_step = PTURRET_EXTERNAL_ARMOUR_ON
					to_chat(user, "<span_class='notice'>You weld the turret's armor down.</span>")
				...
				I.play_tool_sound(src, 75)
				to_chat(user, "<span_class='notice'>You pry off the turret's exterior armor.</span>")
...
/obj/machinery/porta_turret_construct/attack_hand(mob/user)
	...
	switch(build_step)
		if(PTURRET_GUN_EQUIPPED)
			...
			to_chat(user, "<span_class='notice'>You remove [installed_gun] from the turret frame.</span>")
			installed_gun = null

		if(PTURRET_SENSORS_ON)
			to_chat(user, "<span_class='notice'>You remove the prox sensor from the turret frame.</span>")
