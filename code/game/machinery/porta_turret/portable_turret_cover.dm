...
/obj/machinery/porta_turret_cover/attackby(obj/item/I, mob/user, params)
	if(I.tool_behaviour == TOOL_WRENCH && !parent_turret.on)
		...
		if(parent_turret.allowed(user))
			...
			to_chat(user, "<span_class='notice'>Controls are now [parent_turret.locked ? "locked" : "unlocked"].</span>")
			updateUsrDialog()
		else
		...
		to_chat(user, "<span_class='notice'>You add [parent_turret] to multitool buffer.</span>")
	else
		return ..()
...
/obj/machinery/porta_turret_cover/emag_act(mob/user)
	if(!(parent_turret.obj_flags & EMAGGED))
		to_chat(user, "<span_class='notice'>You short out [parent_turret]'s threat assessment circuits.</span>")
