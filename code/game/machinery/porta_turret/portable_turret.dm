...
/obj/machinery/porta_turret/Topic(href, href_list)
	...
	if(href_list["power"] && !locked)
		if(anchored)	//you can't turn a turret on/off if it's not anchored/secured
			...
			to_chat(usr, "<span_class='notice'>It has to be secured first!</span>")
		interact(usr)
		return
...
/obj/machinery/porta_turret/attackby(obj/item/I, mob/user, params)
	if(stat & BROKEN)
		if(I.tool_behaviour == TOOL_CROWBAR)
			...
			to_chat(user, "<span_class='notice'>You begin prying the metal coverings off...</span>")
			if(I.use_tool(src, user, 20))
				if(prob(70))
					if(stored_gun)
						stored_gun.forceMove(loc)
					to_chat(user, "<span_class='notice'>You remove the turret and salvage some components.</span>")
					if(prob(50))
						new /obj/item/stack/sheet/metal(loc, rand(1,4))
					if(prob(50))
						new /obj/item/assembly/prox_sensor(loc)
				else
					to_chat(user, "<span_class='notice'>You remove the turret but did not manage to salvage anything.</span>")
				qdel(src)
		...
		if(!anchored && !isinspace())
			...
			update_icon()
			to_chat(user, "<span_class='notice'>You secure the exterior bolts on the turret.</span>")
			if(has_cover)
				cover = new /obj/machinery/porta_turret_cover(loc) //create a new turret. While this is handled in process(), this is to workaround a bug where the turret becomes invisible for a split second
				cover.parent_turret = src //make the cover's parent src
		else if(anchored)
			setAnchored(FALSE)
			to_chat(user, "<span_class='notice'>You unsecure the exterior bolts on the turret.</span>")
			power_change()
			invisibility = 0
		...
		if(allowed(user))
			locked = !locked
			to_chat(user, "<span_class='notice'>Controls are now [locked ? "locked" : "unlocked"].</span>")
		else
		...
		M.buffer = src
		to_chat(user, "<span_class='notice'>You add [src] to multitool buffer.</span>")
	else
		return ..()
...
/obj/machinery/porta_turret/emag_act(mob/user)
	if(obj_flags & EMAGGED)
		return
	to_chat(user, "<span_class='warning'>You short out [src]'s threat assessment circuits.</span>")
...
/obj/machinery/porta_turret/proc/remove_control(warning_message = TRUE)
	...
	if(remote_controller)
		if(warning_message)
			to_chat(remote_controller, "<span_class='warning'>Your uplink to [src] has been severed!</span>")
...
/obj/machinery/turretid/examine(mob/user)
	...
	if(issilicon(user) && (!stat & BROKEN))
		to_chat(user, "<span_class='notice'>Ctrl-click [src] to [ enabled ? "disable" : "enable"] turrets.</span>")
		to_chat(user, "<span_class='notice'>Alt-click [src] to set turrets to [ lethal ? "stun" : "kill"].</span>")
...
/obj/machinery/turretid/attackby(obj/item/I, mob/user, params)
	...
	if ( get_dist(src, user) == 0 )		// trying to unlock the interface
		if (allowed(usr))
			if(obj_flags & EMAGGED)
				to_chat(user, "<span_class='notice'>The turret control is unresponsive.</span>")
				return

			locked = !locked
			to_chat(user, "<span_class='notice'>You [ locked ? "lock" : "unlock"] the panel.</span>")
			if (locked)
				if (user.machine==src)
					user.unset_machine()
...
/obj/machinery/turretid/emag_act(mob/user)
	if(obj_flags & EMAGGED)
		return
	to_chat(user, "<span_class='danger'>You short out the turret controls' access analysis module.</span>")
	obj_flags |= EMAGGED
	locked = FALSE
...
/obj/machinery/turretid/attack_ai(mob/user)
	if(!ailock || IsAdminGhost(user))
		...
		to_chat(user, "<span_class='notice'>There seems to be a firewall preventing you from accessing this device.</span>")

/obj/machinery/turretid/ui_interact(mob/user)
	. = ..()
	if ( get_dist(src, user) > 0 )
		if ( !(issilicon(user) || IsAdminGhost(user)) )
			to_chat(user, "<span_class='notice'>You are too far away.</span>")
			user.unset_machine()
			user << browse(null, "window=turretid")
			return
