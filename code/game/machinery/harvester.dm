...
/obj/machinery/harvester/proc/start_harvest()
	...
	harvesting = TRUE
	visible_message("<span_class='notice'>The [name] begins warming up!</span>")
...
/obj/machinery/harvester/default_pry_open(obj/item/I) //wew
	...
	if(.)
		...
		visible_message("<span_class='notice'>[usr] pries open \the [src].</span>", "<span_class='notice'>You pry open [src].</span>")
		open_machine()
...
/obj/machinery/harvester/emag_act(mob/user)
	...
	obj_flags |= EMAGGED
	allow_living = TRUE
	to_chat(user, "<span_class='warning'>You overload [src]'s lifesign scanners.</span>")
...
/obj/machinery/harvester/container_resist(mob/living/user)
	if(!harvesting)
		...
		to_chat(user,"<span_class='warning'>[src] is active and can't be opened!</span>") //rip
...
/obj/machinery/harvester/examine(mob/user)
	...
	if(stat & BROKEN)
		return
	if(state_open)
		to_chat(user, "<span_class='notice'>[src] must be closed before harvesting.</span>")
	else if(!harvesting)
		to_chat(user, "<span_class='notice'>Alt-click [src] to start harvesting.</span>")
	if(in_range(user, src) || isobserver(user))
		to_chat(user, "<span_class='notice'>The status display reads: Harvest speed at <b>[interval*0.1]</b> seconds per organ.<span>")
