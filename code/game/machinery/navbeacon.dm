...
/obj/machinery/navbeacon/attackby(obj/item/I, mob/user, params)
	...
	if(I.tool_behaviour == TOOL_SCREWDRIVER)
		...
		user.visible_message("[user] [open ? "opens" : "closes"] the beacon's cover.", "<span_class='notice'>You [open ? "open" : "close"] the beacon's cover.</span>")
		...
		if(open)
			if (src.allowed(user))
				...
				to_chat(user, "<span_class='notice'>Controls are now [src.locked ? "locked" : "unlocked"].</span>")
			else
				to_chat(user, "<span_class='danger'>Access denied.</span>")
			updateDialog()
		else
			to_chat(user, "<span_class='warning'>You must open the cover first!</span>")
	else
		return ..()
...
/obj/machinery/navbeacon/ui_interact(mob/user)
	...
	if(!open && !ai)	// can't alter controls if not open, unless you're an AI
		to_chat(user, "<span_class='warning'>The beacon's control cover is closed!</span>")
		return
