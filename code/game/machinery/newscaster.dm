...
/obj/machinery/newscaster/attackby(obj/item/I, mob/living/user, params)
	if(I.tool_behaviour == TOOL_WRENCH)
		...
		if(I.use_tool(src, user, 60))
			...
			if(stat & BROKEN)
				to_chat(user, "<span_class='warning'>The broken remains of [src] fall on the ground.</span>")
		...
		if(stat & BROKEN)
			...
			to_chat(user, "<span_class='notice'>[src] does not need repairs.</span>")
	else
		return ..()
...
/obj/machinery/newscaster/attack_paw(mob/user)
	if(user.a_intent != INTENT_HARM)
		to_chat(user, "<span_class='warning'>The newscaster controls are far too complicated for your tiny brain!</span>")
	else
...
/obj/machinery/newscaster/proc/AttachPhoto(mob/user)
	...
	if(issilicon(user))
		...
		if(isAI(user))
			...
			to_chat(user, "<span_class='warning'>You cannot interface with silicon photo uploading!</span>")
...
/obj/item/newspaper/suicide_act(mob/user)
	user.visible_message("<span_class='suicide'>[user] is focusing intently on [src]! It looks like [user.p_theyre()] trying to commit sudoku... until [user.p_their()] eyes light up with realization!</span>")
	...
	user.visible_message("<span_class='suicide'>[user] downs the contents of [W.name] in one gulp! Shoulda stuck to sudoku!</span>")
...
/obj/item/newspaper/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/pen))
		if(!user.is_literate())
			...
			return
		if(scribble_page == curr_page)
			to_chat(user, "<span_class='notice'>There's already a scribble in this page... You wouldn't want to make things too cluttered, would you?</span>")
		else
