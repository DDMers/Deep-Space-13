...
/obj/machinery/dna_scannernew/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		to_chat(user, "<span_class='notice'>The status display reads: Radiation pulse accuracy increased by factor <b>[precision_coeff**2]</b>.<br>Radiation pulse damage decreased by factor <b>[damage_coeff**2]</b>.<span>")
...
/obj/machinery/dna_scannernew/proc/toggle_open(mob/user)
	if(panel_open)
		to_chat(user, "<span_class='notice'>Close the maintenance panel first.</span>")
		return
	...
	if(state_open)
		...
		to_chat(user, "<span_class='notice'>The bolts are locked down, securing the door shut.</span>")
		return
