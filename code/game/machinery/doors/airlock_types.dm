...
/obj/machinery/door/airlock/clockwork/examine(mob/user)
	...
	switch(construction_state)
		if(GEAR_SECURE)
			gear_text = "<span_class='brass'>The cogwheel is solidly <b>wrenched</b> to the brass around it.</span>"
		if(GEAR_LOOSE)
			gear_text = "<span_class='alloy'>The cogwheel has been <i>loosened</i>, but remains <b>connected loosely</b> to the door!</span>"
...
/obj/machinery/door/airlock/clockwork/proc/attempt_construction(obj/item/I, mob/living/user)
	if(!I || !user || !user.canUseTopic(src))
		...
		if(construction_state == GEAR_SECURE)
			user.visible_message("<span_class='notice'>[user] begins loosening [src]'s cogwheel...</span>", "<span_class='notice'>You begin loosening [src]'s cogwheel...</span>")
			if(!I.use_tool(src, user, 75, volume=50) || construction_state != GEAR_SECURE)
				return 1
			user.visible_message("<span_class='notice'>[user] loosens [src]'s cogwheel!</span>", "<span_class='notice'>[src]'s cogwheel pops off and dangles loosely.</span>")
			playsound(src, 'sound/items/deconstruct.ogg', 50, 1)
			construction_state = GEAR_LOOSE
		else if(construction_state == GEAR_LOOSE)
			user.visible_message("<span_class='notice'>[user] begins tightening [src]'s cogwheel...</span>", "<span_class='notice'>You begin tightening [src]'s cogwheel into place...</span>")
			if(!I.use_tool(src, user, 75, volume=50) || construction_state != GEAR_LOOSE)
				return 1
			user.visible_message("<span_class='notice'>[user] tightens [src]'s cogwheel!</span>", "<span_class='notice'>You firmly tighten [src]'s cogwheel into place.</span>")
			playsound(src, 'sound/items/deconstruct.ogg', 50, 1)
			construction_state = GEAR_SECURE
		return 1
	else if(I.tool_behaviour == TOOL_CROWBAR)
		if(construction_state == GEAR_SECURE)
			to_chat(user, "<span_class='warning'>[src]'s cogwheel is too tightly secured! Your [I.name] can't reach under it!</span>")
			return 1
		else if(construction_state == GEAR_LOOSE)
			user.visible_message("<span_class='notice'>[user] begins slowly lifting off [src]'s cogwheel...</span>", "<span_class='notice'>You slowly begin lifting off [src]'s cogwheel...</span>")
			if(!I.use_tool(src, user, 75, volume=50) || construction_state != GEAR_LOOSE)
				return 1
			user.visible_message("<span_class='notice'>[user] lifts off [src]'s cogwheel, causing it to fall apart!</span>", \
			"<span_class='notice'>You lift off [src]'s cogwheel, causing it to fall apart!</span>")
			deconstruct(TRUE)
		return 1
	return 0
