	...
	if(I.tool_behaviour == TOOL_WRENCH && can_unanchor)
		if(!anchored)
			if(!isturf(loc) || isspaceturf(loc))
				to_chat(user, "<span_class='notice'>The bomb must be placed on solid ground to attach it.</span>")
			else
				to_chat(user, "<span_class='notice'>You firmly wrench the bomb to the floor.</span>")
				I.play_tool_sound(src)
				setAnchored(TRUE)
				if(active)
					to_chat(user, "<span_class='notice'>The bolts lock in place.</span>")
		else
			if(!active)
				to_chat(user, "<span_class='notice'>You wrench the bomb from the floor.</span>")
				I.play_tool_sound(src)
				setAnchored(FALSE)
			else
				to_chat(user, "<span_class='warning'>The bolts are locked down!</span>")

	else if(I.tool_behaviour == TOOL_SCREWDRIVER)
		open_panel = !open_panel
		update_icon()
		to_chat(user, "<span_class='notice'>You [open_panel ? "open" : "close"] the wire panel.</span>")
		...
		if(open_panel && wires.is_all_cut())
			if(payload)
				to_chat(user, "<span_class='notice'>You carefully pry out [payload].</span>")
				payload.forceMove(drop_location())
				payload = null
			else
				to_chat(user, "<span_class='warning'>There isn't anything in here to remove!</span>")
		else if (open_panel)
			to_chat(user, "<span_class='warning'>The wires connecting the shell to the explosives are holding it down!</span>")
		else
			to_chat(user, "<span_class='warning'>The cover is screwed on, it won't pry off!</span>")
	else if(istype(I, /obj/item/bombcore))
		if(!payload)
			if(!user.transferItemToLoc(I, src))
				return
			payload = I
			to_chat(user, "<span_class='notice'>You place [payload] into [src].</span>")
		else
			to_chat(user, "<span_class='warning'>[payload] is already loaded into [src]! You'll have to remove it first.</span>")
		...
		to_chat(user, "<span_class='notice'>You start to cut [src] apart...</span>")
		if(I.use_tool(src, user, 20, volume=50, amount=5)) //uses up 5 fuel
			to_chat(user, "<span_class='notice'>You cut [src] apart.</span>")
		...
		if((old_integ > obj_integrity) && active && !defused && (payload in src))
			to_chat(user, "<span_class='warning'>That seems like a really bad idea...</span>")
...
/obj/machinery/syndicatebomb/interact(mob/user)
	...
	if(!open_panel)
		if(!active)
			...
			to_chat(user, "<span_class='warning'>The bomb is bolted to the floor!</span>")
...
/obj/machinery/syndicatebomb/proc/settings(mob/user)
	...
	if(in_range(src, user) && isliving(user)) //No running off and setting bombs from across the station
		...
		loc.visible_message("<span_class='notice'>[icon2html(src, viewers(src))] timer set for [timer_set] seconds.</span>")
	if(alert(user,"Would you like to start the countdown now?",,"Yes","No") == "Yes" && in_range(src, user) && isliving(user))
		if(defused || active)
			if(defused)
				visible_message("<span_class='warning'>[icon2html(src, viewers(src))] Device error: User intervention required.</span>")
			return
		else
			visible_message("<span_class='danger'>[icon2html(src, viewers(loc))] [timer_set] seconds until detonation, please clear the area.</span>")
			activate()
			update_icon()
...
/obj/item/bombcore/training/detonate()
	...
	if(istype(holder))
		attempts++
		holder.loc.visible_message("<span_class='danger'>[icon2html(holder, viewers(holder))] Alert: Bomb has detonated. Your score is now [defusals] for [attempts]. Resetting wires...</span>")
		reset()
	else
		qdel(src)
...
/obj/item/bombcore/training/defuse()
	...
	if(istype(holder))
		attempts++
		defusals++
		holder.loc.visible_message("<span_class='notice'>[icon2html(holder, viewers(holder))] Alert: Bomb has been defused. Your score is now [defusals] for [attempts]! Resetting wires in 5 seconds...</span>")
	...
	if(I.tool_behaviour == TOOL_CROWBAR && beakers.len > 0)
		...
		if(beakers.len < max_beakers)
			if(!user.transferItemToLoc(I, src))
				return
			beakers += I
			to_chat(user, "<span_class='notice'>You load [src] with [I].</span>")
		else
			to_chat(user, "<span_class='warning'>[I] won't fit! \The [src] can only hold up to [max_beakers] containers.</span>")
			return
	..()
...
/obj/item/syndicatedetonator/attack_self(mob/user)
	if(timer < world.time)
		...
		to_chat(user, "<span_class='notice'>[existent] found, [detonated] triggered.</span>")
		if(detonated)
			detonated--
