...
/obj/structure/camera_assembly/examine(mob/user)
	...
	switch(state)
		if(STATE_WRENCHED)
			to_chat(user, "<span_class='info'>You can secure it in place with a <b>welder</b>, or removed with a <b>wrench</b>.</span>")
		if(STATE_WELDED)
			to_chat(user, "<span_class='info'>You can add <b>wires</b> to it, or <b>unweld</b> it from the wall.</span>")
		if(STATE_WIRED)
			if(has_upgrades)
				to_chat(user, "<span_class='info'>You can remove the contained upgrades with a <b>crowbar</b>.</span>")
			to_chat(user, "<span_class='info'>You can complete it with a <b>screwdriver</b>, or <b>unwire</b> it to start removal.</span>")
		if(STATE_FINISHED)
			to_chat(user, "<span_class='boldwarning'>You shouldn't be seeing this, tell a coder!</span>")
	...
	switch(state)
		if(STATE_WRENCHED)
			if(W.tool_behaviour == TOOL_WELDER)
				if(weld(W, user))
					to_chat(user, "<span_class='notice'>You weld [src] securely into place.</span>")
					setAnchored(TRUE)
					state = STATE_WELDED
				return
		...
		if(STATE_WELDED)
			if(istype(W, /obj/item/stack/cable_coil))
				...
				if(C.use(2))
					to_chat(user, "<span_class='notice'>You add wires to [src].</span>")
					state = STATE_WIRED
				else
					to_chat(user, "<span_class='warning'>You need two lengths of cable to wire a camera!</span>")
					return
				return
				...
				if(weld(W, user))
					to_chat(user, "<span_class='notice'>You unweld [src] from its place.</span>")
		...
		if(STATE_WIRED)	// Upgrades!
			if(istype(W, /obj/item/stack/sheet/mineral/plasma)) //emp upgrade
				if(emp_module)
					to_chat(user, "<span_class='warning'>[src] already contains a [emp_module]!</span>")
					return
				...
				to_chat(user, "<span_class='notice'>You attach [W] into [src]'s inner circuits.</span>")
				return

			else if(istype(W, /obj/item/analyzer)) //xray upgrade
				if(xray_module)
					to_chat(user, "<span_class='warning'>[src] already contains a [xray_module]!</span>")
					return
				...
				if(proxy_module)
					to_chat(user, "<span_class='warning'>[src] already contains a [proxy_module]!</span>")
					return
				if(!user.transferItemToLoc(W, src))
					return
				to_chat(user, "<span_class='notice'>You attach [W] into [src]'s inner circuits.</span>")
...
/obj/structure/camera_assembly/crowbar_act(mob/user, obj/item/tool)
	...
	if(!choice || !user.canUseTopic(src, BE_CLOSE, FALSE, NO_TK))
		return
	to_chat(user, "<span_class='notice'>You remove [choice] from [src].</span>")
	drop_upgrade(choice)
...
/obj/structure/camera_assembly/screwdriver_act(mob/user, obj/item/tool)
	...
	if(!input)
		to_chat(user, "<span_class='warning'>No input found, please hang up and try your call again!</span>")
		return
	var/list/tempnetwork = splittext(input, ",")
	if(tempnetwork.len < 1)
		to_chat(user, "<span_class='warning'>No network found, please hang up and try your call again!</span>")
		return
...
/obj/structure/camera_assembly/wirecutter_act(mob/user, obj/item/I)
	...
	to_chat(user, "<span_class='notice'>You cut the wires from the circuits.</span>")
	state = STATE_WELDED
	return TRUE
...
/obj/structure/camera_assembly/wrench_act(mob/user, obj/item/I)
	if(state != STATE_WRENCHED)
		return FALSE
	I.play_tool_sound(src)
	to_chat(user, "<span_class='notice'>You detach [src] from its place.</span>")
