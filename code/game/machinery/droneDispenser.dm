...
/obj/machinery/droneDispenser/examine(mob/user)
	...
	if((mode == DRONE_RECHARGING) && !stat && recharging_text)
		to_chat(user, "<span_class='warning'>[recharging_text]</span>")
	...
	switch(mode)
		if(DRONE_READY)
			...
			if(begin_create_message)
				visible_message("<span_class='notice'>[src] [begin_create_message]</span>")
			if(work_sound)
		...
		if(DRONE_PRODUCTION)
			...
			if(end_create_message)
				visible_message("<span_class='notice'>[src] [end_create_message]</span>")
		...
		if(DRONE_RECHARGING)
			...
			if(recharge_message)
				visible_message("<span_class='notice'>[src] [recharge_message]</span>")
...
/obj/machinery/droneDispenser/attackby(obj/item/I, mob/living/user)
	if(I.tool_behaviour == TOOL_CROWBAR)
		...
		to_chat(user, "<span_class='notice'>You retrieve the materials from [src].</span>")

	else if(I.tool_behaviour == TOOL_WELDER)
		if(!(stat & BROKEN))
			to_chat(user, "<span_class='warning'>[src] doesn't need repairs.</span>")
			return
		...
		user.visible_message(
			"<span_class='notice'>[user] begins patching up [src] with [I].</span>",
			"<span_class='notice'>You begin restoring the damage to [src]...</span>")

		if(!I.use_tool(src, user, 40, volume=50, amount=1))
			return

		user.visible_message(
			"<span_class='notice'>[user] fixes [src]!</span>",
			"<span_class='notice'>You restore [src] to operation.</span>")
...
/obj/machinery/droneDispenser/obj_break(damage_flag)
	if(!(flags_1 & NODECONSTRUCT_1))
		if(!(stat & BROKEN))
			if(break_message)
				audible_message("<span_class='warning'>[src] [break_message]</span>")
			if(break_sound)
