...
/obj/machinery/computer/apc_control/attack_ai(mob/user)
	if(!IsAdminGhost(user))
		to_chat(user,"<span_class='warning'>[src] does not support AI control.</span>") //You already have APC access, cheater!
		return
	..(user)
...
/obj/machinery/computer/apc_control/Topic(href, href_list)
	...
	if(href_list["restore_logging"])
		to_chat(usr, "<span_class='robot notice'>[icon2html(src, usr)] Logging functionality restored from backup data.</span>")
		obj_flags &= ~EMAGGED
	...
	if(href_list["access_apc"])
		...
		if(!APC || APC.aidisabled || APC.panel_open || QDELETED(APC))
			to_chat(usr, "<span_class='robot danger'>[icon2html(src, usr)] APC does not return interface request. Remote access may be disabled.</span>")
			return
		if(active_apc)
			to_chat(usr, "<span_class='robot danger'>[icon2html(src, usr)] Disconnected from [active_apc].</span>")
			active_apc.say("Remote access canceled. Interface locked.")
			playsound(active_apc, 'sound/machines/boltsdown.ogg', 25, 0)
			playsound(active_apc, 'sound/machines/terminal_alert.ogg', 50, 0)
			active_apc.locked = TRUE
			...
			active_apc.remote_control = null
			active_apc = null
		to_chat(usr, "<span_class='robot notice'>[icon2html(src, usr)] Connected to APC in [get_area_name(APC.area, TRUE)]. Interface request sent.</span>")
...
/obj/machinery/computer/apc_control/emag_act(mob/user)
	if(!authenticated)
		to_chat(user, "<span_class='warning'>You bypass [src]'s access requirements using your emag.</span>")
		authenticated = TRUE
		log_activity("logged in")
	else if(!(obj_flags & EMAGGED))
		user.visible_message("<span_class='warning'>You emag [src], disabling precise logging and allowing you to clear logs.</span>")
