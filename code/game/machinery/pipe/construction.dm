...
/obj/item/pipe/wrench_act(mob/living/user, obj/item/wrench/W)
	...
	for(var/obj/machinery/atmospherics/M in loc)
		if((M.pipe_flags & flags & PIPING_ONE_PER_TURF))	//Only one dense/requires density object per tile, eg connectors/cryo/heater/coolers.
			to_chat(user, "<span_class='warning'>Something is hogging the tile!</span>")
			return TRUE
		...
		if(M.GetInitDirections() & SSair.get_init_dirs(pipe_type, fixed_dir()))	// matches at least one direction on either type of pipe
			to_chat(user, "<span_class='warning'>There is already a pipe at that location!</span>")
			return TRUE
	// no conflicts found
	...
	user.visible_message( \
		"[user] fastens \the [src].", \
		"<span_class='notice'>You fasten \the [src].</span>", \
		"<span_class='italics'>You hear ratcheting.</span>")
...
/obj/item/pipe/directional/suicide_act(mob/user)
	user.visible_message("<span_class='suicide'>[user] shoves [src] in [user.p_their()] mouth and turns it on! It looks like [user.p_theyre()] trying to commit suicide!</span>")
	if(iscarbon(user))
...
/obj/item/pipe_meter/wrench_act(mob/living/user, obj/item/wrench/W)
	...
	for(var/obj/machinery/atmospherics/pipe/P in loc)
		if(P.piping_layer == piping_layer)
			...
			break
	if(!pipe)
		to_chat(user, "<span_class='warning'>You need to fasten it to a pipe!</span>")
		return TRUE
	new /obj/machinery/meter(loc, piping_layer)
	W.play_tool_sound(src)
	to_chat(user, "<span_class='notice'>You fasten the meter to the pipe.</span>")
	qdel(src)
...
/obj/item/pipe_meter/screwdriver_act(mob/living/user, obj/item/S)
	...
	if(!isturf(loc))
		to_chat(user, "<span_class='warning'>You need to fasten it to the floor!</span>")
		return TRUE

	new /obj/machinery/meter/turf(loc, piping_layer)
	S.play_tool_sound(src)
	to_chat(user, "<span_class='notice'>You fasten the meter to the [loc.name].</span>")
	qdel(src)
