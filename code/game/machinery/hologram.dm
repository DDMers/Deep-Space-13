...
/obj/machinery/holopad/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		to_chat(user, "<span_class='notice'>The status display reads: Current projection range: <b>[holo_range]</b> units.<span>")
...
/obj/machinery/holopad/attackby(obj/item/P, mob/user, params)
	...
	if(istype(P,/obj/item/disk/holodisk))
		if(disk)
			to_chat(user,"<span_class='notice'>There's already a disk inside [src]</span>")
			return
		if (!user.transferItemToLoc(P,src))
			return
		to_chat(user,"<span_class='notice'>You insert [P] into [src]</span>")
		disk = P
		updateDialog()
		return
	...
	if (href_list["AIrequest"])
		if(last_request + 200 < world.time)
			...
			for(var/mob/living/silicon/ai/AI in GLOB.silicon_mobs)
				if(!AI.client)
					continue
				to_chat(AI, "<span_class='info'>Your presence is requested at <a href='?src=[REF(AI)];jumptoholopad=[REF(src)]'>\the [area]</a>.</span>")
		else
...
/obj/machinery/holopad/proc/activate_holo(mob/living/user)
	...
	if(is_operational() && (!AI || AI.eyeobj.loc == loc))//If the projector has power and client eye is on it
		if (AI && istype(AI.current, /obj/machinery/holopad))
			to_chat(user, "<span_class='danger'>ERROR:</span> \black Image feed in progress.")
			return
		...
		visible_message("<span_class='notice'>A holographic image of [user] flickers to life before your eyes!</span>")

		return Hologram
	else
		to_chat(user, "<span_class='danger'>ERROR:</span> Unable to project hologram.")
...
/obj/machinery/holopad/proc/setup_replay_holo(datum/holorecord/record)
	...
	Hologram.set_light(2)	//hologram lighting
	visible_message("<span_class='notice'>A holographic image of [record.caller_name] flickers to life before your eyes!</span>")
	return Hologram
