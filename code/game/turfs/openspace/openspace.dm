...
/turf/open/openspace/update_multiz(prune_on_fail = FALSE, init = FALSE)
	...
	if(!T)
		vis_contents.len = 0
		if(prune_on_fail)
			ChangeTurf(/turf/open/space) //DeepSpace 13 - Multiz explosions
		return FALSE
	if(init)
		vis_contents += T
	return TRUE
...
/turf/open/openspace/attackby(obj/item/C, mob/user, params)
	...
	if(istype(C, /obj/item/stack/rods))
		...
		if(W)
			to_chat(user, "<span_class='warning'>There is already a catwalk here!</span>")
			return
		if(L)
			if(R.use(1))
				to_chat(user, "<span_class='notice'>You construct a catwalk.</span>")
				playsound(src, 'sound/weapons/genhit.ogg', 50, 1)
				new/obj/structure/lattice/catwalk(src)
			else
				to_chat(user, "<span_class='warning'>You need two rods to build a catwalk!</span>")
			return
		if(R.use(1))
