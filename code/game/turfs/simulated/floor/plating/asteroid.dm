...
/turf/open/floor/plating/asteroid/proc/can_dig(mob/user)
	if(!dug)
		return TRUE
	if(user)
		to_chat(user, "<span_class='notice'>Looks like someone has dug here already.</span>")
...
/turf/open/floor/plating/asteroid/attackby(obj/item/W, mob/user, params)
	...
	if(!.)
		if(W.tool_behaviour == TOOL_SHOVEL || W.tool_behaviour == TOOL_MINING)
			...
			to_chat(user, "<span_class='notice'>You start digging...</span>")

			if(W.use_tool(src, user, 40, volume=50))
				if(!can_dig(user))
					return TRUE
				to_chat(user, "<span_class='notice'>You dig a hole.</span>")
				getDug()
...
/turf/open/floor/plating/asteroid/snow/burn_tile()
	if(!burnt)
		visible_message("<span_class='danger'>[src] melts away!.</span>")
		slowdown = 0
		burnt = TRUE
		icon_state = "snow_dug"
		return TRUE
