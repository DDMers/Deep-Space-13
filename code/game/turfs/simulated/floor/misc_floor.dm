...
/turf/open/floor/clockwork/crowbar_act(mob/living/user, obj/item/I)
	if(islist(baseturfs))
		...
		return TRUE
	user.visible_message("<span_class='notice'>[user] begins slowly prying up [src]...</span>", "<span_class='notice'>You begin painstakingly prying up [src]...</span>")
	if(I.use_tool(src, user, 70, volume=80))
		user.visible_message("<span_class='notice'>[user] pries up [src]!</span>", "<span_class='notice'>You pry up [src]!</span>")
		make_plating()
	return TRUE
