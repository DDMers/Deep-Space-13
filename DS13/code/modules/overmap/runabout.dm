...
/obj/item/carkey
	...
	desc = "A small transceiver array which can unlock a linked runabout."
...
/obj/item/carkey/AltClick(mob/user)
	. = ..()
	var/list/options = list("rename", "reskin")
...
/obj/structure/overmap_component/docking_computer/attack_hand(mob/user)
	...
	if(!A.linked_overmap || istype(A, /area/space)) //If there's a linked overmap, that means we're docked to a ship. Space doesnt have a linked overmap.
		...
		linked.pixel_z = 400 //Shove it offscreen so we can land!
		linked.angle = 0
		linked.EditAngle()
		linked.forceMove(get_turf(pick(selected.docking_points)))
		playsound(linked, 'DS13/sound/effects/pod_launch.ogg', 100, 1)
		linked.send_sound_all('DS13/sound/effects/pod_launch.ogg', "<span class='notice'>A huge force weighs down on you as [linked] accelerates.</span>")
		linked.pixel_w = -120
		for(var/i = 0, i < 50, i++)
			linked.pixel_z -= 8
			sleep(1)
